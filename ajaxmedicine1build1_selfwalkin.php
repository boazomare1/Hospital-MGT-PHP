...
$query200 = "select itemcode,itemname,disease,genericname,pkg,`$ratecolumn` from master_medicine where (itemname like '%$searchmedicinename1%' or disease like '%$searchmedicinename1%' or genericname like '%$searchmedicinename1%') and categoryname not in ('MEDICAL SUPPLIES','GENERAL STORES') and status <> 'deleted' order by itemname";
...

{
	...
	$query61 = "select rate from pharma_template_map where productcode = '$itemcode' and templateid='1' order by auto_number desc";
	...
	/*$query57 = "select sum(cum_quantity) as currentstock from transaction_stock where locationcode='".$locationcode."' AND itemcode = '$itemcode' and cum_stockstatus='1'";
