...
if(isset($_REQUEST['docno'])){ $docno = $_REQUEST['docno'];}else{$docno='';}	
	...
	$query3 = "update master_internalstockrequest set recordstatus = 'deleted',discard_username='$username' where docno = '$docno' and recordstatus!='completed' and balance_qty=0";
	$exec3 = mysqli_query($GLOBALS["___mysqli_ston"], $query3) or die ("Error in Query3".mysqli_error($GLOBALS["___mysqli_ston"]));
	$query3 = "update master_internalstockrequest set recordstatus = 'partially deleted',discard_username='$username' where docno = '$docno' and recordstatus!='completed' and balance_qty>0";
	$exec3 = mysqli_query($GLOBALS["___mysqli_ston"], $query3) or die ("Error in Query3".mysqli_error($GLOBALS["___mysqli_ston"]));
	
}
elseif($action == 'approvefunction'){
if(isset($_REQUEST['docno'])){ $docno = $_REQUEST['docno'];}else{$docno='';}	
	
	$query3 = "update master_internalstockrequest set approval_status = 'approved',approved_username='$username' where docno = '$docno' and recordstatus!='completed' and balance_qty=0";
	$exec3 = mysqli_query($GLOBALS["___mysqli_ston"], $query3) or die ("Error in Query3".mysqli_error($GLOBALS["___mysqli_ston"]));
	$query3 = "update master_internalstockrequest set approval_status = 'approved',approved_username='$username' where docno = '$docno' and recordstatus!='completed' and balance_qty>0";
	$exec3 = mysqli_query($GLOBALS["___mysqli_ston"], $query3) or die ("Error in Query3".mysqli_error($GLOBALS["___mysqli_ston"]));
		 ...
		 $query1 = "select fromstore,updatedatetime,docno,tostore,typetransfer,username from master_internalstockrequest where recordstatus='pending' and Date(updatedatetime) between '$fromdate' and '$todate' and tostore='$searchstorecode' AND fromstore LIKE '%$fromstockstore%'  group by docno order by tostore $sortfunc";
            ...
            <tr <?php echo $colorcode; ?> id="TR<?php echo $sno ?>">
              ...
              <td class="bodytext31" valign="center" align="left">
			    ...
			    <div align="left"><input type = "button" id="approve<?php echo $sno ?>" value = "Approve" onclick = "approverequestion('<?php echo $sno ?>','<?php echo $docno ?>')"></div></td>
