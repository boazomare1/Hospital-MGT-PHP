...
// code ends here
	...
	$res4accountname = $res_sc['scheme_name'];
	$accountexpirydate = $res_sc['scheme_expiry'];
$scheme_active_status = $res_sc["scheme_active_status"];
	...
	$customeroveralllimit = $res5["overalllimitop"];
	
	$exclusions = $res5["exclusions"];
...
if($visitlimit>0)
{
	...
	$res51consultationdept= $res51['department'];
	$res51consultationdoc= $res51['consultingdoctorcode'];
	$res51consultationname= $res51['consultingdoctor'];
	
    $res51lastvistdatetime =$lastvisitdate.' '.$res51consultationtime;
...
if($lastvisitdate!=''){	   
		   ...
		   $diff = $currentdatetime - strtotime($res51lastvistdatetime);
		   $days_diff = $diff/3600;
	}else
		$days_diff=0;
	

	$query66 = "select * from master_consultationtype where recordstatus = '' limit 1";
...
//$check_checkbox variable concatianted by murali
	if ($searchresult == '')

	{

		$searchresult = ''.$customercode.'#'.$customername.'#'.$patientmiddlename.'#'.$patientlastname.'#'.$paymenttype.'#'.$subtype.'#'.$billtype.'#'.$accountname.'#'.$accountexpirydate.'#'.$planname.'#'.$planexpirydate.'#'.$visitlimit.'#'.$overalllimit.'#'.$res4paymenttype.'#'.$res4subtype.'#'.$res4accountname.'#'.$res4planname.'#'.$visitcount.'#'.$res4planfixedamount.'#'.$patientspent.'#'.$res4planpercentage.'#'.$availablelimit.'#'.$years.'#'.$gender.'#'.$recordstatus.'#'.$res66consultationtype.'#'.$paymenttypeanum.'#'.$mrdno.'#'.$admissionfees.'#'.$lastvisitdate.'#'.$visitdays.'#'.$res4planstatus.'#'.$res4smartap.'#'.$photoavailable.'#'.$inactivestatus.'#'.$memberno.'#'.$overallplandue.'#'.$registrationfees.'#'.$res4mcc.'#'.$days_diff.'#'.$res51consultationdept.'#'.$res51consultationdoc.'#'.$res51consultationname.'#'.$scheme_id.'#'.$scheme_active_status.'#'.$check_checkbox.'#'.$exclusions;
