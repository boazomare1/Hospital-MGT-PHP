...
$returndocno=isset($_REQUEST['returndocno'])?$_REQUEST['returndocno']:'';
...
$storecode=$getstorecode=isset($_REQUEST['getstorecode'])?$_REQUEST['getstorecode']:'';
...
//ends here
			...
			$query40 = "select rate from transaction_stock where itemcode='$itemcode' and batchnumber ='$batchnumber' and fifo_code='$fifo_code' and storecode ='$getstorecode' order by auto_number desc limit 0,1";
...
//	if($refund == $itemcode)
...

	{
	      ...
	      $query612 = "select sum(return_quantity) as totqty from medicine_return_request where itemcode='".$itemcode."' and patientcode = '$patientcode' and visitcode = '$visitcode' and refno='$refnoget'";
          ...
          $totqty=$rescumstock234["totqty"];

		  $query613 = "select sum(quantity) as alreadyreturn from pharmacysalesreturn_details where itemcode='".$itemcode."' and patientcode = '$patientcode' and visitcode = '$visitcode' and docnumber='$refnoget'";
          $exec613 = mysqli_query($GLOBALS["___mysqli_ston"], $query613) or die ("Error in query613".mysqli_error($GLOBALS["___mysqli_ston"]));
		  $rescumstock235 = mysqli_fetch_array($exec613);
          $alreadyreturn=$rescumstock235["alreadyreturn"];
          
    if($returnquantity<=($totqty-$alreadyreturn)) {
	...
	{ //echo 'in';
		...
		$stockquery2="insert into transaction_stock (fifo_code,tablename,itemcode, itemname, transaction_date,transactionfunction,description,
			...
			transaction_quantity, cum_quantity, entrydocno, docstatus, cum_stockstatus, batch_stockstatus,locationcode,locationname,storecode,storename,username,ipaddress,recorddate,recordtime,updatetime,patientcode,patientvisitcode,patientname,rate,totalprice,ledgercode,ledgername,incomeledger,incomeledgercode)
			...
			'$cum_quantity', '$billnumber', '','1','1', '$getlocationcode','','$getstorecode', '', '$username', '$ipaddress','$dateonly','$timeonly','$datetimeonly','$patientcode','$visitcode','$patientname','$costprice','$totalcp','$ledgercode','$ledgername','$incomeledger','$incomeledgercode')";
		...
		$query43="insert into pharmacysalesreturn_details(fifo_code,patientcode,visitcode,companyanum,billnumber,itemcode,itemname,rate,quantity,totalamount,username,ipaddress,entrydate,batchnumber,billstatus,location,pharmacycoa,categoryname,locationname,locationcode,store,docnumber,salseautonumber,ledgercode,ledgername,incomeledger,incomeledgercode,costprice,totalcp)values('$fifo_code','$patientcode','$visitcode','$companyanum','$billnumber','$itemcode','$medicinename','$rate','$returnquantity','$amount','$username','$ipaddress','$dateonly','$batchnumber','pending','$location1','$pharmacycoa','$categoryname','".$getlocationname."','".$getlocationcode."','".$getstorecode."','".$refnoget."','".$salseautonumber."','$ledgercode','$ledgername','$incomeledger','$incomeledgercode','$costprice','$totalcp')"; 

		$exec43=mysqli_query($GLOBALS["___mysqli_ston"], $query43) or die(mysqli_error($GLOBALS["___mysqli_ston"]));

		

		//$query55 = "insert into paylaterpharmareturns(patientcode,patientvisitcode,patientname,medicinecode,medicinename,rate,quantity,amount,pharmacycoa,username,ipaddress,billdate,accountname,billnumber,locationname,locationcode,accountnameano,accountnameid,subtypeano,ledgercode,ledgername,incomeledger,incomeledgercode)values('$patientcode','$visitcode','$patientname','$itemcode','$medicinename','$rate','$returnquantity','$amount','$pharmacycoa','$username','$ipaddress','$dateonly','$accountname','$billnumber','".$getlocationname."','".$getlocationcode."','$accountnameano','$accountnameid','$patientsubtype','$ledgercode','$ledgername','$incomeledger','$incomeledgercode')";

		//$exec55 = mysql_query($query55) or die(mysql_error());

	

		$queryupdate = "UPDATE medicine_return_request SET completestatus='complete' where itemcode='".$itemcode."' and  visitcode='".$visitcode."' and refno='".$refnoget."' and refundstatus <> 'refund' and fifo_code='$fifo_code'";

		$execupdate = mysqli_query($GLOBALS["___mysqli_ston"], $queryupdate) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
		...
		$query43="insert into pharmacysalesreturn_details(fifo_code,patientcode,visitcode,companyanum,billnumber,itemcode,itemname,rate,quantity,totalamount,username,ipaddress,entrydate,batchnumber,billstatus,pharmacycoa,location,categoryname,locationname,locationcode,store,docnumber,salseautonumber,ledgercode,ledgername,incomeledger,incomeledgercode,costprice,totalcp)values('$fifo_code','$patientcode','$visitcode','$companyanum','$billnumber','$itemcode','$medicinename','$rate','$returnquantity','$amount','$username','$ipaddress','$dateonly','$batchnumber','completed','$pharmacycoa','$location1','$categoryname','".$getlocationname."','".$getlocationcode."','".$getstorecode."','".$refnoget."','".$salseautonumber."','$ledgercode','$ledgername','$incomeledger','$incomeledgercode','$costprice','$totalcp')";
	...
	{ //echo 'ii';
		...
		$query43="insert into pharmacysalesreturn_details(patientcode,visitcode,companyanum,billnumber,itemcode,itemname,rate,quantity,totalamount,username,ipaddress,entrydate,batchnumber,billstatus,pharmacycoa,location,categoryname,locationname,locationcode,store,docnumber,salseautonumber,ledgercode,ledgername,incomeledger,incomeledgercode,costprice,totalcp)values('$patientcode','$visitcode','$companyanum','$billnumber','$itemcode','$medicinename','$rate','$returnquantity','$amount','$username','$ipaddress','$dateonly','$batchnumber','pending','$pharmacycoa','$location1','$categoryname','".$getlocationname."','".$getlocationcode."','".$getstorecode."','".$refnoget."','".$salseautonumber."','$ledgercode','$ledgername','$incomeledger','$incomeledgercode','$costprice','$totalcp')";
        ...
        echo '<script> alert("Some item faild to return/already returned, please check again"); window.location="ipmedicineissuedlist.php";</script>';
	    exit;
	}
  ...
  header("location:ipmedicineissuedlist.php");
...

{
	...
	if(confirm("Are You Want To Save The Record?")==false){document.getElementById("savereturn").disabled=false;return false;}	
...
function balancecalc(varSerialNumber1,qnty1,totalcount,key)
...
totalamount=document.getElementById("amount"+i+"").value;
...
//echo $getstorename = $resget['store'];
  ...
  $storecode=$getstorecode = $resget['store'];
    ...
    <td colspan="4" align="left" valign="middle"  bgcolor="<?php if ($errmsg == '') { echo '#FFFFFF'; } else { echo 'red'; } ?>" class="bodytext3"><strong><?php echo $errmsg;?>&nbsp;</strong></td></tr>
	 ...
	 ?>
      ...
      <tr>
			...
			<td width="8%"  align="left" valign="center" 
                ...
                bgcolor="#ffffff" class="bodytext31"><div align="center"><strong>Amount</strong></div></td>
						...
						$queryentry = "select locationname,locationcode from master_ipvisitentry where patientcode = '$patientcode' and visitcode = '$visitcode' ";
						   ...
						   $execentry = mysqli_query($GLOBALS["___mysqli_ston"], $queryentry) or die(mysqli_error($GLOBALS["___mysqli_ston"]));

						   $resentry = mysqli_fetch_array($execentry);
        ...
        $query9 = "select storecode from master_employeelocation where username = '$username' and locationcode = '$res1locationanum' and defaultstore='default'";
		...
		$query10 = "select storecode from master_store where auto_number = '$res9anum'";

		$exec10 = mysqli_query($GLOBALS["___mysqli_ston"], $query10) or die ("Error in Query10".mysqli_error($GLOBALS["___mysqli_ston"]));

		$res10 = mysqli_fetch_array($exec10);

		$storechk = $res10['storecode'];


		  $queryrefund = "select sum(return_quantity) as return_total,docno,refno,issue_quantity,rateperunit,freestatus,fifo_code from medicine_return_request where patientcode = '$patientcode' and visitcode = '$visitcode' and refundstatus <> 'refund'  and completestatus <> 'complete' and store='$storechk' and docno='$returndocno' GROUP BY refno,itemcode,store,fifo_code"; 

		   $execrefund = mysqli_query($GLOBALS["___mysqli_ston"], $queryrefund) or die(mysqli_error($GLOBALS["___mysqli_ston"]));

		 $nno=  mysqli_num_rows($execrefund); 

		  
			if($nno==0){
			  echo "<p align='center'><strong><font color=red>Forbidden You don't have permission to access this patient. </font><strong></p>";
			}
		   ...
		   $fifo_code=$resrefund['fifo_code'];

			$query61 = "select patientname,patientcode,visitcode,itemname,itemcode,ipdocno,quantity,entrydate,rate,totalamount,fifo_code,freestatus,auto_number,sum(quantity) as quantity1,sum(totalamount) as totalamount1,batchnumber from pharmacysales_details where patientcode = '$patientcode' and visitcode = '$visitcode' and ipdocno='$refnoget' and fifo_code='$fifo_code'";
			...
			$query62 = "select quantity from pharmacysalesreturn_details where patientcode = '$patientcode' and visitcode = '$visitcode' and itemcode='$itemcode' and fifo_code='$fifo_code' and docnumber='".$refnoget."' and salseautonumber='".$salseautonumber."'";
			...
			$res02=mysqli_query($GLOBALS["___mysqli_ston"], $query02);

			$num02=mysqli_fetch_array($res02);
...
?>
  ...
  <td class="bodytext31" valign="center"  align="left"><div align="center">
        ...
        <input type="checkbox" name="ref[<?= $sno; ?>]" id="ref<?php echo $sno; ?>" value="<?php echo $itemcode; ?>" onClick="updatebox('<?php echo $sno; ?>','<?php echo $resquantity;?>','<?php echo $nno; ?>')"/></div></td>

		<td class="bodytext31" valign="center"  align="left"><div align="center"><?php echo $itemname;?></div></td>

		<input type="hidden" name="med[<?= $sno; ?>]" value="<?php echo $itemname;?>">

		<input type="hidden" name="code[<?= $sno; ?>]" value="<?php echo $itemcode; ?>">

        <input type="hidden" name="fifo_code[<?= $sno; ?>]" value="<?php echo $fifo_code; ?>">

			<td class="bodytext31" valign="center"  align="left"><div align="center"><?php echo $batchnumber;?></div></td>

			<input type="hidden" name="batch[<?= $sno; ?>]" value="<?php echo $batchnumber;?>">
			...
			<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="returnquantity[<?= $sno; ?>]" id="returnquantity<?php echo $sno; ?>" onKeyUp="balancecalc('<?php echo $sno; ?>','<?php echo $resquantity;?>','<?php echo $nno; ?>',event)" onKeyPress="return number(event)" size="7" value="<?php echo $returnqty;?>"></div></td>

			<td class="bodytext31" valign="center"  align="left"><div align="center"><?php echo $refundedquantity;?>

            

            <input type="hidden" class="bal" name="docnoget[<?= $sno; ?>]" id="docnoget<?php echo $sno; ?>" size="7" readonly value="<?php echo $docnoget; ?>">

            <input type="hidden" class="bal" name="refnoget[<?= $sno; ?>]" id="refnoget<?php echo $sno; ?>" size="7" readonly value="<?php echo $refnoget; ?>">

            <input type="hidden" class="bal" name="issuequantity[<?= $sno; ?>]" id="issuequantity<?php echo $sno; ?>" size="7" readonly value="<?php echo $issuequantity; ?>">

            <input type="hidden" class="bal" name="rateperunit[<?= $sno; ?>]" id="rateperunit<?php echo $sno; ?>" size="7" readonly value="<?php echo $rateperunit; ?>">

            <input type="hidden" class="bal" name="freestatus[<?= $sno; ?>]" id="freestatus<?php echo $sno; ?>" size="7" readonly value="<?php echo $freestatus; ?>">

            </div></td>

			<input type="hidden" name="quantity[<?= $sno; ?>]" value="<?php echo $resquantity;?>">

		   <td class="bodytext31" valign="center" bordercolor="#f3f3f3"><input type="text" class="bal" name="balamount[<?= $sno; ?>]" id="balamount<?php echo $sno; ?>" size="7" readonly value="<?php echo $balanceqty; ?>"></td>

         <td class="bodytext31" valign="center"  align="left"><div align="center"><?php echo $rate;?><input type="hidden" name="rate[<?= $sno; ?>]" id="rate<?php echo $sno; ?>" value="<?php echo $rate;?>"></div></td>

		 <td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" class="bal" name="amount[<?= $sno; ?>]" id="amount<?php echo $sno; ?>" size="7" readonly></div></td>

		 <input type="hidden" name="docnumber[<?= $sno; ?>]" value="<?php echo $docnumber;?>">

           <input type="hidden" name="salseautonumber[<?= $sno; ?>]" value="<?php echo $salseautonumber;?>">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td width="54%" align="right" valign="top" >
             	  ...
             	  <input name="Submit2223" type="submit" value="Save" id="savereturn" onClick="return check();" accesskey="b" class="button" style="border: 1px solid #001E6A"/>
