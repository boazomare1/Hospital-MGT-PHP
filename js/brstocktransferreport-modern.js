// Branch Stock Transfer Report Modern JavaScript
// Handles form validation, responsive design, and enhanced user experience for branch stock transfer reporting
...
function initializeFormValidation() {
    const searchForm = document.querySelector('form[action*="brstocktransferreport.php"]');
    if (searchForm) {
...
function adjustTablesForMobile() {
    const tables = document.querySelectorAll('.stock-transfer-table');
...
// Stock Transfer Table Enhancement
function initializeStockTransferTable() {
    const tables = document.querySelectorAll('.stock-transfer-table');
    tables.forEach(table => {
...
function addTransferStatusIndicators(table) {
    const rows = table.querySelectorAll('tbody tr');
    rows.forEach(row => {
        const statusCell = row.querySelector('td:contains("pending"), td:contains("approved"), td:contains("rejected"), td:contains("completed"), td:contains("in progress")');
...
function updateStockTransferSummary() {
    const rows = document.querySelectorAll('.stock-transfer-table tbody tr');
    ...
    updateSummaryDisplay(totalTransfers, pendingTransfers, approvedTransfers, completedTransfers, rejectedTransfers, totalAmount, totalQuantity);
}

function addSummaryCards() {
    const container = document.querySelector('.brstocktransfer-container') || document.body;
    const summarySection = document.createElement('section');
    summarySection.className = 'summary-section';
    summarySection.innerHTML = `
        <div class="summary-header">
            <h3><i class="fas fa-exchange-alt"></i> Stock Transfer Summary</h3>
        </div>
        <div class="summary-cards">
            ...
            <div class="summary-card rejected">
                ...
                <div class="summary-card-value" id="rejectedTransfers">0</div>
            </div>
            <div class="summary-card total">
...
function updateSummaryDisplay(total, pending, approved, completed, rejected, amount, quantity) {
    ...
    const rejectedEl = document.getElementById('rejectedTransfers');
...
function addTransferSummary() {
    const container = document.querySelector('.brstocktransfer-container') || document.body;
    const transferSection = document.createElement('section');
    transferSection.className = 'transfer-summary';
    transferSection.innerHTML = `
        <h3><i class="fas fa-chart-pie"></i> Transfer Analysis</h3>
        <div class="transfer-grid">
            <div class="transfer-item pending">
                <div class="transfer-item-label">Pending Analysis</div>
                <div class="transfer-item-value" id="pendingAnalysis">0</div>
            </div>
            <div class="transfer-item approved">
                <div class="transfer-item-label">Approval Rate</div>
                <div class="transfer-item-value" id="approvalRate">0%</div>
            </div>
            <div class="transfer-item completed">
                <div class="transfer-item-label">Completion Rate</div>
                <div class="transfer-item-value" id="completionRate">0%</div>
            </div>
            <div class="transfer-item rejected">
                <div class="transfer-item-label">Rejection Rate</div>
                <div class="transfer-item-value" id="rejectionRate">0%</div>
            </div>
        </div>
    `;
    ...
    // Calculate and update transfer metrics
    updateTransferMetrics();
}

function updateTransferMetrics() {
    const totalEl = document.getElementById('totalTransfers');
    const pendingEl = document.getElementById('pendingTransfers');
    const approvedEl = document.getElementById('approvedTransfers');
    const completedEl = document.getElementById('completedTransfers');
    ...
    if (totalEl && pendingEl && approvedEl && completedEl && rejectedEl) {
...
window.BrStockTransferReportModern = {
    validateSearchForm,
    showNotification,
    updateStockTransferSummary,
    updateTransferMetrics
