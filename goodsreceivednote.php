...
$fromdate=$updatedate = date('Y-m-d');
...
$query3 = "select * from bill_formats where description = 'goods_receivednote'";
...

{
	...
	if(isset($_POST['only_stockable'][$key]))
	{
	$list_mrn=$_POST['only_stockable'][$key];
	}
	else
	{
	$list_mrn='';
   ...
   if($selectitem==$list_mrn)
    {
      
	$qry_pos = "select * from materialreceiptnote_details where billnumber='$list_mrn' and suppliercode='$suppliercode' and grnstatus='' and itemstatus <> 'deleted'";
	...
	if($numrows>0)
	{
		while($mrn_res=mysqli_fetch_array($exec_po))
		{
			
$autoid     = $mrn_res['auto_number'];
$itemcode     = $mrn_res['itemcode'];
$itemname     = $mrn_res['itemname'];
...
$quantity = $mrn_res['quantity'];

$allpackagetotalquantity = $quantity;


$free = $mrn_res['free'];

$itemdiscountpercent = $mrn_res['discountpercentage'];

		$totalamount = $mrn_res['subtotal'];

$itemtaxpercent = $mrn_res['itemtaxpercentage'];

$batchnumber = $mrn_res['batchnumber'];

$salesprice = $rate;

$costprice = $rate;

$expirydate = $mrn_res['expirydate'];

$fxamount1 = $totalamount;

$totalfxpuramount = $totalamount;
...
$fifocode = $mrn_res['fifo_code'];
$ponumber = $mrn_res['ponumber'];

$mrnno = $mrn_res['billnumber'];
$expense = '';
$expensecode ='';
$expenseanum = '';
$cost_center = "";
...
$packagename = $mrn_res['packagename'];
...
$ledgername ='';
			...
			manufactureranum, manufacturername,typeofpurchase,suppliername,suppliercode,supplieranum ,ponumber,supplierbillnumber,costprice,locationcode,store,coa,categoryname,fifo_code,totalfxamount,deliverybillno,mrnno,ledgeranum,ledgercode,ledgername,expense,expensecode,expenseanum,job_title,cost_center) 
			...
			'$manufactureranum', '$manufacturername','Process','$suppliername','$suppliercode','$supplieranum','$ponumber','$supplierbillno','$costprice','$locationcode','$store1','$coa','$categoryname','$fifo_code','$totalfxpuramount','$deliverybillno','$mrnno','$ledgeranum','$ledgercode','$ledgername','$expense','$expensecode','$expenseanum','$job_title','$cost_center')";
...
if(isset($_POST['costcenter'][$key]))
{
	$cost_center = $_POST['costcenter'][$key];
}
else
{
	$cost_center = "";
...
{
	$expense = $_POST['asset'][$key];
	$expensecode = $_POST['assetno'][$key];
	$expenseanum = $_POST['assetanum'][$key];
}
...
$ledgername = $_POST['ledgername'][$key];
		...
		$query3 = "insert into master_purchase (companyanum, billnumber, billdate, suppliercode, suppliername,totalamount,totalquantity,ipaddress,supplierbillnumber,typeofpurchase,currency,fxrate,totalfxamount,locationcode,locationname,purchaseamount,expense,expensecode,expenseanum,mrnno,cost_center)values('$companyanum','$billnumbercode','$billdate','$suppliercode', '$suppliername','$amount','$quantity','$ipaddress','$supplierbillno','Process','$currency','$fxrate','$totalfxamount','$locationcode','$locationname','$amount','$expense','$expensecode','$expenseanum','$mrnno','$cost_center')";
...
$query5 = "select * from materialreceiptnote_details where suppliercode='$supplier_code' and locationcode='$loc_code' order by billnumber";
...

{
	...
	$chkbackdate="select grn_backdate from master_accountname where auto_number='$supplieranum'";
	$exec5grn = mysqli_query($GLOBALS["___mysqli_ston"], $chkbackdate);
	$res5grn = mysqli_fetch_array($exec5grn);
	$isbackdate=$res5grn["grn_backdate"];

	if($isbackdate==1){

		$getfinyr="SELECT fromyear,toyear FROM `master_financialyear` where status='Active'";
		$exec55grn = mysqli_query($GLOBALS["___mysqli_ston"], $getfinyr);
	    $res55grn = mysqli_fetch_array($exec55grn);
	    $fromdate=$res55grn["fromyear"];
...
function checkMrn(mrn,line_number,count,total_count)
{
	if(document.getElementById("selectmrn"+mrn+"").checked==true)
	{
	$(".selectitem"+mrn).attr("checked", "true");
	}else
	{
	$(".selectitem"+mrn).attr("checked",false);
	}
	
	var act_count=parseFloat(line_number)+parseFloat(count);
	for(m=line_number;m<act_count;m++)	
	{
		totalamount(m,total_count);
	}
}
...
var totalcount = document.getElementById("tot_count").value;;
...
var totalitemscnt = $('#tot_count').val();
...

{
	...
	document.getElementById("supplierbillno1").value = supplierbillno;
...

{
	...
	var purchasetype = document.getElementById("purchasetype"+i+"").value;
	if(purchasetype =='ASSETS')
	{
		if(document.getElementById("selectitem"+i+"").checked == true)
	   {

		var assetledgner = document.getElementById("asset"+i+"").value;
	
		if( (document.getElementById("assetanum"+i+"").value == '' || document.getElementById("assetanum"+i+"").value ==undefined) ||  assetledgner.trim()  =="" )

		{

			alert('Please Select Asset Ledger');//aaa
			document.getElementById("asset"+i+"").focus();
		...
		if(document.getElementById("selectitem"+i+"").checked == true)
	   {
		...
		var assetledgner = document.getElementById("expense"+i+"").value;
	
		if( (document.getElementById("expenseno"+i+"").value == '' || document.getElementById("expenseno"+i+"").value ==undefined) ||  assetledgner.trim()  =="" )

		{

			alert('Please Select expense Ledger');//aaa
...
$('#expense'+id).autocomplete({
	...
	source:'autoassetexpensesearch.php', 
				...
				$.ajax({
				  ...
				  success: function (data) { 
				  	...
				  	else
				  	{
	  					...
	  					var costcenter_cnt = totalitemscnt;
  ...
  <tr>
    ...
    <td width="99%" valign="top"><table width="980" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
		...
		<table width="1000" border="0" align="left" cellpadding="2" cellspacing="0" bordercolor="#666666" id="AutoNumber3">
            ...
            <tbody>
			 ...
			 <td    valign="center"  bgcolor="#ecf0f5" class="bodytext3"><strong>Location</strong></td>
						<td  align="left" valign="center"   bgcolor="#ecf0f5">
						<select name="location" id="location" onchange="submitfunc()" >
						...
						?>
						<option value="<?php echo $reslocationanum; ?>" <?php if($loc_code!='')if($loc_code==$reslocationanum){echo "selected";}?>><?php echo $reslocation; ?></option>
						<?php 
						}
						?>
						</select>
                ...
                <td width="27%" align="left" valign="middle" class="bodytext3">
                	...
                	<input  type="text" name="supplier" id="supplier" value="<?php echo $suppliername; ?>" onclick="checkLoc()" size="25" autocomplete="off" style="text-transform:uppercase;width:300px" /> 
             ...
             <td width="11%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>MRN Date </strong></td>
			  ...
			  <td width="10%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>Doc No</strong></td>
                ...
                <td width="15%" align="left" valign="middle" class="bodytext3"><?php echo $billnumbercode; ?>
				...
				<input name="supplierbillno" id="supplierbillno" value="" size="10" rsize="20" autocomplete="off" onKeyUp="return billnotransfer()" onKeyPress="return RestrictSpace()" />			</td>
           ...
           <img src="images2/cal.gif" onClick="javascript:NewCssCal('grndate','yyyyMMdd','','','','','past','<?php echo $fromdate;?>','<?php echo date('Y-m-d');?>')" style="cursor:pointer"/>
                ...
                <td width="15%" align="left" valign="middle" class="bodytext3"><?php echo $address; ?>
                ...
                <td colspan="3" align="left" valign="middle" class="bodytext3"><!-- <input name="deliverybillno" id="deliverybillno" value="<?php echo $deliverybillno; ?>" size="10" rsize="20" autocomplete="off" onKeyUp="return billnotransfer()"/> -->
                	...
                	<input name="deliverybillno" id="deliverybillno" value="" size="10" rsize="20" autocomplete="off" onKeyUp="return billnotransfer()" onKeyPress="return RestrictSpace()"/>
             ...
             <td width="11%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>Time </strong></td>
                ...
                <td width="15%" align="left" valign="middle" class="bodytext3"><?php echo $res5locationname; ?></td>

				   <td width="10%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3" style="display:none"><strong>Store</strong></td>

                <td colspan="3" width="27%" align="left" valign="middle" class="bodytext3" style="display:none">
      ...
      <tr>
		...
		<form action="goodsreceivednote.php" method="post" name="form" id="form" onSubmit="return validationcheck()">
...
<td bgcolor="#ffffff" class="bodytext31" valign="left"  align="left" width="20%" colspan="3"><strong>Ref. No</strong></td>
<td bgcolor="#ffffff" class="bodytext31" valign="center"  align="center" width="20%" colspan="3"><strong>Date</strong></td>
<td bgcolor="#ffffff" class="bodytext31" valign="center"  align="center" width="20%"><strong>Goods Amount</strong></td>
</tr>
...
//echo 'bill.no'.$billnum;
			...
			if ($cbfrmflag1 == 'cbfrmflag1'){
                ...
                $location_cod = $_POST['location'];
				



				if((trim($suppliercode) !="" )&&($location_cod!=''))

				{


			$query10 = "select auto_number from manual_lpo where billnumber='$billnum' and locationcode='$location_cod' and suppliercode='$suppliercode' and recordstatus=''  and recordstatus <> 'deleted'";
			...
			$numb=mysqli_num_rows($exec10);	
					...
					$query76 = "select * from manual_lpo where billnumber='$billnum' and locationcode='$location_cod'  and suppliercode='$suppliercode' and recordstatus='' and recordstatus <> 'deleted'";
					...
					$query76 = "select billnumber,ponumber from materialreceiptnote_details where suppliercode='$suppliercode' and locationcode='$location_cod'  and itemstatus <> 'deleted' group by billnumber order by billnumber desc";
...
$k=1;
					$totitems_qry = "select auto_number from materialreceiptnote_details where suppliercode='$suppliercode' and locationcode='$location_cod'  and grnstatus='' and itemstatus <> 'deleted' order by billnumber desc";
					...
					$ponumber = $respo['ponumber'];
					$maxcount=split("-",$mrn_number);
		            $mgr_find=$maxcount[0];
					...
					$sample_autono='';
					$patientname='';
					$query3811	 = "select sample_table,sample_autono from manual_lpo where billnumber='$ponumber'";
					$exec3811 = mysqli_query($GLOBALS["___mysqli_ston"], $query3811) or die ("Error in Query3811".mysqli_error($GLOBALS["___mysqli_ston"]));
					$res3811 = mysqli_fetch_array($exec3811);
					$sample_table =  $res3811['sample_table']; 
					$sample_autono =  $res3811['sample_autono']; 
					if($sample_table!=''){
					$query381 = "select * from $sample_table where auto_number='$sample_autono'";
					$exec381 = mysqli_query($GLOBALS["___mysqli_ston"], $query381) or die ("Error in Query381".mysqli_error($GLOBALS["___mysqli_ston"]));
					$res381 = mysqli_fetch_array($exec381);
					$patientname =  $res381['patientname']; 
					}
					
					
					//$mgr_find = preg_replace('/[^\\/\-a-z\s]/i', '', $mrn_number);
					
 $query38 = "select * from bill_formats where prefix LIKE '%$mgr_find%'";
...
$paynowdescription =  $res38['description'];

					if($paynowdescription == 'external_mrn')
					{
						
						

						$qry_pos = "select * from materialreceiptnote_details where billnumber='$mrn_number' and suppliercode='$suppliercode' and grnstatus='' and itemstatus <> 'deleted'";
		              ...
		              <td bgcolor="#ffffff" class="bodytext31" valign="center"  align="center"><strong>&nbsp;</strong></td>
                       ...
                       <td bgcolor="#ffffff" class="bodytext31" valign="center"  align="center" width="20%"><strong>Item</strong></td>
						...
						<td id="assetledgertd" bgcolor="#ffffff" class="bodytext31" valign="center"  align="center"><strong>Asset/Expense Ledger1</strong></td>
					    
					  <td id="costcentertd" bgcolor="#ffffff" class="bodytext31" valign="center"  align="center"><strong></strong></td>
						...
						<td bgcolor="#ffffff" class="bodytext31" valign="center"  align="center"><strong>Sale Price</strong></td>
											 ...
											 <tr>
									              ...
									              <td colspan="6" class="bodytext31" valign="center"  align="left" 
									                ...
									                bgcolor="#ecf0f5"><strong><?php  echo $mrn_number.''.'&nbsp;&nbsp;&nbsp;'.''.$ponumber.''.'&nbsp;&nbsp;&nbsp;Patient:&nbsp;&nbsp;&nbsp;'.''.$patientname;?></strong></td>
 ...
 if(strtoupper($purchasetype) == 'ASSETS') { 
		 	$assets_cntr = $assets_cntr + 1;
					...
					<td class="bodytext31" valign="center"  align="left" id="costcenter_tdcontent<?=$sno;?>">
               ...
               $qry_pos = "select * from materialreceiptnote_details where billnumber='$mrn_number' and grnstatus='' and itemstatus <> 'deleted'";
				...
				$querycumstock2 = "select entrydate,sum(subtotal) as mrnprice from materialreceiptnote_details where  billnumber='$mrn_number' ";
					...
					$mrnprice = $rescumstock2["mrnprice"];
					$entrydate = $rescumstock2["entrydate"];
					
					//$querymlpo = "select auto_number from materialreceiptnote_details where ponumber like '%MLPO-%' and  billnumber='$mrn_number' ";
					$querymlpo = "select auto_number from materialreceiptnote_details where itemcode='' and grnstatus='' and  billnumber='$mrn_number' ";

					$execcmlpo = mysqli_query($GLOBALS["___mysqli_ston"], $querymlpo) or die ("Error in CumQuery2".mysqli_error($GLOBALS["___mysqli_ston"]));
					$mlpo_count = mysqli_num_rows($execcmlpo);
				...
				?>
				 ...
				 <tr>
		           ...
		           <td><?php echo ++$sno;?></td>
					<td colspan="1" ><input type="checkbox"  name="selectitem[]" id="selectitem<?php echo $sno ; ?>" value="<?php echo $mrn_number; ?>" onClick="totalamount('<?php  echo $sno+1;?>','<?php echo $number;?>')"></td>

		              <td colspan="3" class="bodytext31" valign="left"  align="left" width="20%" bgcolor="#ecf0f5" >Towards Purchase  By &nbsp;&nbsp; 
					  <a target="_blank" href="print_mrnview.php?billnumber=<?php echo $mrn_number; ?>&&locationcode=<?= $res5locationcode; ?>"><b><?php  echo $mrn_number;?></b></a> 
					  </td>
<td colspan="3" class="bodytext31" valign="left"  align="left" width="20%" bgcolor="#ecf0f5">
<?php echo date('d/m/Y',strtotime($entrydate));?></td>
		              <td colspan="1" class="bodytext31" valign="center"  align="right" width="20%" bgcolor="#ecf0f5" style="font-size:15px"><b><?php  echo number_format($mrnprice,2);?></b>
					  <input type="hidden" id="totalamount<?php echo $sno ; ?>" name="totalamount<?php echo $sno ; ?>" value="<?php echo $mrnprice; ?>">
					  <input type="hidden" id="totalfxamount<?php echo $sno ; ?>" name="totalfxamount<?php echo $sno ; ?>" value="<?php echo $mrnprice; ?>">
					  <input type="hidden" id="only_stockable<?php echo $sno ; ?>" name="only_stockable[]" value="<?php echo $mrn_number; ?>">
					   <input type="hidden" id="purchasetype<?php echo $sno ; ?>" name="purchasetype[]" value="DRUGS">
					   	<input type="hidden" name="itemname[]" value="<?php echo $mrn_number; ?>">
					  </td>
				...
				<tr>
		              ...
		              <td bgcolor="#ffffff" class="bodytext31" valign="center"  align="center"><strong>&nbsp;</strong></td>
                       ...
                       <td bgcolor="#ffffff" class="bodytext31" valign="center"  align="center" width="20%"><strong>Item</strong></td>
						<td bgcolor="#ffffff" class="bodytext31" valign="center"  align="center"><strong>Recd.Qty</strong></td>
						<td id="assetledgertd" bgcolor="#ffffff" class="bodytext31" valign="center"  align="center"><strong>Asset/Expense Ledger2</strong></td>
				 ...
				 <tr>
		              ...
		              <td colspan="1" class="bodytext31" valign="center"  align="center" width="20%" bgcolor="#ecf0f5"><?php  echo $mrn_number;?></td>
			...
			?>
						...
						<tr <?php if($mlpo_count==1000){ ?> style="display:none" <?php } ?> >

					

					<td><input type="checkbox" class="selectitem<?php echo $mrn_number; ?>" name="selectitem[]" id="selectitem<?php echo $sno ; ?>" value="<?php echo $itemcode.$anum; ?>" onClick="totalamount('<?php echo $sno; ?>','<?php echo $number; ?>')"></td>
...
<?php 
     ...
     <td width="20" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">Total FX Purchase Cost</td>
      ...
      <tr>
		 ...
		 <td colspan="2" align="center" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">

		  <input type="hidden" name="frmflag2" value="frmflag2">
<input type="hidden" name="tot_count" id="tot_count" value="<?php echo $sno; ?>">
             <!--   <input name="Submit222" type="submit"  value="Save" id="savebutton" class="button" onClick="return funcsave('<?php echo $items_cnt; ?>')"/>	 -->

                <input style="font-size:22px;width:150px" name="Submit222" type="submit"  value="Save" id="savebutton" class="button"/>	 </td>
