...
$lctname=$res20['locationname'];
header('Content-Type: application/vnd.ms-excel');
header('Content-Disposition: attachment;filename="Detailedlisting['.$lctname.'].['.$updatedatetime.'].xls"');
...
<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody>
            <tr>
              <td align="center" colspan="13" bgcolor="#ffffff" class="bodytext31"><strong>Statement Of Account</strong></td>  
            </tr>
      <tr>
      ...
      */
	  ?>
			 <tr>
				...
				<td width="15%" align="left" valign="center"  
                bgcolor="#ffffff" class="bodytext31"><div align="left"><strong>Disp.ID</strong></div></td>
				<td width="15%" align="left" valign="center"  
                bgcolor="#ffffff" class="bodytext31"><div align="left"><strong>Invoice Date</strong></div></td>
                ...
                bgcolor="#ffffff" class="bodytext31"><strong>Invoice Amount </strong></td>
                <td width="11%" align="right" valign="center"  
                bgcolor="#ffffff" class="bodytext31"><strong> Balance Amount </strong></td>
      ...
      // $subtypes_accountids_str='0';
      // $subtypes_accountnameanos_str='0';
	  ...
	  $subtypeopbal = 0;	
		  ...
		  UNION ALL SELECT (-1*`fxamount`) as fxamount,transactiondate AS transactiondate  from master_transactionpaylater where accountnameano in($subtypes_accountnameanos_str) and  paymenttype in($paymentTypes_names_str) and accountnameid in($subtypes_accountids_str) and transactiontype = 'paylatercredit' and recordstatus <> 'deallocated' and transactiondate < '$ADate1'  and $pass_location
		  
		  UNION ALL SELECT  (-1*`fxamount`) as fxamount,transactiondate AS transactiondate   FROM `master_transactionpaylater` WHERE `accountnameid` in($subtypes_accountids_str) AND `transactiondate` < '$ADate1'  AND `docno` LIKE 'AR-%' AND `transactionstatus` = 'onaccount' AND `transactionmodule` = 'PAYMENT' and $pass_location
		  
		  UNION ALL SELECT (1*`amount`) as fxamount,consultationdate AS transactiondate  FROM `adhoc_debitnote` WHERE `accountcode` in($subtypes_accountids_str) AND `consultationdate` < '$ADate1' and $pass_location group by docno
		  
		  UNION ALL SELECT  (debitamount-creditamount) as fxamount,entrydate AS transactiondate   FROM `master_journalentries` WHERE `ledgerid` in($subtypes_accountids_str) AND `entrydate` < '$ADate1' and $pass_location  group by docno";
	...
	// OUTSTANDING OPENING BALANCE START		
			...
			// FOR ALL FINALIZED BILLS, ar,Cr.N, CRN BILLS 
			
			  		 $opquery12 = "SELECT (fxamount) AS fxamount,transactiondate AS transactiondate from master_transactionpaylater where accountnameano in ($subtypes_accountnameanos_str)  and accountnameid in ($subtypes_accountids_str) and `transactiontype` = 'PAYMENT' AND recordstatus='allocated' and transactiondate < '$ADate1' and fxamount <>'0' and billnumber!='' and $pass_location
		   ";
...
// OUTSTANDING OPENING BALANCE END
    ...
    <!-- <tr >
			...
			<td colspan="4"  align="right" valign="center" bgcolor="#FFF" class="bodytext31" ><strong>Opening Balance Total:</strong></td>
...
$resamount=0;
      ...
      $paymentreceiveddateto = date("Y-m-d", mktime(0, 0, 0, $dotmonth, $dotday + 1, $dotyear));
      $searchsuppliername1 = trim($searchsuppliername1);
      $query42 = "select docno,fxamount,transactiondate,patientcode,patientname,visitcode,particulars,accountnameid,locationname from (select billnumber AS docno,fxamount,transactiondate,patientcode,patientname,visitcode,'Invoice' as particulars,accountnameid,locationname from master_transactionpaylater  where   accountnameano in($subtypes_accountnameanos_str) and accountnameid in($subtypes_accountids_str)  and transactiontype = 'finalize' and transactiondate between '$ADate1' and '$ADate2' and fxamount <>'0' and billnumber not like 'AOP%' and $pass_location
	  
      UNION ALL select billnumber AS docno,transactionamount as fxamount,transactiondate,patientcode,patientname,visitcode,'Opening Balance'particulars,accountnameid,locationname from master_transactionpaylater where accountnameano in($subtypes_accountnameanos_str) and  accountnameid in($subtypes_accountids_str) and transactiontype = 'finalize' and transactiondate between '$ADate1' and '$ADate2' and billnumber like 'AOP%' and $pass_location
	  
      UNION ALL SELECT b.`docno` as docno,  (-1*b.`transactionamount`) as fxamount, b.`transactiondate` as transactiondate,b.patientcode,b.patientname,b.visitcode,b.particulars,b.accountnameid,b.locationname  FROM `master_transactionpaylater` as a JOIN `master_transactionpaylater` as b ON (a.`visitcode` = b.`visitcode`) WHERE b.`accountnameid` in($subtypes_accountids_str) and a.`transactiontype` = 'finalize' and b.`transactiontype` = 'pharmacycredit' and b.`auto_number` > a.`auto_number` AND b.`transactiondate` BETWEEN '$ADate1' AND '$ADate2' and b.billnumber!='' and a.$pass_location
      
	  UNION ALL SELECT docno  as docno, (-1*`fxamount`) as fxamount, `transactiondate` as transactiondate,patientcode,patientname,visitcode,particulars,accountnameid,locationname from master_transactionpaylater where  accountnameano in($subtypes_accountnameanos_str) and paymenttype in ($paymentTypes_names_str) and accountnameid in($subtypes_accountids_str) and transactiontype = 'paylatercredit' and recordstatus <> 'deallocated' and transactiondate between '$ADate1' and '$ADate2'  and $pass_location
      
	  UNION ALL SELECT `docno` as docno, (-1*`fxamount`) as fxamount, `transactiondate` as transactiondate,accountnameid as patientcode, accountname as patientname,'' as visitcode,particulars,accountnameid,locationname  FROM `master_transactionpaylater` WHERE `accountnameid` in($subtypes_accountids_str) AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `docno` LIKE 'AR-%' AND `transactionstatus` = 'onaccount' AND `transactionmodule` = 'PAYMENT' and $pass_location

      UNION ALL SELECT `docno` as docno, (1*`fxamount`) as fxamount, `transactiondate` as transactiondate,patientcode,patientname,visitcode,particulars,accountnameid,locationname from master_transactionpaylater where  accountnameano in($subtypes_accountnameanos_str) and paymenttype in($paymentTypes_names_str)  and accountnameid in($subtypes_accountids_str) and transactiontype = 'paylaterdebit' and recordstatus <> 'deallocated' and transactiondate between '$ADate1' and '$ADate2' and $pass_location

      
      UNION ALL SELECT `docno` as docno, sum(debitamount-creditamount) as fxamount, `entrydate` as transactiondate ,ledgerid as patientcode, ledgername as patientname,'' as visitcode,narration as particulars,ledgerid as accountnameid,locationname   FROM `master_journalentries` WHERE `ledgerid` in($subtypes_accountids_str) AND `entrydate` BETWEEN '$ADate1' AND '$ADate2' and $pass_location group by docno) as t order by t.transactiondate asc";
...
//AND accountnameid in($subtypes_accountids_str) 
				// AND  b.transactiondate BETWEEN '$ADate1' and '$ADate2'
								 // AND  b.transactiondate BETWEEN '$ADate1' and '$ADate2'
								// AND b.transactiondate BETWEEN '$ADate1' and '$ADate2'
								// AND  b.transactiondate   BETWEEN '$ADate1' and '$ADate2'
				// 				$query98 = "SELECT sum(a.fxamount +a.discount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where a.transactiontype = 'PAYMENT' AND a.transactionstatus!= 'onaccount' AND b.transactionstatus= 'onaccount' and a.billnumber='$res2billnumber' and a.recordstatus = 'allocated' and a.accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND a.accountnameid='$accntid' 
	
				// UNION ALL  select sum(a.fxamount +a.discount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where a.transactionmode = 'CREDIT NOTE' AND a.transactiontype= 'PAYMENT' AND b.transactiontype= 'paylatercredit' and a.billnumber='$res2billnumber' and a.recordstatus = 'allocated' and a.accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND a.accountnameid='$accntid' AND  b.transactiondate BETWEEN '$ADate1' and '$ADate2'
	
				// UNION ALL  select sum(-1*a.fxamount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where  a.transactiontype = 'PAYMENT' AND b.transactiontype != 'PAYMENT' and a.docno='$res2billnumber' and a.recordstatus = 'allocated'  and a.accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND a.accountnameid='$accntid' AND a.docno NOT LIKE '%AR%' AND b.transactiondate BETWEEN '$ADate1' and '$ADate2'
	...
	$query98 = "SELECT sum(a.fxamount +a.discount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where a.transactiontype = 'PAYMENT' AND a.transactionstatus!= 'onaccount' AND b.transactionstatus= 'onaccount' and a.billnumber='$res2billnumber' and a.recordstatus = 'allocated' AND a.accountnameid='$accntid'  and a.$pass_location
	
				UNION ALL  select sum(a.fxamount +a.discount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where a.transactionmode = 'CREDIT NOTE' AND a.transactiontype= 'PAYMENT' AND b.transactiontype= 'paylatercredit' and a.billnumber='$res2billnumber' and a.recordstatus = 'allocated'   AND a.accountnameid='$accntid' and a.$pass_location
	
				UNION ALL  select sum(-1*a.fxamount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where  a.transactiontype = 'PAYMENT' AND b.transactiontype != 'PAYMENT' and a.docno='$res2billnumber' and a.recordstatus = 'allocated'  AND a.accountnameid='$accntid' AND a.docno NOT LIKE '%AR%'  and a.$pass_location
				
				UNION ALL select sum(-1*a.fxamount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno)  where  b.transactionstatus = 'onaccount' AND a.transactionstatus != 'onaccount' and a.docno='$res2billnumber' and a.recordstatus = 'allocated' AND  a.docno  LIKE '%AR%'  and a.accountnameid in ($accntid) and a.$pass_location   ";			
...
$pendig_final_value_outstanding = 0;
      ...
      ?>
        <tr  class="acc<?=$res21accountnameano?>" >
          ...
          <td class="bodytext31" valign="center"  align="left"><?=$res2locationname;?></td>
          ...
          <td class="bodytext31" valign="center"  align="left"><?php echo date('Y-m-d',strtotime($res2transactiondate)); ?></td>
          ...
          <td class="bodytext31" valign="center"  align="right" 
          ><?php echo number_format($balanceamount,2,'.',','); ?></td>
          <!-- <td class="bodytext31" valign="center"  align="right"
    ...
    $totalbalamount += $accountbalamount;
      ...
      ?>
       ...
       <td class="bodytext31" valign="center"  align="left"></td>
                <td class="bodytext31" valign="center" colspan="5"  align="right" 
                >Total:</td>
                <td class="bodytext31" valign="center"  align="right" 
                ><?php echo number_format($closetotalamount301,2,'.',','); ?></td>
                <td class="bodytext31" valign="center"  align="right" 
                ><?php echo number_format($accountbalamount,2,'.',','); ?></td>
                 <td class="bodytext31" valign="center"  align="right" 
      ...
      ?>
           ...
           <!-- <tr onClick="showsub(<?=$subtypeanum?>)">
              ...
              bgcolor="#ecf0f5"><strong>Totalss:</strong></td>
              <td class="bodytext31" valign="center"  align="right" 
              bgcolor="#ecf0f5"><strong><?php echo number_format($totalamount301,2,'.',','); ?></strong></td>
              <td class="bodytext31" valign="center"  align="right" 
              bgcolor="#ecf0f5"><strong><?php echo number_format($totalbalamount,2,'.',','); ?></strong></td>
