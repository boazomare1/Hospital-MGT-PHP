...
$totalpending = 0;
	 ...
	 // mysql_query("insert into master_stock(itemname,itemcode,quantity,batchnumber,rateperunit,totalrate,companyanum,transactionmodule,transactionparticular)values('$medicine','$itemcode','$quantity',' $batch','$rate','$amount','$companyanum','SALES','BY SALES (BILL NO: $billnumber )')");
	   ...
	   {   
...

		   {
				...
				//	echo $bat_quantity.','.$itemcode.'<br>';
...

					{
						...
						mysqli_query($GLOBALS["___mysqli_ston"], "update master_consultationpharmissue set quantity='$pending',docnumber='$docnumber' where medicinecode='$itemcode' and patientvisitcode='$visitcode' and consultation_id='$consid1'") or die(mysqli_error($GLOBALS["___mysqli_ston"]));

						mysqli_query($GLOBALS["___mysqli_ston"], "update master_consultationpharm set medicineissue='$issuestatus',docnumber='$docnumber' where medicinecode='$itemcode' and patientvisitcode='$visitcode'");
		...
		mysqli_query($GLOBALS["___mysqli_ston"], "update master_consultationpharm set medicineissue='completed',docnumber='$docnumber' where medicinecode='$itemcode' and patientvisitcode='$visitcode'");
...

{
			...
			$query61 = "select * from master_consultationpharmissue where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and recordstatus <>'deleted' and quantity <> '$zero' and paymentstatus='completed' and amendstatus='2'";
...
echo $query77 = "select itemcode,batchnumber,batch_quantity,description,fifo_code,expirydate  from (select a.itemcode as itemcode,a.batchnumber as batchnumber,a.batch_quantity as batch_quantity,a.description as description,a.fifo_code as fifo_code,b.expirydate as expirydate  from transaction_stock a join materialreceiptnote_details b on (a.batchnumber = b.batchnumber and a.itemcode = b.itemcode) where a.storecode='".$storecode."' AND a.locationcode='".$locationcode."' AND a.itemcode = '$itemcode' and a.batch_stockstatus='1' and b.expirydate >now()
...
echo $currentstock = $exec77["batch_quantity"];
...

{
 ...
 ?>
        ...
        <input type="checkbox" name="ref[]" id="ref<?php echo $sno; ?>" checked value="<?php echo $sno; ?>"/>
