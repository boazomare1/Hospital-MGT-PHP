...
{
	var oTextbox = new AutoSuggestControl(document.getElementById("itemname"), new StateSuggestions());        
}
...
if(document.getElementById("movingitemselection").value == '1')
{
alert("Please Select Moving Item");
document.getElementById("movingitemselection").focus();
return false;
}
}
    ...
    <td valign="top"><table width="98%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td>
			...
			<form name="stockinward" action="reportfastslowmovingitems1.php" method="post" onKeyDown="return disableEnterKey()" onSubmit="return Locationcheck()">
	<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
            bordercolor="#666666" cellspacing="0" cellpadding="4" width="800" 
            align="left" border="0">
      <tbody id="foo">
        <tr>
          <td colspan="5" bgcolor="#ecf0f5" class="bodytext31"><strong>Stock - Report By Fast/Slow Moving Items</strong></td>
          </tr>
        <tr>
...
if (isset($_REQUEST["categoryname"])) { $categoryname = $_REQUEST["categoryname"]; } else { $categoryname = ""; }
	$location=isset($_REQUEST['location'])?$_REQUEST['location']:'';
			...
			if (isset($_REQUEST["categoryname"])) { $categoryname = $_REQUEST["categoryname"]; } else { $categoryname = ""; }
if (isset($_REQUEST["medicinecode"])) { $itemcode = $_REQUEST["medicinecode"]; } else { $itemcode = ""; }
if (isset($_REQUEST["ADate1"])) { $ADate1 = $_REQUEST["ADate1"]; } else { $ADate1 = ""; }
			//$categoryname = $_REQUEST['categoryname'];
			...
			if($itemcode!='')
			{
			 $query3 = "select itemcode, sum(transaction_quantity) as totalsales from transaction_stock where itemcode = '$itemcode' and recordstatus <> 'DELETED' and locationcode='$location' and storecode ='$store' and transaction_date between '$ADate1' and '$ADate2' and transactionfunction='0' and description='Sales' group by itemcode order by totalsales $movingitemquery";// itemcode = '$itemcode' and";
			}
			else
			{
			 $query3 = "select itemcode, sum(transaction_quantity) as totalsales from transaction_stock where recordstatus <> 'DELETED' and locationcode='$location' and storecode ='$store' and transaction_date between '$ADate1' and '$ADate2' and transactionfunction='0' and description='Sales' group by itemcode order by totalsales $movingitemquery";// itemcode = '$itemcode' and";
			}
			$exec3 = mysqli_query($GLOBALS["___mysqli_ston"], $query3) or die ("Error in Query3".mysqli_error($GLOBALS["___mysqli_ston"]));
			while ($res3 = mysqli_fetch_array($exec3))
			{
			$itemcode = $res3['itemcode'];
			$totalsales = $res3['totalsales'];
			
			$res3sumquantity=$totalsales;
			
			
			
			
			$query31 = "select itemcode, sum(transaction_quantity) as purchasequantity from transaction_stock where itemcode = '$itemcode' and recordstatus <> 'DELETED' and locationcode='$location' and storecode ='$store' and transaction_date between '$ADate1' and '$ADate2' and transactionfunction='1'";
			$exec31 = mysqli_query($GLOBALS["___mysqli_ston"], $query31) or die ("Error in Query31".mysqli_error($GLOBALS["___mysqli_ston"]));
			$res31 = mysqli_fetch_array($exec31);
			$purchasequantity = $res31['purchasequantity']; //Value called for purchase calc.
			
			$query1 = "select sum(transaction_quantity) as totalphrmacyminus from transaction_stock where itemcode = '$itemcode' and recordstatus <> 'DELETED' and locationcode='$location' and storecode ='$store' and transaction_date between '$ADate1' and '$ADate2' and transactionfunction='0' and description!='Sales'";
			$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
			$res1 = mysqli_fetch_array($exec1);
			$totalphrmacyminus = $res1['totalphrmacyminus'];
			
			$availablestock=$purchasequantity-($totalsales+$totalphrmacyminus);
			...
			$query2 = "select * from master_itempharmacy where itemcode = '$itemcode' and status <> 'DELETED'";
