...
<a href="emr2.php?patientcode=<?php echo $patientcode?>&&visitcode=<?php echo $visitcode?>"><?php echo $patientcode; ?></a>
  ...
  $query31="select * from resultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and publishstatus = 'completed' and itemcode LIKE '%$itemcode%' and date(recorddate) between '$fromdate' and '$todate' group by itemcode";
			   ...
			   $itemcode2=$res52['itemcode'];
				...
				$labitemanum = $res52['auto_number'];
				 $referencecomments = $res52['referencecomments'];
				  ...
				  $query43 = "select * from resultentry_lab where sampleid='$sampleid' and itemcode = '$itemcode2' and referencename = '$labreferencename' and date(recorddate) between '$fromdate' and '$todate' order by auto_number desc";
				  $exec43 = mysqli_query($GLOBALS["___mysqli_ston"], $query43) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
				  if($type == 'IP')
				  {
					 $query43 = "select * from ipresultentry_lab where sampleid='$sampleid' and itemcode = '$itemcode2' and referencename = '$labreferencename' and date(recorddate) between '$fromdate' and '$todate' order by auto_number desc";
				  $exec43 = mysqli_query($GLOBALS["___mysqli_ston"], $query43) or die(mysqli_error($GLOBALS["___mysqli_ston"])); 
				  }
