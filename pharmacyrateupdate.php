...
$query6 = "select pharmacyformula from master_company where auto_number = '$companyanum'";
            ...
            try {
				...
				foreach($rowData as $key=>$value)
			    {
					if($rowData[$key] == 'Item Code')
					 ...
					 if($rowData[$key] == 'Template ID')
					  $tempidnm = $key;

					 if($rowData[$key] == 'Markup')
					  $markupnm = $key;

					 if($rowData[$key] == 'Fixed Price')
					  $salespricenm = $key;
				...
				for ($row = 2; $row <= $highestRow; $row++){ 
					...
					if($tempid>0){
						if(isset($markupnm)){
							$markup=intval($rowData[$markupnm]);
							$newrate=$markup = str_replace( ',', '', $markup );
							$newrate=$markup = str_replace( '%', '', $markup );
							$query01 = "SELECT purchaseprice from master_medicine WHERE itemcode='$itemcode'";
							...
							$rate_perc = @(($markup/100)* $pprice);
							$new_rate = ($pprice + $rate_perc);
							$new_rate = ceil(number_format($new_rate,2,'.',''));
							$query013 = "SELECT margin as chkval from pharma_template_map WHERE productcode='$itemcode' and templateid='$tempid'order by auto_number desc limit 0,1";
						    $exec013 = mysqli_query($GLOBALS["___mysqli_ston"], $query013);
							$res013 = mysqli_fetch_assoc($exec013);
							$chkval=$res013['chkval'];
							if($chkval=='')
								$chkval=0;


						}elseif(isset($salespricenm)){
							$markup=0;
							$new_rate=$rowData[$salespricenm];
							$new_rate = str_replace( ',', '', $new_rate );
							$newrate=$new_rate = ceil(number_format($new_rate,2,'.',''));
							$query013 = "SELECT rate as chkval from pharma_template_map WHERE productcode='$itemcode' and templateid='$tempid' order by auto_number desc limit 0,1";
						    $exec013 = mysqli_query($GLOBALS["___mysqli_ston"], $query013);
							$res013 = mysqli_fetch_assoc($exec013);	
							$chkval=round($res013['chkval'], 2);
						}
                        ...
                        if($chkval==$newrate){
							...
							while ($res01 = mysqli_fetch_assoc($exec01)){
								...
								$query1 = "INSERT INTO pharma_template_map(templateid, productcode, rate, username,dateadded, recordstatus,margin,ipaddress) VALUES ('$tempid','$itemcode','$new_rate','$username','$updatedatetime','','$markup','$ipaddress')";
								$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query".mysqli_error($GLOBALS["___mysqli_ston"]));

								$query3 = "UPDATE master_medicine SET markup = '$markup' WHERE itemcode = '$itemcode'";
								...
								if(is_array($arr)){
									foreach ($arr as $key => $value) {
										//echo $value.'<br>';
										$col = 'subtype_'.$value;
										$query32 = "UPDATE master_medicine SET $col = '$new_rate' WHERE itemcode = '$itemcode'";
										//echo $query3;
		...
		if($count > 0){
	        foreach($_REQUEST['surveychk'] as $key=>$value){
                ...
                foreach ($obj_ids as $k => $v) {
					...
					$updaterate = str_replace( ',', '', $updaterate );
                    //echo $itemmargin;
...
$('#search1').autocomplete({
  ...
  source:'ajax_master_medicine.php', 
  //alert(source);
  minLength:3,
  delay: 0,
  html: true, 
    select: function(event,ui){
      ...
      $('#itemcode').val(servicescode);
        $('#search1').val(varservicesname);
		...
		//for (var i = 0; i < idsLength; i++) {
		   ...
		   // alert(cost);
		    ...
		    if(itemmargin >=0){
                ...
                var num = Number(Math.ceil(newcost+'e2')+'e-2');
                var newnum = num.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');

                document.getElementById("rate"+tempcode+autono).value = Math.ceil(newcost);
	  ...
	  if (document.form2.upload_file.value == "")
		{
			...
			document.form2.upload_file.focus();
			return false;
		}

      if(!confirm('Do you want to upload the rates?'))
		  return false;
	  else
		  return true;
	}
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tablebackgroundcolor1">
						<tr>
							<td>
								...
								<form name="form2" id="form2" method="post" action="pharmacyrateupdate.php" enctype="multipart/form-data">
								...
								<tbody>
								<tr bgcolor="#011E6A">
									<th colspan="2" bgcolor="#ecf0f5" class="bodytext3">
								...
								<tr bgcolor="#011E6A">
									...
									<td colspan="5" bgcolor="#FFFFFF" class="bodytext3">
										...
										<input name="itemcode" type="hidden" id="itemcode"  value="">
									...
									<td colspan="<?php echo $colspan-5;?>" bgcolor="#FFFFFF" class="bodytext3">
										
										<input type="file" name="upload_file" id="upload_file"> &nbsp;						
						                <input type="submit" name="uploadexcel" id="uploadexcel"   value="Upload Excel" onclick="return chkupload();">
										
									</td>
								</tr><!-- -->
								<tr bgcolor="#011E6A">
									<th width="10%" bgcolor="#ecf0f5" align="center" class="bodytext3">
									...
									<th width="10%" bgcolor="#ecf0f5" class="bodytext3">
										<strong>ID / Code </strong>
									</th>
									<th width="15%" bgcolor="#ecf0f5" class="bodytext3">
										<strong>Category</strong>
									</th>
									<th width="35%" bgcolor="#ecf0f5" class="bodytext3">
										<strong>Pharmacy Item</strong>
									</th>
									<th width="25%" align="right" bgcolor="#ecf0f5" class="bodytext3">
										<strong>Cost Price</strong>
									</th>
									<!-- rate templates headers -->
									<?php 
									  $query_rt = "SELECT templatename,markup,auto_number FROM pharma_rate_template ORDER BY auto_number ASC";
									  $exec_rt = mysqli_query($GLOBALS["___mysqli_ston"], $query_rt) or die ("Error in Query_rt".mysqli_error($GLOBALS["___mysqli_ston"]));
										while ($res_rt = mysqli_fetch_array($exec_rt))
										{
											...
											$col_temp_id = $res_rt['auto_number'];
									?>
									<th colspan="2" align="center" width="20%" bgcolor="#ecf0f5" class="bodytext3">
										<strong> <a href="download_pharmacyrates.php?tmp=<?=$col_temp_id?>&type=<?=$pharmacyformula?>" target="_blank"> <?php echo ucfirst($col_rt);?></a> <?php //if($col_markup != '0'){echo '('.$col_markup.'%)'; }?></strong>  
									</th>
								    <?php } ?>
									<th width="5%" bgcolor="#ecf0f5" align="center" class="bodytext3">
										<div align="center">
								...
								if ($searchflag1 == 'searchflag1'){
									...
									if($itemcode!='')
									{
									$query1 = "select * from master_medicine where  itemcode='$itemcode' and  status <> 'deleted' order by auto_number asc ";
									}else{
								...
								<th align="center"  valign="middle"  class="bodytext3">
									...
									<th align="left" valign="middle" class="bodytext3">
										<div align="center"><?php echo $cnt; ?></div>
									</th>
									<th align="left" valign="middle" class="bodytext3"><?php echo $itemcode; ?> </th>
									<th align="left" valign="middle" class="bodytext3"><?php echo $categoryname; ?> </th>
									<th align="left" valign="middle" class="bodytext3"><?php echo $itemname; ?> </th>
									<th align="right" valign="middle"  class="bodytext3">
								    ...
								    <?php 
								       ...
								       /* $arr11 = array();
								        //query
								        $query_rt21 = "SELECT auto_number FROM pharma_rate_template ORDER BY auto_number ASC";
									    $exec_rt21 = mysql_query($query_rt21) or die ("Error in Query_rt".mysql_error());
										while ($res_rt21 = mysql_fetch_assoc($exec_rt21))
										{   
											$tempid= $res_rt21['auto_number'];
											array_push($arr11, $tempid);
										}
										$tempids = json_encode($arr11); */
										//echo $tempids;
								    ?>
								    <td align="left" valign="middle" class="bodytext3">
								    	<input type="hidden" name="itemcost<?php echo $tempid; ?><?php echo $auto_number; ?>" id="itemcost<?php echo $tempid; ?><?php echo $auto_number; ?>" value="<?php echo $cost; ?>" />
									    <div align="center">
											<input type="text" min="0" name="itemmargin<?php echo $tempid; ?><?php echo $auto_number; ?>" id="itemmargin<?php echo $tempid; ?><?php echo $auto_number; ?>" style="width: 40px; text-align: right; <?php if($margin <= 0){ echo 'background-color: #d1d1d1;border-width:1px;border-color:#d1d1d1;display: none;';} ?>" value="<?php echo $item_margin; ?>" size="5" <?php if($margin <= 0){ echo 'readonly="readonly"';}?> onkeyup='updateRateItem(this,<?php echo $auto_number; ?>,<?php echo $tempid; ?>)'>
										</div>
									</td>
								    <td align="left" valign="middle" class="bodytext3">
									    ...
									    <div align="center">
											<input type="text" name="rate<?php echo $tempid; ?><?php echo $auto_number; ?>" id="rate<?php echo $tempid; ?><?php echo $auto_number; ?>" style="text-align: right; <?php if($margin > 0){ echo 'background-color: #d1d1d1;border-width:1px;border-color:#d1d1d1;';} ?>" value="<?php echo $rate; ?>" size="5" <?php if($margin > 0){ echo 'readonly="readonly"';}?> >
										</div>
									</th>
									 <?php } ?>
									 <?php 
									...
									<td align="center"  valign="middle"  class="bodytext3">
										<span onClick='updaterate(this,<?php echo $auto_number; ?>,<?php echo $tempids; ?>)' class="btn"> Update </span>
									</td> 		
								</tr>
								<?php
									}
							...
							<tr <?php echo $colorcode; ?>>
								...
								<!-- pharmacy template rate-->
									...
									<td align="left" valign="middle" class="bodytext3">
								    	<input type="hidden" name="tempcode<?php echo $auto_number; ?><?php echo $itemcode; ?><?php echo $tempid; ?>" id="tempcode<?php echo $auto_number; ?><?php echo $itemcode; ?><?php echo $tempid; ?>" value="<?php echo $tempid; ?>" />
									    <div align="center">
											<input type="text" min="0" name="rate<?php echo $tempid; ?><?php echo $auto_number; ?>" id="rate<?php echo $tempid; ?><?php echo $auto_number; ?>" style="width: 100px; text-align: right; <?php if($margin > 0){ echo 'background-color: #d1d1d1;border-width:1px;border-color:#d1d1d1;';} ?>" value="<?php echo $rate; ?>" size="5" <?php if($margin > 0){ echo 'readonly="readonly"';}?> >
										</div>
									</td>
								 <?php } ?>
								 <?php 
							...
							<tr>
								<?php 
									  ...
									  $colspan = 8 + ($num_rows * 2);
								 ?>
								<td colspan="<?php echo $colspan;?>" align="center" valign="middle" bgcolor="#FFFFFF" class="bodytext3" >
