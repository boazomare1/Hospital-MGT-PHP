...
<tr>
<td colspan="12" align="center" valign="middle"><strong>OP CGS</strong></td>
</tr>
...
<?php 
$query8 = "SELECT * FROM `billing_consultation` WHERE `billdate` BETWEEN '$ADate1' AND '$ADate2' order by auto_number";
	...
	if($parentid != '' && $tbinclude != '')
	{
		...
		{  
			...
			$querydr1 = "SELECT SUM(a.`totalcp`) as stockpurchase FROM `pharmacysales_details` AS a JOIN master_medicine AS b ON (a.itemcode = b.itemcode) WHERE b.incomeledgercode = '$id' AND a.`entrydate` BETWEEN '$ADate1' AND '$ADate2'
						 UNION ALL SELECT SUM(a.`amount`) as stockpurchase FROM `master_stock_transfer` AS a WHERE a.tostore = '$id' AND a.tostore NOT LIKE 'STO%' AND a.`entrydate` BETWEEN '$ADate1' AND '$ADate2'
						 UNION ALL SELECT SUM(a.`totalcp`) as stockpurchase FROM `pharmacysalesreturn_details` AS a JOIN master_medicine AS b ON (a.itemcode = b.itemcode) WHERE b.inventoryledgercode = '$id' AND a.`entrydate` BETWEEN '$ADate1' AND '$ADate2'";
			...
			$j = 0;
			$crresult = array();
			$querycr1inv = "SELECT SUM(a.`totalcp`) as stockcredit FROM `pharmacysalesreturn_details` AS a JOIN master_medicine AS b ON (a.itemcode = b.itemcode) WHERE b.incomeledgercode = '$id' AND a.`entrydate` BETWEEN '$ADate1' AND '$ADate2'
			...
			$purchase = array_sum($result) - array_sum($crresult);
