// Amend Pending Referral Modern JavaScript - Following Hospital Theme

let allReferralRecords = [];
let filteredReferralRecords = [];
let currentPage = 1;
...
function handleSearch(event) {
    ...
    window.location.href = 'amend_pending_referral.php?' + searchParams.toString();
}
...
function handleLiveSearch(event) {
    ...
    if (searchTerm.length < 2) {
        ...
        filteredReferralRecords = [...allReferralRecords];
        updateResultsDisplay();
        return;
    }
    
    // Filter records based on search term
    filteredReferralRecords = allReferralRecords.filter(record => {
        return (
...
function updatePagination() {
    ...
    const totalPages = Math.ceil(filteredReferralRecords.length / itemsPerPage);
...
function changePage(page) {
    if (page < 1 || page > Math.ceil(filteredReferralRecords.length / itemsPerPage)) {
        return;
    }
...
function updateResultsDisplay() {
    ...
    const currentRecords = filteredReferralRecords.slice(startIndex, endIndex);
...
function exportToCSV() {
    ...
    if (link.download !== undefined) {
        ...
        link.setAttribute('download', 'pending_referrals.csv');
...
function viewReferralDetails(patientcode, visitcode) {
    showAlert(`Viewing referral details: ${patientcode} - ${visitcode}`, 'info');
    
    // This would normally open a modal or navigate to a view page
    console.log('Viewing referral details:', patientcode, visitcode);
}
