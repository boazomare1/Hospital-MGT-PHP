...
if ($upload == 'success')
{
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 1 && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			$itemname_abbreviation = $splitdelimiter[3];
			$itemname_abbreviation = strtoupper($itemname_abbreviation);
			$rateperunit = $splitdelimiter[4];
			$rateperunit = strtoupper($rateperunit);
			$rate2 = $splitdelimiter[5];
			...
			if ($itemcode != '')
			{
			$query2 = "insert into master_radiology (itemcode,itemname,categoryname,itemname_abbreviation,rateperunit,rate2,taxname) values ('$itemcode','$itemname','$categoryname','$itemname_abbreviation','$rateperunit','$rate2','$taxname')";
...
header("location:radiologydataimport1.php?st=success");
}
?>
<style type="text/css">
<!--
