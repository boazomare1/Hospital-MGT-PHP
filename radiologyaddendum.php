...
<?php
if (isset($_REQUEST["viewstatus"])) { $viewstatus = $_REQUEST["viewstatus"]; } else { $errcode = ""; $viewstatus ='';}
...
$getdata = addslashes($_REQUEST["getdata"]);
	    $query26="update resultentry_radiology set addendum = '".$getdata."',addendum_by = '$username', addendum_at = '$updatedatetime' where patientcode='$patientcode' and patientvisitcode='$visitcode' and docnumber='$docnumber' ";
	    $exec26=mysqli_query($GLOBALS["___mysqli_ston"], $query26) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
...
<body>
<form name="frm" id="frmsales" method="post" action="radiologyaddendum.php" onKeyDown="return disableEnterKey(event)">
