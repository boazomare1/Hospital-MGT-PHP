...
$query57 = "select itemcode,batchnumber,batch_quantity,description,fifo_code from (select a.itemcode as itemcode,a.batchnumber as batchnumber,a.batch_quantity as batch_quantity,a.description as description,a.fifo_code as fifo_code from transaction_stock a join materialreceiptnote_details b on (a.batchnumber = b.batchnumber and a.itemcode = b.itemcode) where a.storecode='".$storecode."' AND a.locationcode='".$locationcode."' AND a.itemcode = '$itemcode' and a.batch_stockstatus='1' and b.expirydate >now()
			UNION ALL select a.itemcode as itemcode,a.batchnumber as batchnumber,a.batch_quantity as batch_quantity,a.description as description,a.fifo_code as fifo_code from transaction_stock a join purchase_details b on (a.batchnumber = b.batchnumber and a.itemcode = b.itemcode) where a.storecode='".$storecode."' AND a.locationcode='".$locationcode."' AND a.itemcode = '$itemcode' and a.batch_stockstatus='1' and b.expirydate >now()) as batchl where batch_quantity>0";
