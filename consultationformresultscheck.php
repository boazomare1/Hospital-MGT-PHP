...
$bgcolorcode = "";
$pagename = "";
 $query0="select employeename from master_employee where username='$username'";
$exe002=mysqli_query($GLOBALS["___mysqli_ston"], $query0);
$res000=mysqli_fetch_array($exe002);
 $name2=$res000["employeename"];
...
if ($frmflag1 == 'frmflag1')
{
	...
	$timestamp = $_REQUEST['currenttime'];
	 ...
	 $grandtotal11=$_REQUEST["grandtotal1"];
	  $grandtotal1=(int)preg_replace('[,]','',$grandtotal11);
   ...
   $exec222=mysqli_query($GLOBALS["___mysqli_ston"], $query222) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
	...
	else
	{
		$res21 = mysqli_fetch_array($exec21);
		 $consultationcode = $res21['auto_number'];
		 $consultationcode = $consultationcode + 1;
		 /*$consultationcode = substr($consultationcode, 3, 8);
...
$updatedatetime = date('Y-m-d H:i:s');
	...
	if ($res2 == 0)
	{
		...
		$execicdquery = mysqli_query($GLOBALS["___mysqli_ston"], $icdquery1) or die("Error in icdquery1". mysqli_error($GLOBALS["___mysqli_ston"]));
		...
		$execicdquery2 = mysqli_query($GLOBALS["___mysqli_ston"], $icdquery2) or die("Error in icdquery2". mysqli_error($GLOBALS["___mysqli_ston"]));
		}
		}
		}
		$query34 = "select * from master_company where companystatus = 'Active'";
		$exec34= mysqli_query($GLOBALS["___mysqli_ston"], $query34) or die ("Error in Query3".mysqli_error($GLOBALS["___mysqli_ston"]));
		$res34 = mysqli_fetch_array($exec34);
		...
		if($num65 == 0)
		{
			/*	$default_storecode = 'STO2';
				$query23 = "SELECT b.storecode,b.store,a.defaultstore from master_employeelocation as a join master_store as b on a.storecode=b.auto_number where a.username='$username' and a.locationcode='$locationcodeget' and defaultstore = 'default'";
				$exec23 = mysqli_query($GLOBALS["___mysqli_ston"], $query23) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
				...
				if($num_rows > 0)
				{
				 	...
				 	$res23 = mysqli_fetch_array($exec23);
					$default_storecode = trim($res23['storecode']);
				}
				if(trim($default_storecode)=="")
				{
					$default_storecode = 'STO2';
				}*/
$default_storecode='';				
$querytype = "select type from master_medicine where itemcode='$medicinecode' ";
		...
		/* $query88 = "insert into master_consultation(consultation_id,patientcode,patientauto_number,patientname,patientvisitauto_number,patientvisitcode,recordstatus,recorddate,ipaddress,consultingdoctor,billtype,accountname,paymentstatus,sys,dia,pulse,temp,complaint,drugallergy,foodallergy,consultationtime,request,locationname,locationcode) 
				...
				$exec88 = mysqli_query($GLOBALS["___mysqli_ston"], $query88) or die ("Error in Query88".mysqli_error($GLOBALS["___mysqli_ston"]));*/
					...
					$query93=mysqli_query($GLOBALS["___mysqli_ston"], "update master_visitentry set overallpayment='' where visitcode='$visitcode'") or die(mysqli_error($GLOBALS["___mysqli_ston"])); 
...
$wellnesspkg=$rescheckhc['wellnesspkg'];
		...
		$referalquery2=mysqli_query($GLOBALS["___mysqli_ston"], "update master_visitentry set referalbill='pending' where visitcode='$visitcode'") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
		if($pairvar3 == '0.00')
		{
		if($billingtype == "PAY NOW")
		{
		...
		$newquery=mysqli_query($GLOBALS["___mysqli_ston"], "update master_triage set consultation='completed',urgentstatus='0',complanits='$complaint' where visitcode='$visitcode'") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
...
$exec2221 = mysqli_query($GLOBALS["___mysqli_ston"], $query2221) or die ("Error in Query22221".mysqli_error($GLOBALS["___mysqli_ston"]));
...
$viscode=$_REQUEST['visitcode'];
 ...
 $viscode=""; }
    ...
    $query2 = "select * from master_triage where patientcode = '$patientcode' and visitcode='$viscode' order by auto_number desc";//order by auto_number desc limit 0, 1";
		...
		$quer=mysqli_query($GLOBALS["___mysqli_ston"], "select * from master_location where locationcode='$locationcode'");
...
<!--
<script type="text/javascript" src="js/insertnewitem11_p.js"></script>
<script type="text/javascript" src="js/insertnewitem12_p.js"></script>
<script type="text/javascript" src="js/insertnewitem13_p.js"></script>
<script type="text/javascript" src="js/insertnewitem14_p.js"></script>
<script type="text/javascript" src="js/insertnewitem15_p.js"></script>
...
<script type="text/javascript" src="js/insertnewitem13.js?v=1"></script>
<script type="text/javascript" src="js/insertnewitem14.js?v=1"></script>
<script type="text/javascript" src="js/insertreferrate_new.js"></script>
...
function btnDeleteClick(delID,pharmamount)
{
	...
	var tot4=Number(currenttotal4.replace(/[^0-9\.]+/g,""));
	//alert(currenttotal);
	newtotal4= tot4-pharmamount;
	...
	document.getElementById('total4').value=formatMoney(newgrandtotal4);
...
function btnDeleteClick1(delID1,vrate1)
{
	...
	var current3=document.getElementById('total1').value;
	
	
	var currenttotal3=Number(current3.replace(/[^0-9\.]+/g,""));
	//alert(currenttotal);
...
function btnDeleteClick5(delID5,radrate)
{
	...
	var currenttot=Number(currenttotal2.replace(/[^0-9\.]+/g,""));
	//alert(currenttotal);
	newtotal2= currenttot-radrate;
...
var currenttotal1=document.getElementById('total3').value;
 var currentot = Number(currenttotal1.replace(/[^0-9\.]+/g,""));
	//alert(currenttotal);
	newtotal1= currentot-vrate3;
	
	//alert(newtotal);
	
	document.getElementById('total3').value=formatMoney(newtotal1);
	if(document.getElementById('total').value=='')
	{
	totalamount11=0;
	}
...
function btnDeleteClick4(delID4,vrate4)
{
	...
	var currenttot=Number(currenttotal.replace(/[^0-9\.]+/g,""));
	//alert(currenttotal);
	newtotal= currenttot-vrate4;
...
function sertotal()
{
	...
	var varserr = document.getElementById("rate3").value;
	var varserRates =Number(varserr.replace(/[^0-9\.]+/g,""));
	...
	if (parseInt(varquantityser) > parseInt(varserBaseunit))
	{
		...
		var ref=parseInt(varserRates)+parseInt(resultfinal);
		document.getElementById("serviceamount").value=formatMoney(ref);
	}
	var totalservi = parseFloat(varquantityser) * parseFloat(varserRates);
	document.getElementById("serviceamount").value=formatMoney(totalservi);
}
}
...
var toduty=document.getElementById("ADate6").value;
		...
		alert("Please Select Date Greater Today");
...
var VarRate = document.getElementById("rates").value;
var varr=Number(VarRate.replace(/[^0-9\.]+/g,""));
var ResultAmount = parseFloat(varr * ResultFrequency);
  document.getElementById("amount").value = formatMoney(ResultAmount);
}
}
...
if(document.getElementById("insertrow13").childNodes.length < 2)
	...
	alert("Please Enter the primary disease");
	 ...
	 document.getElementById('Submit222').disabled="";
		return false;
	}
...
<script type="text/javascript" src="js/automedicinecodesearch12_new_consultform.js"></script>
        ...
        <tr>
          <td width="103%"><table   border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
            <tbody>
              <tr bgcolor="#011E6A">
                <td colspan="4" bgcolor="#cccccc" class="style2">Consultation</td>
				 ...
				 <td colspan="4" align="center" valign="middle"  bgcolor="#cccccc" class="bodytext3"><strong><a target="_blank" href="emrresultsviewlist.php?patientcode=<?php echo $patientcode; ?>">Click to go to EMR History</a></strong></td>
                <!--<td colspan="2" bgcolor="#cccccc" class="bodytext3"><?php echo $errmgs; ?>&nbsp;</td>-->
                </tr>
              <tr>
                <td colspan="4" align="left" valign="middle"  bgcolor="<?php if ($errmsg == '') { echo ''; } else { echo '#AAFF00'; } ?>" class="bodytext3"><strong><?php 
				...
				echo $patientfirstname.' '.$patientmiddlename.' '.$patientlastname; ?> , <?php echo $age; ?> , <?php echo $gender; ?></strong></td>
			</td>
			<td class="bodytext3" colspan="2">				
              ...
              <td width="454" rowspan="16"  align="left" valign="middle" bgcolor="#ecf0f5" ><iframe id="preconsultation" src="preconsultationresultsview.php?patientcode=<?php echo $patientcode; ?>&&visitcode=<?php echo $visitcodenum; ?>&&date=<?php echo $consultationdate; ?>&&name=<?php echo $name2;?>" width="450" height="450" frameborder="0"> </iframe></td>
              </tr>
              ...
              <!--<tr>
				...
				<tr>
				 ...
				 <td align="left" valign="middle"  bgcolor="#ecf0f5"><span class="bodytext3"><strong>Account</strong></span></td>
				  ...
				  <tr>
				  <td align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>Subtype</strong></td>
				  <td align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><label><?php echo $res131subtype; ?> </label></td>
				  <td align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">&nbsp;</td>
				  <td align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>Scheme Name</strong></td>
				  <td align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><label><?php echo $scheme_name; ?> </label></td>
				  </tr>
				<tr>
...
-->
				...
				<td colspan="6" align="left" valign="middle"  bgcolor="<?php if ($errmsg == '') { echo ''; } else { echo '#AAFF00'; } ?>" class="bodytext3"><?php echo $errmsg;?>&nbsp;</td>
				 ...
				 <tr bgcolor="">
				  ...
				  <?php
					...
					$exec667 = mysqli_query($GLOBALS["___mysqli_ston"], $query667) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
	...
	while ($res3 = mysql_fetch_array($exec3))
				...
				$netpayment = $totalpayment + $tdsamount21 + $writeoffamount21;
				 ...
				 //  $availablelimit=$availablelimit1+$planfixedamount-($consultationfees);
				   if($patienttype!="PAY NOW")
				   {
					...
					$execbilrad=mysqli_query($GLOBALS["___mysqli_ston"], $querybilrad) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
					...
					$execbilref=mysqli_query($GLOBALS["___mysqli_ston"], $querybilref) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
				  ...
				  $querybilref1="select sum(referalrate) as referalrate from consultation_departmentreferal where patientcode = '$patientcode' and patientvisitcode='$viscode' and paymentstatus = 'completed' and referalrefund <> 'refund'";
					$execbilref1=mysqli_query($GLOBALS["___mysqli_ston"], $querybilref1) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
				...
				$serrate=0;
				  $querybilser="select servicesitemrate*(serviceqty-refundquantity) as servicerate from consultation_services where patientcode = '$patientcode' and patientvisitcode='$viscode' and paymentstatus = 'completed' and approvalstatus <> '2' ";
					$execbilser=mysqli_query($GLOBALS["___mysqli_ston"], $querybilser) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
				  ...
				  $querybillab="select sum(labitemrate) as labrate from consultation_lab where patientcode = '$patientcode' and patientvisitcode='$viscode' and paymentstatus = 'completed' and approvalstatus <> '2' and labrefund <> 'refund'";
					$execbillab=mysqli_query($GLOBALS["___mysqli_ston"], $querybillab) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
				...
				/*
				  $querybilpharm="select sum(mcp.amount) as pharmrate,sum(prd.totalamount) as refundpharm from master_consultationpharm AS mcp LEFT JOIN  pharmacysalesreturn_details AS prd ON mcp.medicinecode=prd.itemcode AND mcp.patientvisitcode = prd.visitcode where mcp.patientcode = '$patientcode' and mcp.patientvisitcode='$viscode' and mcp.paymentstatus = 'completed' and mcp.approvalstatus <> '2' ";
					...
					$pharmrefund=0;
					 $querybilpharm="select amount as pharmrate,medicinecode from master_consultationpharm  where patientcode = '$patientcode' and  patientvisitcode='$viscode'  and  approvalstatus <> '2' ";
					$execbilpharm=mysqli_query($GLOBALS["___mysqli_ston"], $querybilpharm) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
					...
					$medicinecode=$resbilpharm['medicinecode'];
					 ...
					 $execbilpharm1=mysqli_query($GLOBALS["___mysqli_ston"], $querybilpharm1) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
				...
				/* if($billedrate >= $availablelimit)
				 ...
				 {*/
				  if($visitlimit1 != 0)
					{
					    $visitlimit1 = $visitlimit1+$planfixedamount-$billedrate - $consultationfees;
					}
					else
					{
					    $visitlimit1 = 0;
				...
				// mysql_num_rows($execbilrad);
				 ...
				 <tr>
				   <td colspan="11" align="left" valign="middle"  bgcolor="#cccccc" class="style2"  onDblClick="return funcHideView()"  onClick="return funcShowView()">
				   Consultation 
				 ...
				 <tr>
				   <td colspan="11" align="left" valign="middle"  bgcolor="#cccccc" class="bodytext3" onDblClick="return funcpresHideView()" onClick="return funcpresShowView()"><strong>Prescription</strong> <span class="bodytext32"> <img src="images/plus1.gif" width="13" height="13"   onDblClick="return funcpresHideView()" onClick="return funcpresShowView()"> </span></td>
			      </tr>
				 <tr id="pressid">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">
				   <table id="presid" width="621" border="0" cellspacing="1" cellpadding="1">
                     <tr>
                       ...
                       <td width="30" class="bodytext3">Generic</td>
                       <td width="200" class="bodytext3">Medicine Name</td>
                       <td width="38" class="bodytext3">Stk</td>
                       <td width="28" class="bodytext3">Dose</td>
					  ...
					  <input type="hidden" name="medicinecode" id="medicinecode" value="">
			           ...
			           <input name="searchmedicineanum1" id="searchmedicineanum1" value="" type="hidden">
						<input name="hiddenmedicinename" id="hiddenmedicinename" value="" type="hidden">
                          ...
                          <td><input name="generic" type="checkbox" id="generic"/> </td>
                       <td><input name="medicinename" type="text" id="medicinename" size="40" autocomplete="off" onKeyUp="medicinesearch()">					   </td>
                        <td>
				...
				?>
				 ...
				 <tr>
				   <td colspan="11" align="left" valign="middle"  bgcolor="#cccccc" class="bodytext3" onDblClick="return funcLabHideView()"  onClick="return funcLabShowView()"><span class="bodytext32"><span class="style2">Lab <img src="images/plus1.gif" width="13" height="13" onDblClick="return funcLabHideView()"  onClick="return funcLabShowView()"> </span></span></td>
			      </tr>
				 ...
				 <tr>
				   <td colspan="8" align="right" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><span class="style2">Total</span><input type="text" id="total1" readonly size="7"></td>
				  </tr> 
		        
				 <tr>
				   <td colspan="11" align="left" valign="middle"  bgcolor="#cccccc" class="bodytext3"  onDblClick="return funcRadHideView()"  onClick="return funcRadShowView()"><span class="bodytext32"><span class="style2">Radiology <img src="images/plus1.gif" width="13" height="13" onDblClick="return funcRadHideView()"  onClick="return funcRadShowView()"> </span></span></td>
		        </tr>
				 <tr id="radid">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">
				    <table id="presid" width="621" border="0" cellspacing="1" cellpadding="1">
				   ...
				   <td width="30"> <input name="radiology[]" id="radiology" type="text" size="69" autocomplete="off"></td>
				     <td><input name="radiologyinstructions[]" type="text" id="radiologyinstructions" size="20"></td>
				      <td width="30"><input name="rate8[]" type="text" id="rate8" readonly size="8"></td>
                       ...
                       </label></td> <td align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext32"><div align="center">&nbsp;</div></td>
					   <td align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext32"><div align="center">Urgent</div></td>
					   <td height="28" colspan="7" align="left" valign="middle"  bgcolor="#ecf0f5">
				 ...
				 <tr>
				   <td colspan="8" align="right" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><span class="style2">Total</span><input type="text" id="total2" readonly size="7"></td>
				   </tr>
		       
				
				 <tr>
				   <td colspan="11" align="left" valign="middle"  bgcolor="#cccccc" class="bodytext3" onDblClick="return funcSerHideView()" onClick="return funcSerShowView()"><span class="bodytext32"><span class="style2">Service, Procedures and Package Billing <img src="images/plus1.gif" width="13" height="13" onDblClick="return funcSerHideView()" onClick="return funcSerShowView()"> </span></span></td>
		        </tr>
				 <tr id="serid">
				   <td colspan="11" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">
				    <table id="presid" width="621" border="0" cellspacing="1" cellpadding="1">
				   ...
				   <td width="30"><input name="services[]" type="text" id="services" size="69" autocomplete="off"></td>
                    ...
                    <input name="slab[]" type="hidden" id="slab" readonly size="8">
                     ...
                     <td width="30"><input name="serviceqty[]" type="text" id="serviceqty" size="8" autocomplete="off" onKeyDown="return numbervaild(event)" oninput="return sertotal()"></td>
				 ...
				 <tr>
				   <td colspan="8" align="right" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><span class="style2">Total</span><input type="text" id="total3" readonly size="7"></td>
				   </tr>
				 ...
				 <tr id="reffid" style="display:none;">
				    <td colspan="11" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">
				    <table id="presid" width="621" border="0" cellspacing="1" cellpadding="1">
                   ...
                   <input type="text" id="totalr" readonly size="7"></td></tr>
				...
				<tr id="exreffid">
				 ...
				 <tr id="exreffid1">
				  ...
				  <input type="text" id="total5" readonly size="7" style="display: none"></td></tr>
					                <?php 
   ...
   if($sicknum <1)
   {
				  ?>
																																																					   ...
																																																					   <tr>
																																																<td colspan="11" align="left" valign="middle"  bgcolor="#cccccc" class="bodytext3"  onDblClick="return  funcsickleavehide()" onClick="return funcsickleavehideview()"><span class="bodytext32"><span class="style2">Sick Leave
	...
	<input type="hidden" name="visitconsultationdate" id="visitconsultationdate" value="<?= $visitconsultationdate;?>">																																																	   <input name="ADate1" id="ADate1" value="<?php //echo $visitconsultationdate; ?>"  size="10"   onFocus="return datevalidation('ADate1')" onKeyDown="return disableEnterKey()" onChange="return DateDiff();" readonly />
                ...
                <td colspan="6" align="middle"  bgcolor="#ecf0f5"><div align="right" > <font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"> <font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"> <font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"> <font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"> <font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif">
                  ...
                  <input name="Submit222" id="Submit222" type="submit"  value="Save Consultation (Alt+S)" accesskey="s" class="button"  onClick="return fortreatment()" />
...
else
{
	...
	source:'ajaxdrugsearch.php?genericname='+genericname, 
	html: true, 
