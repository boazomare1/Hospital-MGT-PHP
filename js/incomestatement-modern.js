// Income Statement - Modern JavaScript
...
function validateDateRange(dateFrom, dateTo) {
    ...
    if (fromDate > toDate) {
        ...
        showFieldError(dateFromField, 'Start Date cannot be later than End Date.');
        showFieldError(dateToField, 'End Date cannot be earlier than Start Date.');
...
function updateCheckboxVisual(checkbox) {
    ...
    if (checkmark) {
        if (checkbox.checked) {
            checkmark.style.background = 'var(--medstar-primary)';
            checkmark.style.borderColor = 'var(--medstar-primary)';
        } else {
...
function suppliercodesearch1() {
    // Legacy function - can be implemented if needed
    return true;
}
...
function initializeTreeTable() {
    const treeTable = document.getElementById('collapsed');
    if (treeTable) {
        // Add click handlers for expandable rows
        const expandableRows = treeTable.querySelectorAll('tr[data-node-id]');
...
function toggleRow(row) {
    ...
    const childRows = document.querySelectorAll(`tr[data-node-pid="${nodeId}"]`);
...
// Initialize tree table when DOM is ready
document.addEventListener('DOMContentLoaded', function() {
    initializeTreeTable();
});
...
style.textContent = `
    ...
    .data-table tr[data-node-id] {
        cursor: pointer;
    }
    
    .data-table tr[data-node-id]:hover {
