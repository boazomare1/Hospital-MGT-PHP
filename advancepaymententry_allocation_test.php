...
var billamt = billamt;
console.log(varSerialNumber+"----"+totalcount1)
  var textbox = document.getElementById("adjamount"+varSerialNumber+"");
    textbox.value = "";
	alert(document.getElementById("acknow"+varSerialNumber+"").checked);
...
*/
	    ...
	    $credit_amount = $res23['amount'];
				              ////////////////// from allocation of ADJUST PAYMENT ENTRIES//
								$query_adp = "SELECT sum(transactionamount) as transactionamount FROM `advance_payment_allocation` WHERE doctorcode='$suppliercode'  AND `visitcode` =  '$visitcode' and billnumber='$billnumber' and recordstatus='allocated' and (auto_number>'321' or auto_number< 310)  group by billnumber, visitcode order by auto_number desc";
								$exec_adp = mysqli_query($GLOBALS["___mysqli_ston"], $query_adp) or die ("Error in Query_adp".mysqli_error($GLOBALS["___mysqli_ston"]));
								$num_adp=mysqli_num_rows($exec_adp);
								// while($res_adp = mysql_fetch_array($exec_adp)){
								$res_adp = mysqli_fetch_array($exec_adp);
