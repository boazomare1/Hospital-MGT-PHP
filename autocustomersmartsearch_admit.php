...
$query1 = "select * from exchange_files where Member_Nr = '$customersearch' and Progress_Flag = '1' and InOut_Type = '$InOut_Type' and DATE(Smart_Date) = '$registrationdate' order by ID DESC";
$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
$num = mysqli_num_rows($exec1);
if($num<1){
$query1 = "select * from exchange_files where Member_Nr = '$customersearch' and Progress_Flag = '1' and InOut_Type = '$InOut_Type'  order by ID DESC";
...
//echo $Member_Nr = $res1['Member_Nr'];
	...
	if($num > 0)
	{
	  if($visitcode!='' && $amt>0){
		...
		$query2 = "UPDATE master_visitentry SET admitid = '$Admit_ID',availablelimit = '$amt' WHERE patientcode = '".$customersearch."' AND visitcode='".$visitcode."' and admitid=''";
