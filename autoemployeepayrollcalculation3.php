...
{
$query2 = "select auto_number,componentanum,employeecode,employeename,componentname,componentvalue,amounttype,formula from payroll_assign where employeecode = '$employeesearch' and status <> 'deleted' order by type desc";
...
while ($res2 = mysqli_fetch_array($exec2))
{
	...
	if($amounttype == 'Percent')
	{
		...
		if($formulafrom == '1')
		{
			$query4 = "select componentvalue from payroll_assign where employeecode = '$employeesearch' and componentanum = '$formulafrom' and status <> 'deleted'";
		...
		{	
			...
			for($i=1;$i<=2;$i++)
			{
				$query5 = "select componentamount from payroll_assign where employeecode = '$employeesearch' and componentanum = '$i' and status <> 'deleted'";
		...
		{
			$totalgrossper = '';
			$query78 = "select componentamount from payroll_assign where employeecode = '$employeesearch' and typecode = '10' and status <> 'deleted'";
	...
	$sumofcomponent51 = '';
	$query1 = "select rate,totalhours,totaldays,calculationanum from master_otcalculation where componentanum = '$componentanum' and status <> 'deleted'";
	...
	if($rate != '')
	{
		...
		{	
			for($j=1;$j<=2;$j++)
			{
				$query51 = "select componentamount from payroll_assign where employeecode = '$employeesearch' and componentanum = '$j' and status <> 'deleted'";
		...
		{
			$totalgrossper1 = '';
			$query78 = "select componentamount from payroll_assign where employeecode = '$employeesearch' and typecode = '10' and componentanum NOT IN ('3','4') and status <> 'deleted'";
			$exec78 = mysqli_query($GLOBALS["___mysqli_ston"], $query78) or die ("Error in Query78".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($res78 = mysqli_fetch_array($exec78))
			{
				$res78value = $res78['componentamount'];
		...
		{
			$query52 = "select componentamount from payroll_assign where employeecode = '$employeesearch' and componentanum = '$res1formula' and status <> 'deleted'";
	...
	$sumofcomponent55 = '';
	$query13 = "select totaldays,formula from master_absent where componentanum = '$componentanum' and status <> 'deleted'";
	...
	if($totaldays != '')
	{
		...
		{	
			for($k=1;$k<=2;$k++)
			{
				$query55 = "select componentamount from payroll_assign where employeecode = '$employeesearch' and componentanum = '$k' and status <> 'deleted'";
		...
		{
			$query56 = "select componentamount from payroll_assign where employeecode = '$employeesearch' and componentanum = '$res13formula' and status <> 'deleted'";
	...
	$query15 = "select amount,percent,basedon from master_nssf where componentanum = '$componentanum' and status <> 'deleted'";
	...
	else if($res15basedon == 'Percent')
	{
		...
		while($res785 = mysqli_fetch_array($exec785))
		{
			...
			if($res785month == 'Yes')
			{
			$query234 = "select amount from payroll_assignmonthwise where componentanum = '$res785anum' and employeecode = '$employeesearch' and assignmonth = '$assignmonth1' and status <> 'deleted'";
