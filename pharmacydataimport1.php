...
if ($upload == 'success')
{
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 1 && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			$taxname = $splitdelimiter[6];
			$taxname = strtoupper($taxname);
			$purchaseprice = $splitdelimiter[7];
			$purchaseprice = strtoupper($purchaseprice);
			$genericname = $splitdelimiter[8];
			$genericname = strtoupper($genericname);
			$genericname = addslashes($genericname);
     		$minimumstock = $splitdelimiter[9];
			$minimumstock = strtoupper($minimumstock);
			$maximumstock = $splitdelimiter[10];
			$maximumstock = strtoupper($maximumstock);
			$rol = $splitdelimiter[11];
			$rol = strtoupper($rol);
			//$roq = $splitdelimiter[12];
			//$roq = strtoupper($roq);
			$ipmarkup = $splitdelimiter[12];
			$ipmarkup = strtoupper($ipmarkup);
			$spmarkup = $splitdelimiter[13];
			$spmarkup = strtoupper($spmarkup);
			$formula = $splitdelimiter[14];
			$formula = strtoupper($formula);
			$disease = $splitdelimiter[15];
		    $disease = strtoupper($disease);
			$manufacturername = $splitdelimiter[16];
			...
			if ($itemcode != '')
			{
			$query2 = "insert into master_medicine (itemcode,itemname,categoryname,packagename,rateperunit,taxname,purchaseprice,genericname,formula,minimumstock,maximumstock,roq,rol,ipmarkup,spmarkup,disease,manufacturername,unitname_abbreviation) values 
			('$itemcode','$itemname','$categoryname','$packagename','$rateperunit','$taxname','$purchaseprice','$genericname','$formula','$minimumstock','$maximumstock','$strength','$rol','$ipmarkup','$spmarkup','$disease','$manufacturername','$packagename')";
			$exec2 = mysqli_query($GLOBALS["___mysqli_ston"], $query2) or die ("Error in Query2".mysqli_error($GLOBALS["___mysqli_ston"]));
			
			$query3 = "insert into master_itempharmacy (itemcode,itemname,categoryname,packagename,rateperunit,taxname,purchaseprice,genericname,formula,minimumstock,maximumstock,roq,rol,ipmarkup,spmarkup,disease,manufacturername,unitname_abbreviation) values 
			('$itemcode','$itemname','$categoryname','$packagename','$rateperunit','$taxname','$purchaseprice','$genericname','$formula','$minimumstock','$maximumstock','$strength','$rol','$ipmarkup','$spmarkup','$disease','$manufacturername','$packagename')";
	...
	header("location:pharmacydataimport1.php?st=success");
}
?>
<style type="text/css">
<!--
            ...
            <tr>
              <td>
                  <table width="83%" border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
                    <tbody>
                      <tr bgcolor="#011E6A">
                        <td colspan="2" bgcolor="#ecf0f5" class="bodytext3"><strong>Item / Services Data Import From TAB Delimited File </strong></td>
                      </tr>
                      <tr>
