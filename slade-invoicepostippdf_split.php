...
function post_invoice($id,$filename1,$file_path1,$name,$authorization,$invoice_url,$visitcode,$billno,$locationcode,$patientcode,$mainprint)
{
	...
	if ($err) {
	   ...
	   $inv_rslt['msg'] = $err_unable_connect;
		...
		if(isset($obj['id'])){
			...
			header("location:approvedcreditlist.php?billnumber=$billno&patientcode=$patientcode&&visitcode=$visitcode&&locationcode=$locationcode&&account=$mainprint");
exit;
		}else{
...
$query919 = "select accountnameano from billing_ipcreditapprovedtransaction where  visitcode = '$visitcode' and patientcode='$patientcode' and keyprovider='yes'";
$exec919 = mysqli_query($GLOBALS["___mysqli_ston"], $query919) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
$res1 = mysqli_fetch_array($exec919);
 $main_print=$res1['accountnameano'];
...
//while (strtotime($from_limit_date) <= strtotime($to_limit_date)) {
	...
	<td colspan="7" class="bodytext31" align="right"><strong> Net Payable&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo number_format($overalltotal,2,'.',','); ?></strong></td>
	   ...
	   <tr >
            ...
            ?>
           	<td width="140" aligh="right" colspan=""></td>
		   ...
		   $exec41 = mysqli_query($GLOBALS["___mysqli_ston"], $query41) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
			...
			<span><strong><?php echo $postingaccount; ?></strong> &nbsp;&nbsp;&nbsp;<?php echo number_format($transactionamount,2,'.',',');   ?> </</span><br/>
...

{
	...
	post_invoice($claim,$filename1,$file_path2,$customername,$auth,$invoice_url,$visitcode,$billautonumber,$locationcode,$patientcode,$main_print);
}
