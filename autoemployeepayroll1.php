...
$searchyear = $searchyear1[1];
	...
	if($res55finalamount != '')
	{
		$personalrelief = $res55finalamount;
	}
	$datatotal['loan'][2] = ['anum' => '0', 'name' => 'Personal Relief', 'value' => $personalrelief, 'typecode' => '20', 'type' => 'D', 'bgcolor' => '#FF0000'];
	...
	else
	{
      ...
      if($datatotal['loan'][2]['name']!=''){
		$loanname2 = strtoupper($datatotal['loan'][2]['name']);
		$componentrate2 = $datatotal['loan'][2]['value'];
		$componentunit2 = '1';
		$componentamount2 = $datatotal['loan'][2]['value'];	
		$componentanum2 = $datatotal['loan'][2]['anum'];	
		$typecodebgcolor2 = $datatotal['loan'][2]['bgcolor'];
		$type2 = $datatotal['loan'][2]['type'];
		if($loanname2 != '')
		{
			$searchresult = $searchresult.'||^||'.$employeecode.'||'.$employeename.'||'.$loanname2.'||'.number_format($componentrate2,2,'.',',').'||'.$componentunit2.'||'.number_format($componentamount2,2,'.',',').'||'.$componentanum2.'||'.$typecodebgcolor2.'||'.$type2.'||'.number_format($totgross,2,'.',',').'||'.number_format($totdeduct,2,'.',',').'||'.number_format($totnett,2,'.',',').'';
		}
	  }
