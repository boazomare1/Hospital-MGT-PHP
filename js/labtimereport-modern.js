// Lab Time Report Modern JavaScript
...
function initializeExport() {
    ...
    function exportToExcel() {
        ...
        // Create workbook
        const wb = XLSX.utils.table_to_book(table, {sheet: "Lab Time Report"});
        
        // Generate filename with current date
        const today = new Date();
        const dateStr = today.toISOString().split('T')[0];
        const filename = `lab_time_report_${dateStr}.xlsx`;
...
function updateTimeAnalysis() {
    ...
    rows.forEach(row => {
        if (row.style.display !== 'none') {
            ...
            // Calculate time (assuming time columns exist)
            const startTimeCell = row.querySelector('td:nth-child(4)'); // Adjust index as needed
            const endTimeCell = row.querySelector('td:nth-child(5)'); // Adjust index as needed
    ...
    if (completedTests > 0) {
        averageTime = totalTime / completedTests;
    }
    
    // Update time analysis cards
    updateTimeCard('total-tests', totalTests);
    updateTimeCard('completed-tests', completedTests);
    updateTimeCard('pending-tests', pendingTests);
    updateTimeCard('average-time', averageTime, true);
}

function updateTimeCard(cardId, value, isTime = false) {
    const card = document.getElementById(cardId);
    if (card) {
        const valueElement = card.querySelector('.time-metric-value');
        if (valueElement) {
            if (isTime) {
                valueElement.textContent = formatTime(value);
            } else {
...
function calculateTimeDifference(startTime, endTime) {
    // Parse time strings (assuming format like "09:30" or "09:30:00")
