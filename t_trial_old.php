...
{
	window.scrollTo(0,0);
	document.getElemenas date,'expense' as typeyId("imgloader").style.display = "";
}
</script>
<style type="text/css">
<!--
		...
		<div class="row">
				...
				<table width="100%" border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse;">
				    ...
				    <tr>
					  ...
					  <input type="hidden" name="cbfrmflag1" value="cbfrmflag1">
				          ...
				          <input name="reseas date,'expense' as typeutton" type="reset" id="reseas date,'expense' as typeutton"  style="border: 1px solid #001E6A" value="Reset" /></td>
				    </tr>
				  </as date,'expense' as typeody>
				</table>
			</div>
			<div class="col-4">
				...
				<div style="text-align:left;">
					<a href="print_treetrial.php?ADate1=<?php echo $ADate1;?>&&ADate2=<?php echo $ADate2; ?>" id="downloadLink" target="_blank" ><img src="images25/excel-xls-icon.png" alt="excel export" width="50px" height="50px"/></a>
				</div>
			<?php } ?>
			</div>
	     </div>
...
<div id="as date,'expense' as typelData" class="container-full">
	<?php
	 if(isset($_REQUEST["cbfrmflag1"])){
	?>
	<div class="row">
				...
				{       
						...
						$section = $res['section'];
							...
							$openingenddate = date('Y-m-d',strtotime('-1 day', strtotime($from_date)));
								$opening_query = "select transaction_type,sum(transaction_amount*exchange_rate) as transaction_amount from tb_sep2020 where record_status='1' and ledger_id in ('".$ledger_ids."') and ledger_id!='' and transaction_date BETWEEN '$financeyearstart' AND '$openingenddate' group by transaction_type ";
								...
								$opening_query1 = "select transaction_type,sum(transaction_amount*exchange_rate) as transaction_amount from tb_sep2020 where record_status='1' and ledger_id in ('".$ledger_ids."') and ledger_id!='' and transaction_date < '".$financeyearstart."' group by transaction_type ";
								...
								$opening_query = "select transaction_type,sum(transaction_amount*exchange_rate) as transaction_amount from tb_sep2020 where record_status='1' and ledger_id in ('".$ledger_ids."') and ledger_id!='' and transaction_date < '".$from_date."' group by transaction_type ";
								$exec_opening = mysqli_query($GLOBALS["___mysqli_ston"], $opening_query) or die ("Error in opening_query".mysqli_error($GLOBALS["___mysqli_ston"]));; 
								while($res_opening = mysqli_fetch_array($exec_opening)){
									if($res_opening['transaction_type']=="D"){
										$opening_dr_cr += $res_opening['transaction_amount'];
								...
								$transaction_query = "select transaction_type,sum(transaction_amount*exchange_rate)  as transaction_amount from tb_sep2020 where record_status='1' and ledger_id in ('".$ledger_ids."')  and ledger_id!='' and transaction_date between '".$from_date."' and '".$to_date."' group by transaction_type ";
								...
								echo "<tbody>";
						 		echo "<tr style='background-color: #ecf0f5;' data-node-id='".$sno."'><td></td><td width='17%' align='right'>".showAmount($opening_dr_cr,'1')."</td> <td>".$res['code']."</td> <td>".ucwords(strtolower($res['name']))."</td> <td  align='right'>".showAmount($transaction_dr,'0')."</td>  <td  align='right'>".showAmount($transaction_cr,'0')."</td> <td  align='right'>".showAmount($closing_dr_cr,'1')."</td> </tr> </li>";
	              ...
	              <td colspan="7" bgcolor="#ecf0f5" class="bodytext3"><strong>&nbsp;</strong></td>
					...
					while($res = mysqli_fetch_array($exec))
					{
						...
						/*	else{
							 ...
							 //echo $res['uid'];
							 echo "<tr  style='background-color: #ecf0f5;' data-node-id='".$sno."'> <td></td> <td width='17%' align='right'>".showAmount($opening_dr_cr,'1')."</td> <td>".$res['code']."</td> <td>".ucwords(strtolower($res['name']))."</td> <td  align='right'>".showAmount($transaction_dr,'0')."</td>  <td  align='right'>".showAmount($transaction_cr,'0')."</td> <td  align='right'>".showAmount($closing_dr_cr,'1')."</td> </tr>";
							 //echo $res['uid'];
						echo getSubgroups($res['uid'],$from_date,$to_date,$sno);
						...
						echo "<tr style='font-weight:bold'> <td colspan='4' align='right'>Total </td> <td  colspan='2' align='right'>".showAmount($income_statement ,1)." </td> </tr>";
				echo "</table>";
			?>
		</div>
		<div class="col-12">
...
function getLedger($group_id,$from_date,$to_date,$sno,$sno2){
	...
	$opening_query = "select ledger_id,transaction_date,transaction_type,sum(transaction_amount*exchange_rate) as transaction_amount from tb_sep2020 where record_status='1' and ledger_id='".$res['code']."'  and ledger_id!='' and transaction_date < '".$from_date."' group by transaction_type,transaction_date ";
		$exec_opening = mysqli_query($GLOBALS["___mysqli_ston"], $opening_query) or die ("Error in opening_query".mysqli_error($GLOBALS["___mysqli_ston"]));
		while($res_opening = mysqli_fetch_array($exec_opening)){
		...
		$transaction_query = "select transaction_type,sum(transaction_amount*exchange_rate)  as transaction_amount from tb_sep2020 where record_status='1' and ledger_id='".$res['code']."'  and ledger_id!='' and transaction_date between '".$from_date."' and '".$to_date."' group by transaction_type ";
		$exec_transaction = mysqli_query($GLOBALS["___mysqli_ston"], $transaction_query) or die ("Error in transaction_query".mysqli_error($GLOBALS["___mysqli_ston"]));; 
		while($res_transaction = mysqli_fetch_array($exec_transaction)){
			if($res_transaction['transaction_type']=="D"){
				$transaction_dr += $res_transaction['transaction_amount'];
		...
		$closing_dr_cr = $opening_dr_cr + $transaction_dr - $transaction_cr;
		 ...
		 $data .="<tr data-node-id=".$sno.'.'.$sno2.'.'.$sno3." data-node-pid=".$sno.'.'.$sno2." > <td></td><td width='17%' align='right'>".showAmount($opening_dr_cr,'1')."</td><td width='14%'>".$res['code']."</td> <td>".ucwords(strtolower($res['name']))."&nbsp;<a href='ledgerview.php?ledgerid=$ledgercode&&ADate1=$from_date&&ADate2=$to_date' target='_blank' style='font-size:14px;'>&#9432;</a>"."</td> <td  align='right'>".showAmount($transaction_dr,'0')."</td>  <td  align='right'>".showAmount($transaction_cr,'0')."</td> <td  align='right'>".showAmount($closing_dr_cr,'1')."</td> </tr>";
	}
...
function getGroupBalance($group_id,$from_date,$to_date,$sno,$sno2){
	...
	$section = $res1['section'];			
		...
		if($section == 'I' || $section == 'E' ){
			...
			$financeyearstart = $startyear.'-01-01';
			$openingenddate = date('Y-m-d',strtotime('-1 day', strtotime($from_date)));
			 $opening_query = "select transaction_type,sum(transaction_amount*exchange_rate) as transaction_amount from tb_sep2020 where record_status='1' and ledger_id in ('".implode("','",$all_ledgers)."')  and ledger_id!='' and transaction_date BETWEEN '$financeyearstart' AND '$openingenddate'  group by transaction_type ";
			$exec_opening = mysqli_query($GLOBALS["___mysqli_ston"], $opening_query) or die ("Error in opening_query".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($res_opening = mysqli_fetch_array($exec_opening))
			{
				if($res_opening['transaction_type']=="D"){
			...
			$opening_query1 = "select transaction_type,sum(transaction_amount*exchange_rate) as transaction_amount from tb_sep2020 where record_status='1' and ledger_id in ('".implode("','",$all_ledgers)."')  and ledger_id!='' and transaction_date < '$financeyearstart'  group by transaction_type ";
			$exec_opening1 = mysqli_query($GLOBALS["___mysqli_ston"], $opening_query1) or die ("Error in opening_query".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($res_opening1 = mysqli_fetch_array($exec_opening1))
			{
				if($res_opening1['transaction_type']=="D"){
		...
		$GLOBALS['ieledgers'] += $opening_dr_cr1;
			 ...
			 $opening_query = "select transaction_type,sum(transaction_amount*exchange_rate) as transaction_amount from tb_sep2020 where record_status='1' and (ledger_id in ('".implode("','",$all_ledgers)."'))  and ledger_id!='' and transaction_date < '".$from_date."' group by transaction_type ";
		...
		$transaction_query = "select transaction_type,sum(transaction_amount*exchange_rate)  as transaction_amount from tb_sep2020 where record_status='1' and  ledger_id in ('".implode("','",$all_ledgers)."')  and ledger_id!='' and transaction_date between '".$from_date."' and '".$to_date."' group by transaction_type ";
		$exec_transaction = mysqli_query($GLOBALS["___mysqli_ston"], $transaction_query) or die ("Error in transaction_query".mysqli_error($GLOBALS["___mysqli_ston"]));; 
		while($res_transaction = mysqli_fetch_array($exec_transaction)){
			if($res_transaction['transaction_type']=="D"){
				$transaction_dr += $res_transaction['transaction_amount'];
		...
		$closing_dr_cr = $opening_dr_cr + $transaction_dr - $transaction_cr;
         $uuid = $res['uid'] + 1;
		 $data .="<tr  style='background-color: aquamarine;' data-node-id=".$sno.'.'.$sno2." data-node-pid=".$sno."><td></td> <td width='17%' align='right'>".showAmount($opening_dr_cr,'1')."</td> <td>".$res['code']."</td> <td>".ucwords(strtolower($res['name']))."</td> <td  align='right'>".showAmount($transaction_dr,'0')."</td>  <td  align='right'>".showAmount($transaction_cr,'0')."</td> <td  align='right'>".showAmount($closing_dr_cr,'1')."</td> </tr>";
