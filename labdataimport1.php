...
if ($upload == 'success')
{
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 1 && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			$sampletype= $splitdelimiter[3];
			$sampletype = strtoupper($sampletype);
			$itemname_abbreviation = $splitdelimiter[4];
			$itemname_abbreviation = strtoupper($itemname_abbreviation);
			$referencename= $splitdelimiter[5];
			$referencename = strtoupper($referencename);
			$referenceunit= $splitdelimiter[6];
			$referenceunit = strtoupper($referenceunit);
			$referencerange= $splitdelimiter[7];
			$referencerange = strtoupper($referencerange);
			$rateperunit = $splitdelimiter[8];
			$rateperunit = strtoupper($rateperunit);
			$rate2 = $splitdelimiter[9];
			$rate2 = strtoupper($rate2);
			$taxname = $splitdelimiter[10];
			$taxname = strtoupper($taxname);
			$referencevalue = $splitdelimiter[11];
			$referencevalue = strtoupper($referencevalue);
			preg_match ('/[!,^,+,=,[,],;,,,{,},|,\,<,>,?,~]/', $itemname);
			
			if ($itemcode != '')
			{
			$query2 = "insert into master_lab(itemcode,itemname,categoryname,sampletype,itemname_abbreviation,referencename,referenceunit,referencerange,rateperunit,rate2,taxname,referencevalue) values ('$itemcode','$itemname','$categoryname','$sampletype','$itemname_abbreviation','$referencename','$referenceunit','$referencerange','$rateperunit','$rate2','$taxname','$referencevalue')";
