...
if($frompage=='lab'){
	...
	$page= array();
	if($type==OP){
	$query115 = "select * from resultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and publishstatus = 'completed' group by itemcode,sampleid";
	}else{
	$query115 = "select * from ipresultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode'  group by itemcode,sampleid";	
	}
...
<?php
	...
	if($type == 'IP')
	{
		...
		$query32="select rslt.* from ipresultentry_lab as rslt join master_labreference as ref on ref.referencename=rslt.referencename and rslt.itemcode = ref.itemcode where rslt.patientcode = '$patientcode' and rslt.patientvisitcode = '$visitcode' and rslt.itemcode = '$res616itemcode'   and rslt.resultvalue <> '' and rslt.sampleid='$docnumber' $subquery1 group by rslt.referencename order by reforder";
