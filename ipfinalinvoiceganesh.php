...
mysqli_query($GLOBALS["___mysqli_ston"], "insert into billing_ip(billno,patientname,patientcode,visitcode,totalamount,billdate,accountname,subtype,patientbilltype,totalrevenue,discount,deposit,nhif,depositcoa,paymenttype,locationname,locationcode,accountcode,totalamountuhx,curtype,fxrate,accountnameano,accountnameid)values('$billno','$patientname','$patientcode','$visitcode','$totalamount','$currentdate','$accountname','$subtype','$patientbilltype','$totalrevenue','$discount','$deposit','$nhif','$ipdepositscoa','$paymenttype','".$locationnameget."','".$locationcodeget."','$accountcode','".$totalamountuhx."','".$curtype."','".$fxrate."','$accountnameano','$accountcode')") or die(mysqli_error($GLOBALS["___mysqli_ston"]).__LINE__);
...
foreach($_POST['description'] as $key=>$value)
	...
	foreach($_POST['medicinename'] as $key=>$value)
		...
		if($servicesname!="")
		{
		$servicesquery1=mysqli_query($GLOBALS["___mysqli_ston"], "insert into billing_ipservices(patientcode,patientname,patientvisitcode,servicesitemcode,servicesitemname,servicesitemrate,accountname,billdate,billnumber,servicecoa,locationname,locationcode,servicesitemrateuhx,curtype,fxrate,incomeledgercode,incomeledgername,doctorcode,doctorname,wellnesspkg,billstatus)values('$patientcode','$patientname','$visitcode','$servicescode','$servicesname','$servicesrate','$accountname','$currentdate','$billno','$servicecoa','".$locationnameget."','".$locationcodeget."','".$servicesrateuhx."','".$curtype."','".$fxrate."','$serviceledgcode','$serviceledgname','$servicedoctorcode','$servicedoctorname','$wellnesspkg','$stat')") or die(mysqli_error($GLOBALS["___mysqli_ston"]).__LINE__);
		}
		}
		...
		if($servicesname!="")
		{
		$servicesquery1=mysqli_query($GLOBALS["___mysqli_ston"], "insert into billing_ipservices(patientcode,patientname,patientvisitcode,servicesitemcode,servicesitemname,servicesitemrate,accountname,billdate,billnumber,servicecoa,locationname,locationcode,servicesitemrateuhx,curtype,fxrate,incomeledgercode,incomeledgername,doctorcode,doctorname,wellnessitem,billstatus)values('$patientcode','$patientname','$visitcode','$servicescode','$servicesname','$servicesrate','$accountname','$currentdate','$billno','$servicecoa','".$locationnameget."','".$locationcodeget."','".$servicesrateuhx."','".$curtype."','".$fxrate."','$serviceledgcode','$serviceledgname','$servicedoctorcode','$servicedoctorname','1','$stat')") or die(mysqli_error($GLOBALS["___mysqli_ston"]).__LINE__);
		}
			}
  ...
  <td class="bodytext31" valign="center"  align="left"><div align="right"><?php echo number_format(($otbillingrate/$fxrate),2,'.',','); ?></div></td>
	...
	?>
	  <tr>
        <td>&nbsp;</td>
		 <td>&nbsp;</td>
		  <td width="1%" class="bodytext31" align="left">User Name<input type="text" name="username" id="username" value="<?php echo $username; ?>" class="bal1" readonly>&nbsp;&nbsp;&nbsp;&nbsp;
          <?php if($patientbilltype != 'PAY NOW'){ ?>
            <b><input type="text" name="availablelimite" id="availablelimit" value="<?php echo $availablelimit;?>" <?php  if($availablelimit < $overalltotaluhx){?> style="background:#F00;color:#FFF" <?php }?> readonly ></b><?php } ?></td>
		<td width="1%">&nbsp;</td>
		<td width="1%">&nbsp;
         <?php if($admitid!=''){?>
        ...
        <?php if($smartap==1) { ?>
        <input name="Submit222" id="smartfrm" type="submit" value="Save and Post To Smart" class="button" disabled /> <?php } else { ?>
		<input name="Submit222" type="submit" <?php if($billtype!='PAY LATER'){echo 'disabled';} else if($overalltotaluhx < '0' ){echo 'disabled';}?> id="submit" value="Save Bill"  class="button"/>
        <?php } ?>
        </td>
      </tr>
    </table>
