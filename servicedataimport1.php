...
if ($upload == 'success')
{
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 1 && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			if ($itemcode != '')
			{
			$query2 = "insert into master_services (itemcode,itemname,categoryname,itemname_abbreviation,rateperunit,rate2,taxname) values ('$itemcode','$itemname','$categoryname','$itemname_abbreviation','$rateperunit','$rate2','$taxname')";
		...
		header("location:servicedataimport1.php?st=success");
