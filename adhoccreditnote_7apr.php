...
$query91 = "INSERT into master_transactionpaylater (transactiondate, transactiontime, particulars,  
		...
		'','$patientfullname','$patientcode','$visitcode','$account','$billnumbercode','$receivableamount','','','$patienttype11','$patientsubtype11','$username','$pat_accountanum','$pat_accountcode','$locationcodeget','$locationnameget','$subtypeano','$pat_accountcode','$currency','1','$adjamount','','0')";
		...
		<tr>
			   ...
			   <tr>
                ...
                <td align="left" valign="top" class="bodytext3">
                	<?php
                	$sql12="SELECT accountname as accountname, accountcode,accountnameano from master_transactionpaylater where billnumber='$bill_num_get' and transactiontype='finalize' order by auto_number DESC";
                	 // UNION ALL  SELECT accountname  as accountname from billing_ipcreditapproved where billno='$bill_num_get'
						...
						if (mysqli_num_rows($querylab12) > 0) {
							$execlab12=mysqli_fetch_array($querylab12);
                	...
                	?>
				<input type="hidden" name="account" id="account" value="<?php echo $patientaccount_final; ?>" />		
				<input type="hidden" name="pat_accountcode" id="pat_accountcode" value="<?php echo $patientaccount_code; ?>"  />		
				<input type="hidden" name="pat_accountanum" id="pat_accountanum" value="<?php echo $pat_accountanum; ?>"  />		

				<input type="hidden" name="billtypes" id="billtypes" value="<?php echo $billtype; ?>" />

			<?php echo $patientaccount_final; ?>	</td>	
				...
				<tr>
						...
						<?php
						 $query2 = "SELECT billbalanceamount from master_transactionpaylater where billnumber='$bill_num_get' and accountcode = '$patientaccount_code' and transactionstatus <> 'onaccount' and acc_flag = '0' and transactiontype not in ('pharmacycredit','paylatercredit')   order by transactiondate ASC";
						// and transactionmode <> 'CREDIT NOTE'  and transactiondate between '$ADate1' and '$ADate2' 
								$exec2 = mysqli_query($GLOBALS["___mysqli_ston"], $query2) or die ("Error in Query2".mysqli_error($GLOBALS["___mysqli_ston"]));
								while ($res2 = mysqli_fetch_array($exec2)){
          ...
          <tbody id="foo">
            ...
            <tr>
              <td colspan="8" align="right" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><span class="style2">Total Amt  </span>
              	<input type="text" id="total" name="total_amount" readonly size="7"></td>
            </tr>
