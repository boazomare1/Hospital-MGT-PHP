				...
				{  
					...
					if($id != '')
					{
						...
						$querydr1 = "SELECT SUM(`totalamount`) as expenses FROM `expensepurchase_details` WHERE `expensecode` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2' and recordstatus <> 'deleted'
									 ...
									 UNION ALL SELECT SUM(`totalfxamount`) as expenses FROM `purchase_details` WHERE (`expensecode` = '$id' or `ledgercode` = '$id') AND billnumber NOT LIKE 'SUPO%' AND suppliername <> 'OPENINGSTOCK' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2' and recordstatus <> 'deleted'
									 UNION ALL SELECT SUM(`transactionamount`) as expenses FROM `expensesub_details` WHERE `expensecoa` = '$id' AND transactionmode <> 'ADJUSTMENT' AND transactiondate BETWEEN '$ADate1' AND '$ADate2'
