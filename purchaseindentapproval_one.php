...
if($_POST['discard'] != $docno)
{
  foreach($_POST['medicinename'] as $key => $value)
	...
	//echo "<br>".$itemcode;
		...
		if(strtolower(trim($purchasetype))!=strtolower('Expenses') && strtolower(trim($purchasetype))!=strtolower('Others'))
				{
					$query45="update purchase_indent set indent_approval='approved' where docno='$docno' and medicinecode='$itemcode' and auto_number='$medanum'";
				}
				else
				{
					$query45="update purchase_indent set indent_approval='approved' where docno='$docno' and auto_number='$itemcode'";
				}
  ...
  foreach($_POST['medicinename'] as $key => $value)
	...
	//echo "<br>".$itemcode;
		...
		if(strtolower(trim($purchasetype))!=strtolower('Expenses') && strtolower(trim($purchasetype))!=strtolower('Others'))
				{
					$query45="update purchase_indent set indent_approval='rejected' where docno='$docno' and medicinecode='$itemcode' and auto_number='$medanum'";
				}
				else
				{
					$query45="update purchase_indent set indent_approval='rejected' where docno='$docno' and auto_number='$itemcode'";
				}
				$exec45 = mysqli_query($GLOBALS["___mysqli_ston"], $query45) or die(mysqli_error($GLOBALS["___mysqli_ston"]));	
				$actionstatus='reject';
...
<form name="form1" id="frmsales" method="post" action="purchaseindentapproval_one.php" onKeyDown="return disableEnterKey(event)" onSubmit="return process()">
    ...
    <td width="99%" valign="top"><table width="980" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><table width="99%" border="0" align="left" cellpadding="2" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
			...
			{
			$docno = $_REQUEST['docno'];
			 $query12 = "select suppliername,purchasetype,currency,fxamount,bamailfrom,famailfrom,famailcc,locationcode,storecode,povalidity,s_terms from purchase_indent where docno='$docno' and (indent_approval='' or indent_approval='rejected')  and  initial_approval=''";
      ...
      <tr>
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
			...
			$total_taxamount = 0;
			$totalamount=0;			
			$query12 = "select * from purchase_indent where docno='$docno' and (indent_approval='' or indent_approval='rejected')   and is_deleted=0";
			$exec12= mysqli_query($GLOBALS["___mysqli_ston"], $query12) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
			$numb=mysqli_num_rows($exec12);
