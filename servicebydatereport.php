...
source:"ajaxautocomplete_servcode.php",
...
<table width="1900" border="0" cellspacing="0" cellpadding="2">
  ...
  <tr>
    ...
    <td width="99%" valign="top"><table width="116%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="860">
             <form name="cbform1" method="post" action="servicebydatereport.php">
                <table width="700" border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
                  <tbody>
                    <tr bgcolor="#011E6A">
                      <td colspan="2" bgcolor="#ecf0f5" class="bodytext3"><strong>Service Test By Date </strong></td>
        ...
        <td><table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" bordercolor="#666666" cellspacing="0" cellpadding="4" width="600" align="left" border="0">
          <tbody>
			...
			<tr bgcolor="#9999FF">
			 ...
			 <?php
			$totalcount1=0;
			 $query1 = "select servicesitemname,servicesitemcode,consultationdate,patientcode,patientvisitcode  from consultation_services where   servicesitemcode like '%$itemcode%' and consultationdate between '$transactiondatefrom' and '$transactiondateto' and $locationcodenew group by servicesitemcode";
			$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
			$num1 = mysqli_num_rows($exec1);
			//echo $num1;
			while($res1 = mysqli_fetch_array($exec1))
		    {
        	 $res1itemname = $res1['servicesitemname'];
        	 $res1itemcode = $res1['servicesitemcode'];
			 
        	  
			$countitem=0;
			  $query2 = "select  servicesitemname,servicesitemcode,consultationdate,patientcode,patientvisitcode,serviceqty from consultation_services where  consultationdate between '$transactiondatefrom' and '$transactiondateto' and servicesitemcode = '$res1itemcode' and servicesitemrate<>'0.00' and $locationcodenew ";
			$exec2 = mysqli_query($GLOBALS["___mysqli_ston"], $query2) or die ("Error in Query5".mysqli_error($GLOBALS["___mysqli_ston"]));
			$num2 = mysqli_num_rows($exec2);
			while($res2 = mysqli_fetch_array($exec2))
			{
			$res2labitemname = $res2['servicesitemname'];
			$res2itemcode = $res2['servicesitemcode'];
			$res2patientcode = $res2['patientcode'];
			$res2patientvisitcode = $res2['patientvisitcode'];
			$serviceqty = $res2['serviceqty'];
			if($serviceqty=='' || $serviceqty==0)
			{
				$serviceqty=1;
			}
		    $countitem=$countitem+$serviceqty ;
            }
           ...
           <tr <?php echo $colorcode; ?>>
			...
			$totalcount=0;
			$colorloopcount=0;
			 $query1 = "select servicesitemname,servicesitemcode,consultationdate from ipconsultation_services where  servicesitemcode like '%$itemcode%' and consultationdate between '$transactiondatefrom' and '$transactiondateto' and $locationcodenew group by servicesitemcode ";
			$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
			$num1 = mysqli_num_rows($exec1);
			while($res1 = mysqli_fetch_array($exec1))
			{
			$res1itemname = $res1['servicesitemname'];
			$res1labitemcode = $res1['servicesitemcode'];
			
			$countitem=0;
			 $query2 = "select servicesitemname,servicesitemcode,patientcode,patientvisitcode,serviceqty from ipconsultation_services where   consultationdate between '$transactiondatefrom' and '$transactiondateto' and servicesitemcode = '$res1labitemcode' and $locationcodenew";
			...
			$res2patientcode = $res2['patientcode'];
			$res2patientvisitcode = $res2['patientvisitcode'];
			$ipserviceqty = $res2['serviceqty'];
			if($ipserviceqty=='' || $ipserviceqty==0)
			{
				$ipserviceqty=1;
			}
		    $countitem=$countitem+$ipserviceqty ;
            }
			...
			//op start
			$totalcount1=0;
			$query1 = "select servicesitemname,servicesitemcode  from (
			select servicesitemname,servicesitemcode  from consultation_services where  servicesitemcode like '%$itemcode%' and consultationdate between '$transactiondatefrom' and '$transactiondateto' and $locationcodenew 
			UNION ALL 
			select servicesitemname,servicesitemcode from ipconsultation_services where servicesitemcode like '%$itemcode%' and consultationdate between '$transactiondatefrom' and '$transactiondateto' and $locationcodenew) as e group by servicesitemcode";
			$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
			$num1 = mysqli_num_rows($exec1);
			//echo $num1;
			while($res1 = mysqli_fetch_array($exec1))
			...
			$countitem1=0;
			 ...
			 $query2 = "select servicesitemname,servicesitemcode,patientcode,patientvisitcode,serviceqty from ipconsultation_services where   consultationdate between '$transactiondatefrom' and '$transactiondateto' and servicesitemcode = '$res1itemcode' and $locationcodenew";
			$exec2 = mysqli_query($GLOBALS["___mysqli_ston"], $query2) or die ("Error in Query2".mysqli_error($GLOBALS["___mysqli_ston"]));
			$num2 = mysqli_num_rows($exec2);
			while($res2 = mysqli_fetch_array($exec2))
			{
		    ...
		    $countitem1=$countitem1+$ipserviceqty ;
			...
			<tr>
				...
				{
				?>
				<a href="xl_servicestasticst.php?itemname=<?= $itemname ?>&&itemcode=<?= $itemcode ?>&&cbfrmflag1=cbfrmflag1&&ADate1=<?= $transactiondatefrom ?>&&ADate2=<?= $transactiondateto ?>&&location=<?= $location ?>&&dmy=<?= $dmy ?>&&eptype=<?= $eptype ?>" target="_blank"> <img src="images/excel-xls-icon.png" width="30" height="30" /> </a>
				<?php
				}
				?>			  </td> 
			</tr>
