...
if($accountid <>'')
{
	$accountdetails="select accountname,id,auto_number,subtype,paymenttype from master_accountname where id='$accountid'";
	$exeacc=mysqli_query($GLOBALS["___mysqli_ston"], $accountdetails);
	$resacc=mysqli_fetch_array($exeacc);
	$accountname1=$resacc['accountname'];
	$accountid1=$resacc['id'];
	$auto_number=$resacc['auto_number'];
	$paymenttype=$resacc['paymenttype'];
	$subtype=$resacc['subtype'];
	
	$subtypedetails=mysqli_query($GLOBALS["___mysqli_ston"], "select subtype,auto_number,currency,fxrate from master_subtype where auto_number='$subtype'");
	$ressub=mysqli_fetch_array($subtypedetails);
	$subtypename=$ressub['subtype'];
	$subtypeanum=$ressub['auto_number'];
	$currency=$ressub['currency'];
    $fxrate=$ressub['fxrate'];
	
	$paymentdetails=mysqli_query($GLOBALS["___mysqli_ston"], "select paymenttype from master_paymenttype where auto_number='$paymenttype'");
	$respayment=mysqli_fetch_array($paymentdetails);
	$paymentname=$respayment['paymenttype'];
}
...
$accountautonum= isset($_REQUEST['accountautonum'])?$_REQUEST['accountautonum']:'';
$accountname1= isset($_REQUEST['accountname1'])?$_REQUEST['accountname1']:'';
$subtype= isset($_REQUEST['subtype'])?$_REQUEST['subtype']:'';
$subtypeanum= isset($_REQUEST['subtypeanum'])?$_REQUEST['subtypeanum']:'';
$accountid0= isset($_REQUEST['accountid0'])?$_REQUEST['accountid0']:'';
...
else
{
	...
	//echo $companycode;
		...
		if($pairvar!="")
		{
	 	$debitquery="insert into ip_debitnote(billno,accountname,accountnameid,accountnameano,remarks,totalamount,billdate,locationname,locationcode,subtype,subtypeano,patientcode,visitcode,accountcode) values('$billnumbercode','$accountname1','$accountid0','$accountautonum','$pairvar','$amount','$billdate','$locationnameget','$locationcodeget','$subtype','$subtypeanum','$billnumbercode','$billnumbercode','$accountid0')";
		$exedebit=mysqli_query($GLOBALS["___mysqli_ston"], $debitquery)or die(mysql_error);
	
 		$paylaterquery="insert into master_transactionpaylater(docno,transactiondate,accountname,accountnameano,accountnameid,transactiontype,transactionamount,particulars,locationname,locationcode,acc_flag,updatedate,transactiontime,paymenttype,subtype,subtypeano,patientcode,visitcode,billnumber,username,billbalanceamount,accountcode,fxamount,fxrate,exchrate) 
values('$billnumbercode','$billdate','$accountname1','$accountautonum','$accountid0','finalize','$amount','$pairvar','$locationnameget','$locationcodeget','0','$currentdate','$timeonly','$paymenttype','$subtype','$subtypeanum','$billnumbercode','$billnumbercode','$billnumbercode','$username','$fxamount','$accountid0','$fxamount','$fxrate','$fxrate')";
		$exepaylater=mysqli_query($GLOBALS["___mysqli_ston"], $paylaterquery) or die("error in paylaterquery".mysqli_error($GLOBALS["___mysqli_ston"])); 
		
		}
}
	
		header("location:debitnotelist.php");
		exit;
...
<script type="text/javascript" src="js/insertnewitemipmiscbilling1.js"></script>
<script type="text/javascript" src="js/autosuggesaccountsearch2.js"></script>
...
<form name="form1" id="frmsales" method="post" action="debitbill.php" onKeyDown="return disableEnterKey(event)" onSubmit="return validcheck()">
  ...
  <tr>
    ...
    <td width="99%" valign="top"><table width="980" border="0" cellspacing="0" cellpadding="0">
      <tr>
		...
		$query551 = "select * from master_location where locationcode='".$locationcode."'";
		...
		$auto_numberloc=$res551['auto_number'];
		?>
                <tr bgcolor="#011E6A">
                <td colspan="4" bgcolor="#ecf0f5" class="bodytext32"><strong>Account Details</strong></td>
			...
			<input type="hidden" name="location_autonumber" value="<?php echo $auto_numberloc?>">
		 </tr>
		<tr>
				...
				<tr>
                <td width="17%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><span class="bodytext32"> <strong>Account Name</strong>  </span></td>
                <td align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">
				<input name="accountname1" id="accountname1" value="<?php echo $accountname1; ?>" style="border: 1px solid #001E6A; text-transform:uppercase;" size="18" type="hidden">
     <input name="accountautonum" id="accountautonum" value="<?php echo $auto_number; ?>" style="border: 1px solid #001E6A; text-transform:uppercase;" size="18" type="hidden">
                <input name="subtype" id="subtype" value="<?php echo $subtypename; ?>" style="border: 1px solid #001E6A; text-transform:uppercase;" size="18" type="hidden">
                <input name="subtypeanum" id="subtypeanum" value="<?php echo $subtypeanum; ?>" style="border: 1px solid #001E6A; text-transform:uppercase;" size="18" type="hidden">
				<?php echo $accountname1; ?>
				</td>
                <td align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">
				<strong>Account Id</strong></td>
                <td align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">
				<input name="accountid0" id="accountid0" value="<?php echo $accountid1; ?>" style="border: 1px solid #001E6A; text-transform:uppercase;" size="18" type="hidden">
				<input name="paymenttype" id="paymenttype" value="<?php echo $paymentname; ?>" style="border: 1px solid #001E6A; text-transform:uppercase;" size="18" type="hidden">
				<?php echo $accountid1; ?></td>
				</tr>       
				...
				<?php echo $billnumbercode; ?>							</td>
                   ...
                   <tr>
							 <td align="left" valign="middle" class="bodytext3"><strong> Currency / Fx Rate</strong></td>
				<td class="bodytext3"><?php echo $currency.' / '.$fxrate; ?></td>	
      ...
      <tr>
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
				        ...
				        <tr>
				   <td colspan="11" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><span class="bodytext32"><strong>Debit Note</strong> </span></td>
		        </tr>
				<tr id="reffid">
