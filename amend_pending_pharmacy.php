	   ...
	   <tr>
	     <td>	
          ...
          <!--  <tr>
			...
			$query1 = "select * from master_consultationpharm where medicineissue='pending' and patientcode like '%$patientcode1' and patientvisitcode like '%$visitcode1%' and patientname like '%$patientname1%' and recorddate between '$ADate1' and '$ADate2' and  billing = '' and billtype = 'PAY NOW' group by patientvisitcode order by auto_number desc";
		...
		if($num2 > 0)
		{
			...
			?>
			<?php
			$query1 = "select a.patientcode as patientcode, a.patientvisitcode as patientvisitcode, a.recorddate as recorddate from master_consultationpharm AS a JOIN master_visitentry AS b ON (a.patientvisitcode = b.visitcode) where a.medicineissue='pending' and a.patientcode like '%$patientcode1' and a.patientvisitcode like '%$visitcode1%' and a.patientname like '%$patientname1%' and a.recorddate between '$ADate1' and '$ADate2' and  a.billing = '' and a.paymentstatus = 'completed' and b.planpercentage = '0.00' and a.billtype = 'PAY LATER' group by a.patientvisitcode order by a.auto_number desc";
			$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
		...
		if($num2 > 0)
		{
			...
			<?php
			$query1 = "select a.patientcode as patientcode, a.patientvisitcode as patientvisitcode, a.recorddate as recorddate from master_consultationpharm AS a JOIN master_visitentry AS b ON (a.patientvisitcode = b.visitcode) where a.medicineissue='pending' and a.patientcode like '%$patientcode1' and a.patientvisitcode like '%$visitcode1%' and a.patientname like '%$patientname1%' and a.recorddate between '$ADate1' and '$ADate2' and  a.billing = '' and a.paymentstatus = 'pending' and b.planpercentage > '0.00' and a.billtype = 'PAY LATER' group by a.patientvisitcode order by a.auto_number desc";
			$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
