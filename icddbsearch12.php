...
function IcdSearch(id)
{
	...
	//alert(Anum);
	window.opener.document.getElementById('dis1').value = Icd;
	window.opener.document.getElementById('code1').value = Icdcode;
	window.opener.document.getElementById('diseas1').value = Icd;
	window.opener.document.getElementById('chapter1').value = Chapter;
	
	window.opener.insertitem14();
...
<td width="144" align="left" class="bodytext3"><input type="text" size="35" name="searchdisease1" id="searchdisease1" onfocus="this.value = this.value;" value="<?php echo $searchdisease2; ?>" /></td>
...
foreach($searchexplode as $searchdisease1)
{
	...
	$query2 = "select auto_number,disease,icdcode,chapter,description from master_icd where disease like '%$searchdisease1%' and ($searchstring) and recordstatus = 'active' order by auto_number";// order by subtype limit 0, 15";
