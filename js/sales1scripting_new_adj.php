...
$exec1roundoff = mysqli_query($GLOBALS["___mysqli_ston"], $query1roundoff) or die ("Error in Query1roundoff".mysqli_error($GLOBALS["___mysqli_ston"]));

$res1roundoff = mysqli_fetch_array($exec1roundoff);
...

{
   ...
   document.getElementById("insertrow").innerHTML = "";
...

	{
		...
		document.getElementById("adjustamounttr").style.display = 'none';
...

	{
		...
		var mpesaamt = document.getElementById("totalamount").value;
...

	{
		...
		document.getElementById("adjustamounttr").style.display = '';
		...
		document.getElementById("adjustamount").readOnly = false;

		document.getElementById("cashamount").value = document.getElementById("totalamount").value;

		var balance_amt = parseFloat($('#hid_bal_amt').val());

		var has_adv_deposit_amt = document.getElementById("has_adv_deposit_amt").value;

		if(has_adv_deposit_amt == 0 || balance_amt == 0)
...
*/
...

	{
		...
		if (document.getElementById("cashamount").value>0 && (document.getElementById("cashgivenbycustomer").value == "" || document.getElementById("cashgivenbycustomer").value==0))
...

	{
		...
		var nettot=document.getElementById("balanceamt").value;
	...
	var form_flag=document.getElementById("form_flag").value;
	var patientcode=document.getElementById("customercode").value;
	var visitcode=document.getElementById("visitcode").value;
	if(form_flag=='paynow'){


var idsLength = document.getElementById("ref_count").value;
        var arr = [];
		...
		var data = "arr="+json+"&&patientcode="+patientcode+"&&visitcode="+visitcode+"&&form_flag="+form_flag;
		
	 $.ajax({
			
			type : "post",
			url : "ajaxforduplicatepaynow.php",
			data : data,
			catch : false,
			...
			{
				alert('Customer Has Already Paid for this Request. Pl check Reprints');
				window.location="patientbillingstatus_bills.php";
				return true;
				return false;
	...
	var data = "patientcode="+patientcode+"&&visitcode="+visitcode+"&&form_flag="+form_flag;
...
if(document.getElementById("adjustamount").value == '')
...
document.getElementById("chequeamount").value = "0.00";
	...
	if (isNaN(document.getElementById("adjustamount").value))

	{

		alert ("Adjust Amount Can Only Be Numbers.");
	...
	{	
...

		{
			...
			var cashpay=document.getElementById("cashamount").value;
			...
			var allcash=parseFloat(mpaypay)+parseFloat(cheqpay)+parseFloat(cardpay)+parseFloat(onlipay)+parseFloat(adjustpay);
...

			{
				...
				alert("Cant give more change than cash taken");

				document.getElementById("cashgiventocustomer").value =0.00;

				if(parseFloat(adjustpay) > parseFloat(totalamount))
			...
			//	var varVarFinalCashGivenToCustomer1 = parseFloat(varCashGivenToCustomer1).toFixed(2);
				...
				var remainval=parseFloat(varCashGivenByCustomer1)-parseFloat(cashpay);

				if(parseFloat(remainval)<=0)
...

				{
					...
					document.getElementById("cashgiventocustomer").value = parseFloat(remainval).toFixed(2);
	...
	{	
		...
		var varAdjustAmount = document.getElementById("adjustamount").value;

		var varTotalNettAmount = parseFloat(varCashAmount) + parseFloat(varCreditAmount) + parseFloat(varChequeAmount) + parseFloat(varCardAmount) + parseFloat(varOnlineAmount) + parseFloat(varAdjustAmount);
		...
		/*if( (document.getElementById("adjustamount").value !=undefined )  && (!isNaN(document.getElementById("adjustamount").value)) && varAdjustAmount !="")

		{
			...
			if(parseFloat(varAdjustAmount) > adv_dep_total_amt )

			{

				alert('Adjust amount cannot be more than Available Amount');
...

			{
				...
				var adv_dep_bal_amt = adv_dep_total_amt - parseFloat(varAdjustAmount);
			...
			/*var net_amt = parseFloat($('#net_deposit_amt').val());



			if(parseFloat(varAdjustAmount) > parseFloat(net_amt) )
...

			{
				...
				var adv_dep_bal_amt = parseFloat(net_amt) - parseFloat(varAdjustAmount);
