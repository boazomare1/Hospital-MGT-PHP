...
/*
	...
	// OUTSTANDING OPENING BALANCE START		
		  ...
		  UNION ALL SELECT b.`docno` as billnumber,b.visitcode AS visitcode, (-1*b.`transactionamount`) as fxamount, b.`transactiondate` as transactiondate,b.accountnameid FROM `master_transactionpaylater` as a JOIN `master_transactionpaylater` as b ON (a.`visitcode` = b.`visitcode`) WHERE b.`accountnameid` in($subtypes_accountids_str) and a.`transactiontype` = 'finalize' and b.`transactiontype` = 'pharmacycredit' and b.`auto_number` > a.`auto_number` AND b.`transactiondate` < '$ADate1' and b.billnumber!=''
...
//AND `transactiondate` <= '$ADate2' 
 $query982 = "select sum(fxamount +discount ) as transactionamount1,docno from master_transactionpaylater where  transactiontype = 'PAYMENT' and billnumber='$resbillnumber2' and recordstatus = 'allocated'   and accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND accountnameid='$res2accountnameid' 
				UNION ALL 
				select sum(-1*fxamount ) as transactionamount1,docno from master_transactionpaylater where  transactiontype = 'PAYMENT' and docno='$resbillnumber2' and recordstatus = 'allocated'   and accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND accountnameid ='$res2accountnameid' AND docno NOT LIKE '%AR%'
				UNION ALL 
				select sum(-1*fxamount ) as transactionamount1,docno from master_transactionpaylater where  transactiontype = 'PAYMENT' and docno='$resbillnumber2' and recordstatus = 'allocated'   AND docno  LIKE '%AR%'  and accountnameid in (select id from master_accountname where subtype='$subtypeanum') ";	
				$exec982 = mysql_query($query982) or die('ERROR IN  98'.mysql_error());
				$num982 = mysql_num_rows($exec982);
				while($res982 = mysql_fetch_array($exec982))
				{
...
//AND accountnameid in($subtypes_accountids_str) 
 $query98 = "select sum(fxamount +discount ) as transactionamount1,docno from master_transactionpaylater where  transactiontype = 'PAYMENT' and billnumber='$res2billnumber' and recordstatus = 'allocated'  and accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND accountnameid ='$accntid'
				UNION ALL 
				select sum(-1*fxamount ) as transactionamount1,docno from master_transactionpaylater where  transactiontype = 'PAYMENT' and docno='$res2billnumber' and recordstatus = 'allocated'  and accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND accountnameid ='$accntid'	AND docno NOT LIKE '%AR%'
				UNION ALL 
				select sum(-1*fxamount ) as transactionamount1,docno from master_transactionpaylater where  transactiontype = 'PAYMENT' and docno='$res2billnumber' and recordstatus = 'allocated' AND docno  LIKE '%AR%'  and accountnameid in (select id from master_accountname where subtype='$subtypeanum') ";
		...
		{
		   echo $payment = $res98['transactionamount1'];

		  echo '======>'.$allocatedamount = $allocatedamount + $payment;

		echo '<br>';
		}
 
		//$balanceamount = $res2transactionamount - $allocatedamount + $pendig_final_value_outstanding;
        echo $balanceamount = $res2transactionamount-$allocatedamount;
			echo '<br>';	echo '<br>';
...
$pendig_final_value_outstanding = 0;
      ...
      ?>
        <tr  class="acc<?=$res21accountnameano?>" >
          ...
          <td class="bodytext31" valign="center"  align="left"><?php  if($res2billnumber !=''){ echo $res2billnumber;} else{ echo $res2billnumber1; } ?></td>
