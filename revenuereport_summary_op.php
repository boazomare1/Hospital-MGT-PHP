...
$exec_pd = mysqli_query($GLOBALS["___mysqli_ston"], $query_pd) or die ("Error in Query_pd".mysqli_error($GLOBALS["___mysqli_ston"]));
		    ...
		    echo $res_pdrate = $res_pd['amount1'];
...
///// Rebate close DEPOSIT OPENS ///////////
		...
		$depositrefunds=0;
				...
				$query_depositsip = "SELECT (transactionamount) as amountuhx FROM `master_transactionipdeposit` WHERE visitcode in ( SELECT visitcode from billing_paylater where billdate between '$ADate1' and '$ADate2' group by visitcode   
				)";
			$exec_depositsip = mysqli_query($GLOBALS["___mysqli_ston"], $query_depositsip) or die ("Error in Query_depositsip".mysqli_error($GLOBALS["___mysqli_ston"]));
			$num_depositsip=mysqli_num_rows($exec_depositsip);
			while($res_depositsip = mysqli_fetch_array($exec_depositsip)){
		  ...
		  $query_depositrefunds = "SELECT  sum(amount) AS amount from deposit_refund WHERE visitcode in (SELECT visitcode from billing_paylater where billdate between '$ADate1' and '$ADate2' group by visitcode  
				)";
			$exec_depositrefunds = mysqli_query($GLOBALS["___mysqli_ston"], $query_depositrefunds) or die ("Error in Query_depositrefunds".mysqli_error($GLOBALS["___mysqli_ston"]));
			$num_depositrefunds=mysqli_num_rows($exec_depositrefunds);
			while($res_depositrefunds = mysqli_fetch_array($exec_depositrefunds)){
