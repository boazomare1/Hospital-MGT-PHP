/**
 * Modern JavaScript for CAFT Sales Credit Report - caftsalescreditreport.php
 * Enhanced UI interactions, form validation, and dynamic functionality
 */

class CAFTSalesCreditReportManager {
    constructor() {
        this.init();
        this.bindEvents();
    ...
    init() {
        console.log('CAFT Sales Credit Report Manager initialized');
    ...
    bindEvents() {
        ...
        // Table events
        const table = document.querySelector('.caft-sales-credit-table, table');
        if (table) {
    ...
    setupTableEvents(table) {
        ...
        const rows = table.querySelectorAll('tbody tr');
        rows.forEach(row => {
            row.addEventListener('dblclick', (e) => this.viewSalesCreditDetails(e));
        });
    }

    setupFilters() {
        const filterForm = document.querySelector('.caft-sales-credit-filters, .filter-form');
        if (filterForm) {
    ...
    validateField(field) {
        ...
        // Custom validation for sales credit specific fields
    ...
    handleActionClick(e) {
        ...
        const salesCreditId = row?.dataset.id || button.dataset.id;

        switch (action) {
            case 'action-btn-view':
                this.viewSalesCredit(salesCreditId);
                break;
            case 'action-btn-edit':
                this.editSalesCredit(salesCreditId);
                break;
            case 'action-btn-delete':
                this.deleteSalesCredit(salesCreditId);
                break;
            case 'action-btn-approve':
                this.approveSalesCredit(salesCreditId);
                break;
        }
    }
    ...
    viewSalesCreditDetails(e) {
        const row = e.target.closest('tr');
        const salesCreditId = row?.dataset.id;
        if (salesCreditId) {
            this.viewSalesCredit(salesCreditId);
        }
    }
    ...
    viewSalesCredit(id) {
        ...
        setTimeout(() => {
            this.hideLoading();
            this.showNotification(`Viewing sales credit ${id}`, 'info');
    ...
    editSalesCredit(id) {
        ...
        setTimeout(() => {
            this.hideLoading();
            this.showNotification(`Editing sales credit ${id}`, 'info');
            // In a real application, this would open an edit form
        }, 1000);
    }

    deleteSalesCredit(id) {
        if (confirm('Are you sure you want to delete this sales credit record?')) {
            this.showLoading();
            
            setTimeout(() => {
                this.hideLoading();
                this.showNotification(`Sales credit ${id} deleted successfully`, 'success');
                this.removeTableRow(id);
            }, 1000);
        }
    }

    approveSalesCredit(id) {
        this.showLoading();
        
        setTimeout(() => {
            this.hideLoading();
            this.showNotification(`Sales credit ${id} approved successfully`, 'success');
            this.updateSalesCreditStatus(id, 'approved');
        }, 1000);
    }
    ...
    updateSearchResults(visible, total) {
        ...
        if (resultsInfo) {
            resultsInfo.textContent = `Showing ${visible} of ${total} sales credit records`;
        }
    }
    ...
    updateSalesCreditStatus(id, status) {
        const row = document.querySelector(`tr[data-id="${id}"]`);
        if (row) {
    ...
    generateReport(form) {
        ...
        console.log('Generating CAFT Sales Credit Report with data:', reportData);
...
document.addEventListener('DOMContentLoaded', () => {
    new CAFTSalesCreditReportManager();
});

// Export for potential use in other modules
if (typeof module !== 'undefined' && module.exports) {
    module.exports = CAFTSalesCreditReportManager;
}
