...
/*echo $test1="select itemcode,batch_quantity from transaction_stock where (batch_stockstatus='1' and itemcode='INJ00182' and locationcode='LTC-1' and fifo_code='6337' and storecode ='STO1' and batch_quantity>='259') and (batchnumber in(select batchnumber from materialreceiptnote_details where expirydate>now() and itemcode ='INJ00182') or batchnumber in(select batchnumber from purchase_details where expirydate>now() and itemcode ='INJ00182'))";
//select itemcode,batch_quantity from transaction_stock where (batch_stockstatus='1' and itemcode='SCM00209 ' and locationcode='LTC-1' and fifo_code='1689' and storecode ='STO1' and batch_quantity>='258') and (batchnumber in(select batchnumber from materialreceiptnote_details where expirydate>now() and itemcode ='SCM00209 ') or batchnumber in(select batchnumber from purchase_details where expirydate>now() and itemcode ='SCM00209 '))
// union all select itemcode,batch_quantity from transaction_stock where (batch_stockstatus='1' and itemcode='SCM00093 ' and locationcode='LTC-1' and fifo_code='5831' and storecode ='STO1' and batch_quantity>='489') and (batchnumber in(select batchnumber from materialreceiptnote_details where expirydate>now() and itemcode ='SCM00093 ') or batchnumber in(select batchnumber from purchase_details where expirydate>now() and itemcode ='SCM00093 ')) 
//union all select itemcode,batch_quantity from transaction_stock where (batch_stockstatus='1' and itemcode='SCM00210 ' and locationcode='LTC-1' and fifo_code='1923' and storecode ='STO1' and batch_quantity>='24') and (batchnumber in(select batchnumber from materialreceiptnote_details where expirydate>now() and itemcode ='SCM00210 ') or batchnumber in(select batchnumber from purchase_details where expirydate>now() and itemcode ='SCM00210 '))
// union all select itemcode,batch_quantity from transaction_stock where (batch_stockstatus='1' and itemcode='SCM00208 ' and locationcode='LTC-1' and fifo_code='5825' and storecode ='STO1' and batch_quantity>='164') and (batchnumber in(select batchnumber from materialreceiptnote_details where expirydate>now() and itemcode ='SCM00208 ') or batchnumber in(select batchnumber from purchase_details where expirydate>now() and itemcode ='SCM00208 ')) 
//union all select itemcode,batch_quantity from transaction_stock where (batch_stockstatus='1' and itemcode='SCM00207 ' and locationcode='LTC-1' and fifo_code='6334' and storecode ='STO1' and batch_quantity>='155') and (batchnumber in(select batchnumber from materialreceiptnote_details where expirydate>now() and itemcode ='SCM00207 ') or batchnumber in(select batchnumber from purchase_details where expirydate>now() and itemcode ='SCM00207 ')) 
//union all select itemcode,batch_quantity from transaction_stock where (batch_stockstatus='1' and itemcode='INJ00165' and locationcode='LTC-1' and fifo_code='2202' and storecode ='STO1' and batch_quantity>='3') and (batchnumber in(select batchnumber from materialreceiptnote_details where expirydate>now() and itemcode ='INJ00165') or batchnumber in(select batchnumber from purchase_details where expirydate>now() and itemcode ='INJ00165')) 
//union all select itemcode,batch_quantity from transaction_stock where (batch_stockstatus='1' and itemcode='INJ00182' and locationcode='LTC-1' and fifo_code='6337' and storecode ='STO1' and batch_quantity>='259') and (batchnumber in(select batchnumber from materialreceiptnote_details where expirydate>now() and itemcode ='INJ00182') or batchnumber in(select batchnumber from purchase_details where expirydate>now() and itemcode ='INJ00182'))";

$dtaa=mysql_query($test1) or die("sdsd".mysql_error());

print_r(mysql_fetch_array($dtaa));
exit;*/
...
$query01="";
				...
				if($retqty !="0" && $retqty !="")
				{
  					 ...
  					 $query29=mysqli_query($GLOBALS["___mysqli_ston"], "update consultation_services set servicerefund='$refund',refundquantity='$newretqty',remarks='$remarks',process='$process' where patientvisitcode='$visitcode' and patientcode='$patientcode' and servicesitemcode = '$servicesitemcode' and auto_number='$autonumber'")or die("Error in Query29".mysqli_error($GLOBALS["___mysqli_ston"]));
...
foreach($query_array as $query_array_data){
	echo $query_array_data;
 $exec01=mysqli_query($GLOBALS["___mysqli_ston"], $query_array_data) or die("Error in union ". mysqli_error($GLOBALS["___mysqli_ston"]));
 $num01+=mysqli_num_rows($exec01);
}

echo $serialnum." </br> ".$num01;
exit;

echo mysqli_query($GLOBALS["___mysqli_ston"], $query01); 
exit;
 $exec01=mysqli_query($GLOBALS["___mysqli_ston"], $query01) or die("Error in union ". mysqli_error($GLOBALS["___mysqli_ston"]));
exit;
echo "<br>".$num01=mysqli_num_rows($exec01);
echo "<br>".$serialnum;
exit;
	...
	if($_POST['select'][$slctkey] != '')
	{
		...
		$storecodeget=$storecode;
			 ...
			 $querycumstock2 = "select cum_quantity from transaction_stock where cum_stockstatus='1' and itemcode='$itemcode' and locationcode='$locationcodeget'";
				...
				{
				echo	$querybatstock2 = "select batch_quantity from transaction_stock where batch_stockstatus='1' and itemcode='$itemcode' and locationcode='$locationcodeget' and fifo_code='$fifo_code' and storecode ='$storecodeget'  and (batchnumber in(select batchnumber from materialreceiptnote_details where expirydate>now() and itemcode ='$pharmacycode') or batchnumber in(select batchnumber from purchase_details where expirydate>now() and itemcode ='$pharmacycode'))";
				...
				echo "<br>".$bat_quantity = $bat_quantity-$quantity;
...
//						$stockexecquery2=mysql_query($stockquery2) or die(mysql_error());
	...
	//					$exec32 = mysql_query($query32) or die(mysql_error());
		...
		//			 $queryupdate=mysql_query("update consultation_services set process='completed',remarks='$remarks' where patientvisitcode='$visitcode' and patientcode='$patientcode' and servicesitemcode = '$servicesitemcode' and auto_number='$refno'")or die("Error in queryupdate".mysql_error());
					}
					else
					{
...
body {
	margin-left: 0px;
	margin-top: 0px;
	/* [disabled]background-color: #ecf0f5; */
}
.style1 {
	font-size: 36px;
	font-weight: bold;
...
<form name="frmsales" id="frmsales" method="post" action="processservice_test.php" onKeyDown="return disableEnterKey(event)" onSubmit="return validcheck()">
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
			...
			$totalamount=0;
			 ...
			 $query61 = "select serviceqty,refundquantity,servicesitemname,billtype,auto_number,refno from consultation_services where paymentstatus='completed' and patientcode = '$patientcode' and patientvisitcode = '$visitcode' and process='pending' and auto_number='$refnumber'  and servicesitemcode='$service_item_code' and (billtype='PAY NOW' or (billtype='PAY LATER' ))";
