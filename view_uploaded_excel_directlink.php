...
header("location:view_uploaded_excel_directlink.php?docno=$docnum&&searchaccount=$searchaccount_get&&uploadid=$uploadid&&searchdocno=$searchdocno&&fromdate=$fromdate&&todate=$todate");
...
foreach($_POST['billnum'] as $key => $value)
  ...
  header("location:view_uploaded_excel_directlink.php?docno=$docnum1&&searchaccount=$searchaccount_get&&uploadid=$uploadid&&searchdocno=$searchdocno&&fromdate=$fromdate&&todate=$todate");
...
echo "<script>window.location.href = 'accountreceivableentrylist.php?searchaccount=$searchaccount_get&&searchdocno=$searchdocno&&fromdate=$fromdate&&todate=$todate';</script>";
       ...
       $patientcode=$res1['patientcode'];
        ...
        // }
         ...
         if(($x=="0" and $y=="0") or ($subtypeid_docno!=$subtype_id_main)){
            $query1 = "SELECT patientcode, patientname, visitcode, transactionamount, accountname, subtype, accountcode, accountnameid, accountnameano  FROM `master_transactionpaylater` where billnumber='$bill_no' and subtypeano='$subtypeano_docno' and transactionamount<>'0.00' ";
                    $exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
                    $x=mysqli_num_rows($exec1);
                    if($x>0){
                    $res1 = mysqli_fetch_array($exec1);
    ...
    if (!in_array($bill_no, $invoice_number_array))
     ...
     if(($x=="0" and $y=="0")  or($array_found=='1') or ($not_match!=0) or (round($allocation_amount, 2) > round($pending_allocated, 2))  or  ($subtypeid_docno!=$subtype_id_main)  ) { 
        ...
        ?>
         ...
         <tr  <?php  if(($x=="0" and $y=="0")  or($array_found=='1')  or ($not_match!=0) or (round($allocation_amount, 2) > round($pending_allocated, 2))  or  ($subtypeid_docno!=$subtype_id_main)) { echo $not_find; }else{ echo $colorcode; } ?>>        <!-- <tr  <?php  //if(($x=="0" and $y=="0") or ($not_match!=0) or ($accountid_match!=$accountnameid4)) { echo $not_find; }else{ echo $colorcode; } ?>> -->
