...
$qrydoc = "select * from master_doctor where doctorname like '%".$doc."%' and ipdoctor=1";
...

{
	...
	$consultationfees = $resdoc['consultationfees'];


	$a_json_row["id"] = $doctorcode;

	$a_json_row["value"] = trim($doctorname);
	$a_json_row["fees"] = trim($consultationfees);

	//$a_json_row["label"] = '#'.trim($doctorcode).' || '.trim($doctorname).' || '.trim($consultationfees);
	$a_json_row["label"] = trim($doctorname).' || '.trim($consultationfees);
