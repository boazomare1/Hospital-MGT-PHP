...
if ($frmflag2 == 'frmflag2')
{
	...
	if (isset($_POST["save"])) {
		...
		if($count > 0){
	        foreach($_REQUEST['surveychk'] as $key=>$value){
                ...
                //echo $auto_id.'<br>';
                $obj_ids = array();
			    $query_rt211 = "SELECT auto_number,templatename FROM pharma_rate_template WHERE recordstatus <> 'deleted' ORDER BY auto_number ASC";
                ...
                foreach ($obj_ids as $k => $v) {
                    ...
                    $updaterate_string = $_POST['rate'.$tempcode.$auto_id].'<br>';
                    $updaterate = floatval(str_replace(',', '', $updaterate_string));
	...
	header("Location: pharmacyrateupdate_full.php");		
	}
...
function addward1process1()
{
	...
	function updaterate(e,autono,tempids){
		...
		for (var i = 0; i < idsLength; i++) {
		    ...
		    //console.log("rate"+tempcode+autono);
		    //console.log(rate);
	...
	function updateRateItem(e,autono,tempids){
		...
		for (var i = 0; i < idsLength; i++) {
		    ...
		    if(itemmargin > 0){
		    	...
		    	document.getElementById("rate"+tempcode+autono).value = cost;
		    }
		}
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tablebackgroundcolor1">
						<tr>
							<td>
								...
								<form name="form2" id="form2" method="post" action="pharmacyrateupdate_full.php">
								...
								<tr bgcolor="#011E6A">
									...
									<?php 
									  $query_rt = "SELECT templatename,markup FROM pharma_rate_template WHERE recordstatus <> 'deleted' ORDER BY auto_number ASC";
									...
									?>
									<td colspan="2" align="center" width="20%" bgcolor="#ecf0f5" class="bodytext3">
								...
								<td align="center"  valign="middle"  class="bodytext3">
									...
									<?php
									    // Get all templates
									    $query_rt1 = "SELECT auto_number,markup FROM pharma_rate_template WHERE recordstatus <> 'deleted'  ORDER BY auto_number ASC";
								    ...
								    <?php 
								       // get all pharma rate temp codes
								        $arr11 = array();
								        //query
								        $query_rt21 = "SELECT auto_number FROM pharma_rate_template WHERE recordstatus <> 'deleted' ORDER BY auto_number ASC";
									    $exec_rt21 = mysqli_query($GLOBALS["___mysqli_ston"], $query_rt21) or die ("Error in Query_rt".mysqli_error($GLOBALS["___mysqli_ston"]));
										while ($res_rt21 = mysqli_fetch_assoc($exec_rt21))
										{   
											$temp_id= $res_rt21['auto_number'];
											array_push($arr11, $temp_id);
										}
								    ...
								    <td align="left" valign="middle" class="bodytext3">
									    ...
									    <div align="center">
											<input type="text" name="itemmargin<?php echo $tempid; ?><?php echo $auto_number; ?>" id="itemmargin<?php echo $tempid; ?><?php echo $auto_number; ?>" style="width: 40px; text-align: right; <?php if($margin <= 0){ echo 'background-color: #d1d1d1;border-width:1px;border-color:#d1d1d1;display: none;';} ?>" value="<?php echo $item_margin; ?>" size="5" <?php if($margin <= 0){ echo 'readonly="readonly"';}?> onkeyup='updateRateItem(this,<?php echo $auto_number; ?>,<?php echo $tempids; ?>)'>
										</div>
									</td>
									<td align="left" valign="middle" class="bodytext3">
									 ...
									 <?php 
									       // get all pharma rate temp codes
									        $arr1 = array();
									        //query
									        $query_rt2 = "SELECT auto_number FROM pharma_rate_template WHERE recordstatus <> 'deleted' ORDER BY auto_number ASC";
											...
											{   
												$temp_id= $res_rt2['auto_number'];
												array_push($arr1, $temp_id);
											}
