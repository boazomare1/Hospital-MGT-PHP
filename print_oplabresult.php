...
if($type == 'IP')
{
$query31 = "SELECT * from ipresultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode'  and itemcode LIKE '%$itemcode%' and docnumber='$docnumberip' group by itemcode,recorddate order by itemcode,recorddate DESC";
//$query31 = "select * from ipresultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and resultstatus = 'completed' and itemcode LIKE '%$itemcode%' and date(recorddate) between '$fromdate' and '$todate' group by itemcode";
 ...
 $query31 = "SELECT * from resultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and publishstatus = 'completed' and itemcode LIKE '%$itemcode%' and docnumber='$docnumberip' group by itemcode,recorddate order by itemcode,recorddate DESC";
}
...
$res38publisheddatetime  = $res31['recorddate']." ".$res31['recordtime'];
$res123recorddate = "";
$query99 = "select recorddate,recordtime from samplecollection_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and itemcode = '$itemcode' and sampleid = '$sampleid'";
...
if(mysqli_num_rows($exec99) > 0)
{
	...
	$res123recorddate = $res99['recorddate']." ".$res99['recordtime'];
}

$published_by=$res31['username'];
...
if($dob != '0000-00-00')
{
 ...
 $query11 = "select username,lab_comments from consultation_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and labitemcode like '$itemcode' group by username";
...
if($num11 ==0)
{
$query11 = "select username,lab_comments from ipconsultation_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and labitemcode like '$itemcode' group by username";
$exec11 = mysqli_query($GLOBALS["___mysqli_ston"], $query11) or die ("Error in Query11".mysqli_error($GLOBALS["___mysqli_ston"]));
$res11 = mysqli_fetch_array($exec11);
$res11username = $res11['username'];
// $lab_comments = $res11['lab_comments'];
...
<div pagegroup="new" backtop="12mm" backbottom="20mm" backleft="2mm" backright="3mm">
 ...
 <tr>
			...
			<img src="logofiles/<?php echo $companyanum;?>.jpg" width="165" height="85" />
 ...
 <td align="center" class="bodytext23" style="font-size:13px">
		  ...
		  <tr>
              <td align="center" class="bodytext24" style="font-size:13px"> <?php echo '<br>'. $labemobile.'<br>'.$labemail; ?>  </td>
        </tr>
...
-->		  	
	  ...
	  <tr>
		<td class="bodytext27"><span class="style1">Sample Rcvd </span></td>
		  ...
		  <td class="bodytext27" colspan='2'><!--<img src="<?php $sampleanum = substr($sampleid,4); echo $applocation1; ?>/barcode/test.php?text=<?php echo $sampleanum; ?>" width="150" height="50" />--></td>
...
<td>&nbsp;</td>
 ...
 <table width="519" class="test" cellpadding="4" cellspacing="0">
	<tr>
		...
		<?php
	   $query616 = "select *,count(referencename) as nors  from resultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and itemcode = '$itemcode' and sampleid = '$sampleid' and resultvalue <> '' and  docnumber='$docnumberip' group by itemcode,sampleid,referencename";
		$exec616 = mysqli_query($GLOBALS["___mysqli_ston"], $query616) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
		if($type == 'IP')
		{
			$query616 = "select *,count(referencename) as nors  from ipresultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and itemcode = '$itemcode' and sampleid = '$sampleid' and resultvalue <> '' and  docnumber='$docnumberip' group by itemcode,sampleid,referencename";
		$exec616 = mysqli_query($GLOBALS["___mysqli_ston"], $query616) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
		}
	...
	<?php
	$query32="select * from resultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and itemcode = '$res616itemcode' and sampleid = '$sampleid'  and resultvalue <> '' and  docnumber='$docnumberip' group by referencename order by auto_number";
	$exec32=mysqli_query($GLOBALS["___mysqli_ston"], $query32);
	if($type == 'IP')
	{
		$query32="select * from ipresultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and itemcode = '$res616itemcode' and sampleid = '$sampleid'  and resultvalue <> '' and  docnumber='$docnumberip' group by referencename order by auto_number";
	$exec32=mysqli_query($GLOBALS["___mysqli_ston"], $query32);
	}
	...
	$resultvalue = str_replace('>','&gt;',$resultvalue);
	$query99 = "select sample from samplecollection_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and itemcode = '$res616itemcode' and sampleid = '$docnumber'";
	$exec341=mysqli_query($GLOBALS["___mysqli_ston"], $query99);
	$res341=mysqli_fetch_array($exec341);
	$res99 = $res341['sample'];
	$sampletype = $res341['sample'];
	...
	$refcomments = str_replace('border="1"','border="0"',$refcomments);		
	 ...
	 { ?>
     <tr>
	 <td colspan="5" align="left" valign="top" class="bodytext27"><strong>&nbsp;</strong></td>
     </tr>
     <tr>
...
<td>&nbsp;</td><td>&nbsp;</td>
		...
		<tr>
	 <td width="70%"><?php echo strtoupper('Technologist'); ?>:<strong> <?php echo strtoupper($publishedname); ?></strong>
