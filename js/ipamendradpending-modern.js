// IP Amend Radiology Pending Modern JavaScript - Following Hospital Theme

let allRadiologyRecords = [];
let filteredRadiologyRecords = [];
let currentPage = 1;
...
function handleSearch(event) {
    ...
    window.location.href = 'ipamendrad_pending.php?' + searchParams.toString();
}
...
function handleLiveSearch(event) {
    ...
    if (searchTerm.length < 2) {
        ...
        filteredRadiologyRecords = [...allRadiologyRecords];
        updateResultsDisplay();
        return;
    }
    
    // Filter records based on search term
    filteredRadiologyRecords = allRadiologyRecords.filter(record => {
        return (
...
function updatePagination() {
    ...
    const totalPages = Math.ceil(filteredRadiologyRecords.length / itemsPerPage);
...
function changePage(page) {
    if (page < 1 || page > Math.ceil(filteredRadiologyRecords.length / itemsPerPage)) {
        return;
    }
...
function updateResultsDisplay() {
    ...
    const currentRecords = filteredRadiologyRecords.slice(startIndex, endIndex);
...
function exportToCSV() {
    ...
    if (link.download !== undefined) {
        ...
        link.setAttribute('download', 'ip_radiology_pending.csv');
...
// View radiology details
function viewRadiologyDetails(patientcode, visitcode) {
    showAlert(`Viewing radiology details: ${patientcode} - ${visitcode}`, 'info');
    
    // This would normally open a modal or navigate to a view page
    console.log('Viewing radiology details:', patientcode, visitcode);
}
...
document.addEventListener('keydown', function(event) {
    ...
    if ((event.ctrlKey || event.metaKey) && event.key === 'f') {
        event.preventDefault();
        const searchInput = document.getElementById('patientname');
        if (searchInput) {
            searchInput.focus();
        }
    }
