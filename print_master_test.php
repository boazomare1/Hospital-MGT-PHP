...
$updatedatetime= $res43['updatedatetime'];

if($sampleDocNumber==''){
$query43 = "select sample_id as sampleid from pending_test_orders where visitcode='$visitcode' and testcode='$itemcode' and publishdatetime='$updatedatetime'";
$exec43 = mysqli_query($GLOBALS["___mysqli_ston"], $query43) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
$res43 = mysqli_fetch_array($exec43);
$sampleDocNumber = $res43['sampleid'];
if($type == 'IP')
	$sampleid=$sampleDocNumber='IPS-'.$sampleDocNumber;
else
	$sampleid=$sampleDocNumber='OPS-'.$sampleDocNumber;
}
...
if($type == 'IP')
		 echo $query79="SELECT recorddate from ipsamplecollection_lab where patientcode='$patientcode' and patientvisitcode='$visitcode' and sampleid='$sampleDocNumber'";
		else
...
<body>
<form name="frm" id="frmsales" method="post" action="print_labresultsfull_new1_test.php" onKeyDown="return disableEnterKey(event)" onSubmit="return validcheck1()">
     ...
     <tr>
      ...
      <input type="hidden" name="locationcodeget" value="<?php echo $locationcode;?>">
       <input type="hidden" name="locationnameget" value="<?php echo $locationname;?>">
			...
			<td width="4%"  align="left" valign="center" 
				...
				if($type == 'IP')
				{
					 $query31 = "SELECT * from ipresultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode'  and itemcode LIKE '%$itemcode%' and date(recorddate) between '$fromdate' and '$todate' and docnumber='$docnumber_get' group by itemcode,recorddate order by itemcode,recorddate DESC";
					//$query31 = "select * from ipresultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and resultstatus = 'completed' and itemcode LIKE '%$itemcode%' and date(recorddate) between '$fromdate' and '$todate' group by itemcode";
				
				}else{
					$query31 = "SELECT * from resultentry_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and publishstatus = 'completed' and itemcode LIKE '%$itemcode%' and date(recorddate) between '$fromdate' and '$todate' and docnumber='$docnumber_get' group by itemcode,recorddate order by itemcode,recorddate DESC";
				}
	  ...
	  { 
	   ...
	   $labitemcode = $res31['itemcode'];
		...
		if($type == 'IP')
		 echo $query79="SELECT recorddate from ipsamplecollection_lab where patientcode='$patientcode' and patientvisitcode='$visitcode' and sampleid='$sampleid'";
		else
		  $query79="SELECT recorddate from samplecollection_lab where patientcode='$patientcode' and patientvisitcode='$visitcode' and sampleid='$sampleid'";
		 // and itemcode='$itemcode' 
		$exec79=mysqli_query($GLOBALS["___mysqli_ston"], $query79) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
		$res79=mysqli_fetch_array($exec79);
		 $date2_recorddate=$res79['recorddate'];
	   ...
	   $colorloopcount = $colorloopcount + 1;
		...
		?>		  
              ...
              <td class="bodytext31" valign="center"  align="center"><div align="left"><?php echo $recorddate; ?>
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
            ...
            align="left" border="0">
              <tbody>
			   ...
			   $query52="select * from master_labreference where itemcode='$labitemcode' and (gender = '$patientgender' or gender='') and '$birth_days' >= agefrom and '$birth_days' < ageto and status <> 'deleted' group by referencename"; 
			   ...
			   $itemcode2=$res52['itemcode'];
				...
				if($type == 'IP')
				{
					$query49 = "SELECT * from ipresultentry_lab where (docnumber='$docnumber' or sampleid='$sampleid') and itemcode = '$itemcode2' and referencename = '$labreferencename' and date(recorddate) between '$fromdate' and '$todate' and docnumber='$docnumber_get' order by auto_number desc";
				
				}else{
					$query49 = "SELECT * from resultentry_lab where sampleid='$sampleid' and itemcode = '$itemcode2' and referencename = '$labreferencename' and date(recorddate) between '$fromdate' and '$todate' and docnumber='$docnumber_get'  order by auto_number desc";
				}
				//echo "query49 ".$query49;
				$exec43 = mysqli_query($GLOBALS["___mysqli_ston"], $query49) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
				  
				  $res43 = mysqli_fetch_array($exec43);
				  $anum = $res43['auto_number'];
				  $result = $res43['resultvalue'];
				  // $refcomments = $res43['referencecomment'];
			  ...
			  $query641 = "select * from master_labreference where itemcode='$itemcode2' and referencename = '$labreferencename' and status <> 'deleted' group by referencename";
				...
				if($type == 'IP')
				{
					// and auto_number < '$anum'
					 $query49 = "select * from ipresultentry_lab where patientcode='$patientcode' and itemcode='$itemcode2' and referencename='$labreferencename' and auto_number < '$anum' and docnumber='$docnumber_get'  order by auto_number desc limit 0,1";
				
				}else{
					$query49 = "select * from resultentry_lab where patientcode='$patientcode' and itemcode='$itemcode2' and referencename='$labreferencename' and auto_number < '$anum' and docnumber='$docnumber_get'  order by auto_number desc limit 0,1";
				}
				$exec49 = mysqli_query($GLOBALS["___mysqli_ston"], $query49) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
				 ...
				 // $refcomments = $res49['referencecomment'];
				  ?>
				  ...
				  <textarea rows="3" cols="30" name="result[]" id="result<?php echo $sn; ?>" onBlur="return funcrange('<?php echo $sn; ?>')" size="10"  readonly/><?php echo $result; ?></textarea>
				 ...
				 <input type="hidden" name="range111[]" id="range111<?php echo $sn; ?>" value="<?php echo $labreferencerange1[0] ; ?>">
                  ...
                  <td class="bodytext311" valign="center" bordercolor="#f3f3f3" align="left" width="73" style="color:red;"><?php echo $result; ?></td>
