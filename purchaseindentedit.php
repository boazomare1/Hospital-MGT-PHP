...

{
	...
	//$searchsuppliername = $_REQUEST['searchsuppliername'];
		...
		{	
...

			{
			 ...
			 // Update edit flag for Purchase Indent
			 
		 	 $docno_edit = $_REQUEST['docnoedit'];
		 	 $queryedit="UPDATE purchase_indent set is_edited='1',is_edited_on=NOW(),is_edited_by='$username' where docno='$docno_edit'";
		 	 mysqli_query($GLOBALS["___mysqli_ston"], $queryedit) or die(mysqli_error($GLOBALS["___mysqli_ston"]));	
	...
	header("location:purchaseindent.php?success=Failed");
...
if(isset($_REQUEST['docno']))

			{
				...
				$query12 = "select * from purchase_indent where docno='$docno' and is_deleted<>'1' limit 0,1";
...

				{
					...
					$lpotype =  $res12['lpo_type'];
					$indent_memo = $res12['indent_memo'];
...
function btnDeleteClick10_edit(delID4,vrate4)

{
	...
	$('#idTR'+varDeleteID4).remove();
	/*var child4 = document.getElementById('idTR'+varDeleteID4);  

	//alert (child3);//tr name

	document.getElementById ('medicinelisttbl').removeChild(child4);*/
...
function calcedit(sno)

{
	var sno = sno;
	var reqqty=document.getElementById("reqqty"+sno).value;
	//console.log(sno);
	//console.log(reqqty);

	var packsize=document.getElementById("packsize"+sno).value;
	...
	var rt = document.getElementById("rate"+sno).value.replace(/[^0-9\.]+/g,"");

	document.getElementById("fxrate"+sno).value=parseFloat(fxamount*rt);

	var rate=document.getElementById("fxrate"+sno).value.replace(/[^0-9\.]+/g,"");
	...
	document.getElementById("amount"+sno).value=formatMoney(amount.toFixed(2));

	
	calpurchaseindenttotal();
	//console.log('after total fn')
...

	{
		...
		document.getElementById("pkgqty"+sno).value=Math.round(pkgqty);
	...
	document.getElementById("pkgqty"+sno).value=Math.round(1);
...
<script type="text/javascript" src="js/insertnewitem32piedit.js?v=2"></script>
...
<script type="text/javascript" src="js/automedicinecodesearch5kiambu1_multi.js"></script>
...
$( document ).ready(function() {
	...
	if(purchasetype!='Expenses' && purchasetype!='Others' && purchasetype!='non-medical' && purchasetype!='ASSETS')
	...
	minLength: 2,
        ...
        select: function(event, ui) {
        	console.log('in autocomplete medicinename')
	...
	$('.delete_edit_item').click(function(){
		console.log('del clicked');

		var sno = $(this).attr("delsno");
		var amountdel = parseFloat($('#amount'+sno).val());
		console.log(sno)
		btnDeleteClick10_edit(sno,amountdel);

		var id = $(this).attr("rowid");

		  $.ajax({
		  url: 'ajax/purchaseindentitemdel.php',
		  type: 'POST',
		  //async: false,
		  dataType: 'json',
		  //processData: false,    
...
$('#reqqty').focusout(function() {
  ...
  $('#reqqty' ).val($('#reqqty' ).val().replace(/[^0-9\.]+/g,"").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"));
	...
	$('.medicine').keyup(function(e) {

		console.log('in medicine keyup')
...
console.log($(this).attr("serialno"));
...
var serialno = $(this).attr("serialno");
	...
	minLength: 2,
        ...
        select: function(event, ui) {
		    ...
		    $('#medicinecode'+serialno).val(ui.item.itemcode) ;

			//$("#medicinenamel").val(ui.item.value);
			console.log('in autocomplete')

			funcmedicinesearch4multi(serialno);
...

{
	...
	var searchsupplieranum=document.getElementById("searchsupplieranum").value;
...
function calpurchaseindenttotal()
{

	//var itemscnt = $('#itemscnt').val();

	 var sum = 0;
    $('.medicineamt').each(function() {
    
        sum =  parseFloat(sum) + parseFloat($(this).val().replace(/[^0-9\.]+/g,""));
        
    });
    console.log('sum#'+sum+'#')
    var addedsum = 0;
    // Find added items total
    $('.amountaddeddyn').each(function() {
    
        addedsum =  parseFloat(addedsum) + parseFloat($(this).val().replace(/[^0-9\.]+/g,""));
        
    });
    console.log('addedsum'+addedsum);
    var totalsum = parseFloat(sum) + parseFloat(addedsum);
    document.getElementById("total").value=formatMoney(totalsum.toFixed(2));
}
...

{
	...
	var medicinenametype = medicinename;
	...
	var medicinename=document.getElementById("lpotype").value;
...
<form name="form1" id="frmsales" method="post" action="purchaseindentedit.php"  onSubmit="return process()">
        ...
        <td><table width="99%" border="0" align="left" cellpadding="2" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
			...
			<?php }?>
			  ...
			  <tr>
                ...
                <td width="18%" align="left" valign="top" ><input type="text" name="store" id="store" value="<?php echo $store; ?>" size="18" rsize="20" style="border: 1px solid #001E6A" readonly>
...
/*
				...
				$purchasetype1 = $res16["name"];

				?>

                <option value="<?= $purchasetype1; ?>" <?php if($purchasetype == $purchasetype1) echo 'selected'; ?>><?= $purchasetype1; ?></option> 
				...
				?>
				<!-- <option value='ASSETS' <?php //if($purchasetype == 'ASSETS') echo 'selected'; ?>>ASSETS</option> -->
                </select>
...
*/
               ...
               <td width="11%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>Supplier Name</strong></td>
                ...
                <input name="searchsuppliername" type="text" id="searchsuppliername" value="<?php echo $suppliername; ?>" size="30" autocomplete="off">

                <input type="hidden" name="searchsuppliercode" onBlur="return suppliercodesearch1()" id="searchsuppliercode" style="text-transform:uppercase" value="<?php echo $suppliercode; ?>" size="20" />

				 <input type="hidden" name="searchsupplieranum" id="searchsupplieranum" style="text-transform:uppercase" value="<?php echo $supplieranum; ?>" size="20" />

					</td> 
				<?php //} ?>
					<td width="15%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>LPO Type</strong></td>
					  <td width="12%" align="left" valign="top" >

				<select  name="lpotype" id="lpotype" >

                   <!-- <option value="">Select </option> -->
                   <option value="CASH" <?php if($lpotype =='CASH') echo 'selected';?>>CASH </option>
                   <option value="CREDIT" <?php if($lpotype =='CREDIT') echo 'selected';?>>CREDIT </option>
...
-->
      ...
      <tr>
		...
		<table id="medicinelisttbl" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
             ...
             <tr>
                       ...
                       <td bgcolor="#ffffff" class="bodytext31" valign="center"  align="center"><strong>Item Description</strong></td>
			...
			//$query12 = "select * from purchase_indent where docno='$docno' and (approvalstatus LIKE '%reject%')";
			$query12 = "select * from purchase_indent where docno='$docno' and is_deleted <>'1'";
...
?>
  ...
  <tr id="idTR<?=$sno;?>">
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" class="medicine" name="medicinename<?php echo $sno ; ?>" id="medicinename<?php echo $sno ; ?>" serialno="<?php echo $sno ; ?>" value="<?php echo $medicinename;?>"  size="45"></div></td>

		

		<input type="hidden" name="medicinecode<?php echo $sno ; ?>" id="medicinecode<?php echo $sno ; ?>" value="<?php echo $itemcode; ?>">

       

		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="avlqty<?php echo $sno ; ?>" id="avlqty<?php echo $sno ; ?>" value="<?php echo $currentstock;?>"  readonly></div></td>

		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="reqqty<?php echo $sno ; ?>" id="reqqty<?php echo $sno ; ?>" value="<?php echo $reqqty;?>" size="6" onKeyUp="return calcedit('<?php echo $sno; ?>');" ></div></td>

		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="packsize<?php echo $sno ; ?>" id="packsize<?php echo $sno ; ?>" value="<?php echo $package;?>" size="6" class="bal" readonly></div></td>

		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="pkgqty<?php echo $sno ; ?>" id="pkgqty<?php echo $sno ; ?>" size="6" value="<?php echo $packagequantity;?>" class="bal" readonly></div></td>

		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="rate<?php echo $sno ; ?>" id="rate<?php echo $sno ; ?>" value="<?php echo $rate;?>" size="6" class="bal" readonly></div></td>
		<input type="hidden" name="fxrate<?php echo $sno ; ?>" id="fxrate<?php echo $sno ; ?>" value="<?php echo $rate;?>" size="6" class="bal">
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" class="medicineamt" name="amount<?php echo $sno ; ?>" id="amount<?php echo $sno ; ?>" value="<?php echo $amount;?>" size="10" class="bal" readonly>
		<td id="btndelete<?=$sno;?>" align="right" style="background-color: rgb(255, 255, 255); border: 0px solid rgb(0, 30, 106);"><input id="btndelete<?=$sno;?>" rowid="<?php echo $rowid;?>" name="btndelete<?=$sno;?>" type="button" value="Del"  delsno="<?=$sno;?>" class="delete_edit_item" style="border: 1px solid rgb(0, 30, 106);"></td>
		...
		?>
<input type="hidden" name="itemscnt" id="itemscnt" value="<?=$sno;?>">
			  <tr>
              ...
              <td class="bodytext31" valign="center"  align="left" 
                ...
                bgcolor="#ecf0f5"><!-- <input type="text" name="total" id="total" value="<?php echo number_format($totalamount,'2','.',''); ?>" size="10" class="bal" readonly> --></td>
      ...
      <tr id="pressid">
				   ...
				   <table id="presid" width="621" border="0" cellspacing="1" cellpadding="1">
                     ...
                     <tr>
					  ...
					  <input type="hidden" name="medicinecode" id="medicinecode" value="">
                       ...
                       <td><input name="medicinename" type="text" id="medicinename" size="45" autocomplete="off"  onClick="clickmedicine();">
                       ...
                       <td width="48"><input name="rate" type="text" id="rate"  size="8" onKeyUp="return calc();" autocomplete="off" readonly>
				   ...
				   <td colspan="8" align="right" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><span class="style2">Total</span><input type="text" id="total" readonly size="7" value="<?php echo number_format($totalamount,'2','.',''); ?>"></td>
	 ...
	 <td width="66" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">User Name</td>
	   ...
	   <td width="111" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"> <input type="text" name="username" value="<?php echo $username; ?>" size="10" readonly></td>

	    <td width="47" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">Status</td>

	   <td width="120" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"> <input type="text" name="status" value="RFA" size="10"></td>

	    <td width="84" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">Memo</td>

	    <td width="260" valign="middle"  bgcolor="#ecf0f5" class="bodytext3" calign="left"><textarea name="remarks" id="remarks"><?php echo $indent_memo; ?></textarea></td>
	 ...
	 <td colspan="4" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">

	 <input type="radio" name="priority" id="priority1" value="Critical" <?php if($priority =="Critical") echo 'checked';?>><label for="priority1">Critical</label>&nbsp;&nbsp;

	 <input type="radio" name="priority" id="priority2" value="High" <?php if($priority =="High") echo 'checked';?>><label for="priority2">High</label>&nbsp;&nbsp;

	 <input type="radio" name="priority" id="priority3" value="Medium" <?php if($priority =="Medium") echo 'checked';?>><label for="priority3">Medium</label>&nbsp;&nbsp;

	 <input type="radio" name="priority" id="priority4" value="Low" <?php if($priority =="Low") echo 'checked';?>><label for="priority4">Low</label>&nbsp;&nbsp;
      ...
      <tr>
          ...
          <input type="hidden" name="docnoedit" value="<?php echo $docno_edit; ?>" />
