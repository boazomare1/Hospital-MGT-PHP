...
if($_POST['discard'] != $docno)
{
	...
	//echo "<br>".$itemcode;
		...
		foreach($_POST['app'] as $check)
		{
			...
			$status = 'partially';
				...
				$qrypurs = mysqli_query($GLOBALS["___mysqli_ston"], "select limit_amount from master_purchaselimit where purchasetype = '$purchasetype'");
				if($respurs = mysqli_fetch_assoc($qrypurs))
				{
				//echo 'as';
				//echo str_replace(',','',$_POST['totalamount'])*$_POST['fxamount'];
				if($respurs['limit_amount']>str_replace(',','',$_POST['totalamount'])*$_POST['fxamount'])
				{
				 $status = 'approved';
		...
		if(strtolower(trim($purchasetype))!=strtolower('Expenses') && strtolower(trim($purchasetype))!=strtolower('Others'))
				{
					$query45="update purchase_indent set approvalstatus='$status',quantity='$reqqty',packagequantity='$pkgqty',amount='$amount', remarks='$remarks',ceoamailfrom='$ceomailfrom',ceomailcc='$ceomailcc',fausername='$username' where docno='$docno' and medicinecode='$itemcode' and auto_number='$medanum'";
				}
				else
				{
					$query45="update purchase_indent set approvalstatus='$status',quantity='$reqqty',packagequantity='$pkgqty',amount='$amount', remarks='$remarks',ceoamailfrom='$ceomailfrom',ceomailcc='$ceomailcc',fausername='$username' where docno='$docno' and auto_number='$itemcode'";
				}
  ...
  foreach($_POST['medicinename'] as $key => $value)
	...
	//echo "<br>".$itemcode;
		...
		if(strtolower(trim($purchasetype))!=strtolower('Expenses') && strtolower(trim($purchasetype))!=strtolower('Others'))
				{
					$query45="update purchase_indent set approvalstatus='rejected1',quantity='$reqqty',packagequantity='$pkgqty',amount='$amount', remarks='$remarks',ceoamailfrom='$ceomailfrom',ceomailcc='$ceomailcc',fausername='$username' where docno='$docno' and medicinecode='$itemcode' and auto_number='$medanum'";
				}
				else
				{
					$query45="update purchase_indent set approvalstatus='rejected1',quantity='$reqqty',packagequantity='$pkgqty',amount='$amount', remarks='$remarks',ceoamailfrom='$ceomailfrom',ceomailcc='$ceomailcc',fausername='$username' where docno='$docno' and auto_number='$itemcode'";
				}
				$exec45 = mysqli_query($GLOBALS["___mysqli_ston"], $query45) or die(mysqli_error($GLOBALS["___mysqli_ston"]));	
				$actionstatus='reject';
		...
		include('indentmail.php');	
				header("location:viewpurchaseindent2.php");
...
if(grandtotalamount*document.getElementById("fxamount").value>parseFloat(document.getElementById("limitamount").value))
{
...
<form name="form1" id="frmsales" method="post" action="purchaseindentapproval2.php" onKeyDown="return disableEnterKey(event)" onSubmit="return process()">
    ...
    <td width="99%" valign="top"><table width="980" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><table width="99%" border="0" align="left" cellpadding="2" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
			...
			{
			$docno = $_REQUEST['docno'];
			$query12 = "select suppliername,purchasetype,currency,fxamount,bamailfrom,famailfrom,famailcc,locationcode,storecode,povalidity,s_terms from purchase_indent where docno='$docno' and (approvalstatus='1' or approvalstatus='rejected2')";
			...
			$purchasetype1=strtolower(trim($purchasetype));
			$qrypust=mysqli_query($GLOBALS["___mysqli_ston"], "select limit_amount from master_purchaselimit where purchasetype='$purchasetype1'");
			$respust = mysqli_fetch_assoc($qrypust);
			$approvelimit = $respust['limit_amount'];
			}
            ...
            <td width="11%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>Store</strong></td>
             ...
             <td width="11%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>Exchange Rate</strong></td>
                ...
                <?php 
				$query1mail = "select emailto,emailcc from master_email where recordstatus <> 'deleted' and module='Finance Approval' order by auto_number desc";
                ...
                <td colspan="7" align="left" valign="top" ><?php echo $famailfrom;?>
                <input type="hidden" name="bamailfrom" id="bamailfrom" value="<?php echo $bamailfrom;?>">
                <input type="hidden" name="famailfrom" id="famailfrom" value="<?php echo $famailfrom;?>">
                <input type="hidden" name="famailcc" id="famailcc" value="<?php echo $famailcc;?>">
                </td>
				</tr>
				<tr>
                 <td width="8%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>To</strong></td>
                <td colspan="7" align="left" valign="top" ><?php echo $emailto;?><input type="hidden" name="ceomailfrom" id="ceomailfrom" value="<?php echo $emailto;?>"></td>
				</tr>
				<tr>
                <td width="9%" align="left" valign="middle" class="bodytext3" ><strong>CC</strong></td>
                <td colspan="7" align="left" valign="top" ><?php echo $emailcc;?><input type="hidden" name="ceomailcc" id="ceomailcc" value="<?php echo $emailcc;?>"></td>
			    </tr>
			    <tr>
      ...
      <tr>
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
			...
			$total_taxamount = 0;
			$totalamount=0;			
			$query12 = "select * from purchase_indent where docno='$docno' and (approvalstatus='1' or approvalstatus='rejected2') and is_deleted=0";
			$exec12= mysqli_query($GLOBALS["___mysqli_ston"], $query12) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
			$numb=mysqli_num_rows($exec12);
...
?>
  <tr id="tr_<?php echo $row_id; ?>">
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="reqqty[]" id="reqqty<?php echo $sno ; ?>" value="<?php echo $reqqty;?>" size="6"  class="bal" readonly></div></td>
	  ...
	  <tr id="limitmsg" <?php if($totalamount*$fxamount<$approvelimit){echo 'style="display:none;"';} ?>>
	  <td bgcolor="#FF0000"><strong>Indent Exceeds Limit for <?php echo ucwords($purchasetype); ?>. Indent will be available for C.E.O approval.</strong>
	  </td>
	  </tr>
	  <tr>
	  <td>&nbsp;
	 ...
	 <td width="66" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">User Name</td>
	   ...
	   <td width="120" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"> <input type="checkbox" id="selectall" onClick="selectAll(this)"></td>
     	...
     	<td colspan="7">
     		<label>Supplier Terms and conditions: </label>
				                    ...
				                    <textarea id="consultation" cols='50' disabled="" rows='15'  class="ckeditor" name="editor1" ><?php echo $sterms ;?></textarea>
      ...
      <tr>
		 ...
		 <input type="hidden" id="limitamount" value="<?php echo $approvelimit; ?>"/>
