...
if ($upload == 'success')
{
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 3 && $forloopcount != 5  && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			$packsize = $splitdelimiter[5];
			$packsize = strtoupper($packsize);
			$packagequantity = $splitdelimiter[6];
			$packagequantity = strtoupper($packagequantity);
			$rate = $splitdelimiter[7];
			$rate = strtoupper($rate);
			$amount = $packagequantity * $rate;
			$amount = strtoupper($amount);
			$countryoforigin = $splitdelimiter[9];
			$countryoforigin = strtoupper($countryoforigin);
			$expirydate = $splitdelimiter[10];
			...
			if ($itemcode != '')
			{
			$query2 = "insert into master_rfq (medicinecode,medicinename,suppliername,suppliercode,packsize,packagequantity,rate,amount,countryoforigin,expirydate,username,companyanum,ipaddress,date) values ('$itemcode','$itemname','$suppliername','$suppliercode','$packsize','$packagequantity','$rate','$amount','$countryoforigin','$expirydate','$username','$companyanum','$ipaddress','$date')";
...
header("location:uploadrtq.php?st=success&&upload=success");
}
?>
<style type="text/css">
<!--
