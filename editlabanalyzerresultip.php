...
foreach($_POST['lab'] as $key => $value)
		...
		if($resultvalue != '')
		{
			$query26="insert into ipresultentry_lab(patientname,patientcode,patientvisitcode,recorddate,recordtime,itemcode,itemname,resultvalue,referencerange,referenceunit,docnumber,referencename,account,color,username,sampleid,doctorname,resultstatus,analysercode,analysername,refcode)values('$patientname','$patientcode','$visitcode','$dateonly','$timeonly','$itemcode','$labname','$resultvalue','$referencevalue','$unit','$docnumber','$refname','$accountname','$color','$username','$sampleid','$doctorname','completed','$analysercode','$analysername','$refcode')";
	 		$exec26=mysqli_query($GLOBALS["___mysqli_ston"], $query26) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
	   
			$query27 ="update resultentry_laban  set publishstatus='completed',publishdatetime='$updatedatetime' where patientcode='$patientcode' and patientvisitcode='$visitcode' and referencename='$refcode' and publishstatus='' and analysercode='$analysercode'";
...
<body>
<form name="frm" id="frmsales" method="post" action="editlabanalyzerresultip.php" onKeyDown="return disableEnterKey(event)">
  ...
  <tr>
    ...
    <td colspan="3" align="left" valign="middle"  bgcolor="<?php if ($errmsg == '') { echo '#ecf0f5'; } else { echo 'red'; } ?>" class="bodytext3"><strong><?php echo $errmsg;?>&nbsp;</strong></td></tr>
     ...
     <tr>
	  ...
	  $buliditemcode="''";
    $query31="select analysercode,analysername,referencename from resultentry_laban where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and resultstatus = 'completed' and publishstatus='' ";
	...
	{ 
		...
		while($res3111=mysqli_fetch_array($exec3111))
		{
			...
			while($res52=mysqli_fetch_array($exec52))
			{
				...
				$query43 = "select resultvalue from resultentry_laban where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and resultstatus = 'completed' and publishstatus='' and referencename='$refcode' order by auto_number desc";
				...
				if($num64 > 0)
				{
					...
					$query49 = "select * from resultentry_laban where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and resultstatus = 'completed' and publishstatus='completed' and referencename='$refcode' order by auto_number desc limit 0,1";
