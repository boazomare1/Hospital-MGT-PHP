// Balance Sheet Modern JavaScript
...
function validateSearchForm() {
    ...
    if (ADate2 && ADate2.value.trim() === '') {
        markFieldAsInvalid(ADate2, 'Date is required');
        isValid = false;
    }
    
    // Validate date format
    if (ADate2 && ADate2.value.trim() !== '') {
        const dateValue = new Date(ADate2.value);
        if (isNaN(dateValue.getTime())) {
            markFieldAsInvalid(ADate2, 'Please enter a valid date');
            isValid = false;
        }
        ...
        if (dateValue > today) {
            markFieldAsInvalid(ADate2, 'Date cannot be in the future');
            isValid = false;
        }
    }
