...
if (isset($_REQUEST["sortfunc"])) { $sortfunc = $_REQUEST["sortfunc"]; } else { $sortfunc = ""; }
...

			{
				...
				where a.recordstatus='pending' and Date(a.updatedatetime) between '$fromdate' and '$todate' and a.tostore='$searchstorecode' AND a.fromstore LIKE '%$fromstockstore%' and a.approval_status='approved' group by docno order by storename $sortfunc";
		 ...
		 $query1 = "select fromstore,updatedatetime,docno,tostore,typetransfer,username from master_internalstockrequest where recordstatus='pending' and Date(updatedatetime) between '$fromdate' and '$todate' and tostore='$searchstorecode' AND fromstore LIKE '%$fromstockstore%' and approval_status='approved' group by docno order by tostore $sortfunc";
