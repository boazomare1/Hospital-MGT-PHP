...
//ACCOUNT WISE OPENING BALANCE OUTSTANDING END	  
      ...
      while($res42 = mysqli_fetch_array($exec42))
      {
        ...
        if($res2visitcode=='PH-57733'){
			echo "santu";
		
    echo "tar".    $res2transactionamount = $res42['fxamount'];
	echo "</br>";
...
//AND accountnameid in($subtypes_accountids_str) 
	...
	echo $query98 = "SELECT sum(a.fxamount +a.discount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where a.transactiontype = 'PAYMENT' AND a.transactionstatus!= 'onaccount' AND b.transactionstatus= 'onaccount' and a.billnumber='$res2billnumber' and a.recordstatus = 'allocated' AND a.accountnameid='$accntid' 
	
				UNION ALL  select sum(a.fxamount +a.discount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where a.transactionmode = 'CREDIT NOTE' AND a.transactiontype= 'PAYMENT' AND b.transactiontype= 'paylatercredit' and a.billnumber='$res2billnumber' and a.recordstatus = 'allocated'   AND a.accountnameid='$accntid'
	
				UNION ALL  select sum(-1*a.fxamount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where  a.transactiontype = 'PAYMENT' AND b.transactiontype != 'PAYMENT' and a.docno='$res2billnumber' and a.recordstatus = 'allocated'  AND a.accountnameid='$accntid' AND a.docno NOT LIKE '%AR%' 
				
				UNION ALL select sum(-1*a.fxamount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno)  where  b.transactionstatus = 'onaccount' AND a.transactionstatus != 'onaccount' and a.docno='$res2billnumber' and a.recordstatus = 'allocated' AND  a.docno  LIKE '%AR%'  and a.accountnameid in ($accntid)   ";			
...
*/
		...
		echo "allo".  $allocatedamount = $allocatedamount + $payment;
echo "</br>";
		}
...
$pendig_final_value_outstanding = 0;
        ////////////// FOR AR DOCS PENDING AMOUNT //////////////
          ...
          $dot_ar_doc = $dotarray[0];
          if($dot_ar_doc=='AR'){
            echo      $query2 = "SELECT * from master_transactionpaylater where  docno='$res2billnumber' AND transactionmodule = 'PAYMENT' AND transactionstatus = 'onaccount' order by auto_number desc LIMIT 1";
				...
				echo "</br>";
         echo   $query_allocated_amount = "SELECT sum(transactionamount) as amount, sum(discount) as discount from master_transactionpaylater where  recordstatus='allocated' and docno='$res2billnumber'";
