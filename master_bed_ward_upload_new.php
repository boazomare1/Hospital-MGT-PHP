...
{	
		...
		try {
			...
			if($rowData[$key] == 'DAILY REVIEW')
			 $daily_charges1 = $key;

			if($rowData[$key] == 'SUNDRIES')
			 $acc_charges1 = $key;
    		...
    		$rowData = $sheet->rangeToArray('A' . $row . ':' . $highestColumn . $row,
					...
					$daily_charges = $rowData[$daily_charges1];
					$acc_charges = $rowData[$acc_charges1];
		 
			if($bed_charges!="")
				{
 

									$ward_id = trim($ward_name);
 	...
 	// , `wardnamw`, `accommodationcharges`, `cafetariacharges`, `inh_review`, `int_review`, `adms_review`, `accommodationOnly`
									$query1 = "INSERT INTO bed_OTHER_Insurances(`bed`, `ward`, `bedcharges`, `nursingcharges`, `rmocharges`,`accommodationcharges`, `threshold`, `grace`, `locationname`, `locationcode`, `recordstatus`, `ipaddress`, `recorddate`, `username`) 
									VALUES ('$bed_name','$ward_id','$bed_charges','$nur_charges',$daily_charges,$acc_charges,'','','$locationname','$locationcode','','$ipaddress','$updatedatetime','$username')";
					...
					echo "<script>window.location.href = 'master_bed_ward_upload_new.php'</script>";
