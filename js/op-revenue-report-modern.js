/**
 * OP Revenue Report - Modern JavaScript
 * Enhanced functionality for OP revenue report page
 */
...
function initializeFormEnhancements() {
    ...
    if (reportForm) {
        reportForm.addEventListener('submit', function(e) {
            ...
            showLoadingOverlay('Generating OP Revenue Report...');
        });
    }
...
function updateLocationDisplay(locationCode) {
    ...
    if (locationDisplay) {
        const selectedOption = document.querySelector(`#location option[value="${locationCode}"]`);
        if (selectedOption) {
            locationDisplay.innerHTML = `<strong>Location: ${selectedOption.textContent}</strong>`;
        }
    }
}

function updateDepartmentDisplay(departmentCode) {
    const departmentDisplay = document.getElementById('ajaxdepartment');
    if (departmentDisplay) {
        const selectedOption = document.querySelector(`#department option[value="${departmentCode}"]`);
        if (selectedOption) {
            departmentDisplay.innerHTML = `<strong>Department: ${selectedOption.textContent}</strong>`;
        }
    }
}
...
function initializeTableEnhancements() {
    const tables = document.querySelectorAll('.modern-table, .revenue-table');
    tables.forEach(table => {
        ...
        // Add revenue link enhancements
        enhanceRevenueLinks(table);
    });
}
...
function initializeTableSorting(table) {
    ...
    headers.forEach((header, index) => {
        ...
        if (header.textContent.includes('Actions') || 
            header.textContent.includes('Payment Type') ||
...
function enhanceRevenueLinks(table) {
    const links = table.querySelectorAll('a[href*="oprevenuereport_cash_"]');
    links.forEach(link => {
        link.classList.add('revenue-link');
        link.addEventListener('click', function(e) {
            // Add loading state for detail links
            showLoadingOverlay('Loading Revenue Details...');
        });
    });
}
...
function exportToExcel() {
    ...
    const table = document.querySelector('.revenue-table, .modern-table');
    if (!table) {
    ...
    link.setAttribute('download', 'op_revenue_report.csv');
