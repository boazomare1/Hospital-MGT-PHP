...
//get ends here
	$paynowbillprefix = 'OSR';
$paynowbillprefix1=strlen($paynowbillprefix);
$query2 = "select * from other_sales order by auto_number desc limit 0, 1";
...
else
{
	...
	$name = $_REQUEST['os_name'];
		...
		if($pairvar!="")
		{
		$referalquery1=mysqli_query($GLOBALS["___mysqli_ston"], "insert into other_sales(docno,units,amount,description,rate,billingaccountcode,billingaccountname,consultationdate,paymentstatus,consultationtime,remarks,username,ipaddress,name,locationname,locationcode)values('$billnumbercode','$units','$amount','$pairvar','$pairvar1','$accountcode','$accountname','$dateonly','pending','$timeonly','$remarks','$username','$ipaddress','$name','".$locationnameget."','".$locationcodeget."')") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
		
		}
}
	
		header("location:othersales.php");
		exit;
}
...
<script type="text/javascript" src="js/insertnewitemothersales.js"></script>
...
function btnDeleteClick4(delID4,varamount1)
{
	...
	//alert(currenttotal);
	newtotal2= parseFloat(currenttotal2)-parseFloat(misrate);
	
	var availableamount_org=Number(document.getElementById("availableamount_org").value.replace(/[^0-9\.]+/g,""));
	document.getElementById('availableamount_org').value=formatMoney(parseFloat(availableamount_org)+parseFloat(misrate));
...
<body >
<form name="form1" id="frmsales" method="post" action="othersales.php" onKeyDown="return disableEnterKey(event)" onSubmit="return validcheck()">
      ...
      <tr>
        <td width="792"><table width="99%" border="0" align="left" cellpadding="2" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
            <tbody>
                ...
                <tr bgcolor="#011E6A">
                <td colspan="4" bgcolor="#ecf0f5" class="bodytext32"><strong>Other Sales</strong></td>
                 <td colspan="3" class="bodytext31" bgcolor="#ecf0f5"><strong>Location &nbsp;</strong><?php echo $locationname;?></td>
		...
		<tr>
                ...
                <td align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3" >
				<input type="hidden" size="6"  id="grand_total" value="0" readonly style="border: 1px solid red;">
                <input type="text" size="25"  id="os_name" name="os_name" value="" />
                </td>
				  </tr>
