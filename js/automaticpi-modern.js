// Automatic Purchase Indent Modern JavaScript
...
function validateSearchForm() {
    ...
    if (locationcode && locationcode.value.trim() === '') {
        markFieldAsInvalid(locationcode, 'Location is required');
        isValid = false;
    }
    
    // Validate store
    if (storecode && storecode.value.trim() === '') {
        markFieldAsInvalid(storecode, 'Store is required');
        isValid = false;
    }
...
function itemcheck() {
    ...
    if (checkboxes.length === 0) {
        showAlert('Please select at least one item to generate indent.', 'warning');
        return false;
    }
    
    showAlert(`Proceeding with ${checkboxes.length} selected items.`, 'info');
    return true;
}
