...
//ends here
			...
			$query40 = "select rate from transaction_stock where itemcode='$itemcode' and batchnumber ='$batchnumber' and fifo_code='$fifo_code' and storecode ='$getstorecode' limit 0,1";
	...
	{
           $query612 = "select sum(quantity) as totqty from pharmacysales_details where itemcode='".$itemcode."' and patientcode = '$patientcode' and visitcode = '$visitcode' and docnumber='$docnumber'";
		  ...
		  $query613 = "select sum(quantity) as alreadyreturn from pharmacysalesreturn_details where itemcode='".$itemcode."' and patientcode = '$patientcode' and visitcode = '$visitcode' and docnumber='$docnumber'";
	...
	{// echo 'in';
		...
		$execupdatebatstock2 = mysqli_query($GLOBALS["___mysqli_ston"], $queryupdatebatstock2) or die ("Error in updatebatQuery2".mysqli_error($GLOBALS["___mysqli_ston"]));
		 ...
		 $stockquery2="insert into transaction_stock (fifo_code,tablename,itemcode, itemname, transaction_date,transactionfunction,description,
			...
			'$cum_quantity', '$billnumber', '','1','1', '$getlocationcode','','$getstorecode', '', '$username', '$ipaddress','$dateonly','$timeonly','$datetimeonly','$patientcode','$visitcode','$patientname','$costprice','$totalcp','$ledgeranum','$ledgercode','$ledgername','$incomeledger','$incomeledgercode')";
	...
	{ echo 'ii';
        ...
        echo '<script> alert("Some item faild to return/already returned, please check again"); window.location="medicineissuelist.php";</script>';
	    exit;
	}
...
if(confirm("Are You Want To Save The Record?")==false){document.getElementById("savebutton").disabled == false;return false;}	
...
<!-- <body onLoad="funcOnLoadBodyFunctionCall(),onload_totals();"> -->
<body >

<form name="frmsales" id="frmsales" method="post" action="pharmacyrefund.php"  onSubmit="return validcheck()">
			...
			<input name="visitcode" id="visitcode" value="<?php echo $visitcode; ?>" style="border: 1px solid #001E6A" size="18" rsize="20" readonly/>
			  ...
			  <td align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><span class="style4"></span><strong>Age &amp; Gender </strong></td>
				...
				<input name="billnumber" id="billnumber" value="<?php echo $billnumbercode; ?>" style="border: 1px solid #001E6A" size="18" rsize="20" readonly/>
				  ...
				  <tr> 
				  	 <td colspan="5" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"> 
				  <?php
      ...
      <tr>
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
			...
			<td width="8%"  align="left" valign="center" 
                ...
                bgcolor="#ffffff" class="bodytext31"><div align="center"><strong>Amount</strong></div></td>
                  ...
                  <?php
						...
						$query65= "select * from master_employeelocation where username='$username' and defaultstore='default' and locationcode='".$locationcode."'";
			...
			$query71 = "select * from pharmacysales_details where locationcode='".$locationcode."' and patientcode = '$patientcode' and visitcode = '$visitcode' and docnumber='$docnumber' and store='$storecodeget'";
...
$nno=$nno+1;
	...
	//echo $nno;
			...
			$query61 = "select * from pharmacysales_details where locationcode='".$locationcode."' and patientcode = '$patientcode' and visitcode = '$visitcode' and docnumber='$docnumber' and store='$storecodeget'";
...
$returnbillno=$res62['billnumber'];
 $sqlts="select fifo_code,transaction_quantity from transaction_stock where itemcode='$itemcode' and locationcode='$locationcode' and entrydocno='$returnbillno' and storecode ='$storecodeget'";
 $exects62 = mysqli_query($GLOBALS["___mysqli_ston"], $sqlts) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
 $numbtsr=mysqli_num_rows($exects62);
 if($numbtsr>0){
  $rests62 = mysqli_fetch_array($exects62);
     $returnfifo_code=$rests62['fifo_code'];
	 if($fifo_code==$returnfifo_code){
     $avaquantity=$rests62['transaction_quantity'];
...
?>
  ...
  <td class="bodytext31" valign="center"  align="left">
  		<div align="center">
  			<?php if(($num_billno>0) && ($capitation_check=='1')){  ?>
  				<input type="checkbox" checked=""  style=" /*outline:1px solid blue;*/ color: blue;" disabled="">
        		<input type="hidden" name="ref[]" id="ref<?php echo $sno; ?>" value="<?php echo $sno; ?>" onClick="return updatebox('<?php echo $sno; ?>','<?php echo $resquantity;?>','<?php echo $nno; ?>')" checked=''/> 
        	<?php  }else{  ?>
        		 
        			<input type="checkbox" name="ref[]" id="ref<?php echo $sno; ?>" value="<?php echo $sno; ?>" onClick="updatebox('<?php echo $sno; ?>','<?php echo $resquantity;?>','<?php echo $nno; ?>')"/>
        	<?php }  ?>
        </div>
    </td>
        ...
        <input type="hidden" name="fifo_code[]" value="<?php echo $fifo_code; ?>">
			...
			<td class="bodytext31" valign="center"  align="left"><div align="center">
				<?php if(($num_billno>0) && ($capitation_check=='1')){  ?><?=$balanceqty;?>
							<input type="hidden" name="returnquantity[]" id="returnquantity<?php echo $sno; ?>" onKeyUp="balancecalc('<?php echo $sno; ?>','<?php echo $resquantity;?>','<?php echo $nno; ?>')" onKeyPress="return number(event)" value='<?=$balanceqty;?>'  size="7" readonly>
					<?php  }else{  ?>
				<input type="text" name="returnquantity[]" id="returnquantity<?php echo $sno; ?>" onKeyUp="balancecalc('<?php echo $sno; ?>','<?php echo $resquantity;?>','<?php echo $nno; ?>')" onKeyPress="return number(event)"  size="7">
					<?php }  ?>
			</div></td>

			<td class="bodytext31" valign="center"  align="left"><div align="center"><?php echo $refundedquantity;?></div></td>

			<input type="hidden" name="quantity[]" value="<?php echo $resquantity;?>" id="quantity<?php echo $sno; ?>">

		   <td class="bodytext31" valign="center" bordercolor="#f3f3f3">

		   
		   	<?php if(($num_billno>0) && ($capitation_check=='1')){  ?>
								<input type="text" class="bal" name="balamount[]" id="balamount<?php echo $sno; ?>" size="7" readonly value="0">
					<?php  }else{  ?>
					<input type="text" class="bal" name="balamount[]" id="balamount<?php echo $sno; ?>" size="7" readonly value="<?php echo $balanceqty; ?>">
					<?php  }  ?>
		 ...
		 <td class="bodytext31" valign="center"  align="left"><div align="center">
					<input type="text" class="bal" name="amount[]" id="amount<?php echo $sno; ?>" size="7" readonly>
		 </div></td>
		 ...
		 <input type="hidden" name="docnumber[]" value="<?php echo $docnumber;?>">
           ...
           <input type="hidden" name="salseautonumber[]" value="<?php echo $salseautonumber;?>">
					  ...
					  <!-- <element onLoad="balancecalc('<?php echo $sno; ?>','<?php echo $resquantity;?>','<?php echo $nno; ?>')"></element> -->
		...
		?>
			  ...
			  <tr>
			  	<input type="hidden" name="finalno" id="finalno" value="<?php echo $nno;?>">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td width="54%" align="right" valign="top" >
             	  ...
             	  <input name="Submit2223" type="submit" id="savebutton" value="Save"  accesskey="b" class="button" style="border: 1px solid #001E6A"/>
...
// alert(document.getElementById("quantity1").value);
var k;
var grandtotal=0;
...
// alert(finalno);
	for(k=1;k<=finalno;k++)
	{
		var qnty1=document.getElementById("quantity"+k+"").value;
		// alert(qnty1);
		var totalcount1='100';
		// alert(totalcount1);

		if(document.getElementById("ref"+k+"").checked == true)
		{
		      balancecalcCapitation(k,qnty1);

						var totalamount=document.getElementById("amount"+k+"").value;
						// alert(totalamount);
						 grandtotal=parseFloat(grandtotal)+parseFloat(totalamount);
						// alert(grandtotal);
...
function balancecalcCapitation(varSerialNumber1,qnty1)
{
...
for(i=1;i<=k;i++)
{
	...
	//alert(totalamount);
	var grandtotal=parseFloat(grandtotal)+parseFloat(totalamount);
}
document.getElementById("totalamt").value=grandtotal.toFixed(2);
