...
if ($frm1submit1 == 'frm1submit1')
{
	...
	$query21 = "select * from ipconsultation_radiology where patientcode='$patientcode' and patientvisitcode='$visitcode' order by auto_number desc limit 0, 1";
		...
		$radiologyquery= mysqli_query($GLOBALS["___mysqli_ston"], "select * from master_radiology where itemname='$pairvar'") or die ("Error in query21".mysqli_error($GLOBALS["___mysqli_ston"])); 
		...
		$radiologyexecquery1= mysqli_query($GLOBALS["___mysqli_ston"], $radiologyquery1) or die ("Error in radiologyquery1".mysqli_error($GLOBALS["___mysqli_ston"]));

		$radiologyquery2= mysqli_query($GLOBALS["___mysqli_ston"], "update master_visitentry set radiologybill='pending' where visitcode='$visitcode'") or die ("Error in query21".mysqli_error($GLOBALS["___mysqli_ston"])); 
		}
		}
...
$data=mysqli_query($GLOBALS["___mysqli_ston"], "select auto_number,patientcode,patientvisitcode,patientname,refno,radiologyitemcode,radiologyitemname,radiologyitemrate from ipconsultation_radiology where auto_number='$radiologyname'") or die ("Error in radiologyquery1".mysqli_error($GLOBALS["___mysqli_ston"]));
...
$remarks=stripslashes(urldecode($_REQUEST['remarks-'.$radiologyname]));

if($medicinecode<>'')

{

 $query="insert into amendment_details (patientcode,visitcode,patientname,refno,itemcode,itemname,rate,amenddate,amendtime,amendfrom,amendby,ipaddress,type,remarks) values ('$patientcode','$visitcode','$patientname','$refno','$medicinecode','$medicinename','$rate','$date','$time','radiology','$username','$ipaddress','IP','$remarks')";

mysqli_query($GLOBALS["___mysqli_ston"], $query) or die ("Error in query".mysqli_error($GLOBALS["___mysqli_ston"]));

}
mysqli_query($GLOBALS["___mysqli_ston"], "delete from ipconsultation_radiology where auto_number='$radiologyname' and patientvisitcode='$visit'") or die ("Error in query".mysqli_error($GLOBALS["___mysqli_ston"]));
}
...
-->
  <tr>
	...
	$status='completed';
			...
			$query17 = "select * from ipconsultation_radiology where patientvisitcode='$visitcode' and patientcode='$patientcode' and resultentry='pending'";
			...
			?>
			 ...
			 <td class="bodytext31" valign="center"  align="left"><div align="center"><?php echo $consultationdate; ?>
