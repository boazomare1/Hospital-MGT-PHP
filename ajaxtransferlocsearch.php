...
$a_json = array();
$a_json_row = array();
$query1 = "select id,accountname from master_accountname where accountname like '%$customer%' and accountssub = '19' and recordstatus <> 'Deleted' order by auto_number limit 20";
$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
while ($res1 = mysqli_fetch_array($exec1))
{
	$res1customercode = $res1['id'];
	$res1customerfullname=$res1['accountname'];
		
	$res1customercode = trim($res1customercode);
	$res1customerfullname = trim($res1customerfullname);
	...
	$a_json_row["acccode"] = $res1customercode;
	$a_json_row["accname"] = $res1customerfullname;
	$a_json_row["value"] = trim($res1customerfullname);
	$a_json_row["label"] = trim($res1customerfullname);
