...
/*$selectitempono = $_POST['selectitempono'][$key];
...
$itemstatus = '';
			...
			{		
				...
				'$allpackagetotalquantity', '$billnumbercode', 'New Batch','1','1', '$locationcode','','$store1', '', '$username', '$ipaddress','$updatedate','$updatetime','$updatedatetime','$costprice','$totalamount','$ledgeranum','$ledgercode','$ledgername','$incomeledger','$incomeledgercode')";
				...
				values ('$fifo_code','purchase_details','$itemcode', '$itemname', '$updatedate','1', 'Purchase', 
				'$batchnumber', '$allpackagetotalquantity', '$allpackagetotalquantity', 
				'$cum_quantity', '$billnumbercode', 'New Batch','1','1', '$locationcode','','$store1', '', '$username', '$ipaddress','$updatedate','$updatetime','$updatedatetime','$costprice','$totalamount','$ledgeranum','$ledgercode','$ledgername','$incomeledger','$incomeledgercode')";
			...
			'$batchnumber', '$costprice', '$expirydate', '$free', '$quantity', 
			'$packageanum', '$packagename', '$quantityperpackage', '$allpackagetotalquantity', 
			...
			{
				// get po auto_number id for the record
			...
			$sqlchk="select pharmacyformula from master_company";
			$exec101  = mysqli_query($GLOBALS["___mysqli_ston"], $sqlchk) or die ("error in sqlchk".mysqli_error($GLOBALS["___mysqli_ston"]));
			$res101   = mysqli_fetch_array($exec101);
			
			if(isset($res101["pharmacyformula"]) && $res101["pharmacyformula"]==2) {
			...
			while($res_pharma_rate = mysqli_fetch_array($exec_pharma_rate)){
				...
				$query10 = "select margin from pharma_template_map where productcode='".$itemcode."' and templateid='$temp_id' order by auto_number desc limit 0,1";
				$exec10  = mysqli_query($GLOBALS["___mysqli_ston"], $query10) or die ("error in query10".mysqli_error($GLOBALS["___mysqli_ston"]));
				$numb=mysqli_num_rows($exec10);	
				if($numb>0)
				{
                   $res10   = mysqli_fetch_array($exec10);
				   $prev_markup=$res10['margin'];
				}
				...
				$sqlquerymap= "INSERT INTO pharma_template_map(templateid, productcode, rate, username,dateadded, recordstatus,margin) VALUES ('$temp_id','$item_id','$new_price','$username','$updatedatetime','','$markup')";
...
function totalcalc(varserialnumber)
{
	...
	if(receivedqty != '')
	{
		var purchasetype = document.getElementById("purchasetype"+varserialnumber+"").value;
...
// aaa
var purchasetype = document.getElementById("purchasetype"+varserialnumber2+"").value;
...
var taxper = document.getElementById("taxper"+varserialnumber2+"").value;
if(parseFloat(tot)>'0')
{

	if(parseFloat(taxper)>'0'){
	   var costprice1 = parseFloat(totalamount)/parseFloat(tot);
       var costprice2 = costprice1 * (taxper/100);
...
var taxamount = parseFloat(totalamount) * parseFloat((taxper / 100));

document.getElementById("tax"+varserialnumber2+"").value = taxamount.toFixed(2);

var taxamt = document.getElementById("tax"+varserialnumber2+"").value;

totalamount = parseFloat(totalamount) + parseFloat(taxamt);

totalfxamount = parseFloat(totalfxamount) + parseFloat(taxamt);
...
if(purchasetype =='ASSETS' || purchasetype=='non-medical')
{
	document.getElementById("saleprice"+varserialnumber2+"").value = costprice1.toFixed(2);
}
...
var purchasetype = document.getElementById("purchasetype"+i+"").value;
  if(purchasetype !='ASSETS')
  {
	...
	var d = new Date();
    var n = d.getFullYear();
	var curyear = parseInt(n.toString().substring(2));
	if (varItemExpiryDateYear < curyear )
	{
		alert ("Expiry Year Should Be higher or equal to current year.");
		document.getElementById("expirydate"+i+"").focus();
		document.getElementById("savebutton").disabled=false;
		return false;
	}
...
-->
  <tr>
    ...
    <td width="99%" valign="top"><table width="980" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td>
		 <form name="cbform1" id="cbform1" method="post" action="materialreceivednote.php"> 
             ...
             <td width="11%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>Time </strong></td>
                ...
                <td colspan="3" width="27%" align="left" valign="middle" class="bodytext3" style="display:none">
				 <select name="store" id="store" onChange="return storeassign()" >
                  ...
                  <?php
				$query5 = "select * from master_store where location = '$locationanum' and recordstatus = '' and is_grnstore='1'";
				$exec5 = mysqli_query($GLOBALS["___mysqli_ston"], $query5) or die ("Error in Query5".mysqli_error($GLOBALS["___mysqli_ston"]));
				while ($res5 = mysqli_fetch_array($exec5))
				{
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
         ...
         <input type="hidden" name="purchasetype" id="purchasetype" value="<?php echo $purchasetype; ?>">
             <tr>
					  ...
					  <td bgcolor="#ffffff" class="bodytext31" valign="center"  align="center"><strong>Store</strong></td>
...
//echo 'code'.$main_locationcode;
			...
			if($str == 'MLPO-')
			{
			$query767 = "select * from manual_lpo where billnumber='$po_number' and itemcode = '$itemcode' $condn and recordstatus='' and recordstatus <> 'deleted'";
			}
			else
			{
			...
			//$packagequantity =  $res767['packagequantity'];
			$fxtotamount = $fxtotamount + $res767['fxtotamount']; 
			//$fxtotamount = $res767['fxtotamount']; 
			}
...
$store_name = $res75['store'];
			...
			$purchaseindentdocno1 = $res55['purchaseindentdocno'];
			$query477 = "select purchasetype from purchase_indent where docno = '$purchaseindentdocno1'";
			$exec477 = mysql_query($query477) or die ("Error in Query477".mysql_error());
			$res477 = mysql_fetch_array($exec477);
			$purchasetypee = $res477['purchasetype'];*/
...
<?php $sno = $sno + 1; ?>
  <tr>
		...
		<input type="hidden" name="ponumber[]" value="<?php echo $po_number; ?>">

		 <input type="hidden" name="purchasetype<?php echo $sno; ?>" id="purchasetype<?php echo $sno; ?>" value="<?php echo $purchasetypee; ?>">
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="receivedquantity[]" id="receivedquantity<?php echo $sno; ?>" size="6" onKeyUp="return totalcalc('<?php echo $sno; ?>');" class="bodytext21" autocomplete="off" onBlur="return totalamount('<?php echo $sno; ?>','<?php echo $number; ?>');" ></div></td>
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center">
			<input type="text" name="discount[]" id="discount<?php echo $sno; ?>" size="6" onKeyUp="return totalamountdisc('<?php echo $sno; ?>','<?php echo $number; ?>');"  class="bodytext21" style="background-color: gray;"  autocomplete="off" readonly></div></td>
			<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="taxper[]" id="taxper<?php echo $sno; ?>" size="6"  value="<?php  echo $tax_percentage;?>" class="bodytext21" autocomplete="off" readonly></div></td>
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="tax[]" id="tax<?php echo $sno; ?>" size="6"  value="<?php  echo $taxamount;?>" class="bodytext21" autocomplete="off" readonly></div></td>
...
<!--        onKeyUp="return totalamount('<?php echo $sno; ?>','<?php echo $number; ?>');"  -->
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="hidden" name="store_id[]" id="store_id<?php echo $sno; ?>" size="6" class="bodytext21" readonly value="<?php echo $storecode; ?>"><?php echo $store_name;?></div></td>
		<td class="bodytext31" valign="center"  align="left" style="display:none"><div align="center"><input type="text" name="costprice[]" id="costprice<?php echo $sno; ?>" size="6" class="bodytext21" readonly></div></td>
		<td class="bodytext31" valign="center"  align="left" style="display:none"><div align="center">
      ...
      <tr>
		 ...
		 <td colspan="2" align="center" valign="middle"  bgcolor="#ecf0f5" class="bodytext3" >
		  <input type="hidden" name="frmflag2" value="frmflag2">
               <input style="font-size:22px;width:200px" name="Submit222" type="submit"  value="Save" class="button" id="savebutton" onClick="return funcsave('<?php echo $number; ?>')"/>		
			   </td>
      </tr>
	  <?php } ?>
	  </table>
