				...
				{  
					...
					if($id != '')
					{
						...
						$querycr1inv = "SELECT SUM(`creditamount`) as stockcredit FROM `master_journalentries` WHERE `ledgerid` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'
								...
								UNION ALL SELECT SUM(a.`totalcp`) as expenses FROM `pharmacysalesreturn_details` AS a JOIN master_medicine AS b ON (a.itemcode = b.itemcode) WHERE b.incomeledgercode = '$id' AND a.`entrydate` BETWEEN '$ADate1' AND '$ADate2'
										UNION ALL SELECT SUM(`openbalanceamount`) as stockcredit FROM `openingbalancesupplier` WHERE `accountcode` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'
