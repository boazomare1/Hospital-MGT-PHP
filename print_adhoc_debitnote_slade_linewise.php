...
if (isset($_REQUEST["billautonumber"])) { $billautonumber = $_REQUEST["billautonumber"]; } else { $billautonumber = ""; }
	...
	echo $query4 = "SELECT * FROM adhoc_debitnote WHERE patientcode = '$patientcode' AND patientvisitcode = '$visitcode' AND docno = '$billnumber'";
...
$claim_id = $res18["claim_id"];
	...
	$rate = $res5['rate'];
	$units = $res5['units'];
	$visitcode = $res5['patientvisitcode'];
	...
	if ($err) {
	   ...
	   $inv_rslt['msg'] = $err_unable_connect;
		...
		if(isset($obj['id'])){
			...
			$query8 = "update slade_claim set debit_payload='".addslashes($result)."',updated='".date('Y-m-d H:i:s')."' where visitcode = '$visitcode'";
		...
		$exec81 = mysqli_query($GLOBALS["___mysqli_ston"], $query81);
			...
			/* header("location:print_adhoc_debitnote_slade.php?billno=$billnumber&&visitcode=$visitcode&patientcode=$patientcode&locationcode=$locationcode");
				exit;*/
