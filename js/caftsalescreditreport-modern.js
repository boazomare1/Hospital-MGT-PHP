// Modern JavaScript for caftsalescreditreport.php

document.addEventListener('DOMContentLoaded', function() {
    initializeCAFTSalesCreditReport();
});

function initializeCAFTSalesCreditReport() {
...
function exportToExcel() {
    ...
    const url = 'caftsalescreditreport.php?' + new URLSearchParams(formData).toString();
    const link = document.createElement('a');
    link.href = url;
    link.download = 'caft_sales_credit_report.xls';
    link.click();
...
function exportToPDF() {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = 'caft_sales_credit_report.pdf';
    link.click();
...
function exportToCSV() {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = 'caft_sales_credit_report.csv';
    link.click();
...
function initializeCharts() {
    // Initialize summary charts if Chart.js is available
    ...
    const chart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                label: 'Credit Sales',
                data: [3000, 4000, 5000, 3500, 6000, 7000],
                borderColor: '#3498db',
...
function updateSummaryCards() {
    ...
    rows.forEach(row => {
        ...
        if (cells.length >= 4) {
            const salesAmount = parseFloat(cells[1]?.textContent.replace(/[^0-9.-]/g, '') || 0);
            const creditAmount = parseFloat(cells[2]?.textContent.replace(/[^0-9.-]/g, '') || 0);
    ...
    updateSummaryCard('total-sales', totalSales, 'positive');
    updateSummaryCard('total-credit', totalCredit, 'neutral');
    updateSummaryCard('total-paid', totalPaid, 'positive');
    updateSummaryCard('total-pending', totalPending, 'negative');
}
