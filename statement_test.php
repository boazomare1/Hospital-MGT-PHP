...
<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
	  ...
	  $subtypeopbal = 0;	
		  ...
		  UNION ALL SELECT (-1*`fxamount`) as fxamount,transactiondate AS transactiondate  from master_transactionpaylater where accountnameano='$res21accountnameano'  and paymenttype like '%$type%' and accountnameid='$res21accountid' and transactiontype = 'paylatercredit' and recordstatus <> 'deallocated' and transactiondate < '$ADate1' 
 ...
 UNION ALL SELECT  billnumber,'' AS visitcode,(-1*`fxamount`) as fxamount,transactiondate AS transactiondate   FROM `master_transactionpaylater` WHERE `accountnameid` = '$res21accountid' AND `transactiondate` < '$ADate1'  AND `docno` LIKE 'AR-%' AND `transactionstatus` = 'onaccount' AND `transactionmodule` = 'PAYMENT'
		...
		$totalpayment2 = 0;
	$query982 = "select sum(fxamount) as transactionamount1,docno from master_transactionpaylater where visitcode='$resvisitcode2' and transactiontype = 'PAYMENT' and billnumber='$resbillnumber2' and recordstatus = 'allocated'  AND transactiondate <= '$ADate2' and accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND accountnameid='$res21accountid'
	UNION ALL 
	select sum(-1*fxamount) as transactionamount1,docno from master_transactionpaylater where visitcode='$resvisitcode2' and transactiontype = 'PAYMENT' and docno='$resbillnumber2' and recordstatus = 'allocated' AND `transactiondate` <= '$ADate2'  and accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND accountnameid='$res21accountid'";
...
//ACCOUNT WISE OPENING BALANCE OUTSTANDING END	  
      ...
      while($res42 = mysqli_fetch_array($exec42))
      {
        ...
        echo $res2billnumber = $res42['docno'];
        $particulars = $res42['particulars'];
        $res2patientcode = $res42['patientcode'];
        
        echo '===>'.$res2transactionamount = $res42['fxamount'];
/*
...
*/
echo $query98 = "select sum(fxamount) as transactionamount1,docno from master_transactionpaylater where visitcode='$res2visitcode' and transactiontype = 'PAYMENT' and billnumber='$res2billnumber' and recordstatus = 'allocated' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' and accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND accountnameid='$res21accountid'
				UNION ALL 
				select sum(-1*fxamount) as transactionamount1,docno from master_transactionpaylater where visitcode='$res2visitcode' and transactiontype = 'PAYMENT' and docno='$res2billnumber' and recordstatus = 'allocated' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' and accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND accountnameid='$res21accountid' AND docno NOT LIKE '%AR%'
				UNION ALL 
				select sum(-1*fxamount) as transactionamount1,docno from master_transactionpaylater where visitcode='$res2visitcode' and transactiontype = 'PAYMENT' and docno='$res2billnumber' and recordstatus = 'allocated' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND docno  LIKE '%AR%'";
...
$pendig_final_value_outstanding = 0;
        ...
        if($resamount != '0' && $balanceamount!=0)
        {
          $snocount = $snocount + 1;
          $t1 = strtotime($ADate2);
