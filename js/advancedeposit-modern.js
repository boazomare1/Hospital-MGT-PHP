// Advance Deposit Modern JavaScript - Following VAT.php Structure
let allDepositRecords = [];
let filteredDepositRecords = [];
let currentPage = 1;
const itemsPerPage = 10;

// DOM Elements
let depositForm, searchInput, locationSelect, paginationContainer;
...
function initializePagination() {
    ...
    console.log('Advance Deposit page initialized');
}
...
function validateForm() {
    ...
    const requiredFields = ['patientcode', 'patientname', 'accname', 'visitcode', 'billdate', 'paymentmode', 'totalamount'];
...
function hideLoadingState() {
    const submitBtn = document.getElementById('submitBtn');
    if (submitBtn) {
        submitBtn.innerHTML = '<i class="fas fa-save"></i> Add Advance Deposit';
...
function handleLocationChange(event) {
    ...
    // For example, refresh data based on location
}
...
function exportToExcel() {
    ...
    if (link.download !== undefined) {
        ...
        link.setAttribute('download', 'advance_deposits.csv');
...
function confirmDelete(docNo, autoNumber) {
    if (confirm('Are you sure you want to delete this advance deposit? This action cannot be undone.')) {
        window.location.href = 'advancedeposit.php?st=del&anum=' + encodeURIComponent(autoNumber);
    }
}

function confirmActivate(docNo, autoNumber) {
    if (confirm('Are you sure you want to activate this advance deposit?')) {
        window.location.href = 'advancedeposit.php?st=activate&anum=' + encodeURIComponent(autoNumber);
    }
}
