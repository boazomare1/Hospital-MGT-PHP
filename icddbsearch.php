...
function IcdSearch(id)
{
	...
	window.opener.document.getElementById('picdcount').value = parseFloat(window.opener.document.getElementById('picdcount').value)+1;
...
<td width="144" align="left" class="bodytext3"><input type="text" size="35" name="searchdisease1" id="searchdisease1" value="<?php echo $searchdisease2; ?>" /></td>
...
foreach($searchexplode as $searchdisease1)
{
	...
	$query2 = "select auto_number,disease,icdcode,chapter,description from master_icd where rptname like '%$dept%' and ($searchstring) and recordstatus = 'active' order by auto_number";// order by subtype limit 0, 15";
