// IP Discharge Request List Modern JavaScript
let allDischargeRequestRecords = [];
let filteredDischargeRequestRecords = [];
let currentPage = 1;
...
function initializeData() {
    ...
    allDischargeRequestRecords = Array.from(existingRows).map((row, index) => {
    ...
    filteredDischargeRequestRecords = [...allDischargeRequestRecords];
}
...
function exportToExcel() {
    ...
    if (link.download !== undefined) {
        ...
        link.setAttribute('download', 'ip_discharge_request_list_export.csv');
...
// Package processing status handling
function handlePackageProcessingStatus() {
    const processingButtons = document.querySelectorAll('.action-btn.disabled');
    processingButtons.forEach(btn => {
        btn.addEventListener('click', function(e) {
            e.preventDefault();
            showAlert('Package processing is not yet completed. Please wait for the process to finish.', 'info');
        });
    });
}

// Initialize package processing status handling
document.addEventListener('DOMContentLoaded', function() {
    handlePackageProcessingStatus();
});

// Discharge button confirmation
function handleDischargeConfirmation() {
    const dischargeButtons = document.querySelectorAll('.action-btn.discharge');
    dischargeButtons.forEach(btn => {
        btn.addEventListener('click', function(e) {
            const patientName = this.closest('tr').querySelector('.patient-name')?.textContent?.trim();
            if (patientName && !confirm(`Are you sure you want to process discharge for ${patientName}?`)) {
                e.preventDefault();
            }
        });
    });
...
// Initialize discharge confirmation
document.addEventListener('DOMContentLoaded', function() {
    handleDischargeConfirmation();
});
...
// Real-time status updates (for package processing)
function updatePackageProcessingStatus() {
