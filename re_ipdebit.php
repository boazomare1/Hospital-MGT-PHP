...
<?php 
$query8 = "SELECT visitcode,billnumber as docno,fxamount as transactionamount,transactiondate,accountname,accountnameid FROM `master_transactionpaylater` WHERE `billnumber` LIKE '%IPDr%' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `transactiontype` = 'finalize' order by auto_number";
...
$i = 0;
$drresult = array();
$querydr1bnk = "SELECT SUM(`fxamount`) as income FROM `ip_debitnotebrief` WHERE docno = '$docno' and `description` = 'Bed Charges'
UNION ALL SELECT SUM(`fxamount`) as income FROM `ip_debitnotebrief` WHERE docno = '$docno' and `description` = 'Nursing Charges'
UNION ALL SELECT SUM(`fxamount`) as income FROM `ip_debitnotebrief` WHERE docno = '$docno' and `description` = 'RMO Charges'
UNION ALL SELECT SUM(`fxamount`) as income FROM `ip_debitnotebrief` WHERE docno = '$docno' and `description` = 'Lab'
UNION ALL SELECT SUM(`fxamount`) as income FROM `ip_debitnotebrief` WHERE docno = '$docno' and `description` = 'Radiology'
UNION ALL SELECT SUM(`fxamount`) as income FROM `ip_debitnotebrief` WHERE docno = '$docno' and `description` = 'Service'
UNION ALL SELECT SUM(`fxamount`) as income FROM `ip_debitnotebrief` WHERE docno = '$docno' and `description` = 'Others'
       ...
       <tr>
        <td valign="top"><table width="600" id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
			...
			{	
				...
				$query2 = "select accountssub,auto_number,tbinclude,tbledgerview from master_accountssub where auto_number IN ('46') AND accountsmain = '$parentid' and recordstatus <> 'deleted' order by accountssub";
	...
	if($parentid != '')
	{
		if($parentid == '46')
		{
			...
			{  
				...
				$drresult = array();
				$querydr1bnk = "SELECT SUM(fxamount) as paylatercr FROM `master_transactionpaylater` WHERE accountnameid = '$id' AND `billnumber` LIKE 'IPDr%' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `transactiontype` = 'finalize'";
				$execdr1 = mysqli_query($GLOBALS["___mysqli_ston"], $querydr1bnk) or die ("Error in querydr1bnk".mysqli_error($GLOBALS["___mysqli_ston"]));
				while($resdr1 = mysqli_fetch_array($execdr1))
				{
				$i = $i+1;
		...
		else if($parentid == '1')
		{
			$i = 0;
			$crresult = array();
			$querycr1bnk = "SELECT SUM(`fxamount`) as income FROM `ip_debitnotebrief` WHERE consultationdate BETWEEN '$ADate1' AND '$ADate2' and `description` = 'Bed Charges'
							...
							UNION ALL SELECT SUM(`fxamount`) as income FROM `ip_debitnotebrief` WHERE consultationdate BETWEEN '$ADate1' AND '$ADate2' and `description` = 'Radiology'
							...
							UNION ALL SELECT SUM(fxamount) as income FROM `master_transactionpaylater` WHERE `billnumber` LIKE '%IPDr%' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' and billnumber = patientcode";
