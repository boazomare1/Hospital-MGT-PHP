...
$account = $_REQUEST['account'];
 ...
 $autonumber=trim($_REQUEST['refnumber']);
 $servicesitemcode=trim($_REQUEST['servicesitemcode']);
 $rfqty=$_REQUEST['rfqty'];


if($rfqty >0 && $rfqty !="")
{
	$conservice="select refundquantity,serviceqty from ipconsultation_services where patientcode='$patientcode' and patientvisitcode='$visitcode' and auto_number='$autonumber' and servicesitemcode='$servicesitemcode'";
	$exconser=mysqli_query($GLOBALS["___mysqli_ston"], $conservice) or die("error in conservice".mysqli_error($GLOBALS["___mysqli_ston"]));
	$resconser=mysqli_fetch_array($exconser);
	$servicequantity=$resconser['serviceqty'];
	  $refundquantity=$resconser['refundquantity']+$rfqty;
	
	 $newquantity=$servicequantity-$refundquantity;
	if($newquantity==0)
	{
	$process='completed';
	}
	...
	$query29=mysqli_query($GLOBALS["___mysqli_ston"], "update ipconsultation_services set servicerefund='refund',refundquantity='$refundquantity',process='$process' where patientcode='$patientcode' and patientvisitcode='$visitcode' and servicesitemcode = '$servicesitemcode' and auto_number='$autonumber'")or die("Error in Query29".mysqli_error($GLOBALS["___mysqli_ston"]));
 
	 header("location:servicelistip.php");
 				exit;
}
...
$query2 = "select docnumber from ipprocess_service   order by auto_number desc limit 0, 1";
...
if($num01!=$serialnum)
{
	header('location:servicelistip.php?st=matchfailed');
	exit;
}
...
$paynowbillprefix1=strlen($paynowbillprefix);
$query2 = "select docno from ipmedicine_issue where recordstatus = '' order by auto_number desc limit 0, 1";
$exec2 = mysqli_query($GLOBALS["___mysqli_ston"], $query2) or die ("Error in Query2".mysqli_error($GLOBALS["___mysqli_ston"]));
$res2 = mysqli_fetch_array($exec2);
...
else
{
	...
	$docnumber1 = $billnumbercode;
	$entrydocno = $docnumber1;
...
//		$entrydocno = $_POST['entrydocno'][$key];
		...
		$amountget= $quantity*$rate;
			...
			$querycumstock2 = "select cum_quantity from transaction_stock where cum_stockstatus='1' and itemcode='$itemcode' and locationcode='$locationcodeget'";
				...
				if($pharmacyquantityenter>0)
				{
					...
					if($bat_quantity>='0')
					{
						...
						$query32 = "insert into pharmacysales_details(fifo_code,itemname,itemcode,quantity,rate,totalamount,batchnumber,companyanum,patientcode,visitcode,patientname,financialyear,username,ipaddress,entrydate,accountname,docnumber,entrytime,location,store,instructions,categoryname,route,locationname,locationcode,issuedfrom,freestatus,ipdocno,ledgeranum,ledgercode,ledgername,incomeledger,incomeledgercode,costprice,totalcp)values('$fifo_code','$pharmacyname','$itemcode','$quantity','$rate','$amount','$batchno','$companyanum','$patientcode','$visitcode','$patientname','$financialyear','$username','$ipaddress','$dateonly1','$account','$docnumber','$timeonly','$locationnameget','$storecode','','','','$locationnameget','$locationcodeget','ip','$freestatus','$docnumber1','$r_ledgerautonumber','$r_ledgercode','$r_ledgername','$r_incomeledger','$r_incomeledgercode','$r_costprice','$r_totalcp')"; 
						$exec32 = mysqli_query($GLOBALS["___mysqli_ston"], $query32) or die(mysqli_error($GLOBALS["___mysqli_ston"]));


				   $query86 ="insert into ipmedicine_issue(itemname,itemcode,quantity,rateperunit,totalrate,batchnumber,companyanum,patientcode,visitcode,patientname,username,ipaddress,date,account,docno,frequency,dose,days,route,freestatus,dischargemedicine,locationcode,locationname,instructions)values('$pharmacyname','$itemcode','$quantity','$rate','$amount','$batchno','$companyanum','$patientcode','$visitcode','$patientname','$username','$ipaddress','$dateonly1','$account','$docnumber1','','','','','$freestatus','No','$locationcodeget','$locationnameget','')";
				   $exec86 = mysqli_query($GLOBALS["___mysqli_ston"], $query86) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
					
					$queryupdate=mysqli_query($GLOBALS["___mysqli_ston"], "update ipconsultation_services set process='completed' where patientvisitcode='$visitcode' and patientcode='$patientcode' and servicesitemcode = '$servicesitemcode' and auto_number='$autonumber'")or die("Error in queryupdate".mysqli_error($GLOBALS["___mysqli_ston"]));
		 ...
		 $query26="insert into ipprocess_service(patientname,patientcode,patientvisitcode,recorddate,recordtime,itemcode,itemname,acknowledge,refund,docnumber,locationname,locationcode)
		 values('$patientname','$patientcode',
   '$visitcode','$dateonly','$timeonly','$itemcode','$pharmacyname','$status','$status1','$docnumber','".$locationnameget."','".$locationcodeget."')";
...
function validcheck()
{
document.getElementById("smtbutton").disabled = true;
...
//	alert(v1);
	...
	if((varqty>varavlqty))
		{
		document.getElementById("smtbutton").disabled = false;
...
$query78="select subtype from master_ipvisitentry where patientcode='$patientcode' and visitcode='$visitcode'";
...
$subtype="select sertemplate from master_subtype where auto_number='$patientsubtype'";
...
$sertemplate=$ressub['sertemplate'];
...
<form name="frmsales" id="frmsales" method="post" action="processserviceip.php" onKeyDown="return disableEnterKey(event)" onSubmit="return validcheck()">
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
			...
			$totalamount=0;
			 ...
			 $query61 = "select serviceqty,refundquantity,servicesitemname,servicesitemcode,billtype,iptestdocno from ipconsultation_services where  patientcode = '$patientcode' and patientvisitcode = '$visitcode' and process='pending'  and servicesitemcode='$service_item_code' and auto_number='$refnumber' ";
$exec61 = mysqli_query($GLOBALS["___mysqli_ston"], $query61) or die ("Error in Query12".mysqli_error($GLOBALS["___mysqli_ston"]));
 $numb=mysqli_num_rows($exec61);
while($res61 = mysqli_fetch_array($exec61))
{
	 $initialqty=$res61["serviceqty"];
	 $retqty=$res61["refundquantity"];
	 $serviceqty =$initialqty-$retqty;
	/*for($i=0; $i<$serviceqty; $i++)
	{ */
$servicename =$res61["servicesitemname"];
$servicecode =$res61["servicesitemcode"];
$billtype = $res61["billtype"];
$refno = $res61['iptestdocno'];

if($sertemplate =='')
{
$query68="select itemcode from master_services where itemcode='$servicecode' and status <> 'deleted'";
}
else
{
$query68="select itemcode from $sertemplate where itemcode='$servicecode' and status <> 'deleted'";
}
  ...
  <tr id="idTRMain<?php echo $sno; ?>">
        ...
        <input type="hidden" name="serquantity<?php echo $sno; ?>" id="serquantity<?php echo $sno; ?>" value="<?php echo $serviceqty; ?>">
          ...
          <td class="bodytext31" valign="center"  align="center"><?php echo $serviceqty;?>
           ...
           <input type="text" name="avqtyy<?php echo $sno;?>" id="avqtyy<?php echo $sno;?>" readonly value="<?php echo $serviceqty;?>" size="5" style="border:none;background: none;">
       </td>
		<td class="bodytext31" valign="center"  align="center">
        <input type="text" name="rfqty" id="rfqty<?php echo $sno;?>"  value="" size="5" onKeyDown="return numbervaild(event)" onKeyUp="sumtheservice(this.value,<?php echo $sno;?>)">
       </td>
				</tr>
          ...
          <tbody>
            <tr>
              <td width="54%" align="right" valign="top" >
             	  ...
             	  <input name="Submit2223" type="submit" value="Save" id="smtbutton" onClick="return acknowledgevalid(<?php echo $sno;  ?>)" accesskey="b" class="button" style="border: 1px solid #001E6A"/>
               </td>
