        ...
        <td>
          ...
          <tbody>
			...
			<td colspan="11" align = "right" >
				<a href="excel_oprevenuereport_refund_consultation.php?cbfrmflag1=cbfrmflag1&&ADate1=<?php echo $transactiondatefrom; ?>&&ADate2=<?php echo $transactiondateto; ?>&&locationcode=<?php echo $locationcode; ?>"><img  width="30" height="30" src="images/excel-xls-icon.png" style="cursor:pointer;"></a>
			</td>
			</tr>
            <tr>
			...
			$query1 = "select patientcode,patientname,consultation,billdate,locationname,patientvisitcode,billnumber from refund_consultation where  locationcode='$locationcode' and  billdate between '$transactiondatefrom' and '$transactiondateto' and patientvisitcode in (select visitcode from master_visitentry where department like '$department')";
			...
			?>
				...
				$query1 = "select patientcode,patientname,consultation,billdate,locationname,locationcode,patientvisitcode,fxamount,billnumber from refund_paylaterconsultation where  locationname='$locationcode' and  billdate between '$transactiondatefrom' and '$transactiondateto' and patientvisitcode in (select visitcode from master_visitentry where department like '$department')";
			...
			?>
				...
				$query1 = "select consultationfxamount as consultationfxamount, patientcode, patientname, entrydate, visitcode, locationname,billno from billing_patientweivers where locationcode='$locationcode' and entrydate between '$transactiondatefrom' and '$transactiondateto' and consultationfxamount>0";
				
			$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in query1".mysqli_error($GLOBALS["___mysqli_ston"]));
			$discount_count = mysqli_num_rows($exec1);

			if($discount_count >0){
				?>
                <td colspan="12" bgcolor="#ecf0f5" class="bodytext3" align='left'><strong>Discounts</strong></td>
				<?php
			...
			while($res1 = mysqli_fetch_array($exec1)){
				  ...
				  $amount = $res1['consultationfxamount'];
