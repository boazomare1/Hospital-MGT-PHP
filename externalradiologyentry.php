...
if($radiologyname != "")
   ...
   //image part start
if($_FILES['pfile']['name'][$key]){
   
      $file_name = $_FILES['pfile']['name'][$key];
/*      $file_size =$_FILES['image']['size'][$sno];
      $file_tmp =$_FILES['image']['tmp_name'][$sno];
      $file_type=$_FILES['image']['type'][$sno];
      $file_ext=strtolower(end(explode('.',$_FILES['image']['name'][$sno])));
      $filename="ipradio";*/
    /*  $expensions= array("jpg");
      
      if(in_array($file_ext,$expensions)=== false){
        die("extension not allowed, please choose a JPG  file.");
      }
      
      if($file_size > 2097152){
         die('Image size must be excately 2 MB');
      }*/
      

		    mysqli_query($GLOBALS["___mysqli_ston"], "update consultation_radiology set resultentry='$status',docnumber = '$billnumbercode' where billnumber='$billnumber' and radiologyitemname='$radiologyname' and auto_number='$radno'");
			

			
			  $query26="insert into resultentry_radiology(patientname,patientcode,patientvisitcode,recorddate,itemcode,itemname,acknowledge,billnumber,username,docnumber)values('$patientname','walkin',
   'walkinvis','$dateonly','$itemcode','$radiologyname','completed','$billnumber','$username','$billnumbercode')";
   $exec26=mysqli_query($GLOBALS["___mysqli_ston"], $query26) or die(mysqli_error($GLOBALS["___mysqli_ston"]));


 
   $query76 = "update resultentry_radiology set filename= '$file_name' WHERE billnumber='$billnumber' and patientcode = 'walkin'  and docnumber = '$billnumbercode'";
      ...
      <tr>
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
			...
			$sno = '';
			$totalamount=0;			
			$query61 = "select * from consultation_radiology where billnumber='$billnumber' and prepstatus='completed' and imgaquistatus='completed' and resultentry='pending'";
