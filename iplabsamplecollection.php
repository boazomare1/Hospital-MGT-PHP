...
if($isanalyzer==1){	
 ...
 $query26="insert into ipsamplecollection_lab(patientname,patientcode,patientvisitcode,recorddate,recordtime,itemcode,itemname,sample,acknowledge,refund,docnumber,comments,sampleid,locationcode,locationname,transferloccode,transferlocname,username,externallab)values('$patientname','$patientcode',

   '$visitcode','$dateonly','$timeonly','$itemcode','$labname','$sample','$status','$status1','$docnumber','$comments','$sampleid','$locationcode','$locationname','$transferloccode','$transferlocname','$username','$externallab')";

 $exec26=mysqli_query($GLOBALS["___mysqli_ston"], $query26) or die("Error in query26".mysqli_error($GLOBALS["___mysqli_ston"]));

   

   $query261="insert into opipsampleid_lab(patientname,patientcode,patientvisitcode,recorddate,recordtime,itemcode,itemname,sample,acknowledge,refund,docnumber,comments,sampleid,locationcode,locationname,category,isanalyzer)values('$patientname','$patientcode',

   '$visitcode','$dateonly','$timeonly','$itemcode','$labname','$sample','$status','$status1','$docnumber','$comments','$sampleid','$locationcode','$locationname','$categoryname','$isanalyzer')";
      ...
      <tr>
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
			...
			$query61 = "select a.* from ipconsultation_lab as a  JOIN master_lab as b ON a.labitemcode=b.itemcode and b.categoryname='$categoryname'  where a.patientcode = '$patientcode' and a.patientvisitcode = '$visitcode' and a.labsamplecoll='pending' and a.labrefund='norefund' and a.labitemname <> ''  group by a.labitemname,a.auto_number";
...
if($res68["externallab"]=='yes' || $res68["externallab"] == 'on'){
  $externallab = 1;
...
?>
  ...
  <td class="bodytext31" valign="center"  align="left"><div align="center">
        ...
        <input type="checkbox" name="ref[<?= $auto_number ?>]"  class="refcheck" id="ref<?php echo $sno; ?>" value="<?php echo $anumber1; ?>" onClick="return checkboxcheck1('<?php echo $sno; ?>')" <?php echo $dis_it;?>/></div></td>
		...
		<input type="hidden" name="externallab[<?= $auto_number ?>]" value="<?php echo $externallab; ?>">
	   ...
	   <input type="hidden" name="categoryname[<?= $auto_number ?>]" value="<?php echo $categoryname; ?>">
