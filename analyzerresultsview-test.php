...
{  exit;
	...
	if($vtype=='IP') {
		...
		if($resultvalue != '')
		{
			$query26="insert into resultentry_lab(patientname,patientcode,patientvisitcode,recorddate,recordtime,itemcode,itemname,resultvalue,referencerange,referenceunit,docnumber,referencename,account,color,username,sampleid,doctorname,locationname,locationcode,referencecomments,datetime,radiology)values('$patientname','$patientcode',
		'$visitcode','$dateonly','$timeonly','$itemcode','$labname','$resultvalue','$referencevalue','$unit','$docnumber','$refname','$accountname','$color','$username','$sampleid','$doctorname','".$locationnameget."','".$locationcodeget."','$refcomments','$updatedatetime','$radiology')";
			$exec26=mysqli_query($GLOBALS["___mysqli_ston"], $query26) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
		}
...
<form name="frm" id="frmsales" method="post" action="analyzerresultsview-test.php" onKeyDown="return disableEnterKey(event)" >
	  ...
	  <td colspan="5" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext365">
			...
			<td width="4%"  align="left" valign="center" 
                bgcolor="#ffffff" class="bodytext366"><div align="center"><strong>Reference Value</strong></div></td>
				  ...
				  <?php 
	 echo $query31="select * from pending_test_orders where patientcode = '$patientcode' and visitcode = '$visitcode' and sample_id='$docnumber' and testcode='$itemcode' group by testcode";
			  ...
			  $num=mysqli_num_rows($exec52);
			   ...
			   $labname2=$res52['itemname'];
				...
				if($type == 'IP'){
					...
					echo $query49 = "select * from resultentry_lab where patientcode='$patientcode' and itemcode='$itemcode2' and referencename='$labreferencename' and docnumber <> '$docnumber' order by auto_number desc limit 0,1";
				}
