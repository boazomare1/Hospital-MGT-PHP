...
if(isset($parts[0])!='' && isset($parts[1])==''){
 $newcustomersearch = "(customerfullname like '%$parts[0]%')";
}elseif(isset($parts[0])!='' && isset($parts[1])!='' && isset($parts[2])==''){
 $newcustomersearch = "  (customerfullname like '%$parts[0]%' || customerfullname like '%$parts[1]%')";	
}elseif(isset($parts[0])!='' && isset($parts[1])!='' && isset($parts[2])!=''){
$newcustomersearch = "  (customerfullname like '%$parts[0]%' || customerfullname like '%$parts[1]%'|| customerfullname like '%$parts[2]%')";	
}
...
$query1 = "select customercode,customerfullname,nationalidnumber,mobilenumber,accountname,dateofbirth,area,memberno from master_customer  where ($customersearch) and status <> 'Deleted' order by auto_number limit 50";
$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
$searchnum=mysqli_num_rows($exec1);
if($searchnum==0){
 $query11 = "select customercode,customerfullname,nationalidnumber,mobilenumber,accountname,dateofbirth,area,memberno from master_customer  where ($newcustomersearch) and status <> 'Deleted' order by auto_number limit 50";
$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query11) or die ("Error in Query11".mysqli_error($GLOBALS["___mysqli_ston"]));	
}
...
while ($res1 = mysqli_fetch_array($exec1))
{
	...
	$a_json_row["memberno"] = $memberno;
	$a_json_row["nationalidnumber"] = $res1nationalidnumber;
