...
$query2 = "select consultationid as billnumber from consultation_radiology where consultationid like 'EB-%' order by auto_number desc";
...
if(isset($_POST['dis']) && count($_POST['dis'])>1){
		...
		foreach($_POST['radiology'] as $key=>$value){	
			//echo '<br>'.
		// count($_POST['radiology']); exit;
		$pairs= $_POST['radiology'][$key]; 
		$pairvar= $pairs;
		
		$radiologycode= $_POST['raditemcode'][$key];
	    $pairs1= $_POST['rate8'][$key];
		$pairvar1= $pairs1;
		
/* 
		$radiologyquery=mysqli_query($GLOBALS["___mysqli_ston"], "select itemcode from master_radiology where itemname='$pairvar' and status <> 'deleted'");
	...
	/*		 $query1222 = "select bankrefno from bank_refrencenumber  where visitcode='$visitcode' and patientcode='$patientcode' and patientname='$patientfullname' and billnumber='' ";
		...
		/*		if(is_connected()){
					...
					$apibill_name = 'EXTERNAL RADIOLOGY';
					$apibill_date = $opdate;
		...
		header("location:radiology_externalbilling_request.php");
        exit;
...
if (document.getElementById("total2").value == '' || parseFloat(document.getElementById("total2").value)<=0) 
     {
	 alert("Please Select Radiology");
	// document.getElementById("customer").focus();
	 return false;
	 }	
...
$('#customer').autocomplete({
	...
	source:'ajaxexternalcustomernewsearch.php', 
	//alert(source);
	minLength:2,
	delay: 0,
	html: true, 
...
<body onLoad="return funcOnLoadBodyFunctionCall();form1.customer.focus()">
...
<form name="form1" id="frmsales" method="post" action="radiology_externalbilling_request.php" onSubmit="return validcheck()"   onKeyDown="return disableEnterKey(event)">
      ...
      <tr>
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
					         ...
					         <tr >
				   <td colspan="11" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><span class="bodytext32" ><strong>Radiology <img src="images/plus1.gif" width="13" height="13" > </strong></span></td>
		        </tr>
				<tr id="radid">
				   ...
				   <td width="30"><input name="radiology1" id="radiology" type="text" size="69" autocomplete="off">
				   <input name="hiddenradiology" id="hiddenradiology" type="hidden" size="69" autocomplete="off"></td>
				      <td width="30"><input name="rate81" type="text" id="rate8" readonly size="8"></td>
					   <td><label>
				 ...
				 <tr>
				   <td colspan="8" align="right" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>Total </strong>
				     <input type="text" name="total2" id="total2" readonly size="7"><input type="hidden" id="total1" readonly size="7"><input type="hidden" id="total3" readonly size="7"><input type="hidden" id="total4" readonly size="7"></td>
				   </tr>
