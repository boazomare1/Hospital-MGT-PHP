...
//image part start
if(!empty($_FILES['image']['name'][$sno])){
	  $errors= array();
      $file_name = $_FILES['image']['name'][$sno];
      $file_ext=strtolower(end(explode('.',$_FILES['image']['name'][$sno])));
     $filename=$file_name;
}
...
$new_datetime=date('Y-m-d H:i:s');		 
		 ...
		 $query123="update consultation_radiology set  imgaquistatus='completed',imagefilename='$filename',imagefilename2='$filename2',imagefilename3='$filename3',aquisition_datetime='$new_datetime' where patientcode='$patientcode' and  patientvisitcode='$visitcode' and radiologyitemcode='$itemcode' and refno='$refno' "; 
...
if (hasChecked == false )
{
alert("Please acknowledge a sample  or click back button on the browser to exit sample collection");
return false;
}
return true;
}
...
<form name="frmsales" id="frmsales" method="post" action="radiologyimgentry.php" onKeyDown="return disableEnterKey(event)" enctype="multipart/form-data" onSubmit="return validcheck()">
      ...
      <tr>
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
			...
			$sno = '';
			$totalamount=0;			
echo $query61 = "select * from consultation_radiology where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and resultentry='pending' and prepstatus='completed' and paymentstatus='completed' and imgaquistatus='Pending' and radiologyitemcode='$q_itemcode' and refno='$q_refno' and (billtype='PAY NOW' or (billtype='PAY LATER' ))";
...
if($billtype=='PAY LATER'  and $approvalstatus!='1'){
	continue;
}
...
?>
 ...
 <td height="40"  align="left" valign="center" class="bodytext31"><div align="center"><?php echo $radiologyinstructions;?></div></td>		  
	...
	<input type="hidden" name="sno[<?php echo $sno; ?>]" value="<?php echo $sno; ?>">
			  ...
			  <input type="file" name="image[<?php echo $sno; ?>]" id="image<?php echo $sno; ?>" accept="image/JPEG" ><br><br>
