...
// Calendar prototype
function datechanged(clickedid){
	if(clickedid == 'paymententrydate'){
	var firstdate = $('#'+clickedid).val();
	 $('#ADate1').val(firstdate);
	}
	else if(clickedid == 'ADate1'){
		var chequedate = $('#'+clickedid).val();
		var receiptdate = $('#paymententrydate').val();
		var receiptdatefor = $('#paymententrydate').val();

  var receiptdate = new Date(receiptdate);
  var date1 = receiptdate.getTime();  
  
  var chequedate = new Date(chequedate);
  var date2 = chequedate.getTime();
  
  	if(date1 > date2){
		alert('Cheque Date should not be less than Receipt Date');
		$('#'+clickedid).val(receiptdatefor);
	}
...
{ //Generate table cell with value
...

	{
		...
		if (PClickable === true) {
		    ...
		    else { PCellStr = "<td class='calTD' style='text-align:center;cursor:pointer;background-color:" + pColor + "' onmouseover='changeBorder(this, 0);' onmouseout=\"changeBorder(this, 1, '" + pColor + "');\" onClick=\"javascript:callback('" + Cal.Ctrl + "','" + Cal.FormatDate(PValue) + "'),datechanged(Cal.Ctrl );\">" + PValue + "</td>"; }
...

{
...

	{
	    ...
	    vCalHeader += "<td align='center'><select id ='MonthSelector' name='MonthSelector' onChange='javascript:Cal.SwitchMth(this.selectedIndex);RenderCssCal();'>";
...

	{
		...
		//added version 2.1.2  dtTillToday by vasu
        ...
        else if (Cal.EnableDateMode === "past" && ((j > dtToday.getDate()) && (Cal.Month === dtToday.getMonth()) && (Cal.Year === dtToday.getFullYear()) || (Cal.Month > dtToday.getMonth()) && (Cal.Year === dtToday.getFullYear()) || (Cal.Year > dtToday.getFullYear())) || ((j < dtTillToday.getDate()) && (Cal.Month === dtTillToday.getMonth()) && (Cal.Year === dtTillToday.getFullYear()) || (Cal.Month < dtTillToday.getMonth()) && (Cal.Year === dtTillToday.getFullYear()) || (Cal.Year < dtTillToday.getFullYear()))) {
...
function NewCssCal(pCtrl, pFormat, pScroller, pShowTime, pTimeMode, pShowSeconds, pEnableDateMode, ttfromDate, tttoDate)

{
	...
	dtToday = new Date(tttoDate); //to date

	dtTillToday = new Date(ttfromDate); //from date
