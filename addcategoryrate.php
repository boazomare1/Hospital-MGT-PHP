...
if ($frmflag2 == 'frmflag2')
{
	...
	if (isset($_POST["save"])) {
			...
			$subtypeautono1 = $_POST['subtypeautono'];
			if($subtypeautono1 !=""){
				$subtypeautono = $_POST['subtypeautono'];
			}else{
				$subtypeautono = $_POST['subtypeautoid1'];
			}
		...
		if($count > 0){
	        ...
	        foreach($_REQUEST['surveychk'] as $key=>$value){
				...
				$updaterate = $_POST['subtyperate'.$auto_id];
				$itemautono = $_POST['hiddenautono'.$auto_id];
				$upitemcode = $_POST['hiddenitemcode'.$auto_id];
				$sqlquery= "UPDATE `master_medicine` SET `$updatecolumname` = '$updaterate' WHERE `auto_number` = '$itemautono' AND `itemcode` = '$upitemcode' ";
				$exequery = mysqli_query($GLOBALS["___mysqli_ston"], $sqlquery);
			 
				$bgcolorcode = 'Success';	
				$errmsg = "Category Rate Updated Successfully";
			}
		   //exit;	
		}else{
			$bgcolorcode = 'failed';	
	...
	header("Location: addcategoryrate.php");		
	}
...
$('#subtype').autocomplete({
	...
	source:'ajaxsubtypenameserach.php', 
	//alert(source);
	minLength:1,
	delay: 0,
	html: true, 
		select: function(event,ui){
			var subtypename = ui.item.subtypename;
			var subtypeautono = ui.item.subtypeautono;
			$('#subtypeid').val(subtypeautono); 
			$('#subtype').val(subtypename);
...
function addward1process1()
{
	...
	function updaterate(e,autono){
		...
		var itemcode1 = $("#hiddenitemcode"+autono).val();
		var catrate = $("#subtyperate"+autono).val();
		var subtypeid = $("#subtypeautoid").val();
		var subtypename = $("#subtypename"+autono).val();
		//alert(subtypename);
		var data = "autoid="+subtypeid+"&&subtypeval="+catrate+"&&itemcode="+itemcode1+"&&subtypen="+subtypename;
		//alert(data);
		$.ajax({
			
			type : "post",
			url : "ajaxinlineditrate.php",
			data : data,
			catch : false,
			success : function(data){
				//alert(data);
				if(data="yes"){
					alert("The Rate Has Been Updated Successfully for "+subtypename+".");	
				}else{
					alert("The Rate Failed to Update for "+subtypename+".");
				} 
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tablebackgroundcolor1">
						<tr>
							<td>
								<form name="form1" id="form1" method="post" action="addcategoryrate.php" onSubmit="return addward1process11()">
								...
								<table width="600" border="1" align="center" cellpadding="4" cellspacing="2" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
									...
									<tr bgcolor="#011E6A">
										<td colspan="2" bgcolor="#ecf0f5" class="bodytext3">
											<strong> Add Company Rate </strong>
										</td>
									</tr>
									<tr>
									...
									<tr>
										<td width="15%" align="left" valign="middle"  bgcolor="#FFFFFF" class="bodytext3">
											<div align="right"> Select Company </div>
										</td>
										<td width="82%" align="left" valign="middle"  bgcolor="#FFFFFF">
											<input name="subtype" id="subtype" style="border: 1px solid #001E6A; text-transform:uppercase" size="60" autocomplete="off" /> 
											<input type="hidden" name="subtypename" id="subtypename" value=""/>
											<input type="hidden" name="subtypeid" id="subtypeid" value="" />
								...
								<form name="form2" id="form2" method="post" action="addcategoryrate.php">
								...
								<tr bgcolor="#011E6A">
									...
									<td colspan="5" align="right" bgcolor="#ecf0f5" class="bodytext3"><span>
									<?php 
										$colorloopcount=0;
										$sno=0;
									...
									if ($frmflag1 != ''){
										...
										$subtypeautocode = $_POST['subtypeid'];
										$columname = $_POST['subtypename'];
										
									}else if ($sid !=''){
										$frmflag1='';
										
										$subtypeautocode = $_REQUEST['sid'];
										$query3 = "SELECT subtype FROM master_subtype WHERE recordstatus <> 'deleted' AND auto_number = '$subtypeautocode'";
										$exec3 = mysqli_query($GLOBALS["___mysqli_ston"], $query3) or die ("Error in Query3".mysqli_error($GLOBALS["___mysqli_ston"]));
										$res3 = mysqli_fetch_array($exec3);
										$columname = $res3['subtype'];
									} 
									if($searchflag1 !=''){
										$sid='';
										
										$subtypeautocode = $_REQUEST["subtypeautoid1"]; 
										$query4 = "SELECT subtype FROM master_subtype WHERE recordstatus <> 'deleted' AND auto_number = '$subtypeautocode'";
										$exec4 = mysqli_query($GLOBALS["___mysqli_ston"], $query4) or die ("Error in Query4".mysqli_error($GLOBALS["___mysqli_ston"]));
										$res4 = mysqli_fetch_array($exec4);
										$columname = $res4['subtype'];										
									 ...
									 //error_reporting(0);
										...
										*/
										...
										{	
											...
											//previous button
											if ($page > 1) 
												$pagination.= "<a href=\"$targetpage?sid=$subtypeautocode&&page=$prev\" style='color:#3b3b3c;'>previous</a>";
											else
											...
											{	
												for ($counter = 1; $counter <= $lastpage; $counter++)
												{
													if ($counter == $page)
														...
														$pagination.= "<a href=\"$targetpage?sid=$subtypeautocode&&page=$counter\" style='margin:0 0 0 2px; color:#3b3b3c;'>$counter</a>";					
												}
											}
											elseif($lastpage > 5 + ($adjacents * 2))	//enough pages to hide some
											{
												...
												if($page < 1 + ($adjacents * 2))		
												{
													...
													$pagination.= "...";
													$pagination.= "<a href=\"$targetpage?sid=$subtypeautocode&&page=$lpm1\"style='margin:0 0 0 2px; color:#3b3b3c;'>$lpm1</a>";
													$pagination.= "<a href=\"$targetpage?sid=$subtypeautocode&&page=$lastpage\"style='margin:0 0 0 2px; color:#3b3b3c;'>$lastpage</a>";		
												}
												//in middle; hide some front and some back
												elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2))
												{
													$pagination.= "<a href=\"$targetpage?sid=$subtypeautocode&&page=1\" style='margin:0 0 0 2px; color:#3b3b3c;'>1</a>";
													$pagination.= "<a href=\"$targetpage?sid=$subtypeautocode&&page=2\" style='margin:0 0 0 2px; color:#3b3b3c;'>2</a>";
													$pagination.= "...";
												...
												//next button
												if ($page < $counter - 1) 
													$pagination.= "<a href=\"$targetpage?sid=$subtypeautocode&&page=$next\" style='margin:0 0 0 2px; color:#3b3b3c;'>next</a>";
												else
								...
								<tr bgcolor="#011E6A">
									<td colspan="7" bgcolor="#FFFFFF" class="bodytext3">
										...
										<input type="hidden" name="subtypeautoid1" id="subtypeautoid1" value="<?php echo $subtypeautocode; ?>">
								...
								<tr bgcolor="#011E6A">
									<td width="10%" bgcolor="#ecf0f5" align="center" class="bodytext3">
										<div align="center"><strong>Select All</strong>
											<input type="checkbox" id="selectalla" title= "Select All"  onclick="checkAll1(this,<?php echo $subtypeautocode; ?>)" />
											<input type="hidden" name="subtypeautono" value="<?php echo $subtypeautocode; ?>" >
										</strong></div> 
									</td>									
								...
								<td align="center"  valign="middle"  class="bodytext3">
								    <input type="checkbox" name="surveychk[]" id="surveychk<?php echo $auto_number; ?>" class="surveychk<?php echo $subtypeautocode; ?> selectchk" value="<?php echo $auto_number; ?>" >
									<input type="hidden" name="hiddenitemcode<?php echo $auto_number; ?>" id="hiddenitemcode<?php echo $auto_number; ?>" value="<?php echo $itemcode; ?>" />
									<input type="hidden" name="hiddenautono<?php echo $auto_number; ?>" id="hiddenautono<?php echo $auto_number; ?>" value="<?php echo $auto_number; ?>" />
									<input type="hidden" name="subtypeautoid" id="subtypeautoid" value="<?php echo $subtypeautocode; ?>" />
									<input type="hidden" name="subtypename<?php echo $auto_number; ?>" id="subtypename<?php echo $auto_number; ?>" value="<?php echo $itemname;?>"/>
								</td> 
									...
									<td align="left" valign="middle" class="bodytext3">
										<div align="center">
											<input type="text" name="subtyperate<?php echo $auto_number; ?>" id="subtyperate<?php echo $auto_number; ?>" style="text-align: right;" value="<?php echo $rateperunit; ?>" size="8">
										</div>
									</td>
									<td align="center" valign="middle" class="bodytext3">
							...
							?>				
							<tr >
								<td colspan="7" align="center" valign="middle" bgcolor="#FFFFFF" class="bodytext3">
									<font color="#f93635" >
