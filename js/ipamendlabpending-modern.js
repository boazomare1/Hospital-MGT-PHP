// IP Amend Lab Pending Modern JavaScript - Following Hospital Theme
...
function handleSearch(event) {
    ...
    window.location.href = 'ipamendlab_pending.php?' + searchParams.toString();
}
...
function handleLiveSearch(event) {
    ...
    filteredLabRecords = allLabRecords.filter(record => {
        return (
...
function updatePagination() {
    ...
    const totalPages = Math.ceil(filteredLabRecords.length / itemsPerPage);
...
function changePage(page) {
    if (page < 1 || page > Math.ceil(filteredLabRecords.length / itemsPerPage)) {
        return;
    }
...
function updateResultsDisplay() {
    ...
    const currentRecords = filteredLabRecords.slice(startIndex, endIndex);
...
function viewLabDetails(patientcode, visitcode) {
    showAlert(`Viewing lab details: ${patientcode} - ${visitcode}`, 'info');
    
    // This would normally open a modal or navigate to a view page
    console.log('Viewing lab details:', patientcode, visitcode);
}
