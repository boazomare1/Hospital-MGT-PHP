...
function ledgervalue($parentid,$ADate1,$ADate2,$tbinclude)
{
	...
	{
		$query8 = mysqli_query($GLOBALS["___mysqli_ston"], "select accountsmain from master_accountssub where auto_number = '$parentid'");
		$res8 = mysqli_fetch_array($query8);
		//echo $res8['accountsmain'];
		$query81 = mysqli_query($GLOBALS["___mysqli_ston"], "select section from master_accountsmain where auto_number = '".$res8['accountsmain']."'");
		$res81 = mysqli_fetch_array($query81);
		//echo $res81['section'];
		...
		{  
			...
			if($res81['section'] == 'A' || $res81['section'] == 'E')
			{
				$i = 0;
				$result = array();
				$querydr1 = "SELECT SUM(openbalanceamount) as stockpurchase FROM `openingbalanceaccount` WHERE `accountcode` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'
				UNION ALL SELECT SUM(-1*openbalanceamount) as stockpurchase FROM `openingbalancesupplier` WHERE `accountcode` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'";
			...
			else 
			{
				$i = 0;
				$result = array();
				$querydr1 = "SELECT SUM(-1*openbalanceamount) as stockpurchase FROM `openingbalanceaccount` WHERE `accountcode` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'
				UNION ALL SELECT SUM(openbalanceamount) as stockpurchase FROM `openingbalancesupplier` WHERE `accountcode` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'";
				$execdr1 = mysqli_query($GLOBALS["___mysqli_ston"], $querydr1) or die ("Error in Querydr1".mysqli_error($GLOBALS["___mysqli_ston"]));
				while($resdr1 = mysqli_fetch_array($execdr1))
				{
					$i = $i+1;
