...
$is_capitation = $res8['capitation'];

$query9 = "select sum(consultation) as consultationamount from billing_consultation where patientvisitcode = '$visitcode'";

$exec9 = mysqli_query($GLOBALS["___mysqli_ston"], $query9) or die ("Error in Query8".mysqli_error($GLOBALS["___mysqli_ston"]));

$res9 = mysqli_fetch_array($exec9);

$copay_consultation = $res9['consultationamount'];
if($copay_consultation=='')
{
	$copay_consultation=0;
}

$query10 = "select sum(totalamount) as copayamount from billing_paynow where visitcode = '$visitcode'";

$exec10 = mysqli_query($GLOBALS["___mysqli_ston"], $query10) or die ("Error in Query8".mysqli_error($GLOBALS["___mysqli_ston"]));

$res10 = mysqli_fetch_array($exec10);

$copay_amount= $res10['copayamount'];
if($copay_amount=='')
{
	$copay_amount=0;
}
$total_copay_paid=$copay_consultation+$copay_amount;
if($is_capitation=='1')
{
$paylateramount=$paylateramount-$total_copay_paid;
}else
{
$paylateramount=$paylateramount+$total_copay_paid;    
}
...
$fileData1 = $fileData1.addslashes("
	...
	<Code>CONS-".$consultationanum."</Code>
...
$fileData1 = $fileData1.addslashes("
	...
	<Code_Description>COPAY</Code_Description>

	<Quantity>1</Quantity>

	<Total_Amount>".-$total_copay_paid."</Total_Amount>
...
$query8 = "update billing_paylater set orginalsmartxml='".addslashes($importData)."' where visitcode = '$visitcode' and billno='$billautonumber'";
...
if(isset($_REQUEST['slade']) && isset($_REQUEST['claim']) && $_REQUEST['slade']=='yes'){
   $slade_claim_id=$_REQUEST['claim'];
    header("location:slade-balance.php?billautonumber=$billautonumber&&st=success&&printbill=$printbill&&frmflag1=frmflag1&&patientcode=$patientcode&&visitcode=$visitcode&slade=yes&claim=$claim&authorization_id=$authorization_id&amount=$amount&type=op");
   exit;
}else{
