...
$query200 = "select itemcode,itemname,disease,genericname,pkg,`$ratecolumn`,type from master_medicine where (itemname like '%$searchmedicinename1%' or disease like '%$searchmedicinename1%' or genericname like '%$searchmedicinename1%') and categoryname not in ('MEDICAL SUPPLIES','GENERAL STORES') and status <> 'deleted' and itemcode not in (select itemcode from package_items where  package_id='$packageid') order by itemname";
