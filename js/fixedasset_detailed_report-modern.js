// Fixed Asset Detailed Report Modern JavaScript
// Handles form validation, responsive design, and enhanced user experience for fixed asset detailed report management
...
function initializeFormValidation() {
    const searchForm = document.querySelector('form[action*="fixedasset_detailed_report.php"]');
    if (searchForm) {
...
function adjustTablesForMobile() {
    const tables = document.querySelectorAll('.asset-report-table');
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if (e.ctrlKey && e.key === 'r') {
            ...
            if (confirm('Are you sure you want to reset the form?')) {
                document.querySelector('form[action*="fixedasset_detailed_report.php"]').reset();
            }
        }
    });
}
...
function addFormResetConfirmation() {
    const form = document.querySelector('form[action*="fixedasset_detailed_report.php"]');
    if (form) {
...
// Asset Report Table Enhancement
function initializeAssetReportTable() {
    const tables = document.querySelectorAll('.asset-report-table');
    tables.forEach(table => {
        addTableSorting(table);
        addAssetStatusIndicators(table);
        addValueFormatting(table);
...
function sortTable(table, columnIndex) {
    ...
    // Sort rows (excluding category totals and grand totals)
    const dataRows = rows.filter(row => !row.classList.contains('category-total') && !row.classList.contains('grand-total'));
    ...
    // Reorder rows while preserving category totals and grand totals
    const categoryTotals = rows.filter(row => row.classList.contains('category-total'));
    const grandTotal = rows.filter(row => row.classList.contains('grand-total'));
    ...
    // Add category totals
    categoryTotals.forEach(row => tbody.appendChild(row));
    
    // Add grand total
    grandTotal.forEach(row => tbody.appendChild(row));
}

function addAssetStatusIndicators(table) {
    const rows = table.querySelectorAll('tbody tr');
    rows.forEach(row => {
        const assetNameCell = row.querySelector('td:nth-child(3)'); // Asset name column
        if (assetNameCell) {
            const assetName = assetNameCell.textContent.toLowerCase();
            let assetStatus = '';
            
            // Determine asset status based on name or other criteria
            if (assetName.includes('active') || assetName.includes('operational')) {
                assetStatus = 'active';
            } else if (assetName.includes('inactive') || assetName.includes('retired')) {
                assetStatus = 'inactive';
            } else if (assetName.includes('maintenance') || assetName.includes('repair')) {
                assetStatus = 'maintenance';
            } else if (assetName.includes('disposed') || assetName.includes('sold')) {
                assetStatus = 'disposed';
            }
            
            if (assetStatus) {
                const statusIndicator = document.createElement('span');
                statusIndicator.className = `asset-status status-${assetStatus}`;
                statusIndicator.textContent = assetStatus.toUpperCase();
                statusIndicator.style.marginLeft = '0.5rem';
                
                assetNameCell.appendChild(statusIndicator);
            }
        }
    });
}

function addValueFormatting(table) {
    const rows = table.querySelectorAll('tbody tr');
    rows.forEach(row => {
        const purchaseCostCell = row.querySelector('td:nth-child(4)'); // Purchase cost column
        const depreciationCell = row.querySelector('td:nth-child(5)'); // Depreciation column
        const netValueCell = row.querySelector('td:nth-child(6)'); // Net book value column
        
        if (purchaseCostCell && purchaseCostCell.textContent.trim() !== '') {
            purchaseCostCell.classList.add('asset-purchase-cost');
        }
        
        if (depreciationCell && depreciationCell.textContent.trim() !== '') {
            depreciationCell.classList.add('asset-depreciation');
        }
        
        if (netValueCell && netValueCell.textContent.trim() !== '') {
            netValueCell.classList.add('asset-net-value');
...
function addRowHighlighting(table) {
    ...
    rows.forEach(row => {
        ...
        row.addEventListener('mouseenter', function() {
            if (!this.classList.contains('category-total') && !this.classList.contains('grand-total')) {
...
function addCategoryGrouping(table) {
    const rows = table.querySelectorAll('tbody tr');
    rows.forEach(row => {
        // Check if this is a category total row
...
function updateAssetSummary() {
    const rows = document.querySelectorAll('.asset-report-table tbody tr');
    ...
    rows.forEach(row => {
        if (!row.classList.contains('category-total') && !row.classList.contains('grand-total')) {
            const purchaseCostCell = row.querySelector('td:nth-child(4)');
            const depreciationCell = row.querySelector('td:nth-child(5)');
            const netValueCell = row.querySelector('td:nth-child(6)');
    ...
    updateSummaryDisplay(totalPurchaseCost, totalDepreciation, totalNetValue, assetCount);
}

function addSummaryCards() {
    const container = document.querySelector('.asset-report-container') || document.body;
    const summarySection = document.createElement('section');
    summarySection.className = 'summary-section';
    summarySection.innerHTML = `
        <div class="summary-header">
            <h3><i class="fas fa-chart-pie"></i> Asset Summary</h3>
        </div>
        <div class="summary-cards">
            ...
            <div class="summary-card count">
                <div class="summary-card-title">Asset Count</div>
                <div class="summary-card-value" id="assetCount">0</div>
            </div>
        </div>
    `;
...
function updateSummaryDisplay(purchaseCost, depreciation, netValue, count) {
    ...
    if (assetCountEl) assetCountEl.textContent = count;
}
...
window.FixedAssetDetailedReportModern = {
    validateSearchForm,
    showNotification,
    exportToExcel,
    exportToPDF,
