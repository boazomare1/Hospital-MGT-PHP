    ...
    <td width="97%" valign="top"><table width="116%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="860">
              ...
              <form name="cbform1" method="post" action="labanalyzerresultview.php">
...
$querynw1 = "select * from resultentry_lab where patientcode like '%$searchpatientcode%' and patientvisitcode like '%$searchvisitcode%' and patientname like '%$searchpatient%' and recorddate between '$fromdate' and '$todate' and docnumber like '%$docnumber%' and resultstatus='completed' and publishstatus = 'completed' and locationcode='$locationcode' group by docnumber order by auto_number desc";
...
$queryn21 = "select * from consultation_lab where patientcode like 'walkin' and patientvisitcode like 'walkinvis' and resultentry='completed' and consultationdate between '$fromdate' and '$todate' and locationcode='$locationcode'  group by billnumber order by auto_number desc";
...
?>
            ...
            <tr>
              <td width="2%"  align="left" valign="center" 
                bgcolor="#ffffff" class="bodytext31"><div align="center"><strong>No.</strong></div></td>
				 <td colspan="2" width="10%"  align="center" valign="center" 
                bgcolor="#ffffff" class="bodytext31"><strong>Print</strong>
    	...
    	$query1 = "select * from resultentry_lab where patientcode like '%$searchpatientcode%' and patientvisitcode like '%$searchvisitcode%' and patientname like '%$searchpatient%' and recorddate between '$fromdate' and '$todate' and docnumber like '%$docnumber%' and resultstatus='completed' and publishstatus = 'completed' and locationcode='$locationcode' group by docnumber order by patientvisitcode";
		$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
		$num1=mysqli_num_rows($exec1);
		if($num1>0)
		{
		...
		if($num1 !=0)
          ...
          <tr <?php echo $colorcode; ?>>
			...
			<!--<a href="print_master.php?patientcode=<?= $patientcode;?>&visitcode=<?= $visitcode;?>&fromdate=<?=$fromdate?>&todate=<?=$todate?>&docnumber=<?=$docnumber1;?>" target="_blank"><strong>Print</strong></a>-->
			<a href="print_labresultssingle_new1.php?patientcode=<?= $patientcode;?>&visitcode=<?= $visitcode;?>&fromdate=<?=$fromdate?>&todate=<?=$todate?>&docnumber=<?=$docnumber1;?>" target="_blank"><strong>Print</strong></a>
			
			
			</td>
			<td class="bodytext31" valign="center"  align="center">
			<a href="print_labresultsfull1.php?patientcode=<?= $patientcode;?>&visitcode=<?= $visitcode;?>&ADate1=<?=$fromdate?>&ADate2=<?=$todate?>" target="_blank"><strong>View All</strong></a>
		...
		{
			?>
			<tr><td colspan="11" class="bodytext31" bgcolor=""><strong></strong> &nbsp;</td></tr>
            <tr><td colspan="11" class="bodytext31" bgcolor="#ecf0f5"><strong>IP</strong></td></tr>
		<?php
		while($res1 = mysqli_fetch_array($exec1))
		{
		$itemname='';
		...
		if($num1 !=0)
          ...
          <tr <?php echo $colorcode; ?>>
              <td height="45"  align="left" valign="center" class="bodytext31"><div align="center"><?php echo $sno1 = $sno1 + 1; ?></div></td>
			   <td class="bodytext31" valign="center"  align="center"><!--<a href="print_master.php?patientcode=<?= $patientcode;?>&visitcode=<?= $visitcode;?>&fromdate=<?=$fromdate?>&todate=<?=$todate?>&&docnumber=<?=$docnumber;?>" target="_blank"><strong>Print</strong></a>-->
			   	<a href="print_labresultssingle_new1.php?patientcode=<?= $patientcode;?>&visitcode=<?= $visitcode;?>&fromdate=<?=$fromdate?>&todate=<?=$todate?>&docnumber=<?=$docnumber1;?>" target="_blank"><strong>Print</strong></a>
