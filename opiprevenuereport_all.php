  ...
  <tr>
    ...
    <td width="99%" valign="top"><table width="116%" border="0" cellspacing="0" cellpadding="0">
       ...
       <td width="860">
		...
		<form name="cbform1" method="post" action="opiprevenuereport_all.php">
			    ...
			    <select name="location" id="location">
                 <option value="All">All</option>
                    ...
                    <?php
						...
						$query1 = "select locationname,locationcode from master_location where status='' order by locationname";
...
$res8pharmacyitemrate = 0;
			...
			$query1 = "select sum(billamount1) as billamount1 from (
				...
				select sum(consultation-sharingamount) as billamount1 from billing_consultation where  $pass_location and  billdate between '$transactiondatefrom' and '$transactiondateto' 

				UNION ALL

				select sum(totalamount-sharingamount) as billamount1 from billing_paylaterconsultation where $pass_location and  billdate between '$transactiondatefrom' and '$transactiondateto' and accountname = 'CASH - HOSPITAL'
...
*/
			 $query1 = "select SUM(totalamount-sharingamount) AS billamount1 from billing_paylaterconsultation where  $pass_location and  billdate between '$transactiondatefrom' and '$transactiondateto' and accountname != 'CASH - HOSPITAL' and visitcode in (select visitcode from master_visitentry )";
			 $query1 = "select SUM(totalamount-sharingamount) AS billamount1 from billing_paylaterconsultation where  $pass_location and  billdate between '$transactiondatefrom' and '$transactiondateto' and accountname != 'CASH - HOSPITAL' and visitcode in (select visitcode from master_visitentry )";
			$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in query1".mysqli_error($GLOBALS["___mysqli_ston"]));
		  ...
		  //CASH
			...
			//CREDIT
/*
			$query8 = "select sum(amount1) as amount1 from (select sum(amount) as amount1 from billing_paylaterpharmacy where $pass_location and billdate between '$transactiondatefrom' and '$transactiondateto' and accountname != 'CASH - HOSPITAL') as amount";
				*/
...
$totrebate                = 0;
	...
	$query1 = "select  auto_number,patientname,patientcode,visitcode,'billing' as type  from billing_ip where $pass_location and  accountnameano = '603' and billdate between '$fromdate' and '$todate'
		UNION ALL 
	 SELECT auto_number,patientname,patientcode,visitcode,'creditapproved' as type from billing_ipcreditapproved where $pass_location and  accountnameano = '603' and billdate between '$fromdate' and '$todate' group by visitcode  order by auto_number DESC ";
    ...
    //                $query112 = "select sum(amountuhx) as amountuhx from billing_ipbedcharges where locationcode='$locationcode1' and  description NOT IN  ('Bed Charges','Nursing Charges','Resident Doctor Charges') and  visitcode='$visitcode'  and recorddate between '$fromdate' and '$todate'
	    ...
	    $query6 = "select sum(servicesitemrateuhx),sum(sharingamount) from billing_ipservices where $pass_location and patientcode = '$patientcode' and patientvisitcode='$visitcode'   and billdate between '$fromdate' and '$todate'";
		...
		$query8              = "select (transactionamount) as transactionamount, (original_amt-sharingamount) as original_amt, visittype, coa from billing_ipprivatedoctor where  patientcode = '$patientcode' and visitcode='$visitcode'   and billtype <>'' and recorddate between '$fromdate' and '$todate'  ";
                $exec8 = mysqli_query($GLOBALS["___mysqli_ston"], $query8) or die("Error in Query8" . mysqli_error($GLOBALS["___mysqli_ston"]));
                $num8                     = mysqli_num_rows($exec8);
                while($res8 = mysqli_fetch_array($exec8))
				{
		...
		$query11 = "select sum(amount) from billing_ipbedcharges where $pass_location and patientcode = '$patientcode' and visitcode='$visitcode' and (description = 'Daily Review charge' or description = 'RMO Charges') and recorddate between '$fromdate' and '$todate' ";
...

		{
		   ...
		   <?php 
			...
			$rowtot1 = $totaladmissionamount+0+$totalipbedcharges+$totalipnursingcharges+$totaliprmocharges+ $totaliprmocharges1+$totallabamount+$totalradiologyamount+$totalpharmacyamount+$totalservicesamount+$totalambulanceamount+$totalhomecareamount+$totalprivatedoctoramount+$totalipmiscamount+$totalipdiscamount;
...
$totalipdiscamount = 0;
	 ...
	 $query186 = "select  auto_number,patientname,patientcode,visitcode,'billing' as type from billing_ip where $pass_location and billdate between '$fromdate' and '$todate'  and accountnameano != '603' 
	 UNION ALL SELECT auto_number,patientname,patientcode,visitcode,'creditapproved' as type from billing_ipcreditapproved where billdate between '$fromdate' and '$todate' and  $pass_location  and accountnameano != '603'  group by visitcode  order by auto_number DESC ";
...
$chk = "accountnameano != '603'";
 ...
 UNION ALL SELECT sum(-1*rate) as amount FROM `ip_discount` WHERE patientvisitcode IN (select visitcode FROM `billing_ipcreditapprovedtransaction` WHERE $chk and billdate BETWEEN '$fromdate' AND '$todate' group by billno) and patientvisitcode ='$visitcode'";
		...
		$querysearchnew = "select visitcode from billing_ip where billdate between '$fromdate' and '$todate' and accountnameano <> '603'  and  $pass_location  UNION ALL SELECT visitcode from billing_ipcreditapproved where billdate between '$fromdate'  and '$todate' and accountnameano <> '603' and  $pass_location";
