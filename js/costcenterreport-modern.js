// Modern JavaScript for costcenterreport.php

document.addEventListener('DOMContentLoaded', function() {
    initializeCostCenterReport();
});

function initializeCostCenterReport() {
...
function exportToExcel() {
    ...
    const url = 'costcenterreport.php?' + new URLSearchParams(formData).toString();
    const link = document.createElement('a');
    link.href = url;
    link.download = 'cost_center_report.xls';
    link.click();
...
function exportToPDF() {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = 'cost_center_report.pdf';
    link.click();
...
function exportToCSV() {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = 'cost_center_report.csv';
    link.click();
...
function updateSummaryCards() {
    ...
    rows.forEach(row => {
        ...
        if (cells.length >= 3) {
            const revenue = parseFloat(cells[1]?.textContent.replace(/[^0-9.-]/g, '') || 0);
            const expenses = parseFloat(cells[2]?.textContent.replace(/[^0-9.-]/g, '') || 0);
    ...
    updateSummaryCard('revenue', totalRevenue, 'positive');
    updateSummaryCard('expenses', totalExpenses, 'negative');
    updateSummaryCard('profit', totalProfit, totalProfit >= 0 ? 'positive' : 'negative');
}
