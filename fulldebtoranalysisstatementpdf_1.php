...
<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" bordercolor="#666666" cellspacing="0" cellpadding="4" width="1101" align="center" border="1">
	...
	// OUTSTANDING OPENING BALANCE START		
			  ...
			  $opquery2 = "select billnumber,visitcode,fxamount AS fxamount,transactiondate AS transactiondate,accountnameid from master_transactionpaylater where accountnameano in($subtypes_accountnameanos_str) and paymenttype like '%%' and accountnameid in($subtypes_accountids_str) and transactiontype = 'finalize' and transactiondate < '$ADate1' and fxamount <>'0' and billnumber not like 'AOP%'

		  UNION ALL select billnumber,visitcode,transactionamount as fxamount,transactiondate AS transactiondate,accountnameid  from master_transactionpaylater where accountnameano in($subtypes_accountnameanos_str) and accountnameid in($subtypes_accountids_str) and transactiontype = 'finalize' and transactiondate < '$ADate1'  and billnumber like 'AOP%'
	
		  UNION ALL SELECT b.`docno` as billnumber,b.visitcode AS visitcode, (-1*b.`transactionamount`) as fxamount, b.`transactiondate` as transactiondate,b.accountnameid AS accountnameid FROM `master_transactionpaylater` as a JOIN `master_transactionpaylater` as b ON (a.`visitcode` = b.`visitcode`) WHERE b.`accountnameid` in($subtypes_accountids_str) and a.`transactiontype` = 'finalize' and b.`transactiontype` = 'pharmacycredit' and b.`auto_number` > a.`auto_number` AND b.`transactiondate` < '$ADate1' and b.billnumber!=''


	
		 UNION ALL SELECT  docno AS billnumber,'' AS visitcode,(-1*`fxamount`) as fxamount,transactiondate AS transactiondate,accountnameid from master_transactionpaylater where accountnameano in($subtypes_accountnameanos_str) and  paymenttype in($paymentTypes_names_str) and accountnameid in($subtypes_accountids_str) and transactiontype = 'paylatercredit' and recordstatus <> 'deallocated' and transactiondate < '$ADate1'  		  
		  
		    
 UNION ALL SELECT  billnumber,'' AS visitcode,(-1*`fxamount`) as fxamount,transactiondate AS transactiondate,accountnameid   FROM `master_transactionpaylater` WHERE `accountnameid` in($subtypes_accountids_str) AND `transactiondate` < '$ADate1'  AND `docno` LIKE 'AR-%' AND `transactionstatus` = 'onaccount' AND `transactionmodule` = 'PAYMENT'
		  
		  UNION ALL SELECT docno AS billnumber,patientvisitcode AS visitcode,(1*`amount`) as fxamount,consultationdate AS transactiondate,accountcode AS accountnameid  FROM `adhoc_debitnote` WHERE `accountcode` in($subtypes_accountids_str) AND `consultationdate` < '$ADate1'  group by docno
		  
		  UNION ALL SELECT  docno AS billnumber,'' AS visitcode,(debitamount-creditamount) as fxamount,entrydate AS transactiondate,NULL as accountnameid   FROM `master_journalentries` WHERE `ledgerid` in($subtypes_accountids_str) AND `entrydate` < '$ADate1'  group by docno";
		...
		$resbillnumber2 = $opres2['billnumber'];
		$resvisitcode2 = $opres2['visitcode'];
		$res2accountnameid = $opres2['accountnameid'];
		$totalpayment2 = 0;
/*	
	$query982 = "select sum(fxamount +discount ) as transactionamount1,docno from master_transactionpaylater where  transactiontype = 'PAYMENT' and billnumber='$resbillnumber2' and recordstatus = 'allocated' AND `transactiondate` <= '$ADate2'  and accountnameid in (select id from master_accountname where subtype='$subtypeanum') 
	UNION ALL 
	select sum(-1*fxamount ) as transactionamount1,docno from master_transactionpaylater where  transactiontype = 'PAYMENT' and docno='$resbillnumber2' and recordstatus = 'allocated' AND `transactiondate` <= '$ADate2'and accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND accountnameid='$res21accountid'  ";	
*/	
//AND `transactiondate` <= '$ADate2' 
 
			// AND  b.transactiondate <= '$ADate2'	
				
		 $query982 = "select sum(a.fxamount +a.discount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where a.transactiontype = 'PAYMENT' AND a.transactionstatus!= 'onaccount' AND b.transactionstatus= 'onaccount' and a.billnumber='$resbillnumber2' and a.recordstatus = 'allocated' and a.accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND a.accountnameid='$res2accountnameid' 
				UNION ALL
				select sum(a.fxamount +a.discount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where a.transactionmode = 'CREDIT NOTE' AND a.transactiontype= 'PAYMENT' AND b.transactiontype= 'paylatercredit' and a.billnumber='$resbillnumber2' and a.recordstatus = 'allocated' and a.accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND a.accountnameid='$res2accountnameid' 
				UNION ALL 
				select sum(-1*a.fxamount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where  a.transactiontype = 'PAYMENT' AND b.transactiontype != 'PAYMENT' and a.docno='$resbillnumber2' and a.recordstatus = 'allocated'  and a.accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND a.accountnameid='$res2accountnameid' AND a.docno NOT LIKE '%AR%'  
				
					UNION ALL 
				select sum(-1*a.fxamount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno)  where  b.transactionstatus = 'onaccount' AND a.transactionstatus != 'onaccount' and a.docno='$resbillnumber2' and a.recordstatus = 'allocated' AND  a.docno  LIKE '%AR%'  and a.accountnameid in (select id from master_accountname where subtype='$subtypeanum')      ";				
				
	/*			
				UNION ALL 
				select sum(-1*a.fxamount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where  a.transactiontype = 'PAYMENT' AND b.transactiontype != 'PAYMENT' and a.docno='$resbillnumber2' and a.recordstatus = 'allocated' AND a.docno  LIKE '%AR%'  and a.accountnameid in (select id from master_accountname where subtype='$subtypeanum') AND b.transactionstatus='onaccount' AND b.transactiondate  <= '$ADate2'  ";			
*/

				$exec982 = mysqli_query($GLOBALS["___mysqli_ston"], $query982) or die('ERROR IN  98'.mysqli_error($GLOBALS["___mysqli_ston"]));
				$num982 = mysqli_num_rows($exec982);
				while($res982 = mysqli_fetch_array($exec982))
				{
				  $payment2 = $res982['transactionamount1'];

				  $totalpayment2 = $totalpayment2 + $payment2;
				}
									
				//$opbalance2 = $resamount2 - $totalpayment2;	
				$opbalance2 =number_format($resamount2,2,'.','')-number_format($totalpayment2,2,'.','');
				//echo '<br>';
			
	 ////////////// FOR AR DOCS PENDING AMOUNT //////////////
          $dotarray = explode("-", $resbillnumber2);
          $dot_ar_doc = $dotarray[0];
          if($dot_ar_doc=='AR'){
                 $query2 = "SELECT * from master_transactionpaylater where  docno='$resbillnumber2' AND transactionmodule = 'PAYMENT' AND transactionstatus = 'onaccount' order by auto_number desc LIMIT 1";
           ...
           $query_allocated_amount = "SELECT sum(transactionamount) as amount, sum(discount) as discount from master_transactionpaylater where  recordstatus='allocated' and docno='$resbillnumber2'";
          ...
          while($res_allocated_amount = mysqli_fetch_array($exec_allocated_amount)){
                $allocated_amount=$res_allocated_amount['amount'];
                  ...
                  $opbalance2=($totalamount_AR)+$allocated_amount;
     //              if($num_allocated>0){
