...
else
{
	...
	//echo $companycode;
		...
		values ('$patientname','$patientcode','$visitcode','$updatedatetime', '$particulars', 
		'$transactionmode', '$transactiontype','$ipaddress', '$updatedatetime','$companyanum', '$companyname', '$remarks', 
		'$transactionmodule','$accname','$billnumbercode','$cardamount','$cardamount','$cardnumber','$chequedate','$bankname1','$username',CURTIME(),'$ipdepositscoa','".$locationnameget."','".$locationcodeget."','".$comments."','$bankcode')";
		$exec9 = mysqli_query($GLOBALS["___mysqli_ston"], $query9) or die ("Error in Query9".mysqli_error($GLOBALS["___mysqli_ston"]));
...
var data = "patientcode="+patientcode+"&&visitcode="+visitcode+"&&form_flag=deposit";
...
document.getElementById("tdShowTotal").value = formatMoney(newbalance);
}
...
<form name="form1" id="form1" method="post" action="depositform.php?menuid=<?php echo $menuid; ?>" >	
<table width="101%" border="0" cellspacing="0" cellpadding="2">
  ...
  <tr>
    ...
    <td width="97%" valign="top"><table width="116%" border="0" cellspacing="0" cellpadding="0">
		...
		$gender = $res1['gender'];
		$age = $res1['age'];
		$visit_ward = $res1['visit_ward'];
		
		$query671 = "select ward,deposit_amount from master_ward where auto_number='$visit_ward'";
		$exec671 = mysqli_query($GLOBALS["___mysqli_ston"], $query671); 
		$res671 = mysqli_fetch_array($exec671);
		$ward = $res671['ward'];
		$deposit_amount = $res671['deposit_amount'];
        ...
        <td colspan="6">&nbsp;</td>
			...
			<table width="60%" border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
                <tbody>
				  ...
				  <tr>
				  <td colspan="" bgcolor="#ecf0f5" class="bodytext3"><strong>Admitting Ward</strong>&nbsp;<input type="text" name="visit_ward" value="<?php echo $ward; ?>" readOnly></td>
				  <td colspan="" bgcolor="#ecf0f5" class="bodytext3"><strong>Deposit Amount</strong>&nbsp;<input type="text" name="visit_ward" value="<?php echo number_format($deposit_amount,'2','.',','); ?>" readOnly></td>
				  </tr>
			...
			<td colspan="6">
			 <table width="919" border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
				...
				<input type="hidden" name="onlinecoa" value="<?php echo $onlinecoa; ?>"></td>
					...
					<select name="billtype" id="billtype" style="width: 130px;" onChange="return paymentinfo(); getEncAmount();">
 ...
 class="bodytext31" colspan="1" bgcolor="#FFF"><div align="right"><strong> MPESA No. </strong></div></td>
                <td align="left" valign="center"  
 class="bodytext31" colspan="1" bgcolor="#FFF"><input name="mpesanumber" id="mpesanumber" value="" style="border: 1px solid #001E6A; text-align:left; text-transform:uppercase" size="8" <?php if($mpesa_integration == 0){ echo "readonly"; } ?> />
			<?php 
 ...
 class="bodytext31" colspan="1" bgcolor="#FFF"><div align="right"><strong> Date </strong></div></td>
                <td  colspan="1" align="left" valign="center"  
 class="bodytext31" colspan="1" bgcolor="#FFF">  <input name="chequedate" id="chequedate" value="" style="border: 1px solid #001E6A;float:left; text-align:left; text-transform:uppercase" size="8"  /><img src="images2/cal.gif" onClick="javascript:NewCssCal('chequedate','','','','','','','<?= date('Y-m-d'); ?>','<?= date('Y-m-d'); ?>')" style="cursor:pointer;float:left"/></td>
                <td align="left" valign="center"  
 ...
 class="bodytext31" colspan="1" bgcolor="#FFF"><div align="right"><strong> Transaction No </strong></div></td>
                <td align="left" valign="center"  
 class="bodytext31" colspan="1" bgcolor="#FFF"><input name="cardnumber" id="cardnumber" value="" style="border: 1px solid #001E6A; text-align:left; text-transform:uppercase" size="8"  <?php if($barclayscard_integration == 1){ echo "readonly"; } ?>/>
				<!-- iPayments HTML/PHP Code Start -->
	            <?php
                 ...
                 class="bodytext31">&nbsp;</td>
                  ...
                  <tr>
                    ...
                    <td align="left" valign="top"  colspan="5" bgcolor="#FFFFFF"><font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif">
                      ...
                      <input name="Submit" type="submit"  value="Save Payment" id="savebutton" class="button" onclick="return checkvalid();" style="border: 1px solid #001E6A"/>
                    </font></td>
			 ...
			 // $dateofcollection = $res43['transactiondate'];
			  ...
			  //$transactionamount = $res43['transactionamount'];
				...
				if(mysqli_num_rows($bal_exec) > 0)
				{
			  	...
			  	$refund_amt_bal_query = "SELECT sum(amount) as refundamt from deposit_refund where patientcode = '$patientcode' and visitcode='$visitcode'";
