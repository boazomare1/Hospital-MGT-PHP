...
{
 $query70="select billnumber from resultentry_radiology where docnumber='$docnumber' and billnumber='$billnumber'";
$exec70=mysqli_query($GLOBALS["___mysqli_ston"], $query70);
$res70=mysqli_fetch_array($exec70);
 $billnumberext=$res70['billnumber'];

 $query70="select * from billing_external where billno='$billnumberext'";
	$exec70=mysqli_query($GLOBALS["___mysqli_ston"], $query70);
$res70=mysqli_fetch_array($exec70);
 $patientname=$res70['patientname'];
 $patientgender=$res70['gender'];
 $patientage=$res70['age'];


}

$query31="select date(recorddate) as recorddate from resultentry_radiology where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and docnumber = '$docnumber' and acknowledge = 'completed' group by itemname";
	$exec70=mysqli_query($GLOBALS["___mysqli_ston"], $query31);
	$res70=mysqli_fetch_array($exec70);
	$recorddate=$res70['recorddate'];
    if($recorddate==''){
    $query31="select date(recorddate) as recorddate from ipresultentry_radiology where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and docnumber = '$docnumber' and acknowledge = 'completed' group by itemname";
	$exec70=mysqli_query($GLOBALS["___mysqli_ston"], $query31);
	$res70=mysqli_fetch_array($exec70);
	$recorddate=$res70['recorddate'];
	}
    ...
    <td colspan="4" align="left" valign="middle"  bgcolor="<?php if ($errmsg == '') { echo '#ecf0f5'; } else { echo 'red'; } ?>" class="bodytext3"><strong><?php echo $errmsg;?>&nbsp;</strong></td></tr>
	  ...
	  <td width="22%" class="bodytext3" align="left" valign="middle" bgcolor="#ecf0f5">
                ...
                <td width="26%" bgcolor="#ecf0f5" class="bodytext3">
                  ...
                  <input name="ADate" id="ADate" style="border: 1px solid #001E6A" value="<?php echo $recorddate; ?>"  size="8"  readonly="readonly" onKeyDown="return disableEnterKey()" />				</td>
		...
		<?php
		$query35="select imagefilename,imagefilename2,imagefilename3 from consultation_radiology where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and docnumber = '$docnumber'  group by radiologyitemname";
