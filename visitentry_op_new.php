...
$location = $_REQUEST['location'];
   ...
   $accountname = $_REQUEST["accountname"];
	...
	$refferal_promotion = $_REQUEST["refferal_promotion"];
	...
	if($billtype == 'PAY NOW')
	{
		$query3 = "select auto_number from master_visitentry where patientcode = '$patientcode' and billtype = 'PAY NOW' and  consultationdate = '$consultationdate'";
	...
	{
	$totalunbilled=0;
	$query211 = "select visitcode,billtype,consultationdate from master_visitentry where patientcode = '$patientcode' order by auto_number desc";
    ...
    $olconsultationdate = $res211['consultationdate'];
	...
	if($oldvisitcode1 != '')
	{
		if($oldbilltype == 'PAY NOW')
		{
            ...
            if($olconsultationdate==$curnt_date)
            {
			...
			$res2215=1;
			$query2214 = "select auto_number from billing_paynow where visitcode='$oldvisitcode1'";
		...
		else 
		{
			...
			$query22100 = "select auto_number from master_visitentry where visitcode='$oldvisitcode1' and recordstatus='closed'";
			$exec22100 = mysqli_query($GLOBALS["___mysqli_ston"], $query22100) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
			$res22100 = mysqli_num_rows($exec22100);
			
			if ($res221 == 0 && $res22100 == 0)
			{
				header ("location:visitentry_op_new.php?errorcode=errorcode11failed");
				exit();
	...
	if($oldvisitcode1 != '')
	{
		...
		else 
		{
			...
			$query221001 = "select auto_number from master_visitentry where visitcode='$oldvisitcode1' and recordstatus='closed'";
			$exec221001 = mysqli_query($GLOBALS["___mysqli_ston"], $query221001) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
			$res221001 = mysqli_num_rows($exec221001);
			
			if ($res221 == 0 && $res221001 == 0)
			{
				header ("location:visitentry_op_new.php?errorcode=errorcode11failed");
				exit();
...
/* 	
	 ...
	 */
				...
				$query = "select locationname,locationcode,auto_number from login_locationdetails where username='$username' and docno='$docno' order by locationname";
...
$query2 = "select visitcode from master_visitentry  order by auto_number desc limit 0, 1";
...
$visitcode = $testvalue + 1;
	...
	$query4 = "select planstatus,smartap,lablimit,radiologylimit,serviceslimit,pharmacylimit from master_planname where auto_number = '$planname'";
	...
	$queryopchk = "select auto_number from master_visitentry where visitcode = '$visitcode' order by auto_number";
	...
	$query_edition = "select auto_number from master_edition where `from` <= '$currentdate' and `end` >= '$currentdate' and status = 'ACTIVE'";
...
// exit;
	...
	$query56="insert into master_visitentry(patientcode, visitcode, patientfirstname,patientmiddlename, patientlastname,consultationtype,consultingdoctor,consultingdoctorcode,accountname,accountexpirydate,paymenttype,subtype,planname,planexpirydate,visitlimit,overalllimit,availablelimit,billtype,
		visitcount,department,consultationdate,consultationtime,consultationfees,patientspent,planfixedamount,planpercentage,patientfullname,paymentstatus,triagestatus,age,gender,scheme_id,accountfullname,departmentname,username,complaint,consultationrefund,locationname,locationcode,admitid,smartbenefitno,hospitalfees,doctorfees,memberno,plandue,registrationfees,mcc,lablimit,radiologylimit,serviceslimit,pharmacylimit,slade_payload,payer_code,savannah_authid,savannah_authflag,savannahvalid_from,savannahvalid_to,visittype,eclaim_id,is_revisit,offpatient,refferal)values('$patientcode','$visitcode','$patientfirstname','$patientmiddlename','$patientlastname','$consultationtype','$consultingdoctor','$consultingdoctorcode','$accountname','$accountexpirydate','$paymenttype','$subtype','$planname','$planexpirydate','$visitlimit','$overalllimit','$availablelimit','$billtype',
...
//exit;
	...
	if ($res2 == 0)
	{
		...
		$query56="insert into master_visitentry(patientcode, visitcode, patientfirstname,patientmiddlename, patientlastname,consultationtype,consultingdoctor,consultingdoctorcode,accountname,accountexpirydate,paymenttype,subtype,planname,planexpirydate,visitlimit,overalllimit,availablelimit,billtype,visitcount,department,consultationdate,consultationtime,consultationfees,patientspent,planfixedamount,planpercentage,patientfullname,paymentstatus,triagestatus,age,gender,scheme_id,accountfullname,departmentname,username,complaint,consultationrefund,locationname,locationcode,admitid,smartbenefitno,hospitalfees,doctorfees,memberno,plandue,registrationfees,mcc,lablimit,radiologylimit,serviceslimit,pharmacylimit,slade_payload,payer_code,savannah_authid,savannah_authflag,savannahvalid_from,savannahvalid_to,visittype,eclaim_id,is_revisit,copay_forcecheck,offpatient,refferal)
		values('$patientcode','$visitcode','$patientfirstname','$patientmiddlename','$patientlastname','$consultationtype','$consultingdoctor','$consultingdoctorcode','$accountname','$accountexpirydate','$paymenttype','$subtype','$planname','$planexpirydate','$visitlimit','$overalllimit','$availablelimit','$billtype','$visitcount','$department','$consultationdate','$consultationtime','$consultationfees','$patientspent','$planfixedamount','$planpercentageamount','$patientfullname','$paymentstatus','$triagestatus','$age','$gender','$scheme_id','$accname','$departmentname','$username','$triag','torefund','$locationname1','$locationcode','$admitid','$smartbenefitno','$hospitalfees','$doctorfees','$memberno','$plandue','$registrationfees','$mcc','$lablimit','$radiologylimit','$serviceslimit','$pharmacylimit','$slade_authentication_token','$payer_code','$savannah_authid','$savannah_authflag','$savannahvalid_from','$savannahvalid_to','$type','$smartap','$revisit','$copay_forcecheck','$offpatient','$refferal_promotion')";
		$exec56=mysqli_query($GLOBALS["___mysqli_ston"], $query56) ;
		if( mysqli_errno($GLOBALS["___mysqli_ston"]) == 1062) {
           goto visitcreate;
		}
...
//exit;
	...
	{
				$sta_payment='';
        		if(($consultationfees==0)&&($planpercentageamount==0))
        		{
        			$sta_payment='completed';
        		}
        		$query1 = "insert into master_visitentry(patientcode,visitcode,patientfirstname,patientmiddlename,patientlastname,patientfullname,paymentstatus,consultingdoctor,consultingdoctorcode,department,paymenttype,subtype,billtype,accountname,accountexpirydate,planname,planexpirydate,consultationdate,consultationtime,consultationtype,consultationfees,referredby,consultationremarks,complaint,registrationdate,visitlimit,overalllimit,availablelimit,visitcount,patientspent,planpercentage,planfixedamount,age,gender,scheme_id,accountfullname,itemrefund,departmentname,username,consultationrefund,locationname,locationcode,admitid,smartbenefitno,hospitalfees,doctorfees,memberno,plandue,registrationfees,mcc,lablimit,radiologylimit,serviceslimit,pharmacylimit,slade_payload,payer_code,savannah_authid,savannah_authflag,savannahvalid_from,savannahvalid_to,visittype,eclaim_id,is_revisit,offpatient,refferal)
        		values('$patientcode','$visitcode','$patientfirstname','$patientmiddlename','$patientlastname','$patientfullname','$sta_payment','$consultingdoctor','$consultingdoctorcode','$department','$paymenttype','$subtype','$billtype','$accountname','$accountexpirydate','$planname','$planexpirydate','$consultationdate','$consultationtime','$consultationtype','$consultationfees','$referredby', '$consultationremarks','$triag','$registrationdate','$visitlimit','$overalllimit','$availablelimit','$visitcount','$patientspent','$planpercentageamount','$planfixedamount','$age','$gender','$scheme_id','$accname','torefund','$departmentname','$username','torefund','$locationname1','$locationcode','$admitid','$smartbenefitno','$hospitalfees','$doctorfees','$memberno','$plandue','$registrationfees','$mcc','$lablimit','$radiologylimit','$serviceslimit','$pharmacylimit','$slade_authentication_token','$payer_code','$savannah_authid','$savannah_authflag','$savannahvalid_from','$savannahvalid_to','$type','$smartap','$revisit','$offpatient','$refferal_promotion')";
...
<?php
$res11locationcode='';
$query31 = "select consultationprefix from master_company where companystatus = 'Active'";
...
$query21 = "select billnumber from master_billing order by auto_number desc limit 0, 1";
...
if ($patientcode != '')
{
	...
	$query4 = "select auto_number,paymenttype from master_paymenttype where auto_number = '$paymenttype'";
	$exec4 = mysqli_query($GLOBALS["___mysqli_ston"], $query4) or die ("Error in Query4".mysqli_error($GLOBALS["___mysqli_ston"]));
	$res4 = mysqli_fetch_array($exec4);
	...
	$query4 = "select auto_number,mcc from master_accountname where auto_number = '$accountname'";
	$exec4 = mysqli_query($GLOBALS["___mysqli_ston"], $query4) or die ("Error in Query4".mysqli_error($GLOBALS["___mysqli_ston"]));
	$res4 = mysqli_fetch_array($exec4);
	...
	$query_sc = "select scheme_name,scheme_expiry,scheme_active_status from master_planname where scheme_id = '$scheme_id'";
	...
	$planname = $res3['planname'];
	  ...
	  $query4 = "select auto_number,planname,recordstatus,planfixedamount,planpercentage,smartap,planapplicable,opvisitlimit,overalllimitop from master_planname where auto_number = '$planname'";
	$exec4 = mysqli_query($GLOBALS["___mysqli_ston"], $query4) or die ("Error in Query4".mysqli_error($GLOBALS["___mysqli_ston"]));
	$res4 = mysqli_fetch_array($exec4);
	$plannameanum = $res4['auto_number'];
	...
	$query5 = "select auto_number from master_visitentry where patientcode = '$patientcode' and recordstatus = ''";
	...
	$query51 = "select consultationdate from master_visitentry where patientcode = '$patientcode' and recordstatus = '' order by auto_number desc limit 0,1";
...
<?php	$query1111 = "select username,location from master_employee where username = '$username'";
	...
	$query1112 = "select locationname,locationcode,prefix,suffix from master_location where auto_number = '$locationnumber' and status <> 'deleted'";
...
/*	if (document.form1.visittype.value == "")
	...
	*/
	...
	if(document.getElementById("refferal_promotion").value == "")
	{
	alert ("Please Enter Refferal");	
	document.getElementById("refferal_promotion").focus();
	document.getElementById('submit').disabled="";
	  return false;		
  	}
...
else if(smartap==2)
{
	...
	{
	FuncPopup();
	planpercentageamount = document.getElementById('planpercentageamount').value;
	...
	member_number = document.getElementById('memberno').value;
	data = "auth_token="+memberno+"&first_name="+first_name+"&last_name="+last_name+"&member_number="+member_number;
	$.ajax({
	  type : "get",
	  url : "slade-check.php",
	  data : data,
	  ...
	  //alert("check savannah");
	   ...
	   $('#savannah_authflag').val("");
		...
		var visit_limit='0.00';
		var view_reserved_amount='0.00';
		var planfixedamount='0.00';
		
		var visit_limit=jsondata['visit_limit'];
		visit_limit = parseFloat(visit_limit).toFixed(2);
		visit_limit = visit_limit.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
		
		var view_reserved_amount=jsondata['view_reserved_amount'];
		view_reserved_amount = parseFloat(view_reserved_amount).toFixed(2);
		view_reserved_amount = view_reserved_amount.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
		
		var planfixedamount=jsondata['planfixedamount'];
		//alert(planfixedamount);
		if((planfixedamount=='')||(planfixedamount=='NaN'))
		{
		    planfixedamount="0.00";
		}
		planfixedamount = parseFloat(planfixedamount).toFixed(2);
		planfixedamount = planfixedamount.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
	    if((planfixedamount=='')||(planfixedamount=='NaN'))
		{
		    planfixedamount="0.00";
		}	
		$('#view_copay').val(planfixedamount).css({"font-weight": "bold",'font-size':'20px'});
		
		 if(planpercentageamount<=0)
	   {
	   $('#planfixedamount').val(planfixedamount);
	   }
	   
	   var planpercentageamount = document.getElementById('planpercentageamount').value;
 if(planpercentageamount<=0)
	   {
		   var planfixedamount_main = document.getElementById('planfixedamount').value;
		   if(planfixedamount_main>0)
		   {
			   if(planfixedamount_main<jsondata['planfixedamount'])
			   {
				   $('#planfixedamount').val(jsondata['planfixedamount']);  
			   }
		   }
		   else
		   {
		       var slade_copay=jsondata['planfixedamount'];
		       //alert(slade_copay);
		        if((slade_copay=='')||(slade_copay=='NaN')||(slade_copay==null))
        		{
        		    $('#planfixedamount').val('0.00');
        		}else
        		{
        		  $('#planfixedamount').val(slade_copay);   
        		}
        	
		   }
	   }
	   
		$('#view_availablelimit').val(visit_limit);
		$('#view_reserved_amount').val(view_reserved_amount);
		$("#dialog1").dialog();
	...
	alert('Member not covered for Out-Patient');
		...
		alert('Something wrong on Slade. Please Recheck all parameters.');
	     }
	     else
	     {
	     alert(jsondata['error']);    
...
function funcLocationDepartmentChange()
{
		...
		?>
		<?php
		$query10 = "select auto_number,department from master_department where  recordstatus = ''";
...
$('#customer').autocomplete({
	...
	$('#refferal_promotion').autocomplete({
		
	source:'ajaxgetpromotion.php', 
	minLength:1,
	delay: 0,
	html: true, 
		select: function(event,ui){
			var customercode = ui.item.promotion;
			var accountname = ui.item.auto_number;
			$('#refferal_promotion').val(customercode);
			$('#refferal_autonumber ').val(accountname);
			},
    });
});
...
function funfetchsavannahbackup(smartap)
{
if(smartap==1)
{
var customersearch=document.getElementById("patientcode").value;
...
function check_memberno(id,name)
{
var registrationdate=$("#registrationdate").val();
var locationcode=$("#locationcode").val();
var data = "id="+id+"&registrationdate="+registrationdate+"&locationcode="+locationcode;	
$.ajax({
  type : "get",
  url : "ajaxsearchmember.php",
  data : data,
  cache : false,
  success : function (t){
	if(t!='')
...
<table width="103%" border="0" cellspacing="0" cellpadding="2">
          ...
          <td width="860"><table width="1258" height="282" border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
            <tbody>
              <tr bgcolor="#011E6A">
                ...
                <td bgcolor="#ecf0f5"  colspan="4" style="color:red;font-size:20px"></td>
 ...
 -->
				 ...
				 <!-- <td width="12%" align="left" valign="middle"  bgcolor="#ecf0f5">&nbsp;</td>-->
				  <td width="13%" align="left" valign="middle"  bgcolor="#ecf0f5" style="display:none"><img width="150" height="150" id="patientimage" src="">
				  <br/> <input type="button" name="imgbtn" id="imgbtn" value="Show Image" onClick="return ShowImage('<?php echo $patientcode; ?>',this.value);"></td>
					
				
				  <td width="26%" rowspan="6" align="center" valign="middle"  bgcolor="#ecf0f5" colspan='2'>
				  <span >Exclusions</span>
				  <textarea id="schemeexclusions" name='schemeexclusions' class="bal"  rowspan='4' style="height: 68px; width: 292px;" readOnly></textarea>
				  <br/>
				  <div id="fetchbtn" style="display:<?php if($smartap > 0 ){ echo ""; } else { echo "none"; } ?>">
				  <input name="fetch" type="button"  value="<?php if($smartap ==1 ){ echo "Smart"; } elseif($smartap ==2 ) { echo "Slade"; } elseif($smartap ==3 ) { echo "Smart+Slade1"; } ?>" <?php if($smartap > 0 ){ echo ""; } else { echo "disabled"; } ?> class="button" id="fetch" onClick="funfetchsavannah('<?php echo $smartap; ?>');" style="background-color:#FF9900;color:#FFFFFF; font-weight:bold; height:50px; width:100px; cursor:pointer;" /></div>  <input type="hidden" id="smart_ap" name="smart_ap" value="<?php echo $smartap; ?>"/></td>
				  </tr>
				<tr>
				...
				<tr>
				  ...
				  <td align="left" valign="middle"  bgcolor="#ecf0f5"><label>
				  <input type="text" name="planfixedamount" id="planfixedamount"  value="<?php echo $planfixedamount;?>" readonly>
				  </label></td>
				...
				<tr>
				  ...
				  <td align="left" valign="middle"  bgcolor="#ecf0f5"><strong>
					...
					<input name="departmentname" id="departmentname" value="" size='28' placeholder="Search Dept Here" autocomplete="off" />
			...
			<input name="consultingdoctorsearch" id="consultingdoctorsearch" value="" type="hidden" />
				  ...
				  <td align="left" valign="middle"  bgcolor="#ecf0f5"><strong>
				    <select name="consultationtype" id="consultationtype" onChange="return funcConsultationTypeChange()">
                     <option value="" selected="selected">Select Consultation Type</option>
					  ...
					  $que = "select locationname,locationcode from login_locationdetails where username='$username' and docno='$docno' order by auto_number desc";
						...
						$s =0 ;
						$t=0;
				  $query52="select auto_number,consultationtype from master_consultationtype where locationcode = '$resultlocationanum' and recordstatus = ''";
				  ...
				  ?>
                   ...
                   <td align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext32">Refferal &nbsp;&nbsp;
			      <input name="refferal_promotion" id="refferal_promotion" value="" autocomplete="off" Placeholder="Search"/></strong>
                  <input type="hidden" name="refferal_autonumber" id="refferal_autonumber" value="" /></td>
                ...
                <td align="left" valign="middle"  bgcolor="#ecf0f5"><input name="consultationfees" id="consultationfees" value="<?php //echo $res53consultationfees; ?>" onFocus="return funcVisitEntryPatientCodeValidation1()" size="20" readonly/></td>
				 ...
				 <tr>
				   ...
				   <td align="left" valign="middle"  bgcolor="#ecf0f5"><input name="memberno" id="memberno" value="<?= $memberno ?>" size="20" readonly /></td>
				     <td align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><span class="bodytext32"><!--<label onClick="return enabtriage();">--><label>MRD NO </label></span></td>
				   ...
				   <td align="left" valign="middle"  bgcolor="#ecf0f5"><input name="registrationfees" id="registrationfees" value="<?php echo $registrationfees; ?>" readonly size="20" /></td>
					 ...
					 <td align="left" valign="middle"  bgcolor="#ecf0f5" id="copayforce_pay" style="display:none;" >
					 <div style="color:red; FONT-WEIGHT:bold; FONT-SIZE: 20px;">
					  <input type="checkbox" id="copay_forcecheck" name="copay_forcecheck"  /> Force Copay
					  </div>
				  </td>
			 ...
			 <tr id="enabtriag">
				 ...
				 <tr>
                 <td align="left" class="bodytext3">Plan Used <?php echo $smartap; ?></td>
				   ...
				   <td align="left" valign="middle"  bgcolor="#ecf0f5"> <input type="text" name="visitlimit" id="visitlimit"  value="<?php echo $visitlimit;?>" readonly></td>
				   ...
				   <td align="left" valign="middle"  bgcolor="#ecf0f5" id='mccareaval' <?php if($is_mcc==1){ } else { ?> style='display:none' <?php } ?>><input type='hidden' name="mccno" id="mccno" value="1" size="20"  /></td>
				     ...
				     <td align="left" id="offslade" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"  <?php if(($smartap==2)||($smartap==3)){ ?>style="display:block"  <?php }else{ ?>style="display:none" <?php } ?>><label>OFFSLADE</label> &nbsp; 
...
<td width="860"><table width="300" height="282" border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">

<tbody>
<h3>Slade  Details</h3>
<tr>
<td align="left" valign="middle" width="150px"><strong>Available Limit </strong></td>
<td align="left" valign="middle"><input type="text" name="view_availablelimit" id="view_availablelimit"  value=""   readonly size="9"></td>
</tr>

<tr>
<td align="left" valign="middle" width="150px"><strong>Reserved Amount </strong></td>
<td align="left" valign="middle"><input type="text" name="view_reserved_amount" id="view_reserved_amount"  value="" readonly size="9"></td>
</tr>

<tr>
<td align="left" valign="middle" width="150px"><strong>Copay Amount </strong></td>
<td align="left" valign="middle"><input type="text" name="view_copay" id="view_copay"  value="" readonly size="5"></td>
</tr>
    </tbody>
    </table>
    </td>
