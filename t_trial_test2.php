...
function getLedger($group_id,$from_date,$to_date,$sno,$sno2){
	...
	$opening_query = "select ledger_id,transaction_date,transaction_type,sum(transaction_amount*exchange_rate) as transaction_amount from tb where record_status='1' and ledger_id='".$res['code']."'  and ledger_id!='' and transaction_date < '".$from_date."' group by transaction_type ";
		$exec_opening = mysqli_query($GLOBALS["___mysqli_ston"], $opening_query) or die ("Error in opening_query".mysqli_error($GLOBALS["___mysqli_ston"]));
		while($res_opening = mysqli_fetch_array($exec_opening)){
		...
		$closing_dr_cr = $opening_dr_cr + $transaction_dr - $transaction_cr;
		 ...
		 $data .="<tr data-node-id=".$sno.'.'.$sno2.'.'.$sno3." data-node-pid=".$sno.'.'.$sno2." > <td></td><td  align='right'>".showAmount($opening_dr_cr,'1')."</td><td>".$res['code']."</td> <td>".ucwords(strtolower($res['name']))."&nbsp;<a href='ledgerview.php?ledgerid=$ledgercode&&ADate1=$from_date&&ADate2=$to_date' target='_blank' style='font-size:14px;'>&#9432;</a>"."</td> <td  align='right'>".showAmount($transaction_dr,'0')."</td>  <td  align='right'>".showAmount($transaction_cr,'0')."</td> <td  align='right'>".showAmount($closing_dr_cr,'1')."</td> </tr>";
	}
...
function getIEledgers($group_id){
...
function getGroupBalance($group_id,$from_date,$to_date,$sno,$sno2){
	...
	$ie_ledgers = getIEledgers($group_id);
	//$GLOBALS['ieledgers'] = '';
	...
	$section = $res1['section'];			
		...
		if($section == 'I' || $section == 'E' ){
			...
			$financeyearstart = $startyear.'-01-01';
			 $opening_query = "select transaction_type,sum(transaction_amount*exchange_rate) as transaction_amount from tb where record_status='1' and ledger_id in ('".implode("','",$all_ledgers)."')  and ledger_id!='' and transaction_date BETWEEN '$financeyearstart' AND '$from_date'  group by transaction_type ";
			$exec_opening = mysqli_query($GLOBALS["___mysqli_ston"], $opening_query) or die ("Error in opening_query".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($res_opening = mysqli_fetch_array($exec_opening))
			{
				if($res_opening['transaction_type']=="D"){
