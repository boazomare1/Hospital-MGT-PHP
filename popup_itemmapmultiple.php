...
$st = 0;
	if(count($suppliercode)>0){
		foreach($suppliercode as $k=>$v){
			 ...
			 $chkSql = "select * from master_itemmapsupplier where suppliercode='$suppliercodes' and itemcode='$itemcode' and supplier_autoid='$supautocodes' and status!='deleted'";
			 ...
			 if($numSup==0){ 
				mysqli_query($GLOBALS["___mysqli_ston"], "insert into master_itemmapsupplier(itemcode,suppliercode,rate,updateddate,username,supplier_autoid)values('$itemcode','$suppliercodes','$rates','$updatedatetime','$username','$supautocodes')") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
				 $st = 1;
			 }
			 
		}
        header("location:popup_itemmapmultiple.php?item=$itemcode&st=1");
		exit;
	}
}
...
$st = 0;
	...
	if(count($suppliercode)>0){
		foreach($suppliercode as $k=>$v){
			 ...
			 $chkSql = "select * from master_itemmapsupplier where auto_number='$suppliercodes'";
			 $exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $chkSql) or die ("Error in chkSql".mysqli_error($GLOBALS["___mysqli_ston"]));
			 $numSup = mysqli_num_rows($exec1);
			 if($numSup>0){ 
				 if($fav==$suppliercodes)
				    mysqli_query($GLOBALS["___mysqli_ston"], "update master_itemmapsupplier set rate='$rates',fav_supplier='1' where auto_number='$suppliercodes'") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
				 else
					 mysqli_query($GLOBALS["___mysqli_ston"], "update master_itemmapsupplier set rate='$rates',fav_supplier='0' where auto_number='$suppliercodes'") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
				 $st = 2;
			 }
			 
		}
        header("location:popup_itemmapmultiple.php?item=$itemcode&st=2");
		exit;
	}
}
...
$st = 0;
			 ...
			 $chkSql = "select * from master_itemmapsupplier where auto_number='$id'";
			 $exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $chkSql) or die ("Error in chkSql".mysqli_error($GLOBALS["___mysqli_ston"]));
			 $numSup = mysqli_num_rows($exec1);
			 if($numSup>0){ 
				mysqli_query($GLOBALS["___mysqli_ston"], "delete from master_itemmapsupplier where auto_number='$id'") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
				header("location:popup_itemmapmultiple.php?item=$itemcode&st=3");
		        exit;
			 }
...
function del(id,item){
	...
	if (r == true) {
	  window.location='popup_itemmapmultiple.php?cnfform2=cnfform2&item='+item+'&id='+id;
	}else
      return false;
}
</script>
...
<body >
<form name='form1' id='form1' action='popup_itemmapmultiple.php?item=<?php echo $itemcode;?>' method='post' onsubmit="return valid();">
...
<table width="100%" border="0" align="center" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
	...
	<?php
	if($itemcode!=''){
	$query1 = "select itemcode,itemname from master_medicine where itemcode ='".$itemcode."' and status <> 'deleted' ";
	...
	?>
	  <?php if ($errmsg != '') { ?>
		 <tr>
			<td  align="center" valign="middle"  bgcolor="<?php if ($errmsg == '') { echo '#FFFFFF'; } else { echo '#5fe5de'; } ?>" class="bodytext3"><?php echo $errmsg;?></td>
			
		  </tr>
		<?php } ?>
      <tr bgcolor="#011E6A">
        <td  bgcolor="#CCCCCC" class="bodytext3"><strong>Add Suppliers for "<?php echo $res1['itemname'];?>(<?php echo $res1['itemcode'];?>)"</strong></td>
      </tr>
      <tr>
...
<?php
$query2 = "select a.auto_number as id,a.fav_supplier as fav_supplier,a.supplier_autoid as supplier_autoid,a.suppliercode as suppliercode,a.rate as rate,b.accountname as name from master_itemmapsupplier as a join master_accountname as b on a.suppliercode=b.id and a.supplier_autoid = b.auto_number where  a.itemcode='$itemcode' and a.status <> 'deleted' ";
$exec2 = mysqli_query($GLOBALS["___mysqli_ston"], $query2) or die ("Error in query2".mysqli_error($GLOBALS["___mysqli_ston"]));
$numLab1 = mysqli_num_rows($exec2);
if($numLab1>0){ 
?>
<form name='form2' id='form2' action='popup_itemmapmultiple.php?item=<?php echo $itemcode;?>' method='post' onsubmit="return valid2();">
<table width="100%" border="0" align="center" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
    <tr bgcolor="#011E6A">
		<td bgcolor="#CCCCCC" class="bodytext3" colspan='6'>
	...
	<tr bgcolor="#011E6A">
		...
		<td width="7%" bgcolor="#CCCCCC" class="bodytext3"><strong>Supplier Code</strong></td>
