...
$query01="";
				...
				if($retqty !="0" && $retqty !="")
				{
  					 ...
  					 $query29=mysqli_query($GLOBALS["___mysqli_ston"], "update consultation_lab set labrefund='$refund',remarks='$remarks',pkg_process='$pkg_process' where labitemname='$servicename' and patientvisitcode='$visitcode' and patientcode='$patientcode' and labitemcode = '$q_itemcode' and auto_number='$autonumber'")or die("Error in Query29".mysqli_error($GLOBALS["___mysqli_ston"]));
 				 header("location:processlablist.php");
 				exit;
				}
...
if($num01!=$serialnum)
{
	...
	header('location:processlablist.php?st=matchfailed');
	exit;
}
			 ...
			 $querycumstock2 = "select cum_quantity from transaction_stock where cum_stockstatus='1' and itemcode='$itemcode' and locationcode='$locationcodeget'";
				...
				if($pharmacyquantityenter>0)
				{
					...
					if($bat_quantity>='0')
					{
					 ...
					 $queryupdate=mysqli_query($GLOBALS["___mysqli_ston"], "update consultation_lab set pkg_process='completed',remarks='$remarks' where labitemname='$servicename' and patientvisitcode='$visitcode' and patientcode='$patientcode' and labitemcode = '$q_itemcode' and auto_number='$refno'")or die("Error in queryupdate".mysqli_error($GLOBALS["___mysqli_ston"]));
					}
					else
					{
...
<form name="frmsales" id="frmsales" method="post" action="processlab.php" onKeyDown="return disableEnterKey(event)" onSubmit="return validcheck()">
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
			...
			$totalamount=0;
			 ...
			 $query61 = "select labitemname,billtype,auto_number,refno from consultation_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and pkg_process='pending' and auto_number='$refnumber'  and labitemcode='$q_itemcode' and (billtype='PAY NOW' or (billtype='PAY LATER' ))";
...
$servicename =$res61["labitemname"];
$billtype = $res61["billtype"];
$refno = $res61['auto_number'];
$newrefno = $res61['refno'];
 $query68="select itemcode from master_lab where itemname='$servicename'";
