...
$inv_rslt=array();
$inv_line=array();
$query7 = "select * from billing_ip where visitcode = '$visitcode' and billno='$billno' and slade_status!='completed' and slade_claim_id='$claim'";
...
if($num232>0){
    ...
    if($nhifclaim>0){
      $copay_arr[0]["copay_type"]="NHIF";
	  $copay_arr[0]["copay_amount"]=$nhifclaim;
	...
	if($consultationamount >0 ){
	   ...
	   $inv_line[$j]["item_name"]='ADMISSION FEE';
	...
	if($packageanum1 > 0)
    {
		   ...
		   $inv_line[$j]["item_code"]=$packagecode;
		   $inv_line[$j]["item_name"]=$packagename;
		   $inv_line[$j]["charge_date"]=date("c",strtotime($res7['billdate']));
		   $inv_line[$j]["unit_price"]=$packageamount;
		   $inv_line[$j]["quantity"]=1;
		   $inv_line[$j]["line_number"]=$sno;	
		   $sno++;
		   $j++;
	...
	if($num91>0){
		while($res91 = mysqli_fetch_array($exec91))
		{
		   ...
		   $inv_line[$j]["item_code"]='BED-'.$bed;
		   $inv_line[$j]["item_name"]=$charge."(".$bedname.")";
		   $inv_line[$j]["charge_date"]=date("c",strtotime($date));
		   $inv_line[$j]["unit_price"]=number_format($rate,2,'.','');
		   $inv_line[$j]["quantity"]=$quantity;
		   $inv_line[$j]["line_number"]=$sno;
		   $sno++;
		   $j++;
		}
   ...
   /////////////////////// start services  /////////////////////////////////////
    ...
    $query15 = "select * from billing_ipservices where patientvisitcode = '$visitcode' and wellnessitem <> '1'";	
   ...
   $query18 = "select * from master_transactionadvancedeposit where visitcode = '$visitcode' and recordstatus='adjusted'";	
    ...
    if($deposits>0){
	  ...
	  $copay_arr[0]["copay_amount"]=$deposits;
	  $invoice_arry["copays"]=$copay_arr;
	}
   ...
   /////////////////////// end deposit refund /////////////////////////////////////
	...
	$claim_payload=$inv_send=json_encode($invoice_arry);
