...
{  
	...
	$i = 0;
	$result = array();
	$querydr1 = "SELECT SUM(`transactionamount`) as payablesdr FROM `master_transactionpharmacy` WHERE `suppliercode` = '$id' AND `transactionmodule` = 'PAYMENT' AND `docno` LIKE 'SP-%' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
	...
	$j = 0;
	$crresult = array();
	$querycr1 = "SELECT SUM(`transactionamount`) as payables FROM `master_transactionpharmacy` WHERE `suppliercode` = '$id' AND `billnumber` NOT LIKE 'SUPO%' AND `transactiontype` = 'PURCHASE' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2'
