...
foreach($_POST['radiology'] as $key => $value)
	...
	$u1=1;
		...
		$clinical = str_replace(chr(194)," ",$clinical);
...
$exclude = $res612['exclude'];
		...
		if($status2=='refund')
		{	
       $query26="update resultentry_radiology set refund='$status1' and patientvisitcode='$visitcode' and itemcode='$itemcode' ";
...
$query24 = "select * from master_employee where username = '$orderedby'";
 ...
 $query76 = "update master_consultation set results='$status1',username='$orderedbyname',closevisit='' where patientvisitcode='$visitcode'";
 ...
 $new_datetime=date('Y-m-d H:i:s');  
   //echo "update consultation_radiology set resultentry='$status',radiologyrefund='$status1',docnumber='$docnumber',urgent='$u1',resulttime='".date('Y-m-d h:i:s')."' , comments='$comments',urgentstatus='0' where patientvisitcode='$visitcode' and radiologyitemname='$radiologyname' and refno='$refno";
  $query123="update consultation_radiology set refundrequestedby = '$username', clinical_diagnosis='$clinical',prepstatus='$status1', resultentry='$status1',radiologyrefund='$status1',docnumber='$docnumber',urgent='$u1',resulttime='".date('Y-m-d h:i:s')."' , comments='$comments',urgentstatus='0',preparation_datetime='$new_datetime',exclude='$exclude' where patientvisitcode='$visitcode' and radiologyitemcode='$itemcode' and refno='$refno' "; 
...
$new_datetime=date('Y-m-d H:i:s');
   ...
   //echo "update consultation_radiology set resultentry='$status',radiologyrefund='$status1',docnumber='$docnumber',urgent='$u1',resulttime='".date('Y-m-d h:i:s')."' , comments='$comments',urgentstatus='0' where patientvisitcode='$visitcode' and radiologyitemname='$radiologyname' and refno='$refno";
  $query123="update consultation_radiology set clinical_diagnosis='$clinical',prepstatus='$status', radiologyrefund='$status1',docnumber='$docnumber',urgent='$u1',comments='$comments',urgentstatus='0',preparation_datetime='$new_datetime',exclude='$exclude' where patientvisitcode='$visitcode' and radiologyitemcode='$itemcode' and refno='$refno' "; 
mysqli_query($GLOBALS["___mysqli_ston"], $query123) or die(mysqli_error($GLOBALS["___mysqli_ston"]));


//$newquery228=mysqli_query($GLOBALS["___mysqli_ston"], "update tat_detail set rad_in='$indate' where patientcode='$patientcode' and patientvisitcode='$visitcode'") or die(mysqli_error($GLOBALS["___mysqli_ston"]).__LINE__);
...
$query0 = "select * from master_consultationlist where visitcode = '$visitcode'";
$exec0 = mysqli_query($GLOBALS["___mysqli_ston"], $query0) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
$res0 = mysqli_fetch_array($exec0);
$templatedata_cons = $res0['templatedata'];
$templatedata_cons=str_ireplace('<p>','',$templatedata_cons);
$templatedata_cons=str_ireplace('</p>','',$templatedata_cons);  
$templatedata_cons = preg_replace("/&#?[a-z0-9]+;/i"," ",$templatedata_cons); 
$query24 = "select * from master_employee where username = '$orderedby'";
...
<form name="frmsales" id="frmsales" method="post" action="radiologyprepentry.php" onKeyDown="return disableEnterKey(event)" enctype="multipart/form-data" onSubmit="return validcheck()">
			...
			<input name="visitcode" type="hidden" id="visitcode" value="<?php echo $visitcode; ?>" style="border: 1px solid #001E6A" size="18" rsize="20" readonly/><?php echo $visitcode; ?>                  </td>
			  ...
			  <td align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><span class="style4"></span><strong>Age &amp; Gender </strong></td>
				...
				<input type="hidden" name="billtype" id="billtype" value="<?php echo $billtype; ?>">		</td>
                 <td width="9%" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><strong>Scheme Name</strong></td>
                <td colspan="1" align="left" valign="top" class="bodytext3"><?php echo $scheme_name;?></td>		 
				 ...
				 <td width="8%" align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><strong>primary diagnosis</strong></td>
      ...
      <tr>
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
			...
			$sno = '';
			$totalamount=0;			
$query61 = "select * from consultation_radiology where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and prepstatus='pending' and resultentry='pending' and paymentstatus='completed'  and radiologyitemcode='$q_itemcode' and refno='$q_refno' and (billtype='PAY NOW' or (billtype='PAY LATER' ))";
...
if($billtype=='PAY LATER'  && ($approvalstatus!='1' && $approvalstatus!='2')){
	continue;
}
if($radtemplate =='')
{
...
$queryche = "select * from billing_paylater where  patientcode ='".$patientcode."' and visitcode ='".$visitcode."' ";
...
?>
  <tr <?php echo $colorcode;?>>
	...
	<input type="hidden" name="sno[<?php echo $sno; ?>]" value="<?php echo $sno; ?>">
		<td class="bodytext31" valign="center"  align="left"><div align="center">
			<input type="checkbox" name="ref[<?php echo $sno; ?>]"  class="ref[]" id="ref<?php echo $sno; ?>" value="<?php echo $sno; ?>" onClick="return checkboxcheck1('<?php echo $sno; ?>')" <?php if($billpaylatercount>0) echo "disabled"; else echo ''; ?>/>
			  ...
			  <td class="bodytext31" valign="center"  align="left"><div align="center" class="checkcomment">
			  <textarea rows="8" cols="53" name="clinical[<?php echo $sno; ?>]" id="clinical<?php echo $sno; ?>" style="display:none"> <?php echo $templatedata_cons; ?> </textarea></div></td>
          ...
          <tbody>
            <tr>
              <td width="54%" align="right" valign="top" >
                     ...
                     <input type="hidden" name="indate" value="<?php echo $startdate; ?>" />
