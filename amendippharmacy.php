...
$sele=mysqli_query($GLOBALS["___mysqli_ston"], "select * from master_frequency where auto_number='$afrequency'") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
...
$data=mysqli_query($GLOBALS["___mysqli_ston"], "select auto_number,patientcode,visitcode,patientname,docno,itemcode,itemname,prescribed_quantity,rateperunit,totalrate from ipmedicine_prescription where auto_number='$medicinename' and visitcode='$viscode'");
$data1=mysqli_fetch_array($data);
$patientname=$data1['patientname'];
$patientcode=$data1['patientcode'];
$visitcode=$data1['visitcode'];
$refno=$data1['docno'];
$medicinecode=$data1['itemcode'];
$medicinename1=$data1['itemname'];
$rate=$data1['rateperunit'];

				$pharmquantity=$data1['prescribed_quantity'];
	
			$pharmamount=$data1['totalrate'];
...
if($medicinecode<>'')
{
 $query="insert into amendment_details (patientcode,visitcode,patientname,refno,itemcode,itemname,rate,qty,amount,amenddate,amendtime,amendfrom,amendby,ipaddress,type,remarks) values ('$patientcode','$visitcode','$patientname','$refno','$medicinecode','$medicinename1','$rate','$pharmquantity','$pharmamount','$date','$time','Pharmacy','$username','$ipaddress','IP','$remarks')";
mysqli_query($GLOBALS["___mysqli_ston"], $query)or die("Error in query".mysqli_error($GLOBALS["___mysqli_ston"]));
}

mysqli_query($GLOBALS["___mysqli_ston"], "delete from ipmedicine_prescription where auto_number='$medicinename' and visitcode='$viscode'");

header('location:amendippharmacy.php?patientcode='.$patientcode.'&visitcode='.$visitcode);
...
<?php
$querylab7=mysqli_query($GLOBALS["___mysqli_ston"], "select * from master_ipvisitentry where patientcode='$patientcode' and visitcode='$visitcode'");
...
window.location='amendippharmacy.php?delete='+id+'&patientcode='+patient+'&&visitcode='+visit+'&remarks-'+id+'='+msg;
}
			  ...
			  <?php
				...
				?>
				 ...
				 <td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="aamount[]" id="aamount<?php echo $sno; ?>" value="<?php echo $pharmamount; ?>" size="8" class="bal" readonly></div></td>
				  ...
				  <td class="bodytext31" valign="center"  align="left"><div align="center"><input type = "text" name ="remarks-<?php echo $medanum; ?>" id ="remarks-<?php echo $medanum; ?>"></div></td>

				 <td class="bodytext31" valign="center"  align="left"><div align="center"><a onClick="return deletevalid('<?php echo $medanum; ?>','<?php echo $patientcode; ?>','<?php echo $visitcode; ?>')" href='javascript:return false;' >Delete</a></div></td>
