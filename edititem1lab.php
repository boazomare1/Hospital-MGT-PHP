...
$exec79 = mysqli_query($GLOBALS["___mysqli_ston"], $query79) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
	...
	if ($length1<25 && $length2<255)
	{
		if($reference != '')
		{	
			$query25 = "select * from master_labreference where itemcode = '$itemcode' and auto_number = '$refanum'";// or itemname = '$itemname'";
			...
			if($resanum == '')
			{	
				$query251 = "select * from master_labreference where itemcode = '$itemcode' and referencename = '$reference' and gender = '$gender' and criticallow = '$criticallow' and criticalhigh = '$criticalhigh' and agefrom='$agelimitfrom' and ageto = '$agelimitto'";// or itemname = '$itemname'";
				...
				if($res251 == 0)
				{
					$query1 = "insert into master_labreference (itemcode, itemname, categoryname, itemname_abbreviation, rateperunit, expiryperiod, taxanum, taxname, ipaddress, updatetime, description, purchaseprice, referencevalue,ipmarkup,sampletype,location,locationname,referencename,referenceunit,referencerange,rate2,rate3,criticallow,criticalhigh,displayname,gender,agefrom,ageto,subheader,reforder,referencecomments,generic_search) 
					values ('$itemcode', '$itemname', '$categoryname', '$itemname_abbreviation', '', '', '', '', '$ipaddress', '$updatedatetime','', '', '$referencevalue','$ipmarkup','$sampletype','','','$reference','$units','$range','$rate2','$rate3','$criticallow','$criticalhigh','$displayname','$gender','$agelimitfrom','$agelimitto','$subheader','$reforder','$refcomments','$genericsearch')";
					$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));	
				}	
			}
			else
			{
				$query251 = "update master_labreference set itemcode = '$itemcode', itemname = '$itemname', status = '' ,categoryname = '$categoryname', itemname_abbreviation = '$itemname_abbreviation', referencename = '$reference' , updatetime = '$updatedatetime', referencevalue = '$referencevalue', gender = '$gender' ,criticallow = '$criticallow', referenceunit = '$units', referencerange = '$range' ,criticalhigh = '$criticalhigh' , agefrom='$agelimitfrom' , ageto = '$agelimitto',reforder = '$reforder', subheader = '$subheader', referencecomments = '$refcomments',generic_search='$genericsearch' where itemcode = '$itemcode' and auto_number = '$resanum'";// or itemname = '$itemname'";
				$exec251 = mysqli_query($GLOBALS["___mysqli_ston"], $query251) or die ("Error in Query251".mysqli_error($GLOBALS["___mysqli_ston"]));
			}	
		}
		...
		if($res24 == 0)
		{
			$query1 = "insert into $labtemplate (itemcode, itemname, categoryname, itemname_abbreviation, rateperunit, expiryperiod, taxanum, taxname, ipaddress, updatetime, description, purchaseprice, referencevalue,ipmarkup,sampletype,location,locationname,rate2,rate3,displayname,externallab,exclude,pkg,radiology,ledgercode,username) 
			values ('$itemcode', '$itemname', '$categoryname', '$itemname_abbreviation', '$rateperunit', '$expiryperiod', '$taxanum', '$res4taxname', '$ipaddress', '$updatedatetime','$description', '$purchaseprice', '$referencevalue','$ipmarkup','$sampletype','$locationcode','$locationname','$rate2','$rate3','$displayname','$externallab','$exclude','$pkg','$radiology','$ledgercode','$username')";
			$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
		}
			...
			//$itemcode = '';	
		header("location:labitem1temp.php"); //commented
...
if (isset($_REQUEST["itemcode"])) { $itemcode = $_REQUEST["itemcode"]; } else { $itemcode = ""; }
	...
	$exec67 = mysqli_query($GLOBALS["___mysqli_ston"], $query67) or die("67".mysqli_error($GLOBALS["___mysqli_ston"]));
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin:22px 41px">
        <tr>
          <td width="860"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="tablebackgroundcolor1">
                ...
                <table width="900" border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
                  ...
                  <tr id="pressid">
                    <td colspan="11" align="left" valign="middle"   class="bodytext13"><table id="presid" width="300" border="0" cellspacing="1" cellpadding="1">
                      <tbody id="insertrow">
                        <tr>
                       <td width="150" class="bodytext3"><strong>Sub Header</strong></td>
					   <td width="150" class="bodytext3"><strong>Reference</strong></td>
                       <td width="48" class="bodytext3"><strong>Unit</strong></td>
                       <td width="48" class="bodytext3"><strong>Critical Low</strong></td>
					   <td width="48" class="bodytext3"><strong>Critical High</strong></td>
					   <td width="80" class="bodytext3"><strong>Gender</strong></td>
					   <td width="106" class="bodytext3"><input type="text" size="10" value="Age Range" class="bodytext3" readonly style="border:none;background-color:transparent;font-weight:bold;" /></td>
					   <td width="106" class="bodytext3"><strong>Comments</strong></td>
					   <td width="106" class="bodytext3"><input type="text" size="10" value="Ref Order" class="bodytext3" readonly style="border:none;background-color:transparent;font-weight:bold;" /></td>
					   <td width="30" class="bodytext3"><strong>Generic Search</strong></td>
					   <td width="106" class="bodytext3">&nbsp;</td>
                        </tr>
                        <?php
				$itemcount = "";
				...
				?>
                      ...
                      <td colspan="11"  class="bodytext13"><table border="0" cellspacing="1" cellpadding="1">
                        <tbody>
                          <tr >
                            ...
                            <td width="120"><input name="subheader[]" type="text" id="subheader" size="20" autocomplete="off"></td>
						   <td width="120"><input name="reference[]" type="text" id="reference" size="20" autocomplete="off"></td>
						   <td width="48"><input name="units[]" type="text" id="units" size="8" autocomplete="off"></td>
							
						   <td width="48"><input name="rangelow[]" type="text" id="rangelow" size="8" autocomplete="off"></td>
						   <td width="48"><input name="rangehigh[]" type="text" id="rangehigh" size="8" autocomplete="off"></td>
						   <td width="80"><select name="gender[]" id="gender">
						   <option value="">All</option>
						   <option value="Male">Male</option>
						   <option value="Female">Female</option></select></td>
						   <td width="128"><input name="agelimitfrom[]" type="text" id="agelimitfrom" size="1" autocomplete="off" placeholder="From">
						   <input name="agelimitto[]" type="text" id="agelimitto" size="1" autocomplete="off" placeholder="To"></td>
						   <td width="218"><textarea id="refcomments" name="refcomments[]" rows="0" cols="30">
					...
					<script>
						...
						''
						   ...
						   <td width="9"><input name="reforder[]" type="text" id="reforder" size="1" autocomplete="off"></td>
			...
			<input type = "hidden" name="labtemplate" value="<?php echo $labtemplate; ?>">
