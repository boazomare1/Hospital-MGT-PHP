...
else
{
	...
	if(!empty($_FILES['upload_file']))
	{
		...
		try {
			...
			if($value!="S.No" && $value!="Item Code" && $value!="Item Name" && $value!="Rate")
			{
			$qrysubtype = "select * from master_subtype where recordstatus <> 'deleted' and subtype = '$value'";
			$exec = mysqli_query($GLOBALS["___mysqli_ston"], $qrysubtype) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
			while($res =mysqli_fetch_array($exec))
			{
			array_push($subtypes,$key);
    		...
    		$rowData = $sheet->rangeToArray('A' . $row . ':' . $highestColumn . $row,
				...
				foreach($subtypes as $key => $colnm)
				{
					echo $ratevalue =$rowData[$colnm];
					$subtypeano =$subtypeanos[$key];
					$querystr = $querystr.",`subtype_".$subtypeano."` = '".$ratevalue."'";
				}
				if(isset($ratenm))
				{
						
				$rate=floatval($rowData[$ratenm]);
				$querystr = $querystr.",rateperunit = '".$rate."',`$ratecolumn`='".$rate."',`subtype_1`='".$rate."'";
				}
			if($itemcode!="" )
				{
				$medicinequery2="update master_medicine SET ipaddress = '$ipaddress' ".$querystr." where itemcode= '$itemcode'";
			...
			header("location:pharmacyitemrates.php");
			//print_r($objPHPExcel);
...
if(document.cbform12.location.value=="")
	...
	if(document.cbform12.subtype.value=="")
	{
		alert("Please select subtype");
		document.cbform12.subtype.focus();
		return false;
	}
	var location = document.getElementById("location").value;
	var subtype = document.getElementById("subtype").value;
	window.open("medicinerates_csv1.php?frmflag34=frmflag34&&location="+location+"&&subtype="+subtype, "_blank");
	return true;
    ...
    <td width="97%" valign="top"><table width="116%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="860">
              ...
              <form name="cbform12" method="post" action="pharmacyitemrates.php" onSubmit="return medicinecheck()" >
		<table width="570" border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
          <tbody>
            <tr bgcolor="#011E6A">
              <td colspan="1" bgcolor="#ecf0f5" class="bodytext3"><strong> Pharamcy Rates </strong></td>
              ...
              <td   class="bodytext3" bgcolor="FFFFFF"   colspan="3" ><select name="location" id="location" style="border: 1px solid #001E6A;">
             <option value="" >Select Location</option>
              ...
              <td   class="bodytext3" bgcolor="FFFFFF"   colspan="3" ><select name="subtype" id="subtype" style="border: 1px solid #001E6A;">
	 ...
	 <!-- <tr id="pressid">
      ...
      <form name="cbform1" method="post" action="pharmacyitemrates.php" enctype="multipart/form-data" onSubmit="return validcheck()">	
        <?php
