...

{
	...
	$genericname = $res2['genericname'];
	 ...
	 $query571 = "select itemcode,sum(batch_quantity) as currentstock from transaction_stock where locationcode='".$locationcode."' and itemcode='$itemcode' and batch_stockstatus='1' and storecode IN ($storebuild) and (batchnumber in(select batchnumber from purchase_details where expirydate>now() and itemcode ='$itemcode') or batchnumber in(select batchnumber from materialreceiptnote_details where expirydate>now() and itemcode ='$itemcode')) group by itemcode";
