...

{
	...
	$query_sdbtdate = "SELECT * from supplier_debit_transactions where supplier_id = '".$_GET['suppliercode']."' and approve_id='$document'";
		  $exec_sdbtdate = mysqli_query($GLOBALS["___mysqli_ston"], $query_sdbtdate) or die ("Error in Query_sdbtdate".mysqli_error($GLOBALS["___mysqli_ston"]));
		  $res_sdbtdate = mysqli_fetch_array($exec_sdbtdate);
		  	 $created_at1= $res_sdbtdate['created_at'];
		  	 $timestamp1 = strtotime($created_at1);

			$sdbt_date = date('Y-m-d', $timestamp1); // d.m.YYYY
...

				{
					...
					// $ald=$_GET['ald'];
					 ...
					 $query12 = "INSERT into master_transactionpharmacy (transactiondate,docno,transactiontype,  suppliercode, suppliername,  transactionamount, billnumber, recordstatus, approvalstatus,ipaddress, updatedate)
						values ( '$sdbt_date', '$document', 'PAYMENT', '$suppliercode', '$suppliername',  '$adjamount', '$billnum', 'allocated','APPROVED','$ipaddress', '$updatedatetime')";
...
<form name="cbform2" method="post" action="" onSubmit="return check()">
      <tr>
          ...
          <tbody>
		   		...
		   		<tr bgcolor="#011E6A">
                    ...
                    <td colspan="4" bgcolor="#ecf0f5" class="bodytext3"><strong>Supplier Activity - Details </strong></td> <td bgcolor="#ecf0f5" class="bodytext3" colspan="2"> </td>
	  ...
	  <tr>
        ...
        <td>
			...
			$query2 = "select * from master_purchase where suppliercode = '$suppliercode' and recordstatus <> 'deleted' and billnumber != '$document' group by billnumber order by billdate";
...

			{
				...
				$query342 = "select * from master_transactionpharmacy where transactiontype = 'PURCHASE' and transactionmode <> 'CREDIT' and billnumber = '$billnumber' and companyanum='$companyanum' and  approved_payment = '1' ";
		...
		//	$docno1 = $res3['docno']; 
			...
			$query12 = "select * from master_transactionpharmacy where billnumber = '$billnumber' order by auto_number desc limit 0, 1 ";
			...
			$allocatedamount="0";
			$query3 = "SELECT * from master_transactionpharmacy where  billnumber = '$billnumber' and recordstatus = 'allocated'";
				$exec3 = mysqli_query($GLOBALS["___mysqli_ston"], $query3) or die ("Error in Query3".mysqli_error($GLOBALS["___mysqli_ston"]));
				$num=mysqli_num_rows($exec3);
				while ($res3 = mysqli_fetch_array($exec3)){
					$allocatedamount += $res3['transactionamount'];
					$balanceamount=$balanceamount-$res3['transactionamount'];
				}
			...
			?>
             ...
             <tr <?php echo $colorcode; ?>>
			  ...
			  <input type="checkbox" name="ack<?php echo $sno; ?>" id="acknow<?php echo $sno; ?>" value="<?php echo $billnumber; ?>" onClick="updatebox('<?php echo $sno; ?>','<?php echo $balanceamount; ?>','<?php echo $number; ?>',<?=$pending_amountfromdb;?>)"></td>
              ...
              </div></td><input type="hidden" name="billamount" id="bill<?php echo $sno; ?>" value="<?php if ($billtotalamount != '0.00') echo $billtotalamount; ?>">

              <td class="bodytext311" valign="center" bordercolor="#f3f3f3" align="left"><div align="right">

                <?php if ($allocatedamount != '0.00') echo number_format($allocatedamount, 2,'.',','); //echo number_format($billtotalamount, 2); ?>

              </div></td><input type="hidden" name="billfxamount" id="billfx<?php echo $sno; ?>" value="<?php if ($billtotalfxamount != '0.00') echo $billtotalfxamount; ?>">
			  ...
			  <td class="bodytext311" valign="center" bordercolor="#f3f3f3" align="left"><div align="right">
                ...
                <input type="hidden" id="balance_amount<?php echo $sno; ?>" value="<?=$balanceamount; ?>">

              </div></td>

			      <td class="bodytext311" valign="center" bordercolor="#f3f3f3" align="right"><input class="bali" type="text" name="adjamount<?php echo $sno; ?>" id="adjamount<?php echo $sno; ?>" size="7"  onClick="checkboxcheck('<?php echo $sno; ?>')" onKeyUp="balancecalc('<?php echo $sno; ?>','<?php echo $balanceamount; ?>','<?php echo $number; ?>','<?=$pending_amountfromdb;?>')"></td>

            <td class="bodytext311" valign="center" bordercolor="#f3f3f3"><input type="hidden" class="bal" name="balamount<?php echo $sno; ?>" id="balamount<?php echo $sno; ?>" value="" size="7" readonly></td>

            </tr>

            <?php } // if $balance !=0 i.e., the pending balance should not 0;
			...
			<tr>
			  ...
			  <input type="submit" name="submit1" id="submit112" value="Submit" onclick="return confirm('Are you sure you want to Save?');"></td>
		...
		<tr>
          ...
          <tbody>
		   		...
		   		<tr bgcolor="#011E6A">
                    ...
                    <td colspan="4" bgcolor="#ecf0f5" class="bodytext3"><strong>Allocated Supplier - Details </strong></td> <td bgcolor="#ecf0f5" class="bodytext3" colspan="2"> </td>
              ...
              <td width="15%" align="right" valign="center" bordercolor="#f3f3f3" bgcolor="#ffffff" class="bodytext311"><b>Select</b></td>
