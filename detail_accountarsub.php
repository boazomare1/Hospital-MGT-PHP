			...
			<?php } 
				...
				{  
					...
					$querydr1 = "SELECT (`fxamount`) as paylater, patientname as name, patientcode as code, billnumber as billno, transactiondate as date FROM `master_transactionpaylater` WHERE `accountnameid` = '$id' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `transactiontype` = 'finalize' AND `billnumber` LIKE 'CB%'
								 ...
								 UNION ALL SELECT (`transactionamount`) as paylater, patientname as name, patientcode as code, billnumber as billno, transactiondate as date FROM `master_transactionpaylater` WHERE `accountnameid` = '$id' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `transactiontype` = 'finalize' AND `billnumber` LIKE 'DBI-%'
								 UNION ALL SELECT (`fxamount`) as paylater, patientname as name, patientcode as code, billnumber as billno, transactiondate as date FROM `master_transactionpaylater` WHERE `accountnameid` = '$id' AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `transactiontype` = 'finalize' AND `billnumber` LIKE '%IPDr%'
					   			 UNION ALL SELECT (a.`totalamountuhx`) as paylater, a.patientname as name, a.patientcode as code, a.billnumber as billno, a.transactiondate as date FROM `master_transactionip` AS a JOIN master_ipvisitentry AS b ON (a.visitcode = b.visitcode) WHERE a.`accountnameid` = '$id' AND b.billtype = 'PAY LATER' AND a.`transactiondate` BETWEEN '$ADate1' AND '$ADate2'
								 ...
								 UNION ALL SELECT (-1*`openbalanceamount`) as paylater, accountname as name, accountcode as code, docno as billno, entrydate as date FROM `openingbalancesupplier` WHERE `accountcode` = '$id' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2'";
