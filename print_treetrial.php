...
header('Content-Disposition: attachment;filename="Trial Balance '.$ADate1.'to'.$ADate2.'.xls"');
...
<form name="cbform1" method="post" action="t_trial.php">
	...
	<div class="row">
		<div class="col-6">
		   <table>
				...
				<tr>
					<td align="center" colspan="7" ><b>Trial Balance </b></td>
				</tr>
				<tr>
					<td align="center" colspan="7" ><b>From <?php echo $ADate1  ?> to  <?php echo $ADate2  ?></b></td>
				</tr>
			</table>
			<?php
				...
				{       
						...
						$section = $res['section'];
							...
							if($section == 'E'){
								...
								echo "<tbody>";
						 		echo "<tr style='background-color: tomato;' data-node-id='".$sno."'><td></td><td  align='right'>".showAmount($opening_dr_cr,'1')."</td> <td>"."'".$res['code']."</td> <td>".ucwords(strtolower($res['name']))."</td> <td  align='right'>".showAmount($transaction_dr,'0')."</td>  <td  align='right'>".showAmount($transaction_cr,'0')."</td> <td  align='right'>".showAmount($closing_dr_cr,'1')."</td> </tr> </li>";
			   ...
			   // $from_date = $ADate1;
				...
				' cellpadding='1' cellspacing='0'>";
	              ...
	              <td colspan="7" bgcolor="#ecf0f5" class="bodytext3"><strong>&nbsp;</strong></td>
					...
					echo "<tr style='background-color: #4fc7fd;'><td>#</td> <td>Opening</td> <td>Code</td> <td>Ledger</td> <td>Debit</td>  <td>Credit</td> <td>Closing</td> </tr>";
					$query = "select auto_number as uid,id as code,section,accountsmain as name from master_accountsmain where section in ('I','L')  order by section";
					$exec = mysqli_query($GLOBALS["___mysqli_ston"], $query) or die ("Error in Query".mysqli_error($GLOBALS["___mysqli_ston"]));
					$sno = 0;
					while($res = mysqli_fetch_array($exec))
					{
...
function getGroupBalance($group_id,$from_date,$to_date,$sno,$sno2,$includeopenbal,$locationwise,$includezeroballeg){
	...
	$section = $res1['section'];			
		...
		if($closing_dr_cr!=0 || $includezeroballeg=='1'){
         $uuid = $res['uid'] + 1;
		 $data .="<tr  style='background-color: aquamarine;' data-node-id=".$sno.'.'.$sno2." data-node-pid=".$sno."><td></td> <td  align='right'>".showAmount($opening_dr_cr,'1')."</td> 
		 <td>"."'".$res['code']."</td> <td>".ucwords(strtolower($res['name']))."</td> <td  align='right'>".showAmount($transaction_dr,'0')."</td>  <td  align='right'>".showAmount($transaction_cr,'0')."</td> <td  align='right'>".showAmount($closing_dr_cr,'1')."</td> </tr>";
		}
	 if($res['show_ledger']=="1"){
		 	 $data .=getLedger($res['uid'],$from_date,$to_date,$sno,$sno2,$includeopenbal);
		 }
