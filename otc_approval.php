...
$query2 = "select consultationid from consultation_lab where consultationid like 'EB-%'

			union select consultationid from consultation_radiology where consultationid like 'EB-%'

			union select consultationid from consultation_services where consultationid like 'EB-%'

			union select consultation_id as consultationid from master_consultationpharm where consultation_id like 'EB-%'";

$exec2 = mysqli_query($GLOBALS["___mysqli_ston"], $query2) or die ("Error in Query2".mysqli_error($GLOBALS["___mysqli_ston"]));

$res2 = mysqli_fetch_array($exec2);

if($res2["consultationid"]=='')
...
if(isset($_POST['dis']) && count($_POST['dis'])>1){
	...
	*/
	...
	/*		 $query1222 = "select bankrefno from bank_refrencenumber  where visitcode='$visitcode' and patientcode='$patientcode' and patientname='$patientfullname' and billnumber='' ";
		...
		header("location:otc_approval.php");
...
<script type="text/javascript" src="js/autosuggestnewmedicine1_new.js"></script> <!-- For searching customer -->
...
var vrate1 = vrate1;
	...
	var currenttotal3=document.getElementById('total').value;
	...
	document.getElementById('total').value=newtotal3;
...
var radrate=radrate;
	...
	totalamount21=document.getElementById('total').value;
...
var currenttotal=document.getElementById('total3').value;

	//alert(currenttotal);

	newtotal= currenttotal-vrate3;
	...
	document.getElementById('total3').value=newtotal;
	...
	var newgrandtotal=parseInt(totalamount21)+parseInt(totalamount31)+parseInt(newtotal);
...
//}
	...
	if((parseFloat(document.getElementById("availablelimit").value.replace(/,/g,''))-parseFloat(document.getElementById("total").value.replace(/,/g,'')))<0)
	{
	alert("Patient Does Not have Enough Limit To Proceed");
	return false;
	}
...
alert("Please select the patient before adding drugs");
...
$('#customer').autocomplete({
	...
	source:'ajaxexternalcustomernewsearch_pharma.php', 
	...
	html: true, 
		...
		select: function(event,ui){
			...
			var iscapitation = ui.item.iscapitation;
			...
			var avaliable_limit_op = ui.item.avaliable_limit_op;
			var currenticd = ui.item.currenticd;
			var pasticd = ui.item.previousicd;
			...
			$('#subtypeano').val(subtype);



            if(billtype=='PAY LATER'){
				$('#availablelimit').val(avaliable_limit_op);
				$('#paylater_show').show();
			}
			else
			{
			...
			if(currenticd!=''){
              $('#currenticd').html(currenticd);
              $('#current_icd_show').show();
			}else{
              $('#currenticd').html("");
              $('#current_icd_show').hide();
			}

			if(pasticd!=''){
              $('#pasticd').html(pasticd);
              $('#past_icd_show').show();
			}else{
              $('#pasticd').html("");
              $('#past_icd_show').hide();
			...
			if(iscapitation=='1'){
				$('#iscapitation_show').show();
			}else{
				$('#iscapitation_show').hide();
			}	
...
<form name="form1" id="frmsales" method="post" action="otc_approval.php" onSubmit="return validcheck()"   >
             ...
             <tr bgcolor="#011E6A">
              ...
              <tr>
                ...
                <tr>
				  ...
				  <input name="overalllimit" id="overalllimit" value = "external" type = "hidden">
				  ...
				  <input name="subtypeano" id="subtypeano" value="" type="hidden">
        	...
        	<tr>
			   ...
			   <tr>
				...
				<td></td>	
				 ...
				 <td><input name="accountname" id="accountname" value="" type="text" readonly></td>
      ...
      <tr>
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody id="foo">
				   ...
				   <td colspan="11" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><span class="bodytext32"><strong>Pharmacy <img src="images/plus1.gif" width="13" height="13"> </strong></span></td>
				   ...
				   <table id="presid" width="621" border="0" cellspacing="1" cellpadding="1">
                     ...
                     <tr>
					  ...
					  <input type="hidden" name="medicinecode" id="medicinecode" value="">
                       ...
                       <td><input name="medicinename" type="text" id="medicinename" size="40" autocomplete="off" onFocus="return functioncheckpatient();">					   </td>
