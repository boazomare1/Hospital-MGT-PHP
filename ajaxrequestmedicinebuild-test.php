...
echo $query222 = "select itemcode,itemname,disease from master_medicine where itemname like '%$searchmedicinename1%' ".$strtype." and status <> 'Deleted' limit 0,10";
...

{
	...
	//////// all bacthes qty ///////
		echo $querycumstock2 = "SELECT sum(batch_quantity) as total_quantity from transaction_stock where batch_stockstatus='1' and itemcode='$itemcode' and storecode='$storecode' and batchnumber in(select batchnumber from purchase_details where expirydate>now() and itemcode ='$itemcode')";
		...
		$total_quantity = $rescumstock2["total_quantity"];
		$currentstock = $total_quantity;
	//////// all bacthes qty ///////
