...
$cleaned_billno = preg_replace('/[^a-zA-Z0-9]/', '', $visitcode);
...
.underline {text-decoration: underline;}
	...
	<table width="700" border="0" align="center" cellpadding="2" cellspacing="4" style="margin: 0px 0px 0px 0px;">
		...
		$querybill = "select billdate,billno as billno2,preauthcode from billing_ipcreditapproved where patientcode = '$patientcode' and visitcode = '$visitcode'";
	   ...
	   }else
	   {
        ...
        <tr>
		 ...
		 <tr>
         	...
         	<td width="73" align="left" valign="center" class="bodytext31"><strong>Member No:</strong></td>

			<td width="200" align="left" valign="center" class="bodytext31"><?php echo $memberno; ?></td>
         ...
         <tr>
			...
			<td width="73" align="left" valign="center" class="bodytext31"><strong>Pre Auth Code:</strong></td>

			<td width="200" align="left" valign="center" class="bodytext31"><?php echo $preauthcode;?></td>
...
$totalbedtransferamount=0;
			...
			$querya01 = "select * from billing_ipbedcharges where  visitcode='$visitcode' and patientcode='$patientcode' and docno='$billno2' and bed <> '0' order by recorddate";
...
//while (strtotime($from_limit_date) <= strtotime($to_limit_date)) {
			...
			//	echo "<tr><td colspan='7' style='background-color:#ccc'><strong>".date('d M Y',strtotime($from_limit_date))."</strong></td></tr>";
			 ...
			 $titleName ='';
			$storequery ="select p.store as storecode,s.storelable as storelable from pharmacysales_details as p left join master_store as s on p.store=s.storecode where  p.visitcode='$visitcode' and p.patientcode='$patientcode' and p.freestatus = 'No' GROUP BY s.storelable order by s.storelable";
            ...
            while($resStore = mysqli_fetch_array($execStore))
			{
			 ...
			 $query23 = "select * from pharmacysales_details where  visitcode='$visitcode' and patientcode='$patientcode' and freestatus = 'No' and store in (select storecode from master_store where storelable='".$resStore['storelable']."')  GROUP BY ipdocno,itemcode order by entrydate";
		...
		<tr>
			...
			//		$totalser=0;
		     ...
		     $query21 = "select * from ipconsultation_services where locationcode='$locationcode' and patientvisitcode='$visitcode' and patientcode='$patientcode' and servicesitemname <> '' and servicerefund <> 'refund' and wellnessitem <> '1' and freestatus = 'No'  group by servicesitemcode,iptestdocno ";
		...
		<tr>
...

		   {
			...
			$nhif_claimtotalamount=0;
			$query641 = "select * from billing_ipcreditapprovedtransaction where locationcode='$locationcode' and patientcode='$patientcode' and visitcode='$visitcode' and nhifclaim_flag='1'";
			$exec641 = mysqli_query($GLOBALS["___mysqli_ston"], $query641) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
			$num_nhifflag = mysqli_num_rows($exec641);
			if($num_nhifflag>0){
			$res641 = mysqli_fetch_array($exec641);
			$nhif_claimaccountname = $res641['accountname'];
			$nhif_claimbilldate = $res641['billdate'];
			$nhif_claimtotalamount = $res641['totalamount'];
				?>
			<tr>
				...
				<td class="bodytext31" valign="center"  align="left"><?php echo  date('d-m-Y', strtotime($nhif_claimbilldate)); ?></td>

				<td class="bodytext31" valign="center"  align="left"></td>

				<td class="bodytext31" valign="center"  align="left" width="250"><?php echo $nhif_claimaccountname; ?></td>

				<td class="bodytext31" valign="center"  align="right"><?php echo '1'; ?></td>

				<td class="bodytext31" valign="center"  align="right"><?php echo number_format($nhif_claimtotalamount,2,'.',','); ?></td>
			...
			<?php } ?>
			  ...
			  $consultationtotal=$totalop;
			   ...
			   $gross_discount=abs($totaldiscountamount);
			   
			   $gross_amount_total=$payoveralltotal -$gross_discount;
	         ...
	         <tr>
	            ...
	            <td colspan="5" class="bodytext31" align="left"><div width='450'><strong><?php echo $currency; ?></strong>&nbsp;&nbsp;<?php echo str_replace('Kenya Shillings','',$convertedwords); ?></div></td> 
              
	<td colspan="2" class="bodytext31" align="right"><strong> Grand Total&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo number_format($overalltotal,2,'.',','); ?></strong></td>
	...
	<td colspan="7" class="bodytext31" align="right"><strong> Net Payable&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo number_format($overalltotal,2,'.',','); ?></strong></td>
	    ...
	    <tr>
	      ...
	      <td colspan="7" height="10" align="left" valign="center" bordercolor="#f3f3f3" class="bodytext31">&nbsp;</td>
		...
		<td colspan="7" align="left" valign="center" bordercolor="#f3f3f3" class="bodytext31"><strong>Receivable Accounts:</strong></td>
            ...
            $res11subtotalamount=$gross_amount_total;
            $billautonumber =$billnumbers;
...
<tbody>
 <tr>
	      ...
	      <td colspan="" align="left" valign="center" bordercolor="#f3f3f3" class="bodytext31">&nbsp;</td>
