...
function totalipd($miscode, $ADate1, $ADate2){
      ...
      select sum(billing_ipambulance.amountuhx) as amount, A.misreport FROM `billing_ipambulance` join (SELECT DISTINCT(master_accountname.accountname), master_accountname.misreport FROM master_accountname) AS A on A.accountname = billing_ipambulance.accountname where  (billing_ipambulance.recorddate BETWEEN '$ADate1' and '$ADate2')
      UNION ALL
      select sum(billing_ipservices.servicesitemrateuhx) as amount, A.misreport FROM `billing_ipservices` join (select distinct(visitcode) as visitcode, accountnameano from billing_ip where billdate between '$ADate1' and '$ADate2' UNION ALL select distinct(visitcode) as visitcode, accountnameano from billing_ipcreditapproved where billdate between '$ADate1' and '$ADate2') as b ON billing_ipservices.patientvisitcode = b.visitcode JOIN master_accountname as A ON A.auto_number = b.accountnameano where  (billing_ipservices.billdate BETWEEN '$ADate1' and '$ADate2')
      UNION ALL
      select sum(billing_iplab.labitemrate) as amount, A.misreport FROM `billing_iplab` join (select distinct(visitcode) as visitcode, accountnameano from billing_ip where billdate between '$ADate1' and '$ADate2' UNION ALL select distinct(visitcode) as visitcode, accountnameano from billing_ipcreditapproved where billdate between '$ADate1' and '$ADate2') as b ON billing_iplab.patientvisitcode = b.visitcode JOIN master_accountname as A ON A.auto_number = b.accountnameano where  (billing_iplab.billdate BETWEEN '$ADate1' and '$ADate2')
      UNION ALL
      SELECT sum(billing_ipradiology.radiologyitemrateuhx) as amount, A.misreport FROM `billing_ipradiology` join (SELECT DISTINCT(master_accountname.accountname), master_accountname.misreport FROM master_accountname) AS A on A.accountname = billing_ipradiology.accountname where  (billing_ipradiology.billdate BETWEEN '$ADate1' and '$ADate2')
      UNION ALL
      ...
      UNION ALL
      select sum(billing_ipbedcharges.amountuhx) as amount, A.misreport FROM `billing_ipbedcharges` join (select distinct(visitcode) as visitcode, accountnameano from billing_ip where billdate between '$ADate1' and '$ADate2' UNION ALL select distinct(visitcode) as visitcode, accountnameano from billing_ipcreditapproved where billdate between '$ADate1' and '$ADate2') as b ON billing_ipbedcharges.visitcode = b.visitcode JOIN master_accountname as A ON A.auto_number = b.accountnameano where  (billing_ipbedcharges.recorddate BETWEEN '$ADate1' and '$ADate2')
      UNION ALL
      SELECT sum(billing_iphomecare.amount) as amount, A.misreport FROM `billing_iphomecare` join (SELECT DISTINCT(master_accountname.accountname), master_accountname.misreport FROM master_accountname) AS A on A.accountname = billing_iphomecare.accountname where  (billing_iphomecare.recorddate BETWEEN '$ADate1' and '$ADate2')
      UNION ALL
...
// }
 ...
 $query6 = "select count(*) as count from ip_bedallocation where recorddate BETWEEN '$ADate1' AND '$ADate2'  ";
...
select count(a.visitcode) as count, c.misreport as misreport FROM billing_ip as a JOIN ip_bedallocation as b ON a.visitcode = b.visitcode JOIN master_accountname as c ON a.accountnameid = c.id where a.billdate between '$ADate1' and '$ADate2' and c.misreport = '$paymentcode' 
UNION ALL 
select count(a.visitcode) as count, c.misreport as misreport FROM billing_ipcreditapproved as a JOIN ip_bedallocation as b ON a.visitcode = b.visitcode JOIN master_accountname as c ON a.accountnameid = c.id where a.billdate between '$ADate1' and '$ADate2' and c.misreport = '$paymentcode' 
) as count1";
...
$query15 = "select * from master_bed where  recordstatus <> 'deleted'";
...
$smsTxt .= "\nOCCUPANCY = ".number_format($occupancy,2).'%';	
  ...
  $query = "select * from master_sms where reportname = 'dailyflash'";
  $exec = mysqli_query($GLOBALS["___mysqli_ston"], $query) or die("Error in Query");
  $res = mysqli_fetch_array($exec);
  $clientid = $res['apiClientID'];
  $key = $res['api_key']; 
  $secret = $res['shortcode']; 
  $serviceid = $res['pass_type'];
  $phone_numbers = $res['phone_number'];
  $txtMessage = $smsTxt;

  $mno = explode(',',$phone_numbers);

  foreach ($mno as $mobile) {
    ...
    $curl_post_data = array(
         ...
         'message' => $txtMessage,
         'shortcode' => $secret,
         'pass_type' => $serviceid, //bm5 {base64 encode} or plain
    );
    ...
    if ($err) {
      ...
      $status_code = $error['respose-code'];
      $status_message = $error['response-description'];
      $datetime = date('Y-m-d H:i:s');
      $phone_no = $mobile;

      mysqli_query($GLOBALS["___mysqli_ston"], "insert into sms_transactions(message_status, response_status_code, response_status_message, sent_sms, sent_datetime, sent_phone_number) VALUES ('$message_status', '$status_code', '$status_message', '$txtMessage', '$datetime', '$phone_no')");

    } else {

      $responseData = json_decode($curl_response, TRUE);
      foreach ($responseData as $responseItem) {
        foreach ($responseItem as $obj) {
          $message_status = 'Success';
          $status_code = $obj['respose-code'];
          $status_message = $obj['response-description'];
          $unique_id = $obj['messageid'];
          $credits = $obj['networkid'];
          $datetime = date('Y-m-d H:i:s');
          $phone_no = $mobile;

          $url1 = 'https://mysms.celcomafrica.com/api/services/getbalance/';
          $curl1 = curl_init();
          curl_setopt($curl1, CURLOPT_URL, $url1);
          curl_setopt($curl1, CURLOPT_HTTPHEADER, array('Content-Type:application/json')); //setting custom header


          $curl1_post_data = array(
                //Fill in the request parameters with valid values
               'partnerID' => $clientid,
               'apikey' => $key,
          );

          $data_string = json_encode($curl1_post_data);
          ...
          //print_r($curl1_response);

          $response1 = json_decode($curl1_response,true);
          $credits = $response1['credit'];

          mysqli_query($GLOBALS["___mysqli_ston"], "insert into sms_transactions(message_status, response_status_code, response_status_message, response_uniqueid, response_credits, sent_sms, sent_datetime, sent_phone_number) VALUES ('$message_status', '$status_code', '$status_message', '$unique_id', '$credits', '$txtMessage', '$datetime', '$phone_no')");
        }
      } 
    }
  }
...
$message = $txtMessage; 
$chatID = "@premier_excom";
$token = '1249866721:AAFgsemY6C_LWoT6hfLoFWB2LP2WShn5WeE';
