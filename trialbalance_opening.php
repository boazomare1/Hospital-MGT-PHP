    ...
    <td colspan="2" valign="top"><table width="99%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td colspan="2" width="46%">
              ...
              <form name="cbform1" method="post" action="trialbalance_opening.php">
             ...
             <tr>
                ...
                bgcolor="#FFFFFF" class="bodytext31"> From Date </td>
                      <td width="20%" align="left" valign="center"  bgcolor="#FFFFFF"><span class="bodytext31">
					    <input type="text" name="ADate1" id="ADate1" style="border: 1px solid #001E6A" value="<?php echo $ADate1; ?>"  size="10" onKeyDown="return disableEnterKey()" />
        ...
        <td valign="top"><table width="600" id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
			...
			{	
				...
				{  
					...
					$tbledgerview = 'accounttbopen.php';
	...
	if($parentid != '' && $tbinclude != '')
	{
		...
		{  
			...
			if($id != '01-1000')
			{
				$i = 0;
				$result = array();
				$querydr1 = "SELECT SUM(`openbalanceamount`) as openacc FROM `openingbalancesupplier` WHERE `accountcode` = '$id'
							 UNION ALL SELECT SUM(`openbalanceamount`) as openacc FROM `openingbalanceaccount` WHERE `accountcode` = '$id'";
			...
			else if($id == '01-1000')
			{
				$i = 0;
				$result = array();
				$querydr1 = "SELECT SUM(`totalamount`) as openacc FROM `purchase_details` WHERE billnumber LIKE 'SUPO%' AND suppliername = 'OPENINGSTOCK' and recordstatus <> 'deleted'";
				$execdr1 = mysqli_query($GLOBALS["___mysqli_ston"], $querydr1) or die ("Error in Querydr1".mysqli_error($GLOBALS["___mysqli_ston"]));
				while($resdr1 = mysqli_fetch_array($execdr1))
				{
				$i = $i+1;
