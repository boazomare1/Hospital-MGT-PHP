...
if ($upload == 'success')
{
	...
	$filename = $username."_".$filedate."_tabdump.csv";
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 1 && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			$employeecode = $splitdelimiter[1];
			$employeecode = strtoupper($employeecode);
			$employeename = $splitdelimiter[2];
			$logindate= $splitdelimiter[3];
			$logindate = date('Y-m-d',strtotime($logindate));
			$paymonth = $splitdelimiter[4];
			$paymonth = date('M-Y',strtotime($paymonth));
			$logintime = $splitdelimiter[5];
			$logintime = date('H:i:s',strtotime($logintime));
			preg_match ('/[!,^,+,=,[,],;,,,{,},|,\,<,>,?,~]/', $employeename);
			
			if ($employeecode != '')
			{
			$query2 = "insert into absent_upload (employeecode,employeename,assignmonth,attendance_date,attendance_timein,username,ipaddress,updatedatetime)
			values ('$employeecode','$employeename','$paymonth','$logindate','$logintime','$username','$ipaddress','$updatedatetime')";
		...
		header("location:tadataimport1.php?st=success");
