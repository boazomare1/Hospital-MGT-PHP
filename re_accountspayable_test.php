...
<?php
	$query1 = "SELECT a.`totalfxamount` as totalamount,a.itemname as itemname,a.ledgercode as ledgercode,a.ledgername as ledgername,a.entrydate as entrydate,c.accountssub as accsub FROM `purchase_details` AS a JOIN `master_medicine` AS b JOIN `master_accountname` AS c ON (a.itemcode = b.itemcode AND b.inventoryledgercode = c.id) WHERE a.`billnumber` = '$billnumber'";
...
<td align="left"><?= number_format($amount2-$amount1,2); ?></td>
</tr>
<tr>
	...
	if($parentid != '' && $tbinclude != '')
	{
		...
		else
		{
			...
			{  
				...
				$result = array();
				$purchase1=0;
				$querydr1 = "SELECT SUM(`totalamount`) as expenses FROM `purchase_details` WHERE `expensecode` = '$id' AND billnumber NOT LIKE 'SUPO%' AND suppliername <> 'OPENINGSTOCK' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2' and recordstatus <> 'deleted'
						UNION ALL SELECT SUM(a.`totalamount`) as expenses FROM `purchase_details` AS a JOIN master_medicine AS b ON (a.itemcode = b.itemcode) WHERE b.inventoryledgercode = '$id' AND a.billnumber NOT LIKE 'SUPO%' AND a.suppliername <> 'OPENINGSTOCK' AND a.entrydate BETWEEN '$ADate1' AND '$ADate2' and a.recordstatus <> 'deleted'";
				...
				$qrydr1 = "SELECT SUM(`totalamount`) as expenses FROM `purchase_details` WHERE `expensecode` = '$id' AND billnumber NOT LIKE 'SUPO%' AND suppliername <> 'OPENINGSTOCK' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2' and recordstatus <> 'deleted'";
				$exedr1 = mysqli_query($GLOBALS["___mysqli_ston"], $qrydr1) or die ("Error in qrydr1".mysqli_error($GLOBALS["___mysqli_ston"]));
				$resdr11 = mysqli_fetch_array($exedr1);
				$purchase1 += $resdr11['expenses'];
				$qrydr1 = "SELECT SUM(a.`totalamount`) as expenses FROM `purchase_details` AS a JOIN master_medicine AS b ON (a.itemcode = b.itemcode) WHERE b.inventoryledgercode = '$id' AND a.billnumber NOT LIKE 'SUPO%' AND a.suppliername <> 'OPENINGSTOCK' AND a.entrydate BETWEEN '$ADate1' AND '$ADate2' and a.recordstatus <> 'deleted'";
				$exedr1 = mysqli_query($GLOBALS["___mysqli_ston"], $qrydr1) or die ("Error in qrydr1".mysqli_error($GLOBALS["___mysqli_ston"]));
				$resdr11 = mysqli_fetch_array($exedr1);
				$purchase1 += $resdr11['expenses'];
				if($purchase1 != $purchase)
				{
				echo '<br> union failed:'.$id;
				}
...
$qrytest = "select * from purchase_details where expensecode in (select id from master_accountname where accountsmain in (select auto_number from master_accountsmain where recordstatus <> 'deleted' and section IN ('A','E'))) AND billnumber NOT LIKE 'SUPO%' AND suppliername <> 'OPENINGSTOCK' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2' group by billnumber
			union all SELECT a.* FROM `purchase_details` AS a JOIN master_medicine AS b ON (a.itemcode = b.itemcode) WHERE b.inventoryledgercode in (select id from master_accountname where accountsmain in (select auto_number from master_accountsmain where recordstatus <> 'deleted' and section IN ('A','E'))) AND billnumber NOT LIKE 'SUPO%' AND suppliername <> 'OPENINGSTOCK' AND `entrydate` BETWEEN '$ADate1' AND '$ADate2' group by billnumber";
$exetest = mysqli_query($GLOBALS["___mysqli_ston"], $qrytest) or die ("Error in qrytest".mysqli_error($GLOBALS["___mysqli_ston"]));
				while($restest = mysqli_fetch_array($exetest))
				{
				//print_r($restest);
				echo '<br>'.$restest['billnumber'].' - '.$restest['ledgername'].' - '.$restest['ledgercode'];
				$lgcode =$restest['ledgercode'];
				if($lgcode=='')
				{
				$lgcode =$restest['expensecode'];
				}
				$qrytest1 ="select * from master_accountsmain where auto_number in (select accountsmain from master_accountname where id like '$lgcode')";
				$exetest1 = mysqli_query($GLOBALS["___mysqli_ston"], $qrytest1) or die ("Error in qrytest1".mysqli_error($GLOBALS["___mysqli_ston"]));
				$restest1 = mysqli_fetch_array($exetest1);
				echo ' - '.$restest1['accountsmain'];
				}
?>