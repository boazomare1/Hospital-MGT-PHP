...
?>
		...
		function format_interval_dob(DateInterval $interval) {
 		...
 		$query1 = "select patientfullname, patientcode, accountname,billtype,consultationdate,scheme_id, gender, age, consultingdoctor, nhifid, subtype, type from master_ipvisitentry where patientcode='$patientcode' and visitcode='$visitcode'";
		...
		<tr>
			...
			<?php 
				...
				while($res18 = mysqli_fetch_array($exec18))
				{
					...
					$quantity1=$qty_t = $interval->format("%a");
					if($packdays1>$quantity1)
					{
						$quantity1=$quantity1-$packdays1; 
						$packdays1=$packdays1-$qty_t;
					}
					else
					{
						$quantity1=$quantity1-$packdays1;
					...
					foreach($tmp as $rslt)
					{
					  ...
					  ?>
								<tr <?php echo $colorcode; ?>>
									...
									<td width="74" class="bodytext31" valign="center"  align="left"><div align="left"><?php echo date("d/m/y", strtotime($bedallocateddate)); ?></div></td>
	...
	$fxrate = $original_fxrate;
		...
		<tr>
			...
			while($res64 = mysqli_fetch_array($exec64))
		   {
			...
			<tr>  
			<td colspan="6" class="bodytext31" align="right"><strong>Total :</strong></td>
			<td class="bodytext31" align="right"><strong><?php echo number_format($overalltotal,2,'.',','); ?></strong></td>
		   </tr>
           <?php if ($_REQUEST['lable']!='' && $_REQUEST['lableamt']!='') { 
		   $lableamt = $_REQUEST['lableamt'];
		   ?>
		   <tr>  
			<td colspan="6" class="bodytext31" align="right"><strong><?php echo strtoupper($_REQUEST['lable']);?>(Less) :</strong></td>
			<td class="bodytext31" align="right"><strong><?php echo number_format($lableamt,2,'.',','); ?></strong></td>
		   </tr>
           <?php }
		         else
				   $lableamt = 0;
		   ...
		   ?>
		   <tr>  
			<td colspan="6" class="bodytext31" align="right"><strong>Net Total :</strong></td>
			<td class="bodytext31" align="right"><strong><?php echo number_format(($overalltotal-$lableamt),2,'.',','); ?></strong></td>
		   </tr>
