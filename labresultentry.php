...
foreach($_POST['lab'] as $key => $value)
 ...
 $exec77 = mysqli_query($GLOBALS["___mysqli_ston"], $query77) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
  ...
  $query77_auditentry = "update audit_resultentry_lab set resultstatus='completed' where docnumber='$docnumber'";
 $exec77_auditentry = mysqli_query($GLOBALS["___mysqli_ston"], $query77_auditentry) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
 
 $query29="UPDATE consultation_lab set resultentry='$status1',lab_comments='$lab_comments', resultdoc='$docnumber', publishdatetime='$updatedatetime',publishstatus = 'completed' where labitemcode='$itemcode' and patientvisitcode='$visitcode' and docnumber='$samplecollectiondocnumber'";
 // 
...
// for(var i = 1; i<= sno; i++){
	...
	if(checkack == true){
		if(eoid == ''){
			alert('Please select the Equipment the list');
			$("#eqname").focus();
			return false;
		}
	}
...
$sampleid = $_REQUEST['sampleid'];
 ...
 $query79="SELECT recorddate from samplecollection_lab where patientcode='$patientcode' and patientvisitcode='$visitcode' and itemcode='$q_itemcode' and sampleid='$sampleid'";
...
// $query79="select * from master_customer where patientcode='$patientcode' and visitcode='$visitcode'";
// $exec79=mysql_query($query79) or die(mysql_error());
// $res79=mysql_fetch_array($exec79);
// $patientage=$res79['age'];
// $patientgender=$res79['gender'];
...
<a href="emr2.php?patientcode=<?php echo $patientcode?>&&visitcode=<?php echo $visitcode?>"><?php echo $patientcode; ?></a>
     ...
     <tr>
      ...
      <td colspan="5" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext365"><strong>Location : <?php echo $locationname; ?></strong></td>
  ...
  $query31="select * from samplecollection_lab where patientcode = '$patientcode' and patientvisitcode = '$visitcode' and sampleid = '$sampleid' and acknowledge = 'completed' and docnumber='$docnumber' and itemcode='$q_itemcode' group by itemname";
	  ...
	  { 
	   ...
	   $colorloopcount = $colorloopcount + 1;
		...
		?>		  
			  ...
			  <input type="hidden" id="eqid" name="eqid"  value="<?php echo $eqid; ?>" hidden></div></td>
		...
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
            ...
            align="left" border="0">
              <tbody>
			   ...
			   $sn = 0;
			   //echo $birth_days;
			  $query52="select * from master_labreference where  itemcode='$labitemcode' and (gender = '$patientgender' or gender='All' or gender='') and '$birth_days' >= agefrom and '$birth_days' <= ageto and status <> 'deleted' order by reforder asc"; 
			   $exec52=mysqli_query($GLOBALS["___mysqli_ston"], $query52);
			   $num1=mysqli_num_rows($exec52);
			   if($num1<=0)
			   { ?>
			    <tr><td><span style="color:red">Lab References are not found in the master for this patient gender and age.</span></td></tr>   
			 <?php  }
			  ...
			  $query641 = "select * from master_labreference where itemcode='$itemcode2' and referencename = '$labreferencename' and status <> 'deleted'  group by referencename";
				...
				else
				{
				   ...
				   echo $labreferencename;	
				   	...
				   	<?php if($genericsearch == '1'){ ?><button id="genericsearch<?php echo $sn; ?>" onclick="loadGeneric(<?php echo $sn+1; ?>)"><img src="data-search-icon.png" height="15" width="15" /></button><?php } ?>
				   </div></td>
				    <input type="hidden" name="lab[]" value="<?php echo $labname2;?>">
				   <input type="hidden" name="referencename[]" id="referencename<?php echo $sn+1; ?>" value="<?php echo $labreferencename; ?>">
				...
				$labreferencerange1[1] = 1000000;
				 ...
				 $pastresult = $res49['resultvalue'];
				  ...
				  <textarea rows="3" cols="30" name="result[]" id="result<?php echo $sn; ?>" onBlur="return funcrange('<?php echo $sn; ?>')" value="<?php echo $result; ?>" ></textarea>
