...
<tr>
<td colspan="12" align="center" valign="middle"><strong>OP Consultation</strong></td>
</tr>
...
$transactionamount = $res8['consultation'];
	...
	if($parentid != '' && $tbinclude != '')
	{
		if($parentid == '16' || $parentid == '17' || $parentid == '18')
		{
			...
			{  
				...
				$j = 0;
				$crresult = array();
				$querycr1 = "SELECT SUM(`$col`) as bankamount FROM `billing_consultation` WHERE (`bankcode` = '$id' or `cashcode` = '$id' or `mpesacode` = '$id' or `transactionmode` = 'SPLIT') AND `billdate` BETWEEN '$ADate1' AND '$ADate2' AND '$id' in (select ledgercode from financialaccount)";
		...
		else
		{
			...
			{  
				...
				if($id == '03-3002-NHL')
				{
					$querydr1 = "SELECT SUM(`consultation`) as income FROM `billing_consultation` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'";
