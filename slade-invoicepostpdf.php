...
//echo $billautonumber;
$query38 = "select locationcode from billing_paylater where billno = '$billautonumber'";

$exec38 = mysqli_query($GLOBALS["___mysqli_ston"], $query38) or die ("Error in Query3".mysqli_error($GLOBALS["___mysqli_ston"]));

$res38 = mysqli_fetch_array($exec38);

$locationcode = $res38["locationcode"];


function post_invoice($id,$filename1,$file_path1,$name,$authorization,$invoice_url,$visitcode,$billno,$offpatient,$locationcode)
{
	
	  /*$file_path1='C:\\Users\\user\\Downloads\\medbot-slade\\'.$filename1;*/
	  
	 // $file_path1='\Users\user\Downloads\medbot-slade\OPLT-210-23-16.pdf';
	...
	if ($err) {
	   ...
	   $inv_rslt['msg'] = $err_unable_connect;
		...
		if(isset($obj['id'])){
			...
			if($offpatient=='1')
			{
				$query81 = "update master_visitentry set offpatient_status='completed' where visitcode = '$visitcode' ";
				$exec81 = mysqli_query($GLOBALS["___mysqli_ston"], $query81);
				//header("location:offslade_posting.php?cbfrmflag1='cbfrmflag1'");
				header("location:billing_pending_op2.php?billautonumber=$billno&&st=success&&printbill=$billno&&location=$locationcode");
				exit;
			}
			else
			{
...
//		$cstnumber1 = $res2["cstnumber"];
    ...
    if (strpos(strtolower($res2subtype), 'nhif') !== false)
      $isnhif=1;
    else
	  $isnhif=0;
	...
	$res2username = strtoupper($username);
	 ...
	 $querymember = "select planpercentage,planname,subtype,consultationdate,consultationtime,accountname,scheme_id,offpatient from master_visitentry where visitcode='$res2visitcode' and patientcode='$res2patientcode'";
	...
	//$plancount = mysql_num_rows($execmember);
	 ...
	 $offpatient=$resmember['offpatient'];
...
$customername = $res44['customername'];
$customermiddlename = $res44['customermiddlename'];
$customerlastname = $res44['customerlastname'];
$res44customerfullname = $res44['customerfullname'];
$res44customerfullname = $customername.' '.$customerlastname.' '.$customermiddlename;
...
$doctordepartment=$resmember['departmentname'];
}
$res_doctor_fullname="";
 $query_doctor = "SELECT username,department from master_consultationlist where visitcode='$res2visitcode' and patientcode='$res2patientcode' order by auto_number asc limit 0,1 ";
...
$res_doctor_username='';
while($res_doctor  = mysqli_fetch_array($exec_doctor)){
$res_doctor_username = $res_doctor['username'];

if($res_doctor['department']!='')
  $doctordepartment=$res_doctor['department'];
}
...
<td width="110" align="left" valign="center" class="bodytext31"><strong>A/c No:</strong></td>
  ...
  <tr>
    ...
    <td width="361" align="left" valign="center" class="bodytext31"><?php echo $res_doctor_fullname." - ".$doctordepartment;?></td>
...
<tr>
 ...
 <?php
 /*$queryicd = "select * from consultation_icd where patientcode = '$res2patientcode' and patientvisitcode = '$res2visitcode' order by auto_number DESC";
...
$query199 = "select * from master_consultationpharmissue where patientvisitcode='$res2visitcode' and patientcode='$res2patientcode' and medicinecode = '".$res19medicinecode."'";
...
$query200 = "select * from consultation_lab where patientvisitcode='$res2visitcode' and patientcode='$res2patientcode' and paymentstatus='completed' and labitemcode = '".$res20labitemcode."' and `op_package_id` IS NULL";
...
if($isnhif==1)
{
	$res200labitemrate=$res200labitemrate-$labcash_copay;
	$labcash_copay=0;
}

if($res200labitemrate==0){
   continue;
}
...
$query211 = "select * from consultation_radiology where patientvisitcode='$res2visitcode' and patientcode='$res2patientcode' and paymentstatus='completed'  and radiologyitemcode = '".$res21radiologyitemcode."' and `op_package_id` IS NULL ";
...
if($isnhif==1)
{
	$res211referalrate=$res211referalrate-$radcash_copay;
	$radcash_copay=0;
}

if($res211referalrate==0){
   continue;
}
...
$query233 = "select refno,sum(cash_copay) as cash_copay, servicesitemrate,sum(amount) as amount from consultation_services where patientvisitcode='$res2visitcode' and patientcode='$res2patientcode' and servicesitemcode = '".$res23servicesitemcode."' and paymentstatus='completed' group by servicesitemcode";
...
if($isnhif==1)
{
	$res233serviceitemrate=$res233serviceitemrate-$sercash_copay;

	if($res233serviceitemrate <= 0)
        $res233serviceitemrate=$res23servicesitemrate;

	$seramount=$res233['amount']-$sercash_copay;
	$sercash_copay=0;
}
else
...
<td  align="right" valign="left" 
	 ...
	 bgcolor="#ffffff" colspan="5" class="bodytext31"><strong>Total Copay(KES):</strong></td>
...
<td  align="right" valign="left" 
	 ...
	 bgcolor="#ffffff" colspan="5" class="bodytext31"><strong>Amount Payable(KES):</strong></td>
...
//$file_path = 'slade-post'; 
//$file_path1 = 'slade-post\\'.$billautonumber.'.pdf'; 
//$filename1=$billautonumber.'.pdf';

/*$desktopPath = getenv("USERPROFILE") . "\\Desktop";
$folderName = "medbot-slade";
...
//echo $file_path1 = '\Users\user\Downloads\medbot-slade\.'.$billautonumber.'; 
//$file_path1 = 'public_html\TESTRFH\medbot_slade\\'.$billautonumber.'.pdf'; 
...

{
	...
	// move_uploaded_file( "\public_html\TESTRFH\\'.$billautonumber.'.pdf'", $folderName);
	post_invoice($claim,$filename1,$file_path2,$customername,$auth,$invoice_url,$res2visitcode,$billautonumber,$offpatient,$locationcode);
}
