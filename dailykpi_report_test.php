...
//  }
 ...
 $query1 = "select count(*) as count from ip_bedallocation where ward NOT IN ('1','2','3','8','17') and recorddate between '$ADate1' and '$ADate2'";
...
function getTotalBedDays($days){
  $queryipcreditdisc = "select * from master_bed where ward NOT IN ('1','2','3','8','17') and recordstatus <> 'deleted'";
...
function totalipd($miscode, $ADate1, $ADate2){
      ...
      UNION ALL
      SELECT sum(billing_ipprivatedoctor.amount) as amount, A.misreport FROM `billing_ipprivatedoctor` join (select distinct(visitcode) as visitcode, accountnameano from billing_ip where billdate between '$ADate1' and '$ADate2' UNION ALL select distinct(visitcode) as visitcode, accountnameano from billing_ipcreditapproved where billdate between '$ADate1' and '$ADate2') as b ON billing_ipprivatedoctor.visitcode = b.visitcode JOIN master_accountname as A ON A.auto_number = b.accountnameano where A.misreport = '$miscode' and (billing_ipprivatedoctor.recorddate BETWEEN '$ADate1' and '$ADate2') group by billing_ipprivatedoctor.visitcode
...
function getTotalCashRevenue($miscode, $ADate1, $ADate2){
  $query1 = "select sum(amount) as amount from (
    select sum(billing_opambulance.amount) as amount, master_accountname.misreport from billing_opambulance JOIN master_accountname ON billing_opambulance.accountname = master_accountname.accountname  where master_accountname.misreport='$miscode' and (billing_opambulance.recorddate between '$ADate1' and '$ADate2') 
      ...
      select sum(billing_paynowreferal.fxamount) as amount, master_accountname.misreport from billing_paynowreferal JOIN master_accountname ON billing_paynowreferal.accountname = master_accountname.accountname where master_accountname.misreport = '$miscode' and (billing_paynowreferal.billdate between '$ADate1' and '$ADate2')
      UNION ALL
      select sum(billing_ipambulance.amountuhx) as amount, A.misreport FROM `billing_ipambulance` join (SELECT DISTINCT(master_accountname.accountname), master_accountname.auto_number, master_accountname.misreport FROM master_accountname) AS A on A.accountname = billing_ipambulance.accountname where A.misreport = '$miscode' and (billing_ipambulance.recorddate BETWEEN '$ADate1' and '$ADate2') and A.auto_number = '47'
      UNION ALL
      select sum(billing_ipservices.servicesitemrateuhx) as amount, A.misreport FROM `billing_ipservices` join (select distinct(visitcode) as visitcode, accountnameano from billing_ip where billdate between '$ADate1' and '$ADate2' UNION ALL select distinct(visitcode) as visitcode, accountnameano from billing_ipcreditapproved where billdate between '$ADate1' and '$ADate2') as b ON billing_ipservices.patientvisitcode = b.visitcode JOIN master_accountname as A ON A.auto_number = b.accountnameano where A.misreport = '$miscode' and (billing_ipservices.billdate BETWEEN '$ADate1' and '$ADate2') and A.auto_number = '47'
      UNION ALL
      ...
      UNION ALL
      SELECT sum(billing_ipprivatedoctor.amount) as amount, A.misreport FROM `billing_ipprivatedoctor` join (select distinct(visitcode) as visitcode, accountnameano from billing_ip where billdate between '$ADate1' and '$ADate2' UNION ALL select distinct(visitcode) as visitcode, accountnameano from billing_ipcreditapproved where billdate between '$ADate1' and '$ADate2') as b ON billing_ipprivatedoctor.visitcode = b.visitcode JOIN master_accountname as A ON A.auto_number = b.accountnameano where A.misreport = '$miscode' and (billing_ipprivatedoctor.recorddate BETWEEN '$ADate1' and '$ADate2') and A.auto_number = '47' group by billing_ipprivatedoctor.visitcode
	   UNION ALL
      SELECT -1*sum(billing_ipprivatedoctor.sharingamount) as amount, A.misreport FROM `billing_ipprivatedoctor` join (select distinct(visitcode) as visitcode, accountnameano from billing_ip where billdate between '$ADate1' and '$ADate2' UNION ALL select distinct(visitcode) as visitcode, accountnameano from billing_ipcreditapproved where billdate between '$ADate1' and '$ADate2') as b ON billing_ipprivatedoctor.visitcode = b.visitcode JOIN master_accountname as A ON A.auto_number = b.accountnameano where A.misreport = '$miscode' and (billing_ipprivatedoctor.recorddate BETWEEN '$ADate1' and '$ADate2') and A.auto_number = '47' group by billing_ipprivatedoctor.visitcode
      UNION ALL
    ...
    <td width="97%" valign="top"><table width="116%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="860">
              ...
              <form name="cbform1" method="post" action="dailykpi_report_test.php">
		<table width="634" border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
          <tbody>
            <tr bgcolor="#011E6A">
              <td bgcolor="#ecf0f5" colspan="3" class="bodytext3"><strong>Daily KPI Snapshot</strong></td>
             </tr>
             <tr>
      ...
      <tr>
       ...
       <tr>
        <td>
            ...
            <tr>
              <td class="bodytext31" valign="center"  align="left"> 
               <a target="_blank" href="xl_dailykpireport.php?ADate1=<?php echo $ADate1; ?>"><img src="images/excel-xls-icon.png" width="30" height="30"></a>
              </td>
            </tr>
