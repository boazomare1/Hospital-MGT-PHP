...
$totalamount5 = '0.00';
				...
				{  
					...
					if($id == '01-10041-NHL')
					{
						...
						echo '<br>-->'.$balance = array_sum($crresult) - array_sum($drresult);
					}
					else 
					{
	...
	$query66 = "select * from master_ipvisitentry where visitcode NOT IN (select visitcode from billing_ip) and consultationdate between '$ADate1' and '$ADate2'";
		...
		while($res66 = mysqli_fetch_array($exec66))
		{
			...
			if($radiologyfree == 'No')
			{
			echo $queryr51 = "select rateperunit from `$radtable` where itemcode='$radiologyitemcode'";
		   ...
		   $radtotal = $radtotal + $radrateuhx;
			echo '<br>radtot->'.$balance=$radtotal;
		//	echo $balance;
			if ($showcolor == 0)
			{
				//echo "if";
...
$searchsuppliername='';
		  ...
		  {
		  	echo '<br>radcode'.$radcode=$res5['radiologyitemcode'];
			...
			echo '===>'.$res5radiologyitemrate = $res5radiologyitemrate + $radrate;
		  }
		  ...
		  echo '<br>totalamount5'.$totalamount5 = $totalamount5 + $res5radiologyitemrate;
		 ...
		 // $snocount = $snocount + 1;
			...
			else
			{
				...
				echo '<br>aa->'.$balance=$balance+$totalamount5;
					} 
					...
					echo '<br>=>'.$sumbalance = $sumbalance + $balance + $journal;
				}
