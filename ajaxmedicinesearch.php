...
if($genericsearch <>'')
{
	$a_json = array();
$a_json_row = array();
  $query1 = "select genericname from master_medicine where genericname like '%$process%' and itemcode like '%$item%' group by genericname limit 0,15";
$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
while ($res1 = mysqli_fetch_array($exec1))
{
...
if($storesearch <>'')
{
	$a_json = array();
$a_json_row = array();
  $query1 = "select store,storecode from master_store where store like '%$process%' or storecode like '%$process%' and  recordstatus <>'deleted'  order by auto_number desc limit 0,15";
$exec1 = mysqli_query($GLOBALS["___mysqli_ston"], $query1) or die ("Error in Query1".mysqli_error($GLOBALS["___mysqli_ston"]));
while ($res1 = mysqli_fetch_array($exec1))
{
	$store = $res1['store'];
	...
	$a_json_row["storecode"] = trim($storecode);
	$a_json_row["value"] = trim($store);
	$a_json_row["label"] = trim($store);
