...
<td align="right"><strong>Discount (Dr)</strong></td>
<td align="right"><strong>Total</strong></td>
<td align="right"><strong>Diff</strong></td>
<td align="center"><strong>Status</strong></td>
</tr>
<?php 
$query8 = "SELECT visitcode,billnumber,transactionamount,transactiondate,accountname, cashamount, cardamount, creditamount, chequeamount, onlineamount FROM `master_transactionpaynow` WHERE `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `billnumber` LIKE 'OPC%' order by auto_number";
$exec8 = mysqli_query($GLOBALS["___mysqli_ston"], $query8) or die ("Error in Query8".mysqli_error($GLOBALS["___mysqli_ston"]));
while($res8 = mysqli_fetch_array($exec8))
{
$docno = $res8['billnumber'];
$visitcode = $res8['visitcode'];
$transactionamount = ($res8['cashamount'] + $res8['cardamount'] + $res8['chequeamount'] + $res8['creditamount'] + $res8['onlineamount']);
...
$i = 0;
$drresult = array();
$querydr1bnk = "SELECT SUM(`fxamount`) as income FROM `billing_paynowlab` WHERE billnumber = '$docno'
UNION ALL SELECT SUM(`fxamount`) as income FROM `billing_paynowpharmacy` WHERE billnumber = '$docno'
UNION ALL SELECT SUM(`fxamount`) as income FROM `billing_paynowradiology` WHERE billnumber = '$docno'
UNION ALL SELECT SUM(`fxamount`) as income FROM `billing_paynowservices` WHERE billnumber = '$docno' and wellnessitem <> '1'
UNION ALL SELECT SUM(`fxamount`) as income FROM `billing_paynowreferal` WHERE billnumber = '$docno'
UNION ALL SELECT SUM(`fxamount`) as income FROM `billing_referal` WHERE billnumber = '$docno'
UNION ALL SELECT SUM(-1*(`consultationfxamount`+`pharmacyfxamount`+`labfxamount`+`radiologyfxamount`+`servicesfxamount`)) as income FROM `billing_patientweivers` WHERE billno = '$docno'";
	...
	if($parentid != '' && $tbinclude != '')
	{
		if($parentid == '47' || $parentid == '48' || $parentid == '49')
		{
			...
			{  
				...
				$j = 0;
				$crresult = array();
				$querycr1 = "SELECT SUM(`$col`) as bankamount FROM `master_transactionpaynow` WHERE (`bankcode` = '$id' or `cashcode` = '$id' or `mpesacode` = '$id' or `transactionmode` = 'SPLIT') AND `transactiondate` BETWEEN '$ADate1' AND '$ADate2' AND `billnumber` LIKE 'OPC%' AND '$id' in (select ledgercode from financialaccount)";
		...
		{
			$query267 = "select accountname,auto_number,id from master_accountname where accountssub = '$parentid'
						union select accountssub as accountname,auto_number,id from master_accountssub where parentid = '$parentid'";
			$exec267 = mysqli_query($GLOBALS["___mysqli_ston"], $query267) or die ("Error in Query267".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($res267 = mysqli_fetch_array($exec267))
			{  
				$accountsmain2 = $res267['accountname'];
				...
				$id = $res267['id'];
				$i = 0;
				$result = array();$querydr1 ='';
				if($id == '01-1014-01')
				{
				$querydr1 = "SELECT SUM(`fxamount`) as income FROM `billing_paynowradiology` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'
							 UNION ALL SELECT SUM(-1*`radiologyfxamount`) as income FROM `billing_patientweivers` WHERE entrydate BETWEEN '$ADate1' AND '$ADate2' ";
				}
				else if($id == '01-1009')
				{
				$querydr1 = "SELECT SUM(`fxamount`) as income FROM `billing_paynowlab` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'
							 UNION ALL SELECT SUM(-1*`labfxamount`) as income FROM `billing_patientweivers` WHERE entrydate BETWEEN '$ADate1' AND '$ADate2' ";
				}
				else if($id == '01-1007-01')
				{
				$querydr1 = "SELECT SUM(`fxamount`) as income FROM `billing_paynowpharmacy` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'
							 UNION ALL SELECT SUM(-1*`pharmacyfxamount`) as income FROM `billing_patientweivers` WHERE entrydate BETWEEN '$ADate1' AND '$ADate2' ";
				}
				else if($id == '01-1005')
				{
				$querydr1 = "SELECT SUM(`fxamount`) as income FROM `billing_paynowreferal` WHERE billdate BETWEEN '$ADate1' AND '$ADate2'
				...
				if($querydr1 != ''){
					$querydr1 = $querydr1." UNION ALL";
				}				$querydr1 = $querydr1." SELECT SUM(`fxamount`) as income FROM `billing_paynowservices` WHERE billdate BETWEEN '$ADate1' AND '$ADate2' AND wellnessitem <> '1' AND `servicesitemcode` in (SELECT `itemcode` FROM `master_services` where ledgerid='$id')
