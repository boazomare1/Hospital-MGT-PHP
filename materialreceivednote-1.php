...
// $selectitempono = $_POST['ponumber'][$key];
// echo '#o#'.$selectitempono.'<br>';
// foreach($_POST['selectitem'] as $check=>$value1)
// if($_POST['selectitem']!="")
// {
$selectitem = $_POST['selectitem'][$key];
// if (isset($selectitem)) {
//echo '#i#'.$selectpono.'<br>';
// if(1)
// if(1)  
	// if (isset($_POST['selectitem'][$key]))
// if($quantity!='0' || $quantity!="") 
$batchnumber = $_POST['batch'][$key]; 
// if($selectitem==$selectitemcode)  
if($batchnumber!="")
{
	$quantity = $_POST['receivedquantity'][$key];
	//echo '#selectitem#'.$selectitem.'<br>';
	//echo '#selectpo#'.$ponumber.'<br>';

	//$selectitempono = $_POST['selectitempono'][$check];
//echo '#seletedpo#'.$selectitempono.'<br>';
//$selectpono = $_POST['selectitempono'][$check];
...
$itemstatus = '';
			...
			$query5 = "select purchaseindentdocno from purchaseorder_details where suppliercode='$suppliercode' and billnumber='$ponumber'  order by auto_number desc limit 1";
			...
			// Check with previous cost price
			$query10 = "select purchaseprice from master_medicine where itemcode='".$itemcode."'";
			$exec10  = mysqli_query($GLOBALS["___mysqli_ston"], $query10) or die ("error in query10".mysqli_error($GLOBALS["___mysqli_ston"]));
			$numb=mysqli_num_rows($exec10);	
			if($numb)
			{
				$res10   = mysqli_fetch_array($exec10);
				$prev_costprice = $res10["purchaseprice"];
				if($costprice > $prev_costprice)
				{
					$query563 = "update master_medicine set purchaseprice='$costprice' where itemcode='$itemcode'";
					$exec563 = mysqli_query($GLOBALS["___mysqli_ston"], $query563) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
				}
			}
			...
			while($res_pharma_rate = mysqli_fetch_array($exec_pharma_rate)){
				...
				$sqlquerymap= "INSERT INTO pharma_template_map(templateid, productcode, rate, username,dateadded, recordstatus) VALUES ('$temp_id','$item_id','$new_price','$username','$date','')";
				$exequerymap = mysqli_query($GLOBALS["___mysqli_ston"], $sqlquerymap);

				//check subtypes linked // if any update values
				$array_subtype = array();
				$query_st = "SELECT * FROM master_subtype WHERE pharmtemplate = '$temp_id' AND recordstatus <> 'deleted'";
...
{*/
 ...
 $query5 = "select * from purchaseorder_details where suppliercode='$supplier_code' and (recordstatus='autogenerated' || recordstatus='generated') order by auto_number desc limit 1";
//}
$exec5 = mysqli_query($GLOBALS["___mysqli_ston"], $query5) or die ("Error in Query2".mysqli_error($GLOBALS["___mysqli_ston"]));
...
<script>
// $("#add_button_id"+varserialnumber).off('click');
function enabletext(varserialnumber,number)
{
	...
	if(document.getElementById("selectitem"+varserialnumber+"").checked==true)
	{
		...
		// document.getElementById("add_button_id"+varserialnumber+"").disabled=false;
		// document.getElementById("delete"+varserialnumber+"").enabled=false;
	}
	else
	{
		...
		// document.getElementById("add_button_id"+varserialnumber+"").disabled=true;
		// document.getElementById("delete"+varserialnumber+"").enabled=true;
...
// var totalcount5 = document.getElementById("total_rows").value();
 var totalcount5=$('#total_rows').val();
// alert(totalcount5);
...
if(document.getElementById("srchsuppliercode").value =='')
{
...
if(invoice_dup_check)
{
	document.getElementById("supplierbillno").focus();
	//document.getElementById("savebutton").disabled=false;
	return false;
}
if($.trim($('#deliverybillno').val()) =='')
{
...
var varItemExpiryDateLength = varItemExpiryDate.length;
	...
	if (parseInt(varItemExpiryDateMonth) <= (parseInt(date.getMonth())+ 1)  && varItemExpiryDateYear <= parseInt(date.getFullYear().toString().substr(2,2), 10))
	{
		alert ("Expiry Month and Year should Be greater than current month and yerar.");
		document.getElementById("expirydate"+i+"").focus();
		//document.getElementById("savebutton").disabled=false;
		return false;
	}
...
// Check Total Purchase FX Amount is equal to Entered Invoice Amount
var invoiceamount  = $('#invoiceamount').val();
var totalfxamount  = $('#totalfxamount').val();
if(parseFloat(totalfxamount) != parseFloat(invoiceamount))
{
	alert('Total purchase amount is not matching with Invoice amount');
	 //$('#invoiceamount').val('');
	//document.getElementById("savebutton").disabled=false;
	return false;
}
...
$(document).ready(function () {
	...
	$('#invoiceamount').keypress(function (event) {
           return isNumber(event, this)
    });


	 $( "#supplierbillno" ).blur(function() {
		invoice_dup_check = 0;
		// Get supplier code
		var suppliercode = $('#srchsuppliercode').val();
		var invoicenum = $.trim($(this).val());
		if(suppliercode !="" && invoicenum !="")
		{
			$.ajax({
...
<td >&nbsp;</td>
             ...
             <td width="11%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>LPO Date </strong></td>
			    ...
			    <!-- <td width="10%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>Supplier</strong></td>
                <td width="27%" align="left" valign="middle" class="bodytext3">
				<input  type="text" name="supplier" id="supplier" value="<?php echo $suppliername; ?>" size="25" autocomplete="off" style="text-transform:uppercase" />    
				<input type="hidden" name="srchsuppliercode" id="srchsuppliercode" value="<?php echo $suppliercode; ?>">
				 <input type="hidden" name="searchsupplieranum" id="searchsupplieranum"  value="<?php echo $srchsupplieranum ?>" size="20" />
			    </td> -->
			    <td width="11%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>Invoice Amount</strong></td>
                <td  align="left" valign="middle" >
				<input type="text" name="invoiceamount" id="invoiceamount" size="10" rsize="20" />				</td>
             ...
             <td width="11%" align="left" valign="middle"  bgcolor="#ecf0f5" class="bodytext3"><strong>Time </strong></td>
                ...
                <td colspan="3" width="27%" align="left" valign="middle" class="bodytext3">
				 <select name="store" id="store" onChange="return storeassign()" disabled="">
               ...
               <!-- <option value=""> Select Store</option> -->
				...
				?> -->
				 ...
				 <?php
				 $query6 = "SELECT * from master_company order by auto_number desc limit 1";
				$exec6 = mysqli_query($GLOBALS["___mysqli_ston"], $query6) or die ("Error in Query6".mysqli_error($GLOBALS["___mysqli_ston"]));
				$res6 = mysqli_fetch_array($exec6);
				$store_fetch=$res6['store'];

				$query5 = "SELECT * from master_store where auto_number='$store_fetch' ";
				$exec5 = mysqli_query($GLOBALS["___mysqli_ston"], $query5) or die ("Error in Query5".mysqli_error($GLOBALS["___mysqli_ston"]));
				while ($res5 = mysqli_fetch_array($exec5))
				{
				$storeanum = $res5['auto_number'];
      ...
      <tr>
		...
		<form action="materialreceivednote.php" method="post" name="form1" onSubmit="return validationcheck()">
		<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
         ...
         <input type="hidden" name="purchasetype" id="purchasetype" value="<?php echo $purchasetype; ?>">
             <tr>
                       ...
                       <td bgcolor="#ffffff" class="bodytext31" valign="center"  align="center" ><strong>Item</strong></td>
                       ...
                       <td bgcolor="#ffffff" class="bodytext31" valign="center"  align="center"></td>
			...
			if ($cbfrmflag1 == 'cbfrmflag1')
			{
				...
				if(trim($suppliercode) !="" )
				{
					...
					//$query76 = "select * from purchaseorder_details where suppliercode='$suppliercode' and (recordstatus='autogenerated' || recordstatus='generated') and goodsstatus='' and itemstatus <> 'deleted'";
					$query76 = "select billnumber from purchaseorder_details where suppliercode='$suppliercode' and (recordstatus='autogenerated' || recordstatus='generated') and goodsstatus='' and itemstatus <> 'deleted' group by billnumber order by auto_number DESC";
			...
			$totitems_qry = "select auto_number from purchaseorder_details where suppliercode='$suppliercode' and (recordstatus='autogenerated' || recordstatus='generated') and goodsstatus='' and itemstatus <> 'deleted' ";
			...
			while($respo = mysqli_fetch_array($exec76))
			{
				...
				<?php 
				$qry_pos = "select * from purchaseorder_details where billnumber='$po_number' and (recordstatus='autogenerated' || recordstatus='generated') and goodsstatus='' and itemstatus <> 'deleted'";
				...
				{ ?>
					 <tr>
		              <td colspan="2" class="bodytext31" valign="center"  align="left" 
		                bgcolor="#ecf0f5"><?php  echo $po_number;?></td>
			...
			if($str == 'MLPO-')
			{
			$query767 = "select * from manual_lpo where billnumber='$po_number' and itemcode = '$itemcode' and recordstatus='' and recordstatus <> 'deleted'";
			}
			else
			{
			$query767 = "select * from purchaseorder_details where billnumber='$po_number' and itemcode = '$itemcode' and (recordstatus='autogenerated' || recordstatus='generated') and goodsstatus='' and itemstatus <> 'deleted'";
			}
			...
			$query444 = "select * from materialreceiptnote_details where itemcode='$itemcode' and ponumber='$po_number'";
...
<?php $sno = $sno + 1; ?>
   <tr class="tr_clone">
   	...
   	<input type="hidden" name="po_number" id="po_number<?=$sno;?>" value="<?=$po_number; ?>">
		...
		<td class="bodytext31" width="10%" valign="center"  align="left"><div align="center"><?php echo $sno ; ?></div></td>
		<td class="bodytext31" valign="center" width="100%"  align="left"><div align="center" ><?php echo $itemname; ?></div></td>
		<input type="hidden" name="itemname[]"  value="<?php echo $itemname; ?>">
		<input type="hidden" id="itemname123<?=$sno;?>"  value="<?php echo $itemname; ?>">
		<input type="hidden" name="total_count" id="total_count" value="<?php echo $sno; ?>">
		<input type="hidden" id="itemcode<?=$sno;?>" name="itemcode[]" value="<?php echo $itemcode; ?>">

		<input type="hidden" name="selectitemcode[]" id="selectitemcode<?php echo $sno ; ?>" value="<?php echo $itemanum.'|'.$itemcode; ?>">
		<input type="hidden" name="ponumber[]"  id='ponumber<?=$sno;?>' value="<?php echo $po_number; ?>">
		
		<input type="hidden" name="rate[]" id="rate<?=$sno;?>" value="<?php echo $rate; ?>">
		<input type="hidden" name="quantity[]" id="quantity<?=$sno;?>" value="<?php echo $quantity; ?>">
		<input type="hidden" name="totalamount[]" id="totalamount1<?=$sno;?>" value="<?php echo $amount; ?>">
		<input type="hidden" name="jobtitle" id="jobtitle<?=$sno;?>" value="<?php echo $job_title; ?>"> 

		<td class="bodytext31" valign="center"  align="left"><div align="center" id="packagequantity<?=$sno;?>"><?php echo $packagequantity; ?></div></td>

		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="receivedquantity[]" id="receivedquantity<?php echo $sno; ?>" size="6"  class="bodytext21" autocomplete="off" onKeyUp="totalamount('<?php echo $sno; ?>','<?php echo $number; ?>'); totalcalc('<?php echo $sno; ?>');" ></div></td>
		...
		<!-- <td   id="add_button_id<?=$sno;?>" > -->
			<td>
		  <!-- <input type="button" name="add" value="+" class="tr_clone_add"><span><input type="button" class="remove" value="-"></span> -->
		   <!-- <input type="button" id="clone_row<?php echo $sno; ?>" class="btn btn-warning fa fa-clone clonerow" value="clone" onClick="clonerow(<?php echo $sno; ?>);" >  -->
		  <a  href="javascript:void(0);"  class="add_button" title="Add field" id="<?=$sno;?>"><input type="button" value="+" ></a>
		  </td>
        ...
        <td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="fxamount[]" id="fxamount<?php echo $sno; ?>" size="6" onKeyUp="return totalamount('<?php echo $sno; ?>','<?php echo $number; ?>');" class="bodytext21" autocomplete="off" readonly value="<?php echo number_format($fxpkrate,2,'.',''); ?>"></div></td>
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="tax[]" id="tax<?php echo $sno; ?>" size="6" onKeyUp="return totalamount20('<?php echo $sno; ?>','<?php echo $number; ?>');" class="bodytext21" autocomplete="off" readonly></div></td>
		...
		<td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="totalamount1[]" id="totalamount<?=$sno;?>" size="6" class="bodytext21" readonly value="<?php echo $amount; ?>"></div></td>
		...
		<input type="hidden" name="number_f" id="number_f" value="<?=$number; ?>">
		...
		<br>
		<tr class="">
			<td colspan="23" class="after_loop"></td>
		 </tr>
		 <script type="text/javascript">

$(document).ready(function(){
  
 	// $("#savebutton").on("click", funcsave(id));
    var maxField = 100; //Input fields increment limitation
    var addButton = $('.add_button'); //Add button selector
    
    var x = 1; //Initial field counter is 1
    var id =<?php echo $sno;?>; //Once add button is clicked
    $(addButton).click(function(){ //Check maximum number of input fields
        if(id < maxField){ 
            ...
            var po_number=$('#po_number'+this.id).val();
            var itemanum=$('#selectitem'+this.id).val();
            var itemname12=$('#itemname123'+this.id).val();
            var itemcode=$('#itemcode'+this.id).val();
            var selectitemcode=$('#selectitemcode'+this.id).val();
            var ponumber=$('#ponumber'+this.id).val();
            var rate=$('#rate'+this.id).val();
            var quantity=$('#quantity'+this.id).val();
            var totalamount1=$('#totalamount1'+this.id).val();
            var jobtitle=$('#jobtitle'+this.id).val();
            var packagequantity = $('#packagequantity'+this.id).text();
    //         for(i=1;i<=3;i++)
				// {
				// var balqty = $('#balqty'+this.id).val()-$('#receivedquantity'+i).val();
			// }
            var balqty = $('#balqty'+this.id).val()-$('#receivedquantity'+this.id).val();
            var packagesize = $('#packsize'+this.id).val();
            var fxamount = $('#fxamount'+this.id).val();
            var totalfxamount = $('#totalfxamount'+this.id).val();
            var priceperpack = $('#priceperpack'+this.id).val();
            var totalamount = $('#totalamount'+this.id).val();
            var spmarkup = $('#spmarkup'+this.id).val();

            // alert(spmarkup);
            var wrapper = $('.after_loop'); //Input field wrapper
    		var fieldHTML = '<tr style="background-color:unset;background-color:lightgreen;" class="tr_clone'+id+'"><td colspan="2" class="bodytext31" valign="center"  align="left" bgcolor="#ecf0f5">'+po_number+'</td><td colspan="18" class="bodytext31" valign="center"  align="left" bgcolor="#ecf0f5">&nbsp;</td></tr> <tr style="background-color:unset;background-color:lightgreen;" class="tr_clone'+id+'"><td class="bodytext31" valign="center" width="2.3%" align="left"><input type="checkbox" class="selectitem" name="selectitem[]" id="selectitem'+id+'" value="'+itemanum+'" onClick="enabletext('+id+','+id+')"></td><td class="bodytext31" valign="center"  width="3%"  align="right"><div align="center">'+id+'</div></td><td style="color:unset; color:red;" class="bodytext31" valign="center" width="4%" align="left"><div align="center">'+itemname12+'</div></td><input type="hidden" name="itemname[]"  value="'+itemname12+'"><input type="hidden" name="itemcode[]"  value="'+itemcode+'"><input type="hidden" name="selectitemcode[]" value="'+selectitemcode+'"><input type="hidden" name="ponumber[]" value="'+ponumber+'"><input type="hidden" name="rate[]" value="'+rate+'"><input type="hidden" name="quantity[]" value="'+quantity+'"><input type="hidden" name="totalamount[]" value="'+totalamount1+'"><input type="hidden" name="jobtitle" value="'+jobtitle+'"><td class="bodytext31" valign="center" width="4%" align="left"><div align="center">'+packagequantity+'</div></td><td class="bodytext31"  valign="center"  align="left"><div align="center"><input type="text" name="receivedquantity[]" id="receivedquantity'+id+'" size="6"  class="bodytext21" autocomplete="off" onKeyUp="totalamount('+id+','+id+'); totalcalc('+id+');" ></div></td><td class="bodytext31" width="4%" valign="center"  align="left"><div align="center">'+balqty+'<input type="hidden" name="balqty[]" id="balqty'+id+'" value="'+balqty+'"></div></td><td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="batch[]" id="batch'+id+'" size="6" class="bodytext21" autocomplete="off" readonly></div></td><td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="expirydate[]" id="expirydate'+id+'" size="6" autocomplete="off" readonly></div></td><td><input type="button" value="-" class="remove" id="'+id+'"></td><td class="bodytext31" valign="center" width="4%" align="left"><div align="center">'+packagesize+'<input type="hidden" name="packsize[]" id="packsize'+id+'" value="'+packagesize+'"></div></td><td style="width: 500px;" class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="free[]" id="free'+id+'"  size="6" onKeyUp="return totalcalc1('+id+');" class="bodytext21" autocomplete="off" readonly></div></td><td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="totalquantity[]" id="totalquantity'+id+'" size="6" class="bodytext21" readonly></div></td>  <td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="fxamount[]" id="fxamount'+id+'" size="6" onKeyUp="return totalamount('+id+','+id+');" class="bodytext21" autocomplete="off" readonly value="'+fxamount+'"></div></td>  <td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="discount[]" id="discount'+id+'" size="6" onKeyUp="return totalamountdisc("'+id+'","'+id+'");" class="bodytext21" autocomplete="off" readonly></div></td>  <td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="tax[]" id="tax'+id+'" size="6" onKeyUp="return totalamount20('+id+','+id+');" class="bodytext21" autocomplete="off" readonly></div></td>    <td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="totalfxamount1[]" id="totalfxamount'+id+'" size="6" class="bodytext21" readonly value="'+totalfxamount+'"></div></td>  <td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="priceperpack[]" id="priceperpack'+id+'" size="6"  class="bodytext21" autocomplete="off" readonly value="'+priceperpack+'"></div></td> <td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="totalamount1[]" id="totalamount'+id+'" size="6" class="bodytext21" readonly value="'+totalamount+'"></div></td>  <td class="bodytext31" valign="center"  align="left"><div align="center"><input type="text" name="costprice[]" id="costprice'+id+'" size="6" class="bodytext21" readonly></div></td>          <td class="bodytext31" valign="center"  align="left"><div align="center"><input type="hidden" name="spmarkup" id="spmarkup'+id+'" value="'+spmarkup+'"><input type="text" name="saleprice[]" id="saleprice'+id+'" size="6" class="bodytext21" readonly></div></td></tr>'; 
...
$(document).on('click', '.remove', function(){
	// if (confirm("Are you sure?")) {
        ...
        $('.tr_clone'+button_id+'').remove();
        var pre_id1=parseInt(button_id)-1;
        // var pre_id2=parseInt(button_id)-2;
        // var pre_id3=parseInt(button_id)-3;
        // var pre_id4=parseInt(button_id)-4;
        // alert(pre_id);
		// $( '.totalpurchaseamount').removeData('#totalpurchaseamount');
		var inc=1;

for(i=pre_id1;i>=1;i--)
// for(i=1;i<=button_id;i++)
{
	// alert(i);
	// var pre_id=parseInt(button_id)-i;
	if ($("#receivedquantity"+i).val().length != 0){
		pre_id=i;
		// alert(pre_id);
		break; 
		// alert(pre_id);
	...
	// 	hasValue("#batch"+id);
	// if ($("#batch"+pre_id).val().length == 0){
	// 	pre_id=pre_id2;
	// 	// alert(pre_id);
	// }
	// if ($("#batch"+pre_id2).val().length == 0){
	// 	pre_id=pre_id3;
	// 	// alert(pre_id);
	// }
	// if ($("#batch"+pre_id3).val().length == 0){
	// 	pre_id=pre_id4;
	// 	// alert(pre_id);
	// }
		totalamount(pre_id,pre_id);
		// totalcalc(pre_id);
		id--;
		// totalamount(inc,id);
        ...
        <input type="hidden" id="total_rows" name="total_rows" value="">
		 <td colspan="2" align="right" valign="middle"  bgcolor="#ecf0f5" class="bodytext3">
		  <input type="hidden" name="frmflag2" value="frmflag2">
               <input name="Submit222" type="submit"  value="Save" class="button" id="savebutton" onClick="return funcsave()"/>		 </td>
      </tr>
	  </table>
        </form>
         </td>
