// IP Visit Search - Modern JavaScript
...
function validateSearchForm() {
    ...
    if (dateFrom && dateTo) {
        if (new Date(dateFrom) > new Date(dateTo)) {
            showFieldError($('#ADate2'), 'End date must be after start date');
            isValid = false;
        }
    }
...
function initializeSearchFunctionality() {
    ...
    // Add search suggestions for registration number
    $('#patientcode').on('input', function() {
        const query = $(this).val();
        if (query.length >= 2) {
            // TODO: Implement registration number autocomplete
        }
    });
}
...
function handleLocationChange() {
    ...
    if (locationCode) {
        ...
        // Clear patient search fields when location changes
        $('#patient, #patientcode, #visitcode').val('');
        
        // Show notification
        showNotification('Location changed. Patient search fields cleared.', 'info');
    }
}
...
function updateLocationDisplay(locationCode) {
    ...
    $('.location-name').text(`Location Code: ${locationCode}`);
}
            ...
            ">
                ...
                ">
                    <div class="spinner" style="
                        ...
                        margin: 0 auto 1rem;
                    "></div>
                    <p style="color: #1f2937; font-weight: 600;">Searching...</p>
                </div>
            </div>
        `);
