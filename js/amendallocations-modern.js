/**
 * Modern JavaScript for Amend Allocations
 * Following VAT Modern Design Patterns
 */

class AmendAllocationsManager {
    constructor() {
    ...
    handleActionButton(e) {
        ...
        const allocationCode = row.querySelector('.allocation-code')?.textContent || '';
        
        switch (action) {
            case 'amend':
                this.handleAmendAction(allocationCode);
                break;
            case 'view':
                this.handleViewAction(allocationCode);
                break;
            case 'print':
                this.handlePrintAction(allocationCode);
                break;
            case 'delete':
                this.handleDeleteAction(allocationCode);
                break;
        }
    }

    handleAmendAction(allocationCode) {
        if (confirm(`Are you sure you want to amend allocation ${allocationCode}?`)) {
            this.showAlert(`Successfully amended allocation ${allocationCode}`, 'success');
        }
    }

    handleViewAction(allocationCode) {
        window.open(`allocation_view.php?code=${allocationCode}`, '_blank');
    }

    handlePrintAction(allocationCode) {
        window.print();
    }

    handleDeleteAction(allocationCode) {
        if (confirm(`Are you sure you want to delete allocation ${allocationCode}?`)) {
            this.showAlert(`Successfully deleted allocation ${allocationCode}`, 'success');
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    new AmendAllocationsManager();
});

// Export for global access
window.AmendAllocationsManager = AmendAllocationsManager;
