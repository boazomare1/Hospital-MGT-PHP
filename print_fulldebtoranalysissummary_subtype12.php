...
header('Content-Disposition: attachment;filename="FullDebtorAnalysisSummarySubtype.xls"');
...
<table id="AutoNumber3" style="BORDER-COLLAPSE: collapse" 
          ...
          <tbody>
            ...
            <tr>
              ...
              <td align="center" colspan="16" bgcolor="#ffffff" class="bodytext31"><strong>Full Debtor Analysis Summary Subtype Wise</strong></td>  
		 ...
		 $totoptotalamount30 = 0;
		  ...
		  $totoptotalamountgreater = 0;
		   $subtypeopbal = 0;
			...
			$res21accountid = $res2211['id'];
		 	...
		 	$querydebit12 = "select accountnameid from master_transactionpaylater where  accountnameid='$res21accountid' group by accountnameid
			                 union 
			                 SELECT ledgerid FROM `master_journalentries` WHERE ledgerid ='$res21accountid' group by ledgerid
		  ...
		  $opquery = "select billnumber,visitcode,fxamount AS fxamount,transactiondate AS transactiondate from master_transactionpaylater where  paymenttype like '%%' and accountnameid='$res21accountid' and transactiontype = 'finalize' and transactiondate < '$ADate1' and fxamount <>'0' and billnumber not like 'AOP%'

		  UNION ALL select billnumber,visitcode,transactionamount as fxamount,transactiondate AS transactiondate  from master_transactionpaylater where  accountnameid='$res21accountid' and transactiontype = 'finalize' and transactiondate < '$ADate1'  and billnumber like 'AOP%'
		 ...
		 UNION ALL SELECT  docno AS billnumber,'' AS visitcode,(-1*`fxamount`) as fxamount,transactiondate AS transactiondate from master_transactionpaylater where  paymenttype like '%$type%' and accountnameid='$res21accountid' and transactiontype = 'paylatercredit' and recordstatus <> 'deallocated' and transactiondate < '$ADate1'  		  
 ...
 $query98 = "select sum(a.fxamount +a.discount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where a.transactiontype = 'PAYMENT' AND a.transactionstatus!= 'onaccount' AND b.transactionstatus= 'onaccount' and a.billnumber='$resbillnumber' and a.recordstatus = 'allocated' AND a.accountnameid='$res21accountid' AND  b.transactiondate <= '$ADate2'
				UNION ALL 
				select sum(a.fxamount +a.discount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where a.transactionmode = 'CREDIT NOTE' AND a.transactiontype= 'PAYMENT' AND b.transactiontype= 'paylatercredit' and a.billnumber='$resbillnumber' and a.recordstatus = 'allocated' AND a.accountnameid='$res21accountid' AND  b.transactiondate <= '$ADate2'
				UNION ALL 
				select sum(-1*a.fxamount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where  a.transactiontype = 'PAYMENT' AND b.transactiontype != 'PAYMENT' and a.docno='$resbillnumber' and a.recordstatus = 'allocated'  AND a.accountnameid='$res21accountid' AND a.docno NOT LIKE '%AR%'  AND b.transactiondate  <= '$ADate2'
								UNION ALL 
				select sum(-1*a.fxamount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno)  where  b.transactionstatus = 'onaccount' AND a.transactionstatus != 'onaccount' and a.docno='$resbillnumber' and a.recordstatus = 'allocated' AND  a.docno  LIKE '%AR%'   AND  b.transactiondate  <= '$ADate2' ";
				/*
				UNION ALL 
				select sum(-1*a.fxamount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where  a.transactiontype = 'PAYMENT' AND b.transactiontype != 'PAYMENT' and a.docno='$resbillnumber' and a.recordstatus = 'allocated' AND a.docno  LIKE '%AR%'  AND b.transactiondate  <= '$ADate2' ";	
*/
		...
		$openingtransactiondate = $opres['transactiondate'];
            ...
            <td colspan="6"  align="left" valign="center" bgcolor="#FFF" class="bodytext31" ><strong><?php echo $subtype; ?> </strong></td>
			...
			$res21accountid = $res221['id'];
		 	...
		 	//$querydebit1 = "select * from master_transactionpaylater where  accountnameid='$res21accountid'";
			$querydebit1 = "select accountnameid from master_transactionpaylater where  accountnameid='$res21accountid' group by accountnameid
			                 union 
			                 SELECT ledgerid FROM `master_journalentries` WHERE ledgerid ='$res21accountid' group by ledgerid
...
$resamount=0;
		 ...
		 UNION ALL select billnumber AS docno,transactionamount as fxamount,transactiondate,patientcode,patientname,visitcode,'Opening Balance'particulars from master_transactionpaylater where  accountnameid='$res21accountid' and transactiontype = 'finalize' and transactiondate between '$ADate1' and '$ADate2' and billnumber like 'AOP%'
		  ...
		  UNION ALL SELECT b.`docno` as docno,  (-1*b.`transactionamount`) as fxamount, b.`transactiondate` as transactiondate,b.patientcode,b.patientname,b.visitcode,b.particulars  FROM `master_transactionpaylater` as a JOIN `master_transactionpaylater` as b ON (a.`visitcode` = b.`visitcode`) WHERE b.`accountnameid` = '$res21accountid' and a.`transactiontype` = 'finalize' and b.`transactiontype` = 'pharmacycredit' and b.`auto_number` > a.`auto_number` AND b.`transactiondate` BETWEEN '$ADate1' AND '$ADate2' and b.billnumber!=''
		  		  ...
		  		  UNION ALL SELECT docno as docno, (-1*`fxamount`) as fxamount, `transactiondate` as transactiondate,patientcode,patientname,visitcode,particulars from master_transactionpaylater where  paymenttype like '%$type%' and accountnameid='$res21accountid' and transactiontype = 'paylatercredit' and recordstatus <> 'deallocated' and transactiondate between '$ADate1' and '$ADate2' 
...
//echo $res22accountname;
		  ...
		  $opquery2 = "select billnumber,visitcode,fxamount AS fxamount,transactiondate AS transactiondate from master_transactionpaylater where  paymenttype like '%%' and accountnameid='$res21accountid' and transactiontype = 'finalize' and transactiondate < '$ADate1' and fxamount <>'0' and billnumber not like 'AOP%'

		  UNION ALL select billnumber,visitcode,transactionamount as fxamount,transactiondate AS transactiondate  from master_transactionpaylater where  accountnameid='$res21accountid' and transactiontype = 'finalize' and transactiondate < '$ADate1'  and billnumber like 'AOP%'
		  
		  UNION ALL SELECT b.`docno` as billnumber,b.visitcode AS visitcode, (-1*b.`transactionamount`) as fxamount, b.`transactiondate` as transactiondate FROM `master_transactionpaylater` as a JOIN `master_transactionpaylater` as b ON (a.`visitcode` = b.`visitcode`) WHERE b.`accountnameid` = '$res21accountid' and a.`transactiontype` = 'finalize' and b.`transactiontype` = 'pharmacycredit' and b.`auto_number` > a.`auto_number` AND b.`transactiondate` < '$ADate1' and b.billnumber!=''

		  		  UNION ALL SELECT docno AS billnumber,visitcode, (-1*`fxamount`) as fxamount,transactiondate AS transactiondate from master_transactionpaylater where  paymenttype like '%$type%' and accountnameid='$res21accountid' and transactiontype = 'paylatercredit' and recordstatus <> 'deallocated' and transactiondate < '$ADate1'  
...
//ACCOUNT WISE OPENING BALANCE END					
		   ...
		   <td class="bodytext31" valign="center"  align="left"><?=$sno++;?></td>
                <td class="bodytext31" valign="center" colspan="5" align="left"><?php echo $res22accountname; ?></td>
...

		  {
				...
				$query98 = "select sum(a.fxamount +a.discount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where a.transactiontype = 'PAYMENT' AND a.transactionstatus!= 'onaccount' AND b.transactionstatus= 'onaccount' and a.billnumber='$res2billnumber' and a.recordstatus = 'allocated' AND a.accountnameid='$res21accountid' AND  b.transactiondate BETWEEN '$ADate1' and '$ADate2' and a.transactiondate <= '$ADate2'
 
				UNION ALL
				select sum(a.fxamount +a.discount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where a.transactionmode = 'CREDIT NOTE' AND a.transactiontype= 'PAYMENT' AND b.transactiontype= 'paylatercredit'and a.billnumber='$res2billnumber' and a.recordstatus = 'allocated' AND a.accountnameid='$res21accountid' AND  b.transactiondate BETWEEN '$ADate1' and '$ADate2' and a.transactiondate <= '$ADate2'
 
				UNION ALL 
				
select sum(-1*a.fxamount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno) where  a.transactiontype = 'PAYMENT' AND b.transactiontype != 'PAYMENT' and a.docno='$res2billnumber' and a.recordstatus = 'allocated' AND a.accountnameid='$res21accountid' AND a.docno NOT LIKE '%AR%'	 AND b.transactiondate BETWEEN '$ADate1' and '$ADate2'	and a.transactiondate <= '$ADate2'


UNION ALL
				select sum(-1*a.fxamount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno)  where  b.transactionstatus = 'onaccount' AND a.transactionstatus != 'onaccount' and a.docno='$res2billnumber' and a.recordstatus = 'allocated' AND  a.docno  LIKE '%AR%'   AND  b.transactiondate   BETWEEN '$ADate1' and '$ADate2' and a.transactiondate <= '$ADate2'";
/*				
				UNION ALL 
				
select sum(-1*a.fxamount ) as transactionamount1,a.docno from master_transactionpaylater as a left join master_transactionpaylater as b on(b.docno=a.docno)  where  a.transactiontype = 'PAYMENT' AND b.transactiontype != 'PAYMENT' and a.docno='$res2billnumber' and a.recordstatus = 'allocated' AND  a.docno  LIKE '%AR%'   AND b.transactiondate BETWEEN '$ADate1' and '$ADate2'
